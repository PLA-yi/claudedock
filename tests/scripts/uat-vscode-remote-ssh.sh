...
选项:
  ...
  --confirm-destructive     触发实际断言：需要运行中的容器
  --container=NAME          指定容器名（默认自动检测 claudedock-local-*）
...
detect_container() {
  ...
  local name
  name=$(docker ps --filter "label=claudedock-local=true" --format '{{.Names}}' 2>/dev/null | head -1)
  if [ -z "$name" ]; then
    name=$(docker ps --filter "name=claudedock-local" --format '{{.Names}}' 2>/dev/null | head -1)
  fi
  echo "$name"
}
...
scenario_singbox_process() {
  ...
  if [ -z "$container" ]; then
    skip "singbox_process" "未找到运行中的 claudedock-local 容器"
    write_json_report "singbox_process" "skip"
    return 0
  fi
...
scenario_egress_ip() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "egress_ip" "未找到运行中的 claudedock-local 容器"
    write_json_report "egress_ip" "skip"
    return 0
  fi
...
scenario_dns_leak() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "dns_leak" "未找到运行中的 claudedock-local 容器"
    write_json_report "dns_leak" "skip"
    return 0
  fi
...
scenario_vscode_server() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "vscode_server" "未找到运行中的 claudedock-local 容器"
    write_json_report "vscode_server" "skip"
    return 0
  fi
...
scenario_sshd_process() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "sshd_process" "未找到运行中的 claudedock-local 容器"
    write_json_report "sshd_process" "skip"
    return 0
  fi
...
scenario_singbox_log_domains() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "singbox_log_domains" "未找到运行中的 claudedock-local 容器"
    write_json_report "singbox_log_domains" "skip"
    return 0
  fi
...
scenario_direct_tcpip_forward() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "direct_tcpip_forward" "未找到运行中的 claudedock-local 容器"
    write_json_report "direct_tcpip_forward" "skip"
    return 0
  fi
...
scenario_forward_egress_ip() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "forward_egress_ip" "未找到运行中的 claudedock-local 容器"
    write_json_report "forward_egress_ip" "skip"
    return 0
  fi
...
scenario_security_reject() {
  ...
  container="$(detect_container)"
  if [ -z "$container" ]; then
    skip "security_reject" "未找到运行中的 claudedock-local 容器"
    write_json_report "security_reject" "skip"
    return 0
  fi
