// Package claudedock — Phase 32 Plan 03 账号级热同步单例锁。
//
// 在远端容器内通过 flock(1) 创建账号级单例：同一 claude_account 在
// 容器内同一时刻只能有一个 claudedock 后端持有锁。后端拿不到锁即
...
//   - 锁路径 /tmp/claudedock/locks/sync-<accountID>.lock
...
// ErrSyncLocked 表示另一端 claudedock 已持有同一 claude_account 的热同步单例锁。
//
