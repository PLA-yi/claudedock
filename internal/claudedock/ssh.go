...
func ConnectAndRunClaudeV3(cfg SSHConfig, claudeArgs []string, cwd string,
	...
	//
	// 退出码引用 claudedock.Exit* 命名常量（避开 v2.0 main.go ExitConfigError=4 /
...
//
// 调用方 (cmd/claudedock/sessions.go) 用此入口拿 *ssh.Client 直接喂给
// internal/claudedock.RunSessionsLs / RunSessionsAttach。
...
//
//   - 首次连接：将主机公钥 SHA256 指纹写入 ~/.claudedock/known_hosts
...
func newHostKeyCallback() ssh.HostKeyCallback {
	...
	if home, err := os.UserHomeDir(); err == nil {
		knownHostsPath = filepath.Join(home, ".claudedock", "known_hosts")
	}
...
func knownHostsDir() (string, error) {
	...
	dir := filepath.Join(home, ".claudedock")
