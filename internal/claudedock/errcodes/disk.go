...
func init() {
	MustRegister(Entry{
		...
		NextAction: "清理 ~/.claudedock/hotsync/ 或释放本地磁盘",
	})
	...
	MustRegister(Entry{
		...
		Severity:   SeverityWarn,
		Message:    "HotSync 数据目录 ~/.claudedock/hotsync/ 已达 %s，超过 1GB 警戒线",
		NextAction: "运行 rm -rf ~/.claudedock/hotsync/sessions/ 后重启 claudedock",
	})
}
