...
// 凭证注入策略：
//
//	推荐路径 (b)：测试代码直接 import internal/claudedock 包，
...
// runClaudeDock 启动 claudedock 二进制（已编译到 /tmp/claudedock-int），
// 注入临时 ~/.claudedock/config.yaml 指向 fixture（若存在 mock gateway 路径）；
// 推荐方案：测试代码改为直接 claudedock.ConnectAndRunClaudeV3(...) 调用，
// 避开 main.go LoadConfig 网关需求。本骨架保留 binary 调用版本以便 CI 集成。
func runClaudeDock(t *testing.T, mode string, cwd string) (exitCode int, stderr string) {
	t.Helper()
...
func TestIntegration_C4_HotSyncFail_DowngradesToSSHFSOnly(t *testing.T) {
	...
	cwd := t.TempDir()
	_, stderr := runClaudeDock(t, "auto", cwd)
...
func TestIntegration_C5_SafetyGuard_BlocksSync(t *testing.T) {
	...
	cwd := t.TempDir() // 空目录
	code, stderr := runClaudeDock(t, "full", cwd)
	if code == 0 {
...
func TestIntegration_F2B_KillHotSyncAgent_DowngradesIn2s(t *testing.T) {
	...
	_, _ = dockerExec(t, "pkill", "-9", "claudedock")
...
func TestIntegration_F7C_OAuthExpired_ExitsBeforeClaude(t *testing.T) {
	...
	cwd := t.TempDir()
	code, stderr := runClaudeDock(t, "sshfs-only", cwd)
	if code == 0 {
		t.Errorf("期望退出非 0，实际 0")
	}
...
func TestIntegration_Phase32_NetworkDisconnect30s(t *testing.T) {
	...
	// 端到端 PTY 交互（claudedock 启动 → 进 tmux → docker network disconnect →
