...
// Mode 是 claudedock --mount-mode flag 的四档枚举（CONTEXT D-14）。
//
// 状态机契约：
...
type MountConfig struct {
	...
	// Username 是 claudedock 配置中的用户名（供 dashboard 展示）。
	Username string
...
func tryModeReal(connA, connB *ssh.Client, mode Mode, cfg MountConfig, snapshot *LastSessionSnapshot) (cleanup func(), status HotSyncStatus, err error) {
	...
	var pidFile string
	if dirErr != nil {
		pidFile = "/tmp/claudedock-promoter.pid"
	} else {
...
//
//	claudedock-{account_id_or_anon}-{cwd_hash8}
