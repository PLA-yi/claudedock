...
// LastSessionSnapshot 是 ~/.claudedock/last-session.json 的 schema_version=1 结构。
//
...
// DefaultLastSessionPath 返回默认 last-session.json 路径（~/.claudedock/last-session.json）。
...
func DefaultLastSessionPath() (string, error) {
	...
	return filepath.Join(home, ".claudedock", "last-session.json"), nil
}
