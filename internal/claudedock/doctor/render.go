...
func RenderText(r *Report, noColor bool) string {
	...
	// 第一段：banner
	b.WriteString("╭─────────────────────────────────────────╮\n")
	b.WriteString("│  ClaudeDock Doctor v3.0 体检报告       │\n")
	b.WriteString("╰─────────────────────────────────────────╯\n")
	if r.ClaudeDockVer != "" {
...
// 注：claudedock.ColorEnabled 接受 (noColor, fdHolder) 两参；这里传入 os.Stdout 做 TTY 探测。
func pickIcon(s Status, noColor bool) string {
	if !claudedock.ColorEnabled(noColor, os.Stdout) {
		switch s {
		case StatusPass:
	...
	switch s {
	case StatusPass:
		return claudedock.Colorize(iconPass, claudedock.AnsiGreen, true)
	case StatusWarn:
		return claudedock.Colorize(iconWarn, claudedock.AnsiYellow, true)
	case StatusFail:
		return claudedock.Colorize(iconFail, claudedock.AnsiRed, true)
	case StatusSkip:
		return claudedock.Colorize(iconSkip, claudedock.AnsiGray, true)
	}
	return iconPass
}
