...
// doctor 包不能 import cmd/claudedock（main package），故此处复制 4 行 exec 实现。
...
func checkGitProxyEnabled(ctx context.Context) Check {
	...
	// 「~/.claudedock/config.yaml 不存在或解析失败: proxy_commands 未包含 git」
	// 这种与真实场景相反的文案，且 NextAction 错误地建议「运行 claudedock init」。
...
// checkPromoterAlive 远程 promoter 随 claudedock SSH session 生命周期运行，
