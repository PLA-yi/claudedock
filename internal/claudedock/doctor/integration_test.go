...
// runClaudeDockDoctor 编译 claudedock 并跑 doctor mount，返回 exit/stdout/stderr。
func runClaudeDockDoctor(t *testing.T, extraArgs ...string) (int, string, string) {
	t.Helper()
...
func TestIntegration_DoctorMountHappy(t *testing.T) {
	...
	exit, stdout, stderr := runClaudeDockDoctor(t)
	if exit != 0 && exit != 1 {
...
func TestIntegration_DoctorMountFail_MergerfsTampered(t *testing.T) {
	...
	exit, stdout, _ := runClaudeDockDoctor(t)
	if exit != 2 {
