...
if command -v nft >/dev/null 2>&1; then
  if nft list chain inet claudedock forward 2>/dev/null | grep -q "drop"; then
...
info "SC-3 要求完整流程: claudedock → SSH Proxy → 目录映射 → Claude Code 运行"

CONTROL_PLANE_UP=false
if systemctl is-active claudedock >/dev/null 2>&1; then
...
if [[ "$CONTROL_PLANE_UP" == "true" ]]; then
  ...
  info "手工 E2E: claudedock connect <test-host> 并确认 /workspace 内文件可读写"
  pass "端到端前置条件: 控制面就绪"
else
  warn "控制面未运行，跳过端到端验证 (SC-3 需在完整部署后手工执行)"
  info "手工 E2E 验证步骤:"
  info "  1) 启动控制面: systemctl start claudedock"
  info "  2) claudedock connect <host>"
