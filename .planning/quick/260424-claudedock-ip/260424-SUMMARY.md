...
## 任务描述
为 claudedock CLI 添加外层会话信息面板，在文件挂载成功后、Claude Code PTY 接管终端前展示关键会话信息。

## 改动清单

### 新增文件
- `internal/claudedock/dashboard.go` — SessionDashboard 结构体 + CollectDashboard + Print
- `internal/claudedock/dashboard_test.go` — 5 个渲染测试用例

### 修改文件
- `internal/claudedock/mount_strategy.go` — MountConfig 新增 `Username` 字段
- `cmd/claudedock/main.go` — 创建 MountConfig 时注入 `Username: cfg.Username`
- `internal/claudedock/ssh.go` — `ConnectAndRunClaudeV3` 挂载成功后调用 dashboard 输出
...
## 测试
- `go build ./cmd/claudedock/` PASS
- `go test ./internal/claudedock/ -run TestPrintDashboard -v` 5/5 PASS
- `go test ./internal/claudedock/ -count=1` 全包 PASS（45.8s）
