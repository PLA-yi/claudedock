...
files_modified:
  ...
  - docs/zh/guide/claudedock-local.md
autonomous: true
requirements: []
must_haves:
  truths:
...
彻底移除 ClaudeDock 中的端口映射（Port Mapping）特性，包括后端代码、数据库模型、网络层兼容补丁、API 路由、前端组件及相关文案。
...
<task type="auto">
  ...
  <action>
    ...
    4. `cmd/claudedock/local.go` — 将文案 "端口映射" 改为 "SSH 端口"。
    ...
    7. `docs/zh/guide/claudedock-local.md` — 将 "SSH 端口映射" 改为 "SSH 端口"。

    每改完前端文件后运行 `cd web/admin && npm run build`（或 `pnpm build` / `yarn build`，以项目实际 lockfile 为准）确认构建通过。
  </action>
  <verify>
    <automated>cd web/admin &amp;&amp; npm run build &amp;&amp; grep -r "端口映射" web/admin/src/ cmd/claudedock/local.go tests/scripts/uat-vscode-remote-ssh.sh docs/zh/guide/ || echo "No remaining 端口映射 references"</automated>
  </verify>
