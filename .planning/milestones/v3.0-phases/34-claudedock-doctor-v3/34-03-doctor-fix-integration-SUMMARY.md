...
requires:
  - phase: 34-claudedock-doctor-v3
    provides: errcodes 8 域闭合 Registry（42 条）+ ExtendedExplanations + claudedock explain CLI（Plan 01）
  - phase: 34-claudedock-doctor-v3
...
provides:
  ...
  - cmd/claudedock/doctor.go: anyFixerRegistered 真实检查 + `[fix] N 项已修复 / M 项修复失败` 顶部行（text 模式）
  - integration_test.go (build tag integration) — 2 docker fixture 用例（happy / mergerfs 篡改）
  - scripts/ci-doctor-grep.sh — M14/SC#3 三段断言闸门
  - Makefile: claudedock / ci-doctor-grep / ci-gate 三个 target
...
key-decisions:
  - "loadConfig 复用 Plan 02 在 auth.go 中已声明的包级 var（var loadConfig = claudedock.LoadConfig），fix.go 不重新声明 — 避免 redeclared compile error，单测仍可在 fix_test.go 中按 var 注入 mock"
  - "fixtureCtr 常量 = 'cc-fixture'（与 scripts/test-fixture-up.sh 中 docker-compose.yml 的 container_name 字面量一致），不沿用 plan 示例的 'claudedock-fixture' — 这是与 Plan 02 已完成的 docker fixture 的实际锚点"
...
- cmd/claudedock/doctor.go: `anyFixerRegistered` 改为真实 `len(doctor.FixerRegistry) > 0` 检查；fix=true 时调 `doctor.ApplyFixes` 并在 text 模式输出 `[fix] N 项已修复 / M 项修复失败`
...
- Makefile 新增 `claudedock` / `ci-doctor-grep` / `ci-gate` 三 target，CI workflow 直接 `make ci-gate` 即可
...
4. **Task 3.4: cmd/claudedock/doctor.go anyFixerRegistered 真实检查 + `[fix] N/M` 顶部行** — `f38dfd8` (feat)
...
7. **Task 3.7: Makefile 追加 claudedock / ci-doctor-grep / ci-gate target** — `5d4a0d8` (chore)
...
- `internal/claudedock/doctor/fix.go` — FixerRegistry + 6 Fixers + confirmDestructive + ApplyFixes（313 行）
- `internal/claudedock/doctor/fix_test.go` — 16 单测，纯内存（mock 包级 var）（257 行）
- `internal/claudedock/doctor/integration_test.go` — build tag integration + 2 E2E 用例（124 行）
- `scripts/ci-doctor-grep.sh` — bash + jq + awk 三段断言（74 行，chmod +x）

### 修改（4 个）

- `internal/claudedock/doctor/mount.go` — checkFUSEResidual 显式构造 Check{Details["mountpoints"]: []string}
- `internal/claudedock/doctor/ssh.go` — checkKnownHosts warn 分支挂 Details["host_port"]；checkSSHDKeepaliveDrift warn 分支挂 Details["interval"/"count"/"baseline"]
- `cmd/claudedock/doctor.go` — anyFixerRegistered 真实检查；runDoctor 在 fix=true 时调 ApplyFixes + text 模式打印 `[fix] N 项已修复 / M 项修复失败`
- `Makefile` — 末尾追加 claudedock / ci-doctor-grep / ci-gate 三 target
...
| `go test ./internal/claudedock/doctor/ -run "TestFix\|TestConfirmDestructive\|TestApplyFixes"` | PASS（16/16） |
...
| `grep -c 'FixerRegistry\[' internal/claudedock/doctor/fix.go` | **7**（1 声明 + 6 init 注册） |
| `grep -cE "^func Test(Fix\|ConfirmDestructive\|ApplyFixes)" internal/claudedock/doctor/fix_test.go` | **16** ≥ 14 |
...
| `bash scripts/ci-doctor-grep.sh ./claudedock`（本地 smoke） | **PASS** — `OK: claudedock doctor M14 gate passed (schema=1 / next_action / 错误码).` |
| SC#9 host-agent 边界 `rg "agentapi\.(Action\|NewClient\|RunHostAction)" internal/claudedock/doctor/` | empty ✅ |
| `./claudedock explain MOUNT_MUTAGEN_VERSION_SKEW` | exit 0，stdout 含 `详细说明：` |
| `./claudedock doctor --help` | exit 0，列出 `--fix / --verbose / --json / --yes` 4 flag |

## Decisions Made

详见 frontmatter `key-decisions`。核心：

1. **复用 Plan 02 的 `loadConfig` 包级 var**：plan 示例代码独立声明 `loadConfig`，但 Plan 02 已在 auth.go 中按 `var loadConfig = claudedock.LoadConfig` 注入；fix.go 直接复用，避免 redeclared compile error。fix_test.go 仍可按 var 切换 mock。
2. **`fixtureCtr = "cc-fixture"`**：plan 示例代码用 `claudedock-fixture`，但实际 `scripts/test-fixture-up.sh` 写出的 `docker-compose.yml` 的 `container_name` 是 `cc-fixture`。executor 选择遵循实际 fixture 字面量，避免 docker exec 失败。
...
- **Issue:** Plan §3.1 fix.go 示例代码在文件内独立声明 `loadConfig`（虽未明示，但 fixAuthTokenExpired 调用了 `loadConfig()`）；Plan 02 的 `auth.go` 已经按 `var loadConfig = claudedock.LoadConfig` 声明同名包级 var。两份声明在同一 package 编译冲突。
...
- **Files modified:** `internal/claudedock/doctor/fix.go`（少写 1 行 var）+ `internal/claudedock/doctor/fix_test.go`（与 plan 一致）
- **Verification:** `go build ./internal/claudedock/doctor/...` PASS；TestFixAuthTokenExpired_Success PASS。
- **Committed in:** `78626e5`（Task 3.1 commit）

**2. [Rule 1 - 真实 fixture 字面量] fixtureCtr = "cc-fixture"（非 plan 示例的 "claudedock-fixture"）**

- **Found during:** Task 3.5 写 integration_test.go 时核对 `scripts/test-fixture-up.sh` 实际启动的容器名
- **Issue:** Plan §3.5 示例代码 `const fixtureCtr = "claudedock-fixture"`，但实际 `scripts/test-fixture-up.sh` 第 36 行写出的 docker-compose.yml `container_name: cc-fixture`。如沿用 plan 字面量，集成测试 `dockerExec` 永远 docker exec 失败。
- **Fix:** integration_test.go 改用 `const fixtureCtr = "cc-fixture"`，与 fixture 脚本逐字符一致。Plan §3.5 「executor 注意」段已预批此修订（"如脚本内用 `claudedock-fixture` 以外的名字，改本文件这一行"）。
- **Files modified:** `internal/claudedock/doctor/integration_test.go`
...
- **Files modified:** `cmd/claudedock/doctor.go`
- **Verification:** `bash scripts/ci-doctor-grep.sh ./claudedock` PASS（gate 第 1 段 jq schema_version=1 不被破坏）。
...
无。`fixAuthOAuthRefreshFailed` 返回 `[]string{"请在容器内运行 claudedock exec claude login 重新登录"}` 是设计性 NextAction（OAuth 不能自动登录），不算 stub。
...
- **集成测试 CI 化**：build tag integration 路径已就绪，可在 CI 增 `go test -tags=integration ./internal/claudedock/doctor/` 步骤（依赖 docker daemon + `local/managed-user:v3.0.0` 镜像）。
- **v3.1 backlog 候选**：
  ...
  - (b) `claudedock doctor` 维度并发执行（当前 5 维度串行，可并发到 30s）
...
| SC#8 | Plan 01 | claudedock explain CLI 子进程测试 + 38 条长说明 |
...
- [x] `internal/claudedock/doctor/fix.go` exists ✅
- [x] `internal/claudedock/doctor/fix_test.go` exists ✅
- [x] `internal/claudedock/doctor/integration_test.go` exists ✅
- [x] `scripts/ci-doctor-grep.sh` exists + executable ✅
- [x] `internal/claudedock/doctor/mount.go` modified（Details["mountpoints"]） ✅
- [x] `internal/claudedock/doctor/ssh.go` modified（Details["host_port"]） ✅
- [x] `cmd/claudedock/doctor.go` modified（anyFixerRegistered + ApplyFixes） ✅
