...
files_modified:
  ...
  - internal/claudedock/doctor/integration_test.go
...
must_haves:
  ...
  artifacts:
    - path: internal/claudedock/doctor/fix.go
    ...
    - path: internal/claudedock/doctor/integration_test.go
  ...
  key_links:
    ...
    - from: "internal/claudedock/doctor/fix.go::confirmDestructive"
    ...
    - from: "scripts/ci-doctor-grep.sh"
      to: "claudedock doctor --json / doctor 文本 / grep -E '\\[[!✗]\\]'"
...
1. `internal/claudedock/doctor/fix.go` 新建：`FixerRegistry map[errcodes.Code]Fixer` + 5 类修复函数（D-09 表）+ `confirmDestructive` helper + 5 个包级 var mock 注入点；
...
5. 修改 `cmd/claudedock/doctor.go` — 把 Plan 02 的 `anyFixerRegistered() bool` 占位改为真实检查 + `runDoctor` 在 `opts.Fix=true` 时把 FixerRegistry 传入 `doctor.RunDoctor`（或 doctor 包内部 auto-pick）。
...
- 1 个修改的 cmd/claudedock/doctor.go（anyFixerRegistered 真实实现）
</objective>
...
@.planning/phases/34-claudedock-doctor-v3/34-02-doctor-framework-PLAN.md
...
From internal/claudedock/doctor/check.go (Plan 02):
```go
type Check struct {
    Domain     string
    Name       string
...
From internal/claudedock/mount_mutagen.go (line 225-229，既有 daemon start 幂等样板):
```go
...
From internal/claudedock/integration_test.go (line 1-80，build tag + TestMain + dockerExec):
```go
...
From golang.org/x/term (已被 cmd/claudedock/main.go line 10 import):
```go
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/fix.go</files>

  <read_first>
    - internal/claudedock/doctor/check.go（Plan 02 Check struct + Options 字段）
    - internal/runtime/tasks/worker.go（line 687-695 包级 var mock 模板 + line 957-984 dockerVolumeRunner 模板）
    - internal/claudedock/mount_mutagen.go（line 220-235 daemon stop/start 幂等样板）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-09（5 类修复表）+ §D-10（confirmDestructive 三级判定）+ §D-11（不回滚）+ §D-12（输出格式）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §4.1-§4.5（跨 OS 命令 + 幂等性证据）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.9（包级 var mock）
  </read_first>

  <action>
新建 `internal/claudedock/doctor/fix.go`（约 300 行，完整实现 5 类修复）：
...
func realExecMutagenDaemon(ctx context.Context, action string) error {
	...
	// 本阶段直接走 exec.CommandContext；executor 可按项目约定用 claudedock.RunLocalMutagen helper（如存在）。
...
func fixAuthOAuthRefreshFailed(ctx context.Context, opts Options, _ Check) ([]string, []string) {
	...
	return nil, []string{"请在容器内运行 claudedock exec claude login 重新登录"}
}

func realExecEntryRefresh(ctx context.Context, gateway, shortID, password string) (*claudedock.AuthResponse, error) {
...
**executor 注意：** Task 3.5 会让 RunDoctor 调 ApplyFixes；本 task 只暴露 ApplyFixes。
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/fix.go` 退出码 = 0
    - `grep -q "var FixerRegistry = map\[errcodes.Code\]Fixer" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "^var execMutagenDaemon = realExecMutagenDaemon" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "^var execFusermountUnmount = realExecFusermountUnmount" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "^var execSSHKeygenRemove = realExecSSHKeygenRemove" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "^var execEntryRefresh = realExecEntryRefresh" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "^var execDNSFlush = realExecDNSFlush" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "func confirmDestructive(opts Options, promptZH string) (bool, string)" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "term.IsTerminal" internal/claudedock/doctor/fix.go` 命中（非 TTY 判定）
    - `grep -q "FixerRegistry\\[errcodes.MOUNT_MUTAGEN_DAEMON_UNAVAILABLE\\] = fixMutagenDaemonUnavailable" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "FixerRegistry\\[errcodes.SYSTEM_FUSE_RESIDUAL_MOUNT\\] = fixFUSEResidualMount" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "FixerRegistry\\[errcodes.SSH_KNOWN_HOSTS_CONFLICT\\] = fixSSHKnownHostsConflict" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "FixerRegistry\\[errcodes.AUTH_TOKEN_EXPIRED\\] = fixAuthTokenExpired" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "FixerRegistry\\[errcodes.AUTH_OAUTH_REFRESH_FAILED\\]" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "FixerRegistry\\[errcodes.SYSTEM_DNS_RESOLVE_FAILED\\]" internal/claudedock/doctor/fix.go` 命中
    - `grep -q "func ApplyFixes" internal/claudedock/doctor/fix.go` 命中
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/fix_test.go</files>

  <read_first>
    - internal/claudedock/doctor/fix.go（Task 3.1 新建）
    - internal/runtime/tasks/worker_volume_lifecycle_test.go（Phase 33 包级 var mock 注入测试模板）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-09 / §D-10 / §D-11
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.12（fix_test 单测模板）
  </read_first>

  <action>
新建 `internal/claudedock/doctor/fix_test.go`：
...
func TestFixAuthTokenExpired_Success(t *testing.T) {
	...
	execEntryRefresh = func(ctx context.Context, gw, id, pw string) (*claudedock.AuthResponse, error) {
		return &claudedock.AuthResponse{}, nil
	}
...
- 测试依赖 stdin 交互（`confirmDestructive` 在 Yes / JSON / 非 TTY 三分支覆盖，没有真实 stdin 测试）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/fix_test.go` 退出码 = 0
    - `grep -cE "^func Test(Fix|ConfirmDestructive|ApplyFixes)" internal/claudedock/doctor/fix_test.go` ≥ 14
    - `grep -q "TestConfirmDestructive_Yes_True" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestConfirmDestructive_NonTTY_False" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestFixMutagenDaemon_Idempotent" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestFixFUSEResidualMount_Yes_UnmountsAll" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestFixSSHKnownHostsConflict_NotFound_Idempotent" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestFixDNSResolveFailed_NonTTY_Rejected" internal/claudedock/doctor/fix_test.go` 命中
    - `grep -q "TestApplyFixes_StatusNotDowngraded" internal/claudedock/doctor/fix_test.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestFix|TestConfirmDestructive|TestApplyFixes" -count=1 -v` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 3.3: 修改 mount.go + ssh.go — 填充 Details</name>
  <files>internal/claudedock/doctor/mount.go, internal/claudedock/doctor/ssh.go</files>

  <read_first>
    - internal/claudedock/doctor/mount.go（Plan 02 Task 2.7）
    - internal/claudedock/doctor/ssh.go（Plan 02 Task 2.6）
    - internal/claudedock/doctor/fix.go（Task 3.1：Fixer 从 Details 读 host_port / mountpoints）
  </read_first>

  <action>
**(a) 修改 `internal/claudedock/doctor/mount.go` 的 `checkFUSEResidual`：** 把 newWarn 的返回改为显式 `Check{...}` 并在 Details 中放入 `mountpoints []string`：
...
**(b) 修改 `internal/claudedock/doctor/ssh.go` 的 `checkKnownHosts`：** 在 warn 分支填充 `Details["host_port"]`：
...
- 在 Details 中放原 stdout 大对象（只放结构化关键字段）
  ...
  <acceptance_criteria>
    - `grep -q 'Details: map\[string\]any{"mountpoints": points}' internal/claudedock/doctor/mount.go` 命中
    - `grep -q 'Details = map\[string\]any{"host_port": authHostPort}' internal/claudedock/doctor/ssh.go` 命中
    - `grep -q '"interval":' internal/claudedock/doctor/ssh.go` 命中（sshd_keepalive Details 扩展）
    - `go build ./internal/claudedock/doctor/...` 退出码 = 0
    - Plan 02 既有测试不回归：`go test ./internal/claudedock/doctor/ -count=1 -short -run "TestCheckFUSE|TestCheckKnownHosts|TestCheckSSHDKeepalive"` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 3.4: 修改 cmd/claudedock/doctor.go — anyFixerRegistered 真实检查 + fix 后 stdout 顶部 [fix] 行（CONTEXT D-12/D-16）</name>
  <files>cmd/claudedock/doctor.go</files>

  <read_first>
    - cmd/claudedock/doctor.go（Plan 02 Task 2.11 占位）
    - internal/claudedock/doctor/fix.go（Task 3.1：FixerRegistry + ApplyFixes）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-12（fix 模式 stdout 格式）+ §D-16（退出码）
  </read_first>

  <action>
修改 `cmd/claudedock/doctor.go`：
...
- 在 fix 模式下跳过 render（用户必须看到完整报告 + fix 摘要）
  ...
  <acceptance_criteria>
    - `grep -q "return len(doctor.FixerRegistry) > 0" cmd/claudedock/doctor.go` 命中
    - `grep -q "doctor.ApplyFixes(ctx, opts, report.Checks)" cmd/claudedock/doctor.go` 命中
    - `grep -q '"\\[fix\\] %d 项已修复 / %d 项修复失败' cmd/claudedock/doctor.go` 命中
    - `! grep -q "--fix 自动修复将在 doctor.fix.go 实现（Plan 03）" cmd/claudedock/doctor.go` 为真（Plan 02 占位行已移除）
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/integration_test.go</files>

  <read_first>
    - internal/claudedock/integration_test.go（line 1-111 build tag + TestMain + dockerExec + runCloudClaude）
    - scripts/test-fixture-up.sh / test-fixture-down.sh（既有 fixture）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-24 第 3 条（docker fixture t.Skip 降级）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.13（integration_test 模板）
    - .planning/specifics.md §3 / §4（SC#7 测试锚点：mergerfs 篡改 + 修复命令字面量）
  </read_first>

  <action>
新建 `internal/claudedock/doctor/integration_test.go`：

```go
//go:build integration
// +build integration

// Phase 34 Plan 03 集成测试 — 在 Phase 29 镜像 fixture 容器上跑 claudedock doctor，验证：
...
//      doctor mount 输出含 MOUNT_MERGERFS_FAILED + NextAction 含 'claudedock doctor mount --fix'（SC#7 锚点）
//
// 本文件默认 `go test ./...` 不触发（受 build tag `integration` 隔离）；完整执行需：
//
//   bash scripts/test-fixture-up.sh
//   go test -tags=integration -count=1 -v ./internal/claudedock/doctor/
...
const fixtureCtr = "claudedock-fixture" // 与 scripts/test-fixture-up.sh 一致
...
// runCloudClaudeDoctor 编译 claudedock 并跑 doctor mount，返回 exit/stdout/stderr。
func runCloudClaudeDoctor(t *testing.T, extraArgs ...string) (int, string, string) {
	t.Helper()
	bin := "/tmp/claudedock-doctor-int"
	if _, err := os.Stat(bin); err != nil {
		cmd := exec.Command("go", "build", "-o", bin, "./cmd/claudedock")
	...
	if e, ok := err.(*exec.ExitError); ok {
		...
		t.Logf("claudedock doctor 执行错误: %v", err)
		exitCode = -1
	}
...
**executor 注意：** `fixtureCtr` 常量值需与 `scripts/test-fixture-up.sh` 实际启动的容器名一致；如脚本内用 `claudedock-fixture` 以外的名字，改本文件这一行。
...
- 在 CI 环境 require SSH 拨号成功（mount 维度的远端命令由 fixture 容器自提供）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/integration_test.go` 退出码 = 0
    - `head -2 internal/claudedock/doctor/integration_test.go | grep -q '^//go:build integration$'` 命中
    - `head -2 internal/claudedock/doctor/integration_test.go | tail -1 | grep -q '^// +build integration$'` 命中
    - `grep -q "func TestIntegration_DoctorMountHappy" internal/claudedock/doctor/integration_test.go` 命中
    - `grep -q "func TestIntegration_DoctorMountFail_MergerfsTampered" internal/claudedock/doctor/integration_test.go` 命中
    - `grep -q "MOUNT_MERGERFS_FAILED" internal/claudedock/doctor/integration_test.go` 命中
    - `grep -q 'scripts/test-fixture-up.sh' internal/claudedock/doctor/integration_test.go` 命中
    - 默认 build 不触发集成测试：`go build ./internal/claudedock/doctor/...` 退出码 = 0
    - 默认 test 不触发集成测试：`go test ./internal/claudedock/doctor/ -count=1 -short` 退出码 = 0
    - 可选（本地有 docker 时）：`go test -tags=integration ./internal/claudedock/doctor/ -count=1 -v` — CI 中由 `scripts/ci-doctor-grep.sh` 独立跑，不强求本 acceptance
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <read_first>
    ...
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §6.1（**完整 70 行脚本已给**，直接复制）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-24 第 5 条（CI grep 断言）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §4.1
  </read_first>
...
**禁止：**
- 依赖 `~/.claudedock/config.yaml` 存在（脚本必须在 CI 裸环境跑通 — doctor 未 init 时 network/auth 走本地仍输出合法 JSON）
- 过滤第 1 段 `DIFFPATH` 降级 banner 输出（awk 已守护 section boundary）
  ...
  <acceptance_criteria>
    ...
    - `grep -q "OK: claudedock doctor M14 gate passed" scripts/ci-doctor-grep.sh` 命中
    - `bash -n scripts/ci-doctor-grep.sh` 退出码 = 0（语法正确）
    - 人工 smoke（本地 build claudedock 后）：`bash scripts/ci-doctor-grep.sh ./claudedock` 退出码 = 0（acceptance 允许 CI 上跑，本地无 claudedock 二进制时不阻塞本 task）
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <read_first>
    ...
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §6.1 末尾（CI 挂载位置说明）
  </read_first>
...
```makefile
# Phase 34 Plan 03 Task 3.7: claudedock doctor M14 终验闸门（ROADMAP §Phase 34 SC#3）
.PHONY: ci-doctor-grep
ci-doctor-grep: claudedock
	bash scripts/ci-doctor-grep.sh ./claudedock

# 依赖 target：构建 claudedock 二进制；如 Makefile 已有 `build-claudedock` 等同名 target，改依赖即可。
.PHONY: claudedock
claudedock:
...
- 如 `Makefile` 已有 `.PHONY: claudedock` 或同名 target，直接复用 — 删除本 task 追加的 `claudedock:` 块
- 如有 `.DEFAULT_GOAL` 指定默认 target，保持不变（本 task 只加，不动）

**verbatim 守恒：**
- target 名 `ci-doctor-grep` / `ci-gate`
- `bash scripts/ci-doctor-grep.sh ./claudedock` 命令字面量（exit code 传播，**不**加 `|| true`）

**禁止：**
- 在 Makefile 中 hardcode GOPATH / GOBIN（保留相对路径）
- 追加自动 `rm -f claudedock`（保留二进制供 smoke / explain_test 复用）
  </action>

  <acceptance_criteria>
    - `grep -q "^ci-doctor-grep:" Makefile` 命中
    - `grep -q "^ci-gate:" Makefile` 命中
    - `grep -q "bash scripts/ci-doctor-grep.sh ./claudedock" Makefile` 命中
...
<task type="execute">
  ...
  <read_first>
    - .planning/phases/34-claudedock-doctor-v3/34-01-errcodes-explain-PLAN.md（全 plan）
    - .planning/phases/34-claudedock-doctor-v3/34-02-doctor-framework-PLAN.md（全 plan）
  </read_first>
...
go test ./internal/claudedock/doctor/ -count=1 -short -v

# 4. cmd/claudedock explain + doctor 测试
go test ./cmd/claudedock/ -count=1 -v

# 5. 构建 claudedock 二进制，跑 M14 gate
make claudedock
bash scripts/ci-doctor-grep.sh ./claudedock || echo "NOTE: 本地未 init 时 gate 可能因无 warn/fail 输出而通过 — 仍需 CI 真机验证"

# 6. SC#3 / SC#4 / SC#5 / SC#6 / SC#7 / SC#8 / SC#9 锚点回归
go test ./internal/claudedock/doctor/ -run "TestDowngradeBannerRendersChain|TestRenderTextContainsNextAction|TestJSONSchemaV1Lock|TestApplyFixes" -count=1 -v

# 7. M14 双保险：errcodes Registry 层面
go test ./internal/claudedock/errcodes/ -run "TestAllCodesHaveExplanations|TestNoLegacyLowercaseCodes|TestAllDomainsClosed" -count=1 -v

# 8. explain 子进程级测试
go test ./cmd/claudedock/ -run "TestExplain_" -count=1 -v

# 9. host-agent 边界守恒（SC#9）
! rg -nE "agentapi\.(Action[A-Z]|NewClient|RunHostAction)" internal/claudedock/doctor/

# 10. 凭据泄漏守恒
! rg -nE "cfg\.Password|SSHPass|entry_password|credentials\.json" internal/claudedock/doctor/
```

**如任一命令退出码非 0，本 task 视为未通过。**

**产物清单：**
- `claudedock` 二进制（由 make claudedock 生成）
...
- 跳过任一 SC 锚点测试
  ...
  <acceptance_criteria>
    ...
    - `go test ./internal/claudedock/doctor/ -run "TestDowngradeBannerRendersChain" -count=1` 退出码 = 0（SC#6 终验）
    - `go test ./internal/claudedock/doctor/ -run "TestRenderTextContainsNextAction" -count=1` 退出码 = 0（M14）
    - `go test ./internal/claudedock/doctor/ -run "TestJSONSchemaV1Lock" -count=1` 退出码 = 0（SC#5）
    - `go test ./internal/claudedock/doctor/ -run "TestApplyFixes_StatusNotDowngraded" -count=1` 退出码 = 0（CONTEXT D-16）
    - `go test ./internal/claudedock/errcodes/ -run "TestAllCodesHaveExplanations" -count=1` 退出码 = 0（SC#8 前置）
    - `go test ./cmd/claudedock/ -run "TestExplain_" -count=1` 退出码 = 0（SC#8）
    - `! rg -nE "agentapi\\.(Action[A-Z]\|NewClient\|RunHostAction)" internal/claudedock/doctor/` 为真（SC#9）
    - `! rg -nE "cfg\\.Password\|SSHPass" internal/claudedock/doctor/` 为真（凭据泄漏守恒）
    - `./claudedock explain MOUNT_MUTAGEN_VERSION_SKEW` 退出码 = 0 且 stdout 含「详细说明：」（SC#8 人工 smoke）
    - `./claudedock doctor --help` 退出码 = 0 且 stdout 列出 `--fix / --verbose / --json / --yes`（SC#2）
  </acceptance_criteria>
</task>
...
# bash scripts/test-fixture-up.sh
# go test -tags=integration ./internal/claudedock/doctor/ -count=1 -v
# bash scripts/test-fixture-down.sh
...
go test ./internal/claudedock/doctor/ -run "TestApplyFixes_Fix_TriggersRegistry" -count=1 -v
```
...
- [ ] `./claudedock doctor --fix` 输出顶部含 `[fix] N 项已修复 / M 项修复失败`
...
<output>
After completion, create `.planning/phases/34-claudedock-doctor-v3/34-03-SUMMARY.md` with:
- 8 个 task 的实际 commit SHA + 关键 diff 片段引用
- FixerRegistry 实际注册条目数（`grep -c "FixerRegistry\[" internal/claudedock/doctor/fix.go`）
...
- carry-over：v3.1 backlog 候选：(a) FixerRegistry 扩展更多 code；(b) `claudedock doctor` 并发执行；(c) v2.0 lower-case 码迁移
</output>
