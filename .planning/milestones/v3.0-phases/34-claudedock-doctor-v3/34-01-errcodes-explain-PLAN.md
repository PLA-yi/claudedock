...
files_modified:
  ...
  - internal/claudedock/errcodes/state.go
  - internal/claudedock/errcodes/system.go
  - internal/claudedock/errcodes/ssh.go
  - internal/claudedock/errcodes/auth.go
  - internal/claudedock/errcodes/disk.go
  - internal/claudedock/errcodes/explanations.go
  - internal/claudedock/errcodes/explanations_test.go
  - internal/claudedock/errcodes/codes_test.go
  - cmd/claudedock/explain.go
...
must_haves:
  truths:
    ...
    - "claudedock explain <CODE> 已注册 exit 0 + stdout 含 Format(code) 两段 + 详细说明；未注册 code exit 4（REQ-F8-C / CONTEXT D-17 / ROADMAP §Phase 34 SC#8）"
  ...
  key_links:
    ...
    - from: "cmd/claudedock/explain.go::newExplainCmd"
    ...
    - from: "cmd/claudedock/main.go::rootCmd.AddCommand"
      to: "newExplainCmd()"
...
<objective>
收口 Phase 31 / 32 / 33 分散落码后遗留的错误码命名空间，交付 8 域闭合的 Registry + claudedock explain 子命令。具体实现：
1. `internal/claudedock/errcodes/codes.go` 常量块末尾追加 17 个新 Code 字面量（STATE × 3 / SYSTEM × 4 / SSH × 2 / AUTH × 4 / DISK × 3，与 CONTEXT D-21 表逐条对齐）；
...
5. 新建 `cmd/claudedock/explain.go` + `explain_test.go` + 修改 `cmd/claudedock/main.go` 注册子命令（AddCommand + DisableFlagParsing switch 追加）。

Purpose: 本 Plan 作为 Phase 34 的 **Wave 1 基础设施**，为 Wave 2 `claudedock doctor` 提供统一错误码查找 + Wave 3 `--fix` 5 类修复提供字面量常量；同时独立交付 REQ-F8-C `claudedock explain <code>` CLI（对标 `rustc --explain`）。
...
- 2 个新增 cmd/claudedock 文件（explain.go + explain_test.go）
...
From internal/claudedock/errcodes/codes.go:
```go
type Code string
type Severity int
...
From internal/claudedock/errcodes/mount.go（样板）:
```go
package errcodes
...
From cmd/claudedock/main.go (line 93 / 97-102):
```go
...
<task type="execute">
  ...
  <files>internal/claudedock/errcodes/codes.go</files>

  <read_first>
    - internal/claudedock/errcodes/codes.go（特别是 line 119-146 const 块）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-21（17 条新码完整表）+ §D-23（8 域闭合）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.1..§1.5（五域文件模板 + adapt 表）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §2.1（errcodes 注册示范复制要点）
  </read_first>

  <action>
在 `internal/claudedock/errcodes/codes.go` 的常量块（line 119-146，以 `NET_TCP_KEEPALIVE_UNSUPPORTED` 结尾）**内部、闭合右括号 `)` 之前**追加如下 17 行常量字面量（**变量名与字面值必须逐字符一致**，否则 `MustRegister` 编译期匹配不上）：
...
- 在 const 块外新建独立 `const (...)` 块
  ...
  <acceptance_criteria>
    - `grep -c "STATE_LAST_SESSION_MISSING\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "STATE_VOLUME_IN_USE_001\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "SYSTEM_APPARMOR_FUSERMOUNT3_MISSING\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "SYSTEM_CHECK_TIMEOUT\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "SSH_KNOWN_HOSTS_CONFLICT\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "SSH_SSHD_KEEPALIVE_DRIFT\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "AUTH_CONFIG_MISSING\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "AUTH_OAUTH_REFRESH_FAILED\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "NET_EGRESS_IP_DRIFT\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "DISK_LOCAL_LOW\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "DISK_CONTAINER_LOW\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `grep -c "DISK_MUTAGEN_DATA_BLOAT\s*Code\s*=" internal/claudedock/errcodes/codes.go` 输出 = 1
    - `go build ./internal/claudedock/errcodes/...` 退出码 = 0（既有 init() 不引用新常量故 build 独立通过；Task 1.2-1.4 补完 init 后 MustRegister 正则 + 防重断言再次 build 验证）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 1.2: 新建 errcodes/state.go — STATE_* 3 条（CONTEXT D-21 + D-27 补 STATE_VOLUME_IN_USE_001 / PATTERNS §1.1）</name>
  <files>internal/claudedock/errcodes/state.go</files>

  <read_first>
    - internal/claudedock/errcodes/mount.go（样板，line 1-91）
    - internal/claudedock/errcodes/net.go（NET_OAUTH_* 样板参考 Severity 选择）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-21（STATE 3 条字面量）+ §D-27（Phase 33 STATE_VOLUME_IN_USE_001 字面量不变）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.1（state.go adapt 表，逐字符 Message/NextAction）
    - internal/controlplane/http/admin_claude_accounts.go（仅查看 `STATE_VOLUME_IN_USE_001` 硬编码位置，不改动）
  </read_first>

  <action>
新建 `internal/claudedock/errcodes/state.go`，**逐字符复刻 mount.go 头部 + init 模式**：
...
func init() {
	MustRegister(Entry{
		...
		Message:    "未找到上次会话快照（%s）",
		NextAction: "首次运行 claudedock 后再 doctor 即可看到",
	})
	...
	MustRegister(Entry{
		...
		NextAction: "先停止容器：claudedock admin hosts stop <id>",
	})
	...
	MustRegister(Entry{
		...
		NextAction: "运行 claudedock admin hosts start <id> 启动容器",
	})
}
```
...
- NextAction ≤ 80 runes（codes_test.go:43 强制断言）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/errcodes/state.go` 退出码 = 0
    - `grep -c "^package errcodes$" internal/claudedock/errcodes/state.go` 输出 = 1
    - `grep -c "^//nolint:lll" internal/claudedock/errcodes/state.go` 输出 = 1
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/state.go` 输出 = 3
    - `grep -q "Code:\s*STATE_LAST_SESSION_MISSING" internal/claudedock/errcodes/state.go` 命中
    - `grep -q "Code:\s*STATE_VOLUME_IN_USE_001" internal/claudedock/errcodes/state.go` 命中
    - `grep -q "Code:\s*STATE_CONTAINER_NOT_RUNNING" internal/claudedock/errcodes/state.go` 命中
    - `grep -q "SeverityInfo" internal/claudedock/errcodes/state.go` 命中（STATE_LAST_SESSION_MISSING）
    - `grep -q "SeverityError" internal/claudedock/errcodes/state.go` 命中（STATE_VOLUME_IN_USE_001）
    - `grep -q "SeverityWarn" internal/claudedock/errcodes/state.go` 命中（STATE_CONTAINER_NOT_RUNNING）
    - `go build ./internal/claudedock/errcodes/...` 退出码 = 0（init 运行无 panic）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 1.3: 新建 errcodes/system.go — SYSTEM_* 4 条（CONTEXT D-21 / PATTERNS §1.2）</name>
  <files>internal/claudedock/errcodes/system.go</files>

  <read_first>
    - internal/claudedock/errcodes/mount.go（样板）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-21 SYSTEM 4 条表
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.2（字面 Message/NextAction）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.4 fuse_residual / §4.5 DNS flush / §8.3 AppArmor 5-Gate
  </read_first>

  <action>
新建 `internal/claudedock/errcodes/system.go`：
...
**禁止：** 添加额外 SYSTEM_* 码（本阶段 4 条定稿；新增需 plan-checker 审批）。
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/errcodes/system.go` 退出码 = 0
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/system.go` 输出 = 4
    - `grep -q "Code:\s*SYSTEM_APPARMOR_FUSERMOUNT3_MISSING" internal/claudedock/errcodes/system.go` 命中
    - `grep -q "Code:\s*SYSTEM_FUSE_RESIDUAL_MOUNT" internal/claudedock/errcodes/system.go` 命中
    - `grep -q "Code:\s*SYSTEM_DNS_RESOLVE_FAILED" internal/claudedock/errcodes/system.go` 命中
    - `grep -q "Code:\s*SYSTEM_CHECK_TIMEOUT" internal/claudedock/errcodes/system.go` 命中
    - `grep -q 'NextAction: "运行 claudedock doctor mount --fix 自动解挂"' internal/claudedock/errcodes/system.go` 命中（verbatim 锚点）
    - `grep -q 'NextAction: "运行 claudedock doctor network --fix 刷新 DNS 缓存"' internal/claudedock/errcodes/system.go` 命中
    - `go build ./internal/claudedock/errcodes/...` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 1.4: 新建 errcodes/ssh.go + errcodes/auth.go + errcodes/disk.go（SSH 2 + AUTH 4 + DISK 3 + NET_EGRESS_IP_DRIFT 1 = 10 条）</name>
  <files>internal/claudedock/errcodes/ssh.go, internal/claudedock/errcodes/auth.go, internal/claudedock/errcodes/disk.go</files>

  <read_first>
    - internal/claudedock/errcodes/mount.go（样板）
    - internal/claudedock/errcodes/net.go（NET_OAUTH_* 三态样板，auth 维度语义参考）
    - internal/claudedock/errcodes/session.go（SSH 相关 Severity 选择参考）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-21 SSH 2 + AUTH 4 + DISK 3 + NET 1 条表
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.3 / §1.4 / §1.5（逐字 Message/NextAction）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §9 第 7 条（AUTH_OAUTH_REFRESH_FAILED.NextAction 与 NET_OAUTH_EXPIRED / NET_OAUTH_NOT_FOUND 保持一致）
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/errcodes/ssh.go`（SSH 2 条）：**
...
func init() {
	MustRegister(Entry{
		...
		NextAction: "运行 claudedock doctor ssh --fix 自动 ssh-keygen -R",
	})
...
**(b) 新建 `internal/claudedock/errcodes/auth.go`（AUTH 4 条 + NET_EGRESS_IP_DRIFT 1 条）：**
...
**(c) 新建 `internal/claudedock/errcodes/disk.go`（DISK 3 条）：**
...
func init() {
	MustRegister(Entry{
		Code:       DISK_LOCAL_LOW,
		Severity:   SeverityWarn,
		Message:    "本地 ~/.claudedock/ 可用空间 %dMB，低于警戒线",
		NextAction: "清理 ~/.claudedock/mutagen/ 或释放本地磁盘",
	})
	...
	MustRegister(Entry{
		...
		Severity:   SeverityWarn,
		Message:    "Mutagen 数据目录 ~/.claudedock/mutagen/ 已达 %s，超过 1GB 警戒线",
		NextAction: "运行 mutagen daemon stop && rm -rf ~/.claudedock/mutagen/sessions/",
	})
}
```
...
- `AUTH_OAUTH_REFRESH_FAILED.NextAction` 与 `net.go` 中 `NET_OAUTH_EXPIRED.NextAction` 不一致（RESEARCH §9 第 7 条要求字面量一致）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/errcodes/ssh.go && test -f internal/claudedock/errcodes/auth.go && test -f internal/claudedock/errcodes/disk.go` 退出码 = 0
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/ssh.go` 输出 = 2
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/auth.go` 输出 = 5（AUTH 4 + NET_EGRESS_IP_DRIFT 1）
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/disk.go` 输出 = 3
    - `grep -q "Code:\s*SSH_KNOWN_HOSTS_CONFLICT" internal/claudedock/errcodes/ssh.go` 命中
    - `grep -q "Code:\s*SSH_SSHD_KEEPALIVE_DRIFT" internal/claudedock/errcodes/ssh.go` 命中
    - `grep -q "Code:\s*AUTH_CONFIG_MISSING" internal/claudedock/errcodes/auth.go` 命中
    - `grep -q "Code:\s*AUTH_OAUTH_REFRESH_FAILED" internal/claudedock/errcodes/auth.go` 命中
    - `grep -q "Code:\s*NET_EGRESS_IP_DRIFT" internal/claudedock/errcodes/auth.go` 命中
    - `grep -q "Code:\s*DISK_LOCAL_LOW" internal/claudedock/errcodes/disk.go` 命中
    - `grep -q "Code:\s*DISK_MUTAGEN_DATA_BLOAT" internal/claudedock/errcodes/disk.go` 命中
    - `grep -q 'NextAction: "在容器内运行 claudedock exec claude login 重新登录"' internal/claudedock/errcodes/auth.go` 命中（字面量与 net.go NET_OAUTH_* 一致）
    - `go build ./internal/claudedock/errcodes/...` 退出码 = 0（所有 init panic 防御通过）
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <files>internal/claudedock/errcodes/explanations.go</files>

  <read_first>
    - internal/claudedock/errcodes/codes.go（line 59-77 MustRegister 三段 panic 防御模板）
    - internal/claudedock/errcodes/mount.go + session.go + net.go（既有 Code 字面量完整清单）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-02（ExtendedExplanations + ExplainExempt）+ §D-18（5 段 ≥ 200 字符模板）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.6（verbatim 防御 panic 模式 + 示例 entry）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §6.2 TestAllCodesHaveExplanations 断言要求
    - .planning/research/PITFALLS.md C2 / C4 / C6 / C8 / M13 / M14（长说明「关联文档」字段引用源）
  </read_first>

  <action>
新建 `internal/claudedock/errcodes/explanations.go`（结构完整，init 内注册至少 **30 条** entry：17 条新 Phase 34 + 既有 13 条非 informational 码，见下文完整清单）：

**文件头 + 数据结构 + registerExplanation helper（**verbatim 复刻 PATTERNS §1.6**）：**

```go
// Package errcodes — Phase 34 D-02 / D-18：
// 为 claudedock explain 子命令提供每个非 informational Code 的长说明。
...
```go
	registerExplanation(MOUNT_MUTAGEN_VERSION_SKEW, `触发场景：claudedock 客户端 embed 的 Mutagen 二进制版本与容器内 /etc/claudedock/mutagen.version 不匹配。
根本原因：Mutagen agent / client 协议版本必须严格一致，否则 sync session 创建会握手失败。claudedock 在启动热同步前会做 TrimPrefix+Contains 双保险比对，版本漂移立即降级到 sshfs-only。
复现方式：docker exec <ctr> sed -i 's/v0.18.1/v0.99.99/' /etc/claudedock/mutagen.version
修复路径：升级容器镜像到 v3.0.0+（含 mutagen v0.18.1 agent），或重装 claudedock；也可运行 claudedock doctor mount --fix 自动重启 daemon 复测。
...
- 为 `ExplainExempt` 中的 5 条 Code 再注册 ExtendedExplanations（避免冗余）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/errcodes/explanations.go` 退出码 = 0
    - `grep -c "^package errcodes$" internal/claudedock/errcodes/explanations.go` 输出 = 1
    - `grep -q "var\s*ExtendedExplanations\s*=\s*map\[Code\]string" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "ExplainExempt\s*=\s*map\[Code\]struct{}" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "func registerExplanation(c Code, text string)" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "panic(fmt.Sprintf(\"errcodes: 重复注册 ExtendedExplanations" internal/claudedock/errcodes/explanations.go` 命中（防御三段式）
    - `grep -c "registerExplanation(" internal/claudedock/errcodes/explanations.go` ≥ 31（1 函数定义 + ≥ 30 注册调用）
    - `grep -q "registerExplanation(MOUNT_MUTAGEN_VERSION_SKEW" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "registerExplanation(STATE_VOLUME_IN_USE_001" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "registerExplanation(AUTH_OAUTH_REFRESH_FAILED" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "registerExplanation(DISK_MUTAGEN_DATA_BLOAT" internal/claudedock/errcodes/explanations.go` 命中
    - `grep -q "MOUNT_APFS_CASE_INSENSITIVE: {}" internal/claudedock/errcodes/explanations.go` 命中（ExplainExempt）
    - `grep -q "STATE_LAST_SESSION_MISSING:\s*{}" internal/claudedock/errcodes/explanations.go` 命中（ExplainExempt）
    - `go build ./internal/claudedock/errcodes/...` 退出码 = 0（init panic 防御通过）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 1.6: 新建 errcodes/explanations_test.go + 扩展 codes_test.go（RESEARCH §6.2 三断言 + 下限 30）</name>
  <files>internal/claudedock/errcodes/explanations_test.go, internal/claudedock/errcodes/codes_test.go</files>

  <read_first>
    - internal/claudedock/errcodes/codes_test.go（既有 5 条测试，line 1-85）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §6.2（TestAllCodesHaveExplanations + TestNoLegacyLowercaseCodes + TestAllDomainsClosed 三份完整 Go 代码）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §1.7（explanations_test.go 结构）+ §1.8（codes_test.go 下限调整）
  </read_first>

  <action>
**(a) 修改 `internal/claudedock/errcodes/codes_test.go`（**仅 1 处）：** 把 line 13-15：
...
**(b) 新建 `internal/claudedock/errcodes/explanations_test.go`（**verbatim 复制 RESEARCH §6.2 三份断言**）：**
...
- 在 codes_test.go 中删改既有 5 条测试
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/errcodes/explanations_test.go` 退出码 = 0
    - `grep -q "if len(reg) < 30" internal/claudedock/errcodes/codes_test.go` 命中
    - `grep -q "func TestAllCodesHaveExplanations" internal/claudedock/errcodes/explanations_test.go` 命中
    - `grep -q "func TestNoLegacyLowercaseCodes" internal/claudedock/errcodes/explanations_test.go` 命中
    - `grep -q "func TestAllDomainsClosed" internal/claudedock/errcodes/explanations_test.go` 命中
    - `grep -q "func TestExplainExemptOnlyInformational" internal/claudedock/errcodes/explanations_test.go` 命中
    - `grep -qE '"MOUNT":\s*true' internal/claudedock/errcodes/explanations_test.go` 命中（8 域字面量）
    - `grep -qE '"DISK":\s*true' internal/claudedock/errcodes/explanations_test.go` 命中
    - `go test ./internal/claudedock/errcodes/ -run "TestErrcodesRegistry|TestAllCodesHaveExplanations|TestNoLegacyLowercaseCodes|TestAllDomainsClosed|TestExplainExemptOnlyInformational" -count=1 -v` 退出码 = 0
    - 既有测试不回归：`go test ./internal/claudedock/errcodes/ -count=1` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 1.7: 新建 cmd/claudedock/explain.go + 修改 main.go 注册子命令（CONTEXT D-03 / D-17 / PATTERNS §3.2）</name>
  <files>cmd/claudedock/explain.go, cmd/claudedock/main.go</files>

  <read_first>
    - cmd/claudedock/main.go（特别是 line 85-102 AddCommand + switch os.Args[1]）
    - cmd/claudedock/sessions.go（line 22-47 newSessionsCmd / line 37-44 attachCmd ExactArgs(1) 三段式）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-03 / §D-17（explain 三段输出 + exit 4）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §3.2（explain.go verbatim 模式）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §8.4（大小写敏感必须保留，禁止自动修正）
  </read_first>

  <action>
**(a) 新建 `cmd/claudedock/explain.go`：**
...
- Short 文案 `"解释 claudedock 错误码（对标 rustc --explain）"`
- 未找到 stderr 模板 `"未找到错误码 %s；运行 claudedock doctor 查看可用检查项\n"`（与 CONTEXT D-17 字面一致，CI grep 依赖）
- `os.Exit(exitConfigError)` — 复用 main.go:23 的 `exitConfigError = 4` 常量（不新增常量）
- fallback 文案 `"未提供详细说明，运行 claudedock doctor <domain> 查看相关检查项"`
...
- 修改既有 `exitInternalError` / `runRoot` 等，只加不改
  ...
  <acceptance_criteria>
    - `test -f cmd/claudedock/explain.go` 退出码 = 0
    - `grep -q "^func newExplainCmd()" cmd/claudedock/explain.go` 命中
    - `grep -q "^func runExplain(cmd \*cobra.Command, args \[\]string) error" cmd/claudedock/explain.go` 命中
    - `grep -q "cobra.ExactArgs(1)" cmd/claudedock/explain.go` 命中
    - `grep -q 'errcodes.Lookup(errcodes.Code(args\[0\]))' cmd/claudedock/explain.go` 命中
    - `grep -q '"未找到错误码 %s；运行 claudedock doctor 查看可用检查项\\\\n"' cmd/claudedock/explain.go` 命中
    - `grep -q "os.Exit(exitConfigError)" cmd/claudedock/explain.go` 命中
    - `grep -q "newExplainCmd()" cmd/claudedock/main.go` 命中
    - `grep -qE 'case "init", "env", "ssh", "sync", "sessions", "explain",' cmd/claudedock/main.go` 命中
...
<task type="execute">
  <name>Task 1.8: 新建 cmd/claudedock/explain_test.go — os/exec 子进程级测试（PATTERNS §3.3 / RESEARCH 子进程测试模板）</name>
  <files>cmd/claudedock/explain_test.go</files>

  <read_first>
    - cmd/claudedock/explain.go（Task 1.7 新建）
    - internal/claudedock/integration_test.go（line 86-111 `runCloudClaude` go build + exec.CommandContext + ExitError 断言模式）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §3.3（explain_test.go 测试用例）
  </read_first>

  <action>
新建 `cmd/claudedock/explain_test.go`：
...
// buildOnceExplainBin 把 claudedock 编译到 /tmp/claudedock-explain-test，子测试共享；
// 与 internal/claudedock/integration_test.go:86-111 的 runCloudClaude 同 pattern。
func buildOnceExplainBin(t *testing.T) string {
	t.Helper()
	bin := "/tmp/claudedock-explain-test"
...
func runExplainBin(t *testing.T, bin string, args ...string) (exitCode int, stdout, stderr string) {
	...
	if err != nil {
		t.Logf("claudedock explain 执行错误: %v", err)
...
// claudedock explain MOUNT_MUTAGEN_VERSION_SKEW 必须 exit 0 + stdout 含错误码字面量 + "建议:" 子串。
...
// claudedock explain FAKE_CODE_X 必须 exit 4 + stderr 含 "未找到错误码"。
...
// claudedock explain mount_mutagen_version_skew（小写）必须 exit 4，禁止自动修正。
...
**禁止：** `t.Parallel()`（避免二进制 build 竞争）；构建到 repo 内路径（污染 git）。
  ...
  <acceptance_criteria>
    - `test -f cmd/claudedock/explain_test.go` 退出码 = 0
    - `grep -q "func TestExplain_KnownCode_Exit0" cmd/claudedock/explain_test.go` 命中
    - `grep -q "func TestExplain_UnknownCode_Exit4" cmd/claudedock/explain_test.go` 命中
    - `grep -q "func TestExplain_CaseSensitive_LowerCaseUnknown" cmd/claudedock/explain_test.go` 命中
    - `go test ./cmd/claudedock/ -run "TestExplain_" -count=1 -v` 退出码 = 0（3 条子测全 PASS，需先 build 成功）
  </acceptance_criteria>
</task>
...
go test ./internal/claudedock/errcodes/ -count=1 -v

# 3. explain 子进程级测试（3 条）
go test ./cmd/claudedock/ -run TestExplain_ -count=1 -v
...
go run -tags none ./dev/registry-count 2>/dev/null || \
  grep -cE '^\s*[A-Z_]+ *Code *= *"[A-Z_0-9]+"$' internal/claudedock/errcodes/codes.go
# 应输出 ≥ 42
...
[ "$(grep -c 'registerExplanation(' internal/claudedock/errcodes/explanations.go)" -ge 31 ]
```
...
| SC#8（claudedock explain <code> 中文说明 + 修复步骤） | Task 1.5 ExtendedExplanations ≥ 30 条；Task 1.7 explain.go 三段输出；Task 1.8 三子测断言 |
...
- [ ] `go test ./internal/claudedock/errcodes/ -count=1` 退出码 = 0（9 条测试全 PASS）
- [ ] `go test ./cmd/claudedock/ -run TestExplain_ -count=1` 退出码 = 0（3 条子测全 PASS）
...
|----------|-------------|
| 用户输入 `claudedock explain <code>` args[0] → errcodes.Lookup | 用户可输入任意字符串；Lookup 内部仅做 map 查找，无 shell / 反射 |
...
<output>
After completion, create `.planning/phases/34-claudedock-doctor-v3/34-01-SUMMARY.md` with:
