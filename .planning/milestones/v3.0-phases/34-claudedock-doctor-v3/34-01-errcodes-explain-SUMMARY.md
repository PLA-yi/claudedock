...
provides:
  ...
  - claudedock explain <code> CLI 子命令（对标 rustc --explain，未注册 exit 4）
...
key-decisions:
  ...
  - "AUTH_OAUTH_REFRESH_FAILED.NextAction 与 net.go NET_OAUTH_EXPIRED 字面量保持一致（在容器内运行 claudedock exec claude login 重新登录）"
...
**8 域闭合错误码 Registry（42 条）+ rustc-style claudedock explain CLI（38 条 ≥ 200 中文字符长说明）+ 4 条覆盖率断言**
...
- `claudedock explain <code>` CLI 子命令上线，对标 `rustc --explain`
...
7. **Task 1.7: claudedock explain <code> 子命令** - `75ae2d7` (feat)
...
- `internal/claudedock/errcodes/state.go` - STATE_* 3 条注册（含 D-27 兼容 STATE_VOLUME_IN_USE_001）
- `internal/claudedock/errcodes/system.go` - SYSTEM_* 4 条注册（FUSE / DNS / AppArmor / timeout）
- `internal/claudedock/errcodes/ssh.go` - SSH_* 2 条注册（known_hosts 冲突 + sshd 基线漂移）
- `internal/claudedock/errcodes/auth.go` - AUTH_* 4 条 + NET_EGRESS_IP_DRIFT
- `internal/claudedock/errcodes/disk.go` - DISK_* 3 条（local / container / mutagen bloat）
- `internal/claudedock/errcodes/explanations.go` - ExtendedExplanations 38 条 + ExplainExempt 4 条
- `internal/claudedock/errcodes/explanations_test.go` - 4 条断言（覆盖 / lower-case / 8 域 / Info-only）
- `cmd/claudedock/explain.go` - claudedock explain <code> 子命令
- `cmd/claudedock/explain_test.go` - 3 条子进程级测试

### 修改（3 个）

- `internal/claudedock/errcodes/codes.go` - 常量块追加 17 行 Phase 34 新 Code 字面量
- `internal/claudedock/errcodes/codes_test.go` - Registry 下限 15 → 30
- `cmd/claudedock/main.go` - AddCommand 追加 newExplainCmd() + DisableFlagParsing switch case "explain"
...
| ---- | ---- |
| `go build ./...` | PASS |
| `go test ./internal/claudedock/errcodes/ -count=1` | PASS（9 测试，含 4 新增） |
| `go test ./cmd/claudedock/ -run TestExplain_ -count=1` | PASS（3 子进程测试） |
...
- **Files modified:** `internal/claudedock/errcodes/explanations.go`
...
- ✅ ROADMAP §Phase 34 Success Criteria SC#1（Registry 唯一/中文/next_action/无前缀冲突）+ SC#8（claudedock explain CLI）全部交付
- ⚠️ Plan 02 / 03 新增 Code 时务必同步注册 ExtendedExplanations，否则 CI TestAllCodesHaveExplanations 会立即失败（设计如此）

## Self-Check

- [x] `internal/claudedock/errcodes/state.go` exists ✅
- [x] `internal/claudedock/errcodes/system.go` exists ✅
- [x] `internal/claudedock/errcodes/ssh.go` exists ✅
- [x] `internal/claudedock/errcodes/auth.go` exists ✅
- [x] `internal/claudedock/errcodes/disk.go` exists ✅
- [x] `internal/claudedock/errcodes/explanations.go` exists ✅
- [x] `internal/claudedock/errcodes/explanations_test.go` exists ✅
- [x] `cmd/claudedock/explain.go` exists ✅
- [x] `cmd/claudedock/explain_test.go` exists ✅
