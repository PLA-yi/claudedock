# Phase 34: claudedock doctor v3 + 错误码统一 - Context
...
1. **`claudedock doctor` 五维度自检**（network / auth / ssh / mount / disk），每项输出 `[符号] 中文原因（建议: ... | 错误码: ...）` 四要素（REQ-F6-A / B、PITFALLS M14）
2. **`claudedock doctor --fix`** 至少 5 类自动修复（mutagen agent restart / FUSE 残留挂载清理 / known_hosts 冲突 / OAuth token 过期 refresh / DNS 缓存污染 flush），Q9 拍板默认全幂等 + 危险操作 stdin `y/N` + CI `--yes` 跳过（REQ-F6-C）
3. **`claudedock doctor --verbose|--json|NO_COLOR`** + 退出码 `0/1/2`（与 `brew doctor` 语义对齐，REQ-F6-D）
4. **`claudedock explain <code>`** 子命令（对标 `rustc --explain`），数据源 = Registry Entry 的 `Message`/`NextAction` + 同包追加的 `ExtendedExplanations map[Code]string`（REQ-F8-C）
...
6. **降级历史第一屏**：doctor 启动时**首屏**即从 `~/.claudedock/last-session.json` 读 `DowngradeChain` + `ActualMode` + `ConflictCount` + `ReconnectCount`，渲染历史 banner 后再跑 5 维度检查（PITFALLS M13 终验）
7. **mergerfs / Mutagen 版本 / AppArmor 三大已知坑**通过 mount 维度检查项落地：
   ...
   - Mutagen：客户端 embed 版本 vs 容器内 `/etc/claudedock/mutagen.version` 比对（PITFALLS C4）
...
- 重写 Phase 31/32 既有 `internal/claudedock/ssh_doctor.go`（v2.0 quick task 260417-0w4 已 ship 的 SSH 密钥体检子命令）—— 本阶段把 `ssh_doctor` 作为 doctor `ssh` 维度的**一个检查项**集成进来，不动既有签名
...
- **D-01**：本阶段新增 `internal/claudedock/doctor/` 子包（与 `errcodes/` 同级），按 5 个维度 + 共享渲染拆分：
  ...
  - `doctor/disk.go` — 本地 `~/.claudedock/` 可用空间 / 远端 `/workspace` `/var/lib/claude-persist` 容器内 disk usage / mutagen daemon data 目录大小（无限增长警告）
...
- **D-02**：`internal/claudedock/errcodes/` 包追加 5 个新文件 + 1 个 explanations 文件：
  ...
  - `errcodes/explanations.go` — 包级 `ExtendedExplanations map[Code]string`，每条对应一段 ≥ 200 中文字符的 `claudedock explain` 长说明（含触发场景 / 根本原因 / 复现方式 / 修复路径），由 `init()` 注册（与 `MustRegister` 同样防御重复）
...
- **D-03**：`cmd/claudedock/main.go` 新增两个 cobra 子命令：
  - `doctor [domain]`（domain ∈ `network|auth|ssh|mount|disk|all`，默认 `all`），flag：`--fix` / `--verbose` / `--json` / `--yes`；尊重环境变量 `NO_COLOR`
  - `explain <code>`，args 必填（CODE 大小写敏感即可，与 Registry 保持一致），输出 = `Format(code)` 两段 + 空行 + ExtendedExplanations[code]（缺失时 fallback 到「未提供详细说明，运行 claudedock doctor <domain> 查看相关检查项」）
  - 在 `runRoot` 旁边的 `case` 列表（DisableFlagParsing 关闭判定）追加 `"doctor", "explain"` 两个 keyword
- **D-04**：保持 `claudedock ssh doctor` 子命令字面入口不变（v2.0 quick task 260417-0w4 已发布，不破坏用户肌肉记忆），但 `claudedock doctor ssh` 内部调用同一个 `RunSSHDoctor`（共享底层 + 双入口共存，便于过渡）；后续 v3.1 可考虑 deprecate 旧入口

### `claudedock doctor` 子命令结构（GA-2）
...
- **D-05**：cobra 结构选定 **`doctor [domain]` 子命令 + 顶层 flag**，**不**采用 `doctor --domain=mount` flag 风格：
  ...
  3. 历史 `claudedock ssh doctor` 子命令 = `claudedock doctor ssh`（D-04 双入口）正交不撞
  4. `--fix`/`--verbose`/`--json`/`--yes` 都是顶层 flag（不是 `doctor` 子命令独占），便于 v3.1 给 `claudedock env check --fix` 等其它子命令复用
- **D-06**：`doctor` 子命令**不需要先 init**也能跑 `network` / `auth`（仅本地 config / DNS）维度；`mount`/`ssh`/`disk` 需要 init + Entry API auth 才能 SSH 到容器；缺 init 时把对应维度标记 `StatusSkip` + 中文原因「未配置网关，跳过；运行 claudedock init 配置后重试」（不算 fail）
...
- **D-08**：单个 check timeout 默认 5s，可被 `Options.CheckTimeout` 覆盖；`--verbose` 模式自动放宽到 30s；timeout 命中时检查项标记 `StatusFail` + 中文原因「检查超时（>%s）」 + 错误码 `SYSTEM_CHECK_TIMEOUT`
  ...
  *[auto] GA-2 推荐项：subcommand 风格（替代 flag 风格：(a) 命令行行业惯例；(b) 与 cobra ValidArgs 内置校验天然契合；(c) `claudedock doctor ssh` 自然兼容现有 `ssh doctor` 入口的双向迁移）*
...
- **D-13**：doctor 启动 banner 输出顺序（M13 验收锚点）：
  1. **第一屏**：`Cloud Claude Doctor` 标题 + 当前 claudedock 版本 + 远端 image_version（如 init + auth 已成功）
  2. **第二段**：上次会话快照（来源 `~/.claudedock/last-session.json`）：
     ...
     - last-session.json 缺失时 → `[!] 未找到上次会话快照（首次运行 claudedock 后再 doctor 即可看到）` + 错误码 `STATE_LAST_SESSION_MISSING` 但不算 fail
...
### `claudedock explain <code>` 数据源（GA-5）

- **D-17**：`explain` 子命令流程：
  1. 大小写敏感匹配 args[0] 到 Registry；未注册 → stderr `未找到错误码 %s；运行 claudedock doctor 查看可用检查项` + exit 4（与 ConfigError 一致）
  2. 输出三段：
     - 段 1：`Format(code)` 两段（错误码 + 中文原因 + 建议），与 doctor / 其它路径完全一致的字面量
     - 段 2：空行后 `详细说明：` + `ExtendedExplanations[code]` 或 fallback 到「未提供详细说明，运行 claudedock doctor <domain> 查看相关检查项」
     - 段 3（Verbose 时追加，`--verbose` flag）：触发场景 / 根本原因 / 复现命令（如有）/ 相关 doctor check 名（来源：`internal/claudedock/doctor/explain_index.go` 的 `RelatedChecks map[Code][]string`，非必填字段）
  3. 退出码：找到 = 0；未找到 = 4
- **D-18**：`ExtendedExplanations` 单条建议 200~500 中文字符；模板：
  ...
  修复路径：<完整修复步骤，与 doctor --fix 自动修复重叠时显式标注「也可运行 claudedock doctor mount --fix 自动处理」>
...
- **D-19**：5 维度检查项分布（**总计 ≥ 17 项**，REQ-F6-A 「五维度」字面达标）：
  ...
  | mount | `mutagen_version_match` | 远端（`/etc/claudedock/mutagen.version` 比对） | `MOUNT_MUTAGEN_VERSION_SKEW` |
  ...
  | disk | `local_disk` | 本地（`statfs(~/.claudedock/)`，< 500MB warn / < 100MB fail） | `DISK_LOCAL_LOW` |
  | disk | `container_disk` | 远端（`df -BM /workspace`，< 500MB warn / < 100MB fail） | `DISK_CONTAINER_LOW` |
  | disk | `mutagen_data_size` | 本地（`du -sh ~/.claudedock/mutagen/`，> 1GB warn） | `DISK_MUTAGEN_DATA_BLOAT` |

- **D-20**：远端 SSH 检查统一通过 `internal/claudedock/doctor/remote_runner.go` 的 `RemoteRunner{conn *ssh.Client}.RunScript(name, script string) (stdout, stderr, err)` 包装（沿用 `ssh_doctor.go` 的 `runSSHSession` 模式）；conn 由 `RunDoctor` 在第一次需要远端的 check 之前 lazy 建立，跑完后 `defer conn.Close()`；如远端连接失败，所有标记 `RequiresRemote=true` 的 check 标记 `StatusSkip` + 中文原因「未能连接远端容器（原因: ...）」；不让一次远端断开把整轮 doctor 拖死
...
- **D-21**：本阶段必须新增的错误码（≥ 11 条，与既有 25 条合计 ≥ 36 条；命名匹配 `^[A-Z]+_[A-Z]+_[A-Z0-9]+(_[A-Z0-9]+)*$`，已 Phase 31 D-02 放宽允许 4 段）：
  ...
  |------|----------|----------|-----------|
  | `STATE_LAST_SESSION_MISSING` | Info | doctor 第一屏读 `~/.claudedock/last-session.json` 不存在 | M13 / D-13 |
  ...
  | `SYSTEM_FUSE_RESIDUAL_MOUNT` | Warn | 本地 `mount` 输出仍含旧的 sshfs / macfuse 挂载（claudedock 异常退出留下） | C3 / REQ-F6-C |
  ...
  | `AUTH_CONFIG_MISSING` | Fatal | `~/.claudedock/config.yaml` 不存在 / 解析失败 | REQ-F6-A 前置 |
  ...
  | `DISK_LOCAL_LOW` | Warn / Error | `statfs(~/.claudedock/)` 可用 < 500MB / 100MB | REQ-F6-A |
  | `DISK_CONTAINER_LOW` | Warn / Error | 远端 `df /workspace` 可用 < 500MB / 100MB | REQ-F6-A |
  | `DISK_MUTAGEN_DATA_BLOAT` | Warn | `du -sh ~/.claudedock/mutagen/` > 1GB | M16 边缘 |
...
- **D-24**：Plan 02 / 03 的测试策略：
  ...
  3. **远端集成测试**（Plan 03）：沿用 Phase 31 `integration_test.go` 风格，docker compose fixture（已有 `internal/claudedock/test-fixture-up.sh`）跑 1 个 happy path（`doctor mount` 在 v3 镜像上 5 项全 pass）+ 1 个 fail 注入（`docker exec` 内 `pkill mutagen-agent` 后 doctor mount 必命中 `MOUNT_MUTAGEN_VERSION_SKEW` 或 `MOUNT_MUTAGEN_DAEMON_UNAVAILABLE` 之一）；docker daemon 不可用时 `t.Skip` 留 Phase 35 真机
  4. **explain 测试**：遍历 `errcodes.Registry()`，断言每个 Code 在 `ExtendedExplanations` 或 `ExplainExempt` 中至少出现一次；`claudedock explain MOUNT_MUTAGEN_VERSION_SKEW` 子进程级测试（用 `os/exec` 跑 build 出来的 claudedock 二进制）放 `cmd/claudedock/explain_test.go`
  5. **CI grep 断言**（M14 终验）：`go run ./cmd/claudedock doctor --json` 输出 jq 解析所有 status=warn|fail 的 check 必须含 `next_action` 字段；非 JSON 模式 `grep -L "建议:"` 结果必须为空（doctor 二进制 sample 输出在 CI artifact 里）
...
- **D-25**：从 Phase 31 D-32 / Phase 32 D-27 预留接口直接复用：
  ...
  - `~/.claudedock/last-session.json` schema 已含 `DowngradeChain` / `ConflictCount` / `TmuxSession` / `ClientRole` / `ReconnectCount`，**本阶段读取，不扩 schema**（如需 doctor-only 字段，开 v3.1 phase 升 schema_version=2）
  - `internal/claudedock/oauth_check.go` 在 Phase 31 已实现远端 OAuth 三态判定（NotFound / Expired / ExpiringSoon），doctor `auth.oauth_credentials` check 直接复用
- **D-26**：从 Phase 29 落地的容器侧基线（doctor 远端验收对象）：
  - `/etc/claudedock/mutagen.version`（D-05）→ doctor `mount.mutagen_version_match`
...
- **D-29**：本阶段拆分为 **3 plans**（与 ROADMAP 字面 `0/3 plans` 一致）：
  - **Plan 01 — 错误码注册表收口 + `claudedock explain`**（Wave 1，无依赖）
    ...
    - `cmd/claudedock/explain.go` 新建：cobra 子命令 + Args 校验 + 输出三段
    - `cmd/claudedock/main.go`：`AddCommand(newExplainCmd())` + DisableFlagParsing case 追加 `"explain"`
    - 单元测试：`cmd/claudedock/explain_test.go`（os/exec 跑二进制）+ `errcodes/explanations_test.go`（map 完备性）
  - **Plan 02 — `claudedock doctor` 5 维度框架**（Wave 2，depends_on Plan 01）
    - `internal/claudedock/doctor/` 新包：`doctor.go` / `check.go` / `network.go` / `auth.go` / `ssh.go` / `mount.go` / `disk.go` / `remote_runner.go` / `render.go`（不含 `fix.go`，Plan 03 落）
    - 17 项 check 的 pass/warn/fail/skip 4 路径全实现
    - 第一屏降级历史渲染（D-13）+ JSON schema（D-15）+ 退出码（D-16）
    - `cmd/claudedock/doctor.go` 新建：cobra 子命令 + ValidArgs + `--verbose|--json|--yes`（`--fix` 此 plan 仅注册 flag，转发到 Plan 03 的 fixer registry，但 fixer 为空时只输出 `[!] --fix 自动修复将在 doctor.fix.go 实现（Plan 03）`）
    - `cmd/claudedock/main.go`：`AddCommand(newDoctorCmd())` + DisableFlagParsing case 追加 `"doctor"`
  ...
  - **Plan 03 — `doctor --fix` 5 类自动修复 + 集成测试**（Wave 3，depends_on Plan 02）
    ...
    - CI grep 断言（M14 终验）脚本：`scripts/ci-doctor-grep.sh` 跑一次 `claudedock doctor --json` + jq 校验所有 warn/fail 含 `next_action`，非 JSON 输出 `grep -L "建议:"` 结果为空
...
- `claudedock doctor --json` 是否兼容 `jq -c` 的紧凑输出（建议默认 indent 2，由 jq 自己处理）
- `AUTH_OAUTH_REFRESH_FAILED` 的修复路径是否打印"请在容器内 `claude login`"（建议是，与 Phase 31 `NET_OAUTH_NOT_FOUND` next_action 一致）
- `SSH_KNOWN_HOSTS_CONFLICT` 的 `ssh-keygen -R` 是否支持指定 known_hosts 文件路径（建议读 `~/.ssh/known_hosts` 默认即可，与 Phase 33 `claudedock ssh doctor` 一致）
- explain 是否支持模糊匹配（如输入 `MOUNT_MUTAGEN` 列出全部 6 条 MOUNT_MUTAGEN_*）（建议本阶段不做，留 v3.1）
- `claudedock doctor` 不带 init 时是否自动跳过远端 check（建议是，D-06 行为）
...
- `.planning/phases/29-v3-worker/29-CONTEXT.md` §D-05 / D-11 / D-14 / D-23 — `/etc/claudedock/mutagen.version` 文件位置 / mergerfs 挂载参数（C2 验收锚点）/ sshd KeepAlive 基线 / AppArmor override 路径（C6）
...
- `internal/claudedock/errcodes/codes.go` — `Registry` / `MustRegister` / `Lookup` / `Format` / `Severity` / `codeRe`（本阶段 5 个新文件遵循同样模式）
- `internal/claudedock/errcodes/mount.go` — Phase 31 12 条 MOUNT_* + NET_* 注册示范
- `internal/claudedock/errcodes/session.go` — Phase 32 7 条 SESSION_* + NET_* 注册示范
- `internal/claudedock/errcodes/net.go` — NET_* 已有 OAuth + reconnect 5 条
- `internal/claudedock/ssh_doctor.go` — `RunSSHDoctor` / `runSSHSession` / `parseScanOutput` / `applyFixes` / `Print`（本阶段 doctor `ssh.workspace_ssh_keys` check 复用 + render.go 借鉴样式）
- `internal/claudedock/last_session.go` — `LastSessionSnapshot` schema（doctor 第一屏数据源）+ `WriteLastSession` 原子写入模式
- `internal/claudedock/oauth_check.go` — Phase 31 OAuth 三态检查（doctor `auth.oauth_credentials` check 复用）
- `internal/claudedock/exitcodes.go` — `ExitOK/AuthFailed/NetworkError/Timeout/ConfigError/InternalError/OAuthNotFound/OAuthExpired/MountForceFailed`（doctor 退出码语义对齐）
- `internal/claudedock/colors.go` — ansi helper（render.go 直接复用）
- `internal/claudedock/entry.go` — `EntryClient.AuthenticateAndWait`（doctor `auth.entry_token_valid` 复用 + `--fix` AUTH_TOKEN_EXPIRED 调）
- `internal/claudedock/mount_strategy.go` — `MountConfig` 字段（KeepAliveInterval / NoColor / 等，doctor `ssh.keepalive_config` 校验入口）
- `internal/claudedock/sshfs_watcher.go` — sshfs 抖动 watcher 状态（doctor `mount.sshfs_mountpoint` 可读取最近一次状态）
- `internal/claudedock/mount_mutagen.go` — Mutagen daemon / sync session helper（doctor mount 检查 + `--fix` daemon restart 复用）
- `internal/claudedock/mutagen_bin.go` — embed 二进制版本字符串（doctor `mount.mutagen_version_match` 客户端侧版本来源）
- `cmd/claudedock/main.go` — cobra root + `DisableFlagParsing` 关闭判定 case 列表 + 子命令注册位置
- `cmd/claudedock/sessions.go` / `sync.go` — 现有 cobra 子命令注册示范
...
- `deploy/docker/managed-user/Dockerfile` — `/etc/claudedock/mutagen.version` 写入位置（doctor `mount.mutagen_version_match` 远端读取来源）
- `internal/controlplane/http/admin_claude_accounts.go` — `STATE_VOLUME_IN_USE_001` 字面量（D-27 本阶段补 Registry 注册）
- `internal/claudedock/integration_test.go` — Phase 31/32 docker fixture 测试样例（Plan 03 集成测试沿用）
- `internal/claudedock/test-fixture-up.sh` / `test-fixture-down.sh` — docker compose fixture 启停脚本

### 新增文件预告

- `internal/claudedock/doctor/doctor.go` — 顶层 RunDoctor + Options + Report
- `internal/claudedock/doctor/check.go` — Check struct + Checker interface + runWithTimeout
- `internal/claudedock/doctor/network.go` / `auth.go` / `ssh.go` / `mount.go` / `disk.go` — 5 维度检查项
- `internal/claudedock/doctor/remote_runner.go` — SSH 远端命令执行包装
- `internal/claudedock/doctor/fix.go` — FixerRegistry + 5 类修复 + confirmDestructive
- `internal/claudedock/doctor/render.go` — Text + JSON 双输出
- `internal/claudedock/doctor/explain_index.go` — RelatedChecks map[Code][]string（可选）
- `internal/claudedock/errcodes/state.go` / `system.go` / `ssh.go` / `auth.go` / `disk.go` — ≥ 17 条新 Code 注册
- `internal/claudedock/errcodes/explanations.go` — ExtendedExplanations map + ExplainExempt set
- `internal/claudedock/errcodes/explanations_test.go` — map 完备性 + Registry 覆盖断言
- `cmd/claudedock/doctor.go` — cobra 子命令
- `cmd/claudedock/explain.go` — cobra 子命令
- `cmd/claudedock/explain_test.go` — os/exec 子进程级测试
...
- 现有 `claudedock ssh doctor` 子命令（v2.0 quick task 260417-0w4）→ 不动入口，本阶段 `doctor ssh` 维度内部调用 `RunSSHDoctor` 共享底层

### 既有反模式 / 已知坑

- `claudedock` 二进制在 Phase 31 已 embed Mutagen 4 平台二进制 ~12MB；本阶段不能再大幅膨胀（doctor 文件全 Go 源代码不影响 size，但 ExtendedExplanations 17 条 × ~400 字符 ≈ 7KB 字符串可忽略）
...
- 历史 `claudedock ssh doctor` 子命令的 cobra 入口字面量是 `claudedock ssh doctor`（subcommand 嵌套），本阶段新加的 `claudedock doctor [domain]` 是顶层 `doctor` + ValidArgs，不与之冲突；但 `--help` 会同时列出，文档需要解释清楚两者关系（Plan 02 README 章节）
...
- ROADMAP §Phase 34 Success Criteria 第 3 条 `CI grep claudedock doctor 输出，所有 [!] 与 [✗] 行必须含"建议:"子串`：是 PITFALLS M14 的关键回归 — 本阶段必须有 `scripts/ci-doctor-grep.sh` 并接入 CI（建议挂 Plan 03 收尾 task）
- ROADMAP §Phase 34 Success Criteria 第 6 条 `强制让 claudedock 上次连接静默降级到 sshfs-only 后，claudedock doctor 第一屏必须展示降级历史`：M13 终验由本阶段闭合 — Plan 02 必须有用例构造 last-session.json 含 DowngradeChain 后跑 doctor 断言 stdout 包含 `[降级]` 行
- ROADMAP §Phase 34 Success Criteria 第 7 条 `mergerfs 参数被人为篡改后，doctor mount 必须输出错误码 + 修复命令`：是 C2 + M14 联合验收 — Plan 03 集成测试构造 `docker exec <ctr> umount /workspace && mount -o cor:1 ...`（修改 readdir 参数）后断言 doctor mount 输出含 `MOUNT_MERGERFS_FAILED` + next_action 中的 `claudedock doctor mount --fix`
- ROADMAP §Phase 34 Success Criteria 第 8 条 `claudedock explain MOUNT_MUTAGEN_VERSION_SKEW 等任意错误码均能返回中文详细说明 + 常见修复步骤`：本阶段 ExtendedExplanations 必须覆盖所有非 informational Code（D-18），CI 测试遍历 Registry 断言（D-24 第 4 条）
...
- v3.0 PROJECT.md 强调「零增量特权」——本阶段 doctor 在容器内 exec 的所有命令都不需要新增 capability（mergerfs xattr / sshd -T / df / du 全部用户态）；本地 fix 涉及 `dscacheutil killall -HUP mDNSResponder`（macOS）需要 root 时，提示用户 `sudo` 而非 claudedock 自动 elevation
...
- **doctor 历史报告归档**：每次 `doctor` 把 Report 落盘到 `~/.claudedock/doctor-reports/<timestamp>.json`；运维场景有用但 v3.0 不做（与 OOS-A11 不上报服务端配套）
- **`claudedock explain --list` 列出所有 Code**：本阶段 explain 必须传一个 Code；列表能力推迟 v3.1（也可以直接 `claudedock doctor --json | jq '.checks[].code'` 间接获得）
...
- **doctor `mount.mergerfs_branches` 自动 remount 修复**：mergerfs 重挂涉及容器侧 mount 操作（C3 风险），本阶段 `--fix` 不自动重挂；只输出 next_action 提示用户 `claudedock exec sudo mount -o ...`，自动重挂留 Phase 35 真机验证后再决定
- **`claudedock doctor --report-bug`**：把 Report 复制到剪贴板 + 打开浏览器到 GitHub Issues；OOS-A11 周边，推迟 v3.1
