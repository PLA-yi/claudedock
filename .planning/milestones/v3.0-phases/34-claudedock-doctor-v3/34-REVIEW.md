...
本次审阅覆盖 Phase 34（claudedock doctor v3 + explain + errcodes 8 域闭合）涉及的 41 个源文件，包括：
`cmd/claudedock/{doctor,explain,main}.go`、`internal/claudedock/doctor/` 全套包、
`internal/claudedock/errcodes/` 8 域注册表与长说明、`internal/claudedock/{input_buffer,session,mount_strategy,last_session,colors}.go`、
...
**File:** `internal/claudedock/input_buffer.go:99-110`
**Issue:**
...
**File:** `internal/claudedock/session.go:774-788`（写）+ `internal/claudedock/session.go:692, 654-658, 666`（读 / 复位）
**Issue:**
...
**File:** `internal/claudedock/doctor/check.go:35-63` + `internal/claudedock/doctor/remote_runner.go:13-15`
**Issue:**
...
**File:** `internal/claudedock/doctor/auth.go:39-57`
**Issue:**
...
**File:** `internal/claudedock/doctor/fix.go:107-111`
**Issue:**
...
claudedock 实际跑 mutagen 用的是 `internal/claudedock/mutagen_bin/<os>_<arch>/mutagen`（Phase 31 embed 路径，
`MutagenBinaryVersion` 常量与 mount.go::checkMutagenVersionMatch 里读取的远端 `/etc/claudedock/mutagen.version` 比对）。
...
```go
import "github.com/claudedock/claudedock/internal/claudedock" // 已 import

func realExecMutagenDaemon(ctx context.Context, action string) error {
    bin := claudedock.MutagenBinaryPath() // 需要在 claudedock 包导出
    if bin == "" {
...
如果 `claudedock` 包暂时没有导出 path 解析，可以直接复用 `claudedock.MutagenBinaryVersion` 同模块的辅助函数。
...
**File:** `internal/claudedock/doctor/disk.go:62-71`
**Issue:**
...
**File:** `internal/claudedock/doctor/fix.go:117-125`
**Issue:**
...
**File:** `internal/claudedock/doctor/render.go:78-80` + `doctor.go:267`
**Issue:**
...
**File:** `internal/claudedock/doctor/doctor.go:110, 176`
**Issue:**
...
**File:** `internal/claudedock/doctor/mount.go:147-153`
**Issue:**
...
**File:** `internal/claudedock/doctor/ssh.go:73-76`
**Issue:**
...
**File:** `internal/claudedock/doctor/render.go:31-33`
**Issue:**
...
**File:** `internal/claudedock/doctor/fix.go:262-274`
**Issue:**
`opts.Yes=true` 跳过 `confirmDestructive` 内部 prompt，但底层 sudo 自己会要密码。
若用户没有 NOPASSWD sudoers 或者运行环境没有 TTY（CI），sudo 将无限阻塞等密码 → claudedock 看着就是 hang。
...
**File:** `cmd/claudedock/explain.go:37-38`、`cmd/claudedock/doctor.go:71-73, 92-94, 104-110`
**Issue:**
...
**File:** `internal/claudedock/doctor/doctor.go:135-151`
**Issue:**
...
SSH 拨号默认 `claudedock.SSHConnect` 没有 timeout（或者很长），最坏 4 个域 × 拨号 timeout = 体检整体超时。
