...
files_modified:
  ...
  - internal/claudedock/doctor/check.go
  - internal/claudedock/doctor/network.go
  - internal/claudedock/doctor/auth.go
  - internal/claudedock/doctor/ssh.go
  - internal/claudedock/doctor/mount.go
  - internal/claudedock/doctor/disk.go
  - internal/claudedock/doctor/remote_runner.go
  - internal/claudedock/doctor/render.go
  - internal/claudedock/doctor/doctor_test.go
  - internal/claudedock/doctor/network_test.go
  - internal/claudedock/doctor/auth_test.go
  - internal/claudedock/doctor/ssh_test.go
  - internal/claudedock/doctor/mount_test.go
  - internal/claudedock/doctor/disk_test.go
  - internal/claudedock/doctor/render_test.go
  - internal/claudedock/colors.go
  - cmd/claudedock/doctor.go
  - cmd/claudedock/main.go

must_haves:
  truths:
    - "claudedock doctor 覆盖 5 维度（network / auth / ssh / mount / disk），共 ≥ 17 项 check（REQ-F6-A / ROADMAP §Phase 34 SC#2 / CONTEXT D-19）"
  ...
  key_links:
    - from: "internal/claudedock/doctor/doctor.go::RunDoctor"
    ...
    - from: "internal/claudedock/doctor/render.go::renderDowngradeBanner"
      to: "claudedock.LastSessionSnapshot.DowngradeChain"
      via: "每个 DowngradeStep 输出 '[降级] <from> → <to> | 原因 [<reason_code>] <reason_message>'"
      pattern: "\\[降级\\]"
    - from: "cmd/claudedock/doctor.go::runDoctor"
...
<objective>
交付 `claudedock doctor` 5 维度自检框架（不含 --fix 实施，Plan 03 落）。具体实现：
1. `internal/claudedock/doctor/` 新包，按 CONTEXT D-01 / PATTERNS §2.1-§2.10 拆分 10 个 .go 文件 + 6 个 _test.go；
...
7. `cmd/claudedock/doctor.go` 新建 cobra 子命令 + 4 flag + ValidArgs；`cmd/claudedock/main.go` 注册；
8. `internal/claudedock/colors.go` 内部 `colorEnabled` / `colorize` 首字母改大写导出，供 doctor 包复用（CONTEXT Established Patterns / PATTERNS §2.10 要求）。

Purpose: 本 Plan 作为 **Wave 2 主框架**：提供所有 17 项 check 的 pass/warn/fail/skip 4 路径 + 第一屏降级历史渲染，让用户独立 ship 即可跑 `claudedock doctor` 看完整诊断；`--fix` 由 Plan 03 闭合（本 plan 仅把 flag 注册到位 + 输出占位提示，避免 Plan 03 阻塞本 plan ship）。

Output:
- 1 个新增 doctor/ 子包（9 个 .go 源文件 + 7 个 _test.go）
- 1 个修改的 colors.go（导出 helper）
- 1 个新增 cmd/claudedock/doctor.go
...
@.planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md
@.planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md
@.planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md
@.planning/phases/34-claudedock-doctor-v3/34-01-errcodes-explain-PLAN.md
...
@internal/claudedock/envcheck.go
...
From internal/claudedock/errcodes/codes.go (Plan 01 完成后):
```go
...
From internal/claudedock/last_session.go:
```go
...
// 读取函数（Plan 02 直接用）
func LoadLastSession() (*LastSessionSnapshot, error) // 路径 ~/.claudedock/last-session.json
```

From internal/claudedock/oauth_check.go:
```go
...
From internal/claudedock/ssh_doctor.go:
```go
func RunSSHDoctor(cfg SSHConfig, opts SSHDoctorOptions) (*SSHDoctorResult, error)
```

From internal/claudedock/mount_mutagen.go (line 230-237):
```go
// 3) 版本握手
if connA != nil {
...
From internal/claudedock/mutagen_bin.go:
```go
const MutagenBinaryVersion = "v0.18.1"
```

From internal/claudedock/colors.go (既有 lower-case helper，本 plan Task 2.1 改大写导出):
```go
...
From internal/claudedock/mount_strategy.go:
```go
...
From internal/claudedock/config.go:
```go
func LoadConfig() (*Config, error)      // 读 ~/.claudedock/config.yaml
...
From cmd/claudedock/main.go (line 18-25):
```go
const (
    exitOK            = 0
    exitAuthFailed    = 1
...
<task type="execute">
  ...
  <files>internal/claudedock/colors.go</files>

  <read_first>
    - internal/claudedock/colors.go（完整文件）
    - 全部调用点（grep 索引）：`rg -n "colorEnabled\|colorize" internal/claudedock/`
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.10（「不引入新颜色 helper」→ 必须导出既有）
  </read_first>

  <action>
**第 1 步：** 在 `internal/claudedock/colors.go` 中把 `func colorEnabled` 改名为 `func ColorEnabled`，`func colorize` 改名为 `func Colorize`（首字母大写导出）；如有内部调用也同步改名。

**第 2 步：** 使用 Grep 检索仓库内所有 `colorEnabled(` 与 `colorize(` 调用位置（不限于 `internal/claudedock/` 包），逐一替换成 `ColorEnabled(` / `Colorize(`。

**第 3 步（ANSI 常量导出）：** 导出 4 个 ANSI 常量：把 `internal/claudedock/colors.go` 中的 `ansiGreen` → `AnsiGreen`、`ansiYellow` → `AnsiYellow`、`ansiRed` → `AnsiRed`、`ansiGray` → `AnsiGray`（首字母大写导出），同时全仓 grep 替换所有调用点（包括 `ssh_doctor.go` 等 v2 文件内的 `ansiGreen / ansiYellow / ansiRed / ansiGray` 引用）。Task 2.9 render.go 的 `pickIcon` 直接依赖 `claudedock.AnsiGreen` 等导出名，若未做此步编译失败。

**executor 注意（SSHConnect 无需任何改动）：** `claudedock.SSHConnect` 已于 Phase 32-02 在 `internal/claudedock/ssh.go:200` 作为 export 包装就位（`func SSHConnect(cfg SSHConfig) (*ssh.Client, error) { return sshConnect(cfg) }`），Task 2.10 RunDoctor 的 lazy connect 分支 `claudedock.SSHConnect(sshCfg)` 直接调用即可，本 plan 无需任何重命名 / 导出操作；**不要**去改 `internal/claudedock/ssh_doctor.go`（该文件只调用 `sshConnect`，没有定义；改名会破坏 Phase 32-02 已交付的公共 API）。
...
- 既有 `internal/claudedock/` 包内测试 `go test ./internal/claudedock/ -count=1 -short` PASS
...
- 改动 ANSI 常量字面值（`\x1b[32m` 等）
  ...
  <acceptance_criteria>
    - `grep -q "^func ColorEnabled" internal/claudedock/colors.go` 命中
    - `grep -q "^func Colorize" internal/claudedock/colors.go` 命中
    - `grep -qE "^\s*AnsiGreen\s*=" internal/claudedock/colors.go` 命中（colors.go 使用分组 `const ( ... )` 形式）
    - `grep -qE "\bAnsiYellow\b" internal/claudedock/colors.go` 命中
    - `grep -qE "\bAnsiRed\b" internal/claudedock/colors.go` 命中
    - `grep -qE "\bAnsiGray\b" internal/claudedock/colors.go` 命中
    - `! grep -nE "^func colorEnabled\b|^func colorize\b" internal/claudedock/colors.go` 为空（小写版本已完全删除）
    - `! rg -nE "\bcolorEnabled\(|\bcolorize\(" internal/claudedock/ cmd/claudedock/` 结果为空（所有调用点已改大写）
    - `! rg -nE "\bansiGreen\b|\bansiYellow\b|\bansiRed\b|\bansiGray\b" internal/claudedock/ cmd/claudedock/` 结果为空（所有 ANSI 小写引用已改大写）
    - `go build ./...` 退出码 = 0
    - `go vet ./...` 退出码 = 0
    - `go test ./internal/claudedock/ -count=1 -short` 退出码 = 0（既有测试不回归）
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/doctor.go, internal/claudedock/doctor/check.go</files>

  <read_first>
    - internal/claudedock/ssh_doctor.go（line 18-94 FileReport + RunSSHDoctor 控制流）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-01（Report/Options/Status 字段）+ §D-07（串行执行）+ §D-08（5s/30s timeout）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §5.1（RESEARCH Go struct tag 表）+ §5.2（退出码）+ §5.3（Status 不降级）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.1 / §2.2 adapt 列表
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/doctor.go`（**逐字符按 RESEARCH §5.1 struct tag 表**）：**

```go
// Package doctor — Phase 34 Plan 02：claudedock doctor 五维度自检框架。
//
...
// DowngradeStep 源自 claudedock.LastSessionSnapshot.DowngradeChain；doctor 只读不写。
type DowngradeStep struct {
...
type Report struct {
	...
	CloudClaudeVer   string           `json:"claudedock_version,omitempty"`
...
func RunDoctor(ctx context.Context, opts Options) (*Report, error) {
	...
	// 4) 未 init (claudedock.LoadConfig 失败) → mount/ssh/disk 全部 StatusSkip + 中文原因（D-06）
	// 5) 聚合 Summary 返回
	panic("executor: 按本注释 5 步实现 RunDoctor；参考 ssh_doctor.RunSSHDoctor 控制流 + PATTERNS §2.1")
}
```

**(b) 新建 `internal/claudedock/doctor/check.go`（**RESEARCH §5.1 Check struct + PATTERNS §2.2 Checker interface**）：**
...
- 把 `SchemaVersion` 字段加 `omitempty`（必须始终输出 `"schema_version": 1`）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/doctor.go && test -f internal/claudedock/doctor/check.go` 退出码 = 0
    - `grep -q "^package doctor$" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "type Report struct" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "SchemaVersion\s*int\s*\`json:\"schema_version\"\`$" internal/claudedock/doctor/doctor.go` 命中（**不带 omitempty**）
    - `grep -q "type Options struct" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "type DowngradeBanner struct" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "func RunDoctor(ctx context.Context, opts Options) (\\*Report, error)" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "StatusPass Status = \"pass\"" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "StatusSkip Status = \"skip\"" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "type Check struct" internal/claudedock/doctor/check.go` 命中
    - `grep -q "type Checker interface" internal/claudedock/doctor/check.go` 命中
    - `grep -q "func runWithTimeout" internal/claudedock/doctor/check.go` 命中
    - `grep -q "errcodes.SYSTEM_CHECK_TIMEOUT" internal/claudedock/doctor/check.go` 命中（timeout 命中映射）
    - 本 task 完成后单独 build 可能失败（RunDoctor panic 占位 + fmtSprintf 占位），**Task 2.10 会修复**；本 task acceptance 允许 `go vet ./internal/claudedock/doctor/...` 不通过（结构先到位）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.3: 新建 doctor/remote_runner.go — RemoteRunner interface + sshRemoteRunner 实现（CONTEXT D-20 / PATTERNS §2.8）</name>
  <files>internal/claudedock/doctor/remote_runner.go</files>

  <read_first>
    - internal/claudedock/ssh_doctor.go（line 96-117 `runSSHSession` stdout/stderr 收集模板）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-20（lazy connect + StatusSkip 降级）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §9 第 1 条（interface vs struct 拍板为 interface）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.8
  </read_first>

  <action>
新建 `internal/claudedock/doctor/remote_runner.go`：
...
- 修改 `ssh_doctor.go` 的既有 `runSSHSession`（双份实现允许，保持 Plan 02 与 ssh_doctor 解耦）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/remote_runner.go` 退出码 = 0
    - `grep -q "type RemoteRunner interface" internal/claudedock/doctor/remote_runner.go` 命中
    - `grep -q "RunScript(name, script string) (stdout, stderr string, err error)" internal/claudedock/doctor/remote_runner.go` 命中
    - `grep -q "type sshRemoteRunner struct" internal/claudedock/doctor/remote_runner.go` 命中
    - `grep -q "func NewSSHRemoteRunner" internal/claudedock/doctor/remote_runner.go` 命中
    - `grep -q "golang.org/x/crypto/ssh" internal/claudedock/doctor/remote_runner.go` 命中
    - `go build ./internal/claudedock/doctor/...` 退出码 = 0（独立可编译，不依赖未完成 task）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.4: 新建 doctor/network.go + network_test.go — 3 check（dns_resolve / gateway_reachable / egress_ip_visible）</name>
  <files>internal/claudedock/doctor/network.go, internal/claudedock/doctor/network_test.go</files>

  <read_first>
    - internal/claudedock/envcheck.go（line 30-65 run helper 模式 + 远端 curl）
    - internal/claudedock/entry.go（HTTP client + AuthenticateAndWait / gateway 解析）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-19（network 3 check 错误码映射）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.1 完整命令表 + gotcha
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.3（包级 var mock `lookupHost` / HTTP client）
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/network.go`（3 check + 包级 var mock 注入点）：**
...
func checkDNSResolve(ctx context.Context, host string) Check {
	if host == "" {
		return newSkip("network", "dns_resolve", "未配置网关，跳过；运行 claudedock init 配置后重试")
	}
...
func checkGatewayReachable(ctx context.Context, gateway string) Check {
	if gateway == "" {
		return newSkip("network", "gateway_reachable", "未配置网关，跳过；运行 claudedock init 配置后重试")
	}
...
**(b) 新建 `internal/claudedock/doctor/network_test.go`：**
...
**verbatim 守恒：** 3 条 check 函数名（`checkDNSResolve / checkGatewayReachable / checkEgressIPVisible`）；错误码字面量与 errcodes 完全一致。
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/network.go && test -f internal/claudedock/doctor/network_test.go` 退出码 = 0
    - `grep -q "var lookupHost = net.LookupHost" internal/claudedock/doctor/network.go` 命中（mock 注入点）
    - `grep -q "var httpGet" internal/claudedock/doctor/network.go` 命中
    - `grep -q "func checkDNSResolve" internal/claudedock/doctor/network.go` 命中
    - `grep -q "func checkGatewayReachable" internal/claudedock/doctor/network.go` 命中
    - `grep -q "func checkEgressIPVisible" internal/claudedock/doctor/network.go` 命中
    - `grep -q "errcodes.SYSTEM_DNS_RESOLVE_FAILED" internal/claudedock/doctor/network.go` 命中
    - `grep -q "errcodes.AUTH_GATEWAY_UNREACHABLE" internal/claudedock/doctor/network.go` 命中
    - `grep -q "errcodes.NET_EGRESS_IP_DRIFT" internal/claudedock/doctor/network.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestCheckDNSResolve|TestCheckGatewayReachable|TestCheckEgressIPVisible" -count=1 -v` 退出码 = 0（8 条子测全 PASS）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.5: 新建 doctor/auth.go + auth_test.go — 3 check（config_present / entry_token_valid / oauth_credentials）</name>
  <files>internal/claudedock/doctor/auth.go, internal/claudedock/doctor/auth_test.go</files>

  <read_first>
    - internal/claudedock/config.go（LoadConfig 签名 + 错误信息格式）
    - internal/claudedock/entry.go（AuthenticateAndWait 签名 + AuthResponse 字段）
    - internal/claudedock/oauth_check.go（line 44-63 CheckOAuthCredentials + OAuthStatus 三态）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-19（auth 3 check 错误码映射）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.2（auth 各 check 返回值）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.4
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/auth.go`（3 check）：**
...
import (
	...
	"github.com/claudedock/claudedock/internal/claudedock"
	"github.com/claudedock/claudedock/internal/claudedock/errcodes"
)

// loadConfig 包级 var 注入点（auth_test.go mock 无配置场景）。
var loadConfig = claudedock.LoadConfig

// entryAuthenticate 包级 var（mock Entry API）。签名与 EntryClient.AuthenticateAndWait 对齐但返回简化。
var entryAuthenticate = func(ctx context.Context, gateway, shortID, password string) (*claudedock.AuthResponse, error) {
	client := claudedock.NewEntryClient(gateway)
...
func checkConfigPresent(ctx context.Context) (Check, *claudedock.Config) {
	cfg, err := loadConfig()
	if err != nil {
		c := newFail("auth", "config_present", errcodes.AUTH_CONFIG_MISSING, err.Error())
		return c, nil
	}
	return newPass("auth", "config_present", "~/.claudedock/config.yaml 已加载"), cfg
}
...
func checkEntryTokenValid(ctx context.Context, cfg *claudedock.Config) (Check, *claudedock.AuthResponse) {
	if cfg == nil {
...
// checkOAuthCredentials 复用 claudedock.CheckOAuthCredentials 三态 switch（CONTEXT D-19 / RESEARCH §3.2）。
...
**executor 注意：** `claudedock.AuthResponse` 的实际字段名以 `entry.go` 为准；若 `ImageVersion` / `ClaudeAccountID` 字段名不同，按实际 struct 调整。

**(b) 新建 `internal/claudedock/doctor/auth_test.go`（≥ 6 条子测，覆盖 4 态 + skip + config fail）：**
...
func TestCheckConfigPresent_Fail(t *testing.T) {
	orig := loadConfig
	loadConfig = func() (*claudedock.Config, error) { return nil, fmt.Errorf("~/.claudedock/config.yaml 不存在") }
...
func TestCheckConfigPresent_Pass(t *testing.T) {
	orig := loadConfig
	loadConfig = func() (*claudedock.Config, error) {
		return &claudedock.Config{Gateway: "https://gw.example.com", ShortID: "x", Password: "y"}, nil
	}
	t.Cleanup(func() { loadConfig = orig })
	c, cfg := checkConfigPresent(context.Background())
	if c.Status != StatusPass {
...
func TestCheckEntryTokenValid_401_Warn(t *testing.T) {
	...
	entryAuthenticate = func(ctx context.Context, gw, id, pw string) (*claudedock.AuthResponse, error) {
		return nil, fmt.Errorf("认证失败: HTTP 401")
	}
	t.Cleanup(func() { entryAuthenticate = orig })
	cfg := &claudedock.Config{Gateway: "https://gw.example.com"}
...
func TestCheckEntryTokenValid_Success_Pass(t *testing.T) {
	orig := entryAuthenticate
	entryAuthenticate = func(ctx context.Context, gw, id, pw string) (*claudedock.AuthResponse, error) {
		return &claudedock.AuthResponse{ImageVersion: "v3.0.0", ClaudeAccountID: "acct-1"}, nil
	}
	t.Cleanup(func() { entryAuthenticate = orig })
	cfg := &claudedock.Config{Gateway: "https://gw.example.com"}
	c, resp := checkEntryTokenValid(context.Background(), cfg)
...
- 直接存储用户密码到 `Check.Details`（凭据泄漏风险）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/auth.go && test -f internal/claudedock/doctor/auth_test.go` 退出码 = 0
    - `grep -q "^var loadConfig = claudedock.LoadConfig" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "^var entryAuthenticate" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "func checkConfigPresent" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "func checkEntryTokenValid" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "func checkOAuthCredentials" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "claudedock.CheckOAuthCredentials" internal/claudedock/doctor/auth.go` 命中（复用，不自己写）
    - `grep -q "errcodes.AUTH_CONFIG_MISSING" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "errcodes.AUTH_TOKEN_EXPIRED" internal/claudedock/doctor/auth.go` 命中
    - `grep -q "errcodes.NET_OAUTH_" internal/claudedock/doctor/auth.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestCheckConfigPresent|TestCheckEntryTokenValid|TestCheckOAuthCredentials" -count=1 -v` 退出码 = 0
    - 禁止凭据：`! grep -n "password\|Password" internal/claudedock/doctor/auth.go | grep -v "cfg.Password"` （只允许入参传递，不允许落 Details / log）
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/ssh.go, internal/claudedock/doctor/ssh_test.go</files>

  <read_first>
    - internal/claudedock/ssh_doctor.go（RunSSHDoctor / SSHDoctorResult 完整契约）
    - internal/claudedock/mount_strategy.go（MountConfig.KeepAliveInterval 读法）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-19（ssh 4 check）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.3（sshd -T 命令 + known_hosts HASHED 陷阱）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.5
    - deploy/docker/managed-user/sshd_config（基线 15/8）
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/ssh.go`（4 check）：**
...
func checkWorkspaceSSHKeys(ctx context.Context, sshCfg claudedock.SSHConfig) Check {
	res, err := claudedock.RunSSHDoctor(sshCfg, claudedock.SSHDoctorOptions{Fix: false})
	if err != nil {
		return Check{
			Domain: "ssh", Name: "workspace_ssh_keys", Status: StatusFail,
			Message:    "RunSSHDoctor 失败: " + err.Error(),
			NextAction: "运行 claudedock ssh doctor 查看详细报告",
		}
	}
	status := StatusPass
	problemCount := 0
	...
	return Check{
		...
		NextAction: "运行 claudedock ssh doctor --fix 自动修复",
...
**(b) 新建 `internal/claudedock/doctor/ssh_test.go`（≥ 7 条子测）：**
...
**注意：** `checkWorkspaceSSHKeys` 依赖真实 SSH conn；本 plan 单测层不覆盖（集成测试由 Plan 03 docker fixture 兜底）。
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/ssh.go && test -f internal/claudedock/doctor/ssh_test.go` 退出码 = 0
    - `grep -q "func checkKeepaliveConfig" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "func checkSSHDKeepaliveDrift" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "func checkKnownHosts" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "func checkWorkspaceSSHKeys" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "claudedock.RunSSHDoctor" internal/claudedock/doctor/ssh.go` 命中（复用 v2.0）
    - `grep -q "errcodes.SESSION_KEEPALIVE_TOO_AGGRESSIVE" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "errcodes.SSH_SSHD_KEEPALIVE_DRIFT" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "errcodes.SSH_KNOWN_HOSTS_CONFLICT" internal/claudedock/doctor/ssh.go` 命中
    - `grep -q "golang.org/x/crypto/ssh/knownhosts" internal/claudedock/doctor/ssh.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestCheckKeepalive|TestCheckSSHDKeepalive|TestCheckKnownHosts" -count=1 -v` 退出码 = 0
  </acceptance_criteria>
</task>
...
<task type="execute">
  ...
  <files>internal/claudedock/doctor/mount.go, internal/claudedock/doctor/mount_test.go</files>

  <read_first>
    - internal/claudedock/mount_mutagen.go（line 230-237 版本握手复用点）
    - internal/claudedock/mutagen_bin.go（MutagenBinaryVersion 常量）
    - deploy/scripts/host-preflight.sh（AppArmor 检测逻辑 — Go 改写源）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-19（mount 5 check）+ §D-26 Phase 29 基线
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.4（5 check 命令表）+ §8.1 mergerfs 6 字面量 + §8.3 AppArmor 5-Gate
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.6
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/mount.go`（5 check）：**
...
// checkMutagenVersionMatch 远端 /etc/claudedock/mutagen.version 与 embed 版本比对（C4）。
...
func checkMutagenVersionMatch(ctx context.Context, runner RemoteRunner) Check {
	...
	stdout, _, err := runner.RunScript("mutagen_version", "cat /etc/claudedock/mutagen.version 2>/dev/null")
	if err != nil {
		return newFail("mount", "mutagen_version_match", errcodes.MOUNT_MUTAGEN_VERSION_SKEW,
			claudedock.MutagenBinaryVersion, "(读取失败: "+err.Error()+")")
	}
	remote := strings.TrimSpace(stdout)
	local := claudedock.MutagenBinaryVersion
	if remote == "" {
...
**(b) 新建 `internal/claudedock/doctor/mount_test.go`（≥ 10 条子测）：**
...
- `checkAppArmorFusermount3` exec `apparmor_parser`（只读文件，单测友好）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/mount.go && test -f internal/claudedock/doctor/mount_test.go` 退出码 = 0
    - `grep -q "func checkMutagenVersionMatch" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "func checkMergerfsBranches" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "func checkSSHFSMountpoint" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "func checkFUSEResidual" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "func checkAppArmorFusermount3" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "claudedock.MutagenBinaryVersion" internal/claudedock/doctor/mount.go` 命中
    - `grep -q '"func.readdir=cor:4"' internal/claudedock/doctor/mount.go` 命中（6 字面量之一）
    - `grep -q '"category.create=ff"' internal/claudedock/doctor/mount.go` 命中（6 字面量之一）
    - `grep -q "errcodes.MOUNT_MUTAGEN_VERSION_SKEW" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "errcodes.MOUNT_MERGERFS_FAILED" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "errcodes.MOUNT_SSHFS_DISCONNECTED" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "errcodes.SYSTEM_FUSE_RESIDUAL_MOUNT" internal/claudedock/doctor/mount.go` 命中
    - `grep -q "errcodes.SYSTEM_APPARMOR_FUSERMOUNT3_MISSING" internal/claudedock/doctor/mount.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestCheckMutagen|TestCheckMergerfs|TestCheckSSHFS|TestCheckFUSE|TestCheckAppArmor" -count=1 -v` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.8: 新建 doctor/disk.go + disk_test.go — 3 check（local_disk / container_disk / mutagen_data_size）</name>
  <files>internal/claudedock/doctor/disk.go, internal/claudedock/doctor/disk_test.go</files>

  <read_first>
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-19（disk 3 check） + §Discretion §5（硬编码阈值）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §3.5（Statfs 跨 OS + df 命令）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.7（golang.org/x/sys/unix.Statfs 模板）
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/disk.go`（3 check）：**
...
func checkLocalDisk(ctx context.Context) Check {
	...
	target := filepath.Join(home, ".claudedock")
...
func checkMutagenDataSize(ctx context.Context) Check {
	...
	target := filepath.Join(home, ".claudedock", "mutagen")
	if _, err := os.Stat(target); err != nil {
...
**(b) 新建 `internal/claudedock/doctor/disk_test.go`：**
...
**executor 注意：** `disk.go` 中 `Bsize` 类型在 Linux/Darwin 分别为 `int64`/`uint32`；代码里 `int64(stat.Bsize)` 转换是跨 OS 安全的；测试里直接赋值 `4096` 通用。
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/disk.go && test -f internal/claudedock/doctor/disk_test.go` 退出码 = 0
    - `grep -q "func checkLocalDisk" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "func checkContainerDisk" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "func checkMutagenDataSize" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "golang.org/x/sys/unix" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "diskWarnMB\s*=\s*500" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "diskFailMB\s*=\s*100" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "mutagenBloatMB\s*=\s*1024" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "errcodes.DISK_LOCAL_LOW" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "errcodes.DISK_CONTAINER_LOW" internal/claudedock/doctor/disk.go` 命中
    - `grep -q "errcodes.DISK_MUTAGEN_DATA_BLOAT" internal/claudedock/doctor/disk.go` 命中
    - `go test ./internal/claudedock/doctor/ -run "TestCheckLocalDisk|TestCheckContainerDisk|TestParseDu|TestCheckMutagenDataSize" -count=1 -v` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.9: 新建 doctor/render.go + render_test.go — Text + JSON 双输出 + 降级 banner（CONTEXT D-13/D-15 / PATTERNS §2.10）</name>
  <files>internal/claudedock/doctor/render.go, internal/claudedock/doctor/render_test.go</files>

  <read_first>
    - internal/claudedock/ssh_doctor.go（line 370-454 Print + printGroup 样板）
    - internal/claudedock/colors.go（Task 2.1 后的 ColorEnabled / Colorize）
    - internal/claudedock/last_session.go（LastSessionSnapshot schema）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-13（第一屏 4 段）+ §D-14（彩色规则）+ §D-15（JSON schema）
    - .planning/phases/34-claudedock-doctor-v3/34-RESEARCH.md §5.1 JSON 锁死 + §8.5 M13 断言要求
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §2.10
  </read_first>

  <action>
**(a) 新建 `internal/claudedock/doctor/render.go`：**
...
func renderDowngradeBanner(banner *DowngradeBanner) string {
	...
	if banner == nil {
		b.WriteString("  [!] 未找到上次会话快照（首次运行 claudedock 后再 doctor 即可看到）\n")
...
func pickIcon(s Status, noColor bool) string {
	if noColor || !claudedock.ColorEnabled() {
		switch s {
		case StatusPass:
	...
	switch s {
	case StatusPass:
		return claudedock.Colorize(iconPass, claudedock.AnsiGreen)
	case StatusWarn:
		return claudedock.Colorize(iconWarn, claudedock.AnsiYellow)
	case StatusFail:
		return claudedock.Colorize(iconFail, claudedock.AnsiRed)
	case StatusSkip:
		return claudedock.Colorize(iconSkip, claudedock.AnsiGray)
	}
	return iconPass
}
...
**executor 注意：** `claudedock.Colorize` / `claudedock.ColorEnabled` / `claudedock.AnsiGreen` 等是 Task 2.1 导出后的符号；如 ansi 常量未改大写导出，这里可以改为在 doctor 包内再声明私有常量（不引入新 helper 即可），或者 Task 2.1 一并导出 `AnsiGreen/AnsiYellow/AnsiRed/AnsiGray`。

**(b) 新建 `internal/claudedock/doctor/render_test.go`（**含 M13 + JSON schema 两大锚点**）：**
...
- 在 `RenderJSON` 中过滤 Details 字段（开放透传，只去除 nil）
  ...
  <acceptance_criteria>
    - `test -f internal/claudedock/doctor/render.go && test -f internal/claudedock/doctor/render_test.go` 退出码 = 0
    - `grep -q "func RenderText" internal/claudedock/doctor/render.go` 命中
    - `grep -q "func RenderJSON" internal/claudedock/doctor/render.go` 命中
    - `grep -q "func renderDowngradeBanner" internal/claudedock/doctor/render.go` 命中
    - `grep -q "\\[降级\\]" internal/claudedock/doctor/render.go` 命中（M13 字面量锚点）
    - `grep -q "STATE_LAST_SESSION_MISSING" internal/claudedock/doctor/render.go` 命中
    - `grep -q "iconPass\\s*=\\s*\"\\[✓\\]\"" internal/claudedock/doctor/render.go` 命中
    - `grep -q "iconFail\\s*=\\s*\"\\[✗\\]\"" internal/claudedock/doctor/render.go` 命中
    - `grep -q "json.MarshalIndent" internal/claudedock/doctor/render.go` 命中
    - `grep -q "func TestDowngradeBannerRendersChain" internal/claudedock/doctor/render_test.go` 命中（M13 锚点）
    - `grep -q "func TestJSONSchemaV1Lock" internal/claudedock/doctor/render_test.go` 命中
    - `grep -q "func TestRenderTextContainsNextAction" internal/claudedock/doctor/render_test.go` 命中（M14 锚点）
    - `go test ./internal/claudedock/doctor/ -run "TestDowngradeBanner|TestJSONSchema|TestRenderText" -count=1 -v` 退出码 = 0
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.10: 实现 doctor/doctor.go::RunDoctor 主流程（串行执行 + lazy SSH + Summary 聚合）+ 导入测试 doctor_test.go</name>
  <files>internal/claudedock/doctor/doctor.go, internal/claudedock/doctor/doctor_test.go</files>

  <read_first>
    - internal/claudedock/doctor/doctor.go（Task 2.2 框架 + RunDoctor panic 占位）
    - internal/claudedock/doctor/*.go（Task 2.3-2.9 所有 check 函数签名）
    - internal/claudedock/ssh_doctor.go::sshConnect（line 119-150 SSH 拨号 helper）
    - internal/claudedock/last_session.go::LoadLastSession（LastSessionSnapshot 读取）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-06 / §D-07 / §D-19 / §D-20（执行策略）
  </read_first>

  <action>
**(a) 修改 `internal/claudedock/doctor/doctor.go` — 把 `RunDoctor` 的 `panic(...)` 占位替换为真实实现：**
...
func RunDoctor(ctx context.Context, opts Options) (*Report, error) {
	...
	var authResp *claudedock.AuthResponse
	closeRemote := func() {
		if remoteConn != nil {
			_ = remoteConn.Close()
			remoteConn = nil
	...
	ensureRemote := func() {
		...
		conn, err := claudedock.SSHConnect(sshCfg)
		if err != nil {
...
```go
// convertSnapshotToBanner 把 claudedock.LastSessionSnapshot 转为 doctor DowngradeBanner。
func convertSnapshotToBanner(snap *claudedock.LastSessionSnapshot) *DowngradeBanner {
...
func authRespClaudeAccountID(r *claudedock.AuthResponse) string {
	if r == nil {
		return ""
	}
...
func authRespExpectedEgressIP(r *claudedock.AuthResponse) string {
	if r == nil {
		return ""
	}
...
**executor 注意：** 本 task 需要引入 `net / os / path/filepath / strings` + `claudedock` + `golang.org/x/crypto/ssh`。`claudedock.SSHConnect` 已由 Phase 32-02 在 `internal/claudedock/ssh.go:200` 导出（`return sshConnect(cfg)` 包装），本 plan 不需任何重命名 / 导出动作。`AuthResponse.ExpectedEgressIP` 字段：executor 先尝试编译，若 entry 包 `AuthResponse` 未定义该字段，把 `authRespExpectedEgressIP` 改为恒返回 `""`（`return ""`）并把该现象记录到 `34-02-SUMMARY.md` carry-over，由 v3.1 backlog 补齐 entry.go 字段。此时 `checkEgressIPVisible` 走 `expectedIP == ""` 分支，不触发 drift 警告，仅 Details 里记 `expected: "<unknown — 字段未交付>"`。

**(b) 新建 `internal/claudedock/doctor/doctor_test.go`（RunDoctor 集成点单测，mock 所有包级 var）：**
...
func TestRunDoctor_NoInit_NetworkAuthOnlyLocal(t *testing.T) {
	origCfg := loadConfig
	loadConfig = func() (*claudedock.Config, error) { return nil, fmt.Errorf("config 不存在") }
...
- 任何 `go test` 命令预期 docker daemon 存在（单测必须纯内存 mock）
  ...
  <acceptance_criteria>
    - `grep -q "func RunDoctor(ctx context.Context, opts Options) (\\*Report, error)" internal/claudedock/doctor/doctor.go` 命中
    - `! grep -q "panic(\"executor" internal/claudedock/doctor/doctor.go` 为真（占位 panic 已移除）
    - `grep -q "convertSnapshotToBanner" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "claudedock.LoadLastSession" internal/claudedock/doctor/doctor.go` 命中
    - `grep -q "NewSSHRemoteRunner" internal/claudedock/doctor/doctor.go` 命中
    - `test -f internal/claudedock/doctor/doctor_test.go` 退出码 = 0
    - `grep -q "func TestRunDoctor_NoInit_NetworkAuthOnlyLocal" internal/claudedock/doctor/doctor_test.go` 命中（D-06 回归）
    - `grep -q "func TestRunDoctor_DomainFilter" internal/claudedock/doctor/doctor_test.go` 命中
    - `go build ./internal/claudedock/doctor/...` 退出码 = 0
    - `go vet ./internal/claudedock/doctor/...` 退出码 = 0
    - `go test ./internal/claudedock/doctor/ -count=1 -short -run "TestRunDoctor_" -v` 退出码 = 0
    - **全包单测汇总（本 task 启用整个 doctor 包）**：`go test ./internal/claudedock/doctor/ -count=1 -short` 退出码 = 0（含 Task 2.2-2.9 所有子测）
  </acceptance_criteria>
</task>

<task type="execute">
  <name>Task 2.11: 新建 cmd/claudedock/doctor.go + 修改 main.go 注册（CONTEXT D-03 / D-05 / PATTERNS §3.1）</name>
  <files>cmd/claudedock/doctor.go, cmd/claudedock/main.go</files>

  <read_first>
    - cmd/claudedock/main.go（Plan 01 Task 1.7 完成后的 AddCommand + switch case）
    - cmd/claudedock/sessions.go（三段式 cobra 模板）
    - .planning/phases/34-claudedock-doctor-v3/34-CONTEXT.md §D-03 / §D-05（cobra ValidArgs）+ §D-16（退出码）
    - .planning/phases/34-claudedock-doctor-v3/34-PATTERNS.md §3.1
  </read_first>

  <action>
**(a) 新建 `cmd/claudedock/doctor.go`：**

```go
// cmd/claudedock/doctor.go — Phase 34 Plan 02 Task 2.11
//
// claudedock doctor [domain] 子命令：五维度自检。
// 与 claudedock ssh doctor（v2.0 quick task 入口）双入口共存（CONTEXT D-04）。
package main
...
import (
	...
	"github.com/claudedock/claudedock/internal/claudedock/doctor"
)
...
func newDoctorCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "doctor [domain]",
		Short: "claudedock 五维度自检（network/auth/ssh/mount/disk）",
		Long: "运行 claudedock doctor 检测当前环境健康度，每项输出 [符号] + 中文原因 + 建议 + 错误码。\n" +
...
**executor 注意：** `contextWithDoctorTimeout` 使用标准 `context.Context` / `context.CancelFunc` 签名；需在 `cmd/claudedock/doctor.go` 的 import 中补上 `"context"` 与 `"time"`。

**(b) 修改 `cmd/claudedock/main.go` — 在 Plan 01 修改过的 AddCommand 行末尾再追加 `newDoctorCmd()`：**
...
- Plan 03 占位输出 `"[!] --fix 自动修复将在 doctor.fix.go 实现（Plan 03）"`（M14 兼容 — 该行不走 warn/fail check 渲染，不触发 grep gate）
  ...
  <acceptance_criteria>
    - `test -f cmd/claudedock/doctor.go` 退出码 = 0
    - `grep -q "^func newDoctorCmd" cmd/claudedock/doctor.go` 命中
    - `grep -q "^func runDoctor" cmd/claudedock/doctor.go` 命中
    - `grep -q 'ValidArgs:\s*\[\]string{"network", "auth", "ssh", "mount", "disk", "all"}' cmd/claudedock/doctor.go` 命中
    - `grep -q "doctorExitFail\s*=\s*2" cmd/claudedock/doctor.go` 命中
    - `grep -q "doctor.RunDoctor" cmd/claudedock/doctor.go` 命中
    - `grep -q "doctor.RenderJSON" cmd/claudedock/doctor.go` 命中
    - `grep -q "doctor.RenderText" cmd/claudedock/doctor.go` 命中
    - `grep -q "newDoctorCmd()" cmd/claudedock/main.go` 命中
    - `grep -qE 'case "init", "env", "ssh", "sync", "sessions", "explain", "doctor",' cmd/claudedock/main.go` 命中
    - `grep -q "context.CancelFunc" cmd/claudedock/doctor.go` 命中（contextWithDoctorTimeout 真实签名）
    - `grep -q "context.WithTimeout(parent" cmd/claudedock/doctor.go` 命中（contextWithDoctorTimeout 真实实现）
    - `go build ./cmd/claudedock/...` 退出码 = 0
    - `go vet ./cmd/claudedock/...` 退出码 = 0
    - 人工 smoke：`./claudedock doctor --help` 输出含 `--fix` / `--json` / `--verbose` / `--yes`（可在 done 中作 manual 项；CI 不强求）
  </acceptance_criteria>
</task>
...
go test ./internal/claudedock/doctor/ -count=1 -v

# 3. 既有 claudedock 包无回归（Task 2.1 colors 导出影响面）
go test ./internal/claudedock/ -count=1 -short

# 4. errcodes 包无回归（Plan 01 Task 验证）
go test ./internal/claudedock/errcodes/ -count=1

# 5. cmd/claudedock 包（含 explain + doctor）
go test ./cmd/claudedock/ -count=1

# 6. M13 终验（TestDowngradeBannerRendersChain 是 SC#6 验收锚点）
go test ./internal/claudedock/doctor/ -run TestDowngradeBannerRendersChain -count=1 -v

# 7. M14 终验（TestRenderTextContainsNextAction）
go test ./internal/claudedock/doctor/ -run TestRenderTextContainsNextAction -count=1 -v

# 8. JSON schema 锁（TestJSONSchemaV1Lock）
go test ./internal/claudedock/doctor/ -run TestJSONSchemaV1Lock -count=1 -v

# 9. host-agent endpoint 边界守恒（SC#9）— 本 plan 不引入任何 host-agent 调用
! rg -nE "agentapi\.(Action|NewClient|RunHostAction)" internal/claudedock/doctor/

# 10. 17 项 check 枚举（粗略：grep check function 定义）
[ "$(rg -cE '^func check[A-Z][A-Za-z]+' internal/claudedock/doctor/*.go | awk -F: '{s+=$NF} END{print s}')" -ge 17 ]
```
...
- [ ] `go test ./internal/claudedock/doctor/ -count=1 -short` 退出码 = 0（≥ 40 条单测全 PASS）
...
- [ ] `claudedock doctor --help` 列出 4 flag + ValidArgs 6 项
...
|----------|-------------|
| `claudedock doctor` 本地 | 读 ~/.claudedock/config.yaml + ~/.claudedock/last-session.json + ~/.ssh/known_hosts |
| `claudedock doctor` → Entry API | HTTP GET /healthz + AuthenticateAndWait（复用 EntryClient token 鉴权） |
| `claudedock doctor` → 远端容器 SSH | 复用 EntryClient 返回的 SSH 凭据（host/port/user/password）；sessions 在容器内执行只读命令 |
...
| T-34-02-03 | Information Disclosure | `Check.Details` 意外写入 OAuth 凭据 | medium | mitigate | `auth.go` `checkOAuthCredentials` 只写 `MinutesToExpire` 数字，不写 token 字面量；其它 check 的 Details 走白名单（mount.branches_xattr / mount.mount / disk.addrs / disk.avail），禁止写 `cfg.Password` / `authResp.SSHPass`；acceptance_criteria `! grep -n "cfg.Password\|SSHPass" internal/claudedock/doctor/*.go`（Plan 03 集成测试阶段再次验证） |
| T-34-02-04 | Information Disclosure | JSON 输出泄漏内部路径 / hostname | low | accept | rationale：doctor 是用户自助排障工具，输出目标路径 `~/.claudedock/` / hostPort 是必要信息；CONTEXT §security_threat_model T5 仅要求 ExpiresAt 不入 JSON，已通过 OAuth 只存 MinutesToExpire 实现 |
...
| T-34-02-06 | Tampering | LastSessionSnapshot 被恶意 JSON 注入导致 `renderDowngradeBanner` panic | low | mitigate | `claudedock.LoadLastSession` 已做 json.Unmarshal 错误处理（Phase 31/32 D-18 实现）；doctor 包 `convertSnapshotToBanner` 只拷贝结构化字段，不 eval |
...
<output>
After completion, create `.planning/phases/34-claudedock-doctor-v3/34-02-SUMMARY.md` with:
- 11 个 task 的实际 commit SHA + 关键 diff 片段引用
- doctor 包实际 check 总数（`grep -c "^func check" internal/claudedock/doctor/*.go`）
...
- `claudedock doctor --help` 输出样本（人工 smoke test）
