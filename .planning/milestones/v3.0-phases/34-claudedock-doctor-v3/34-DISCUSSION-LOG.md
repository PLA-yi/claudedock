# Phase 34: claudedock doctor v3 + 错误码统一 - Discussion Log
...
## GA-2：`claudedock doctor` 子命令结构

| Option | Description | Selected |
|--------|-------------|----------|
| `doctor [domain]` 子命令 + 顶层 flag | 与 `brew doctor` / `cargo check` 行业惯例一致；`cobra ValidArgs` 内置校验；与现有 `claudedock ssh doctor` 双入口共存 | ✓ |
...
**Notes:** ValidArgs={network,auth,ssh,mount,disk,all}，默认 all（D-05）。`--fix`/`--verbose`/`--json`/`--yes` 是顶层 flag 便于其它子命令复用（D-05 第 4 条）。`claudedock ssh doctor`（v2.0 quick task 260417-0w4 已 ship）保持兼容（D-04）。
...
## GA-5：`claudedock explain <code>` 数据源
...
- 错误码 i18n（v3.1）
- doctor 历史报告归档（v3.1）
- `claudedock explain --list` / 模糊匹配（v3.1）
- doctor 检查项并发执行（v3.1）
...
- `claudedock doctor --report-bug`（v3.1，与 OOS-A11 配套）
