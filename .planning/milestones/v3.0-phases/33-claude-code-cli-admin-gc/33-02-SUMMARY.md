...
| SC2 | volume 命名 + label 一致 (REQ-F7-A) | Plan 01 BuildClaudeStateVolumeName + worker label set | UAT Step 1：`docker volume ls --filter label=com.claudedock.account_id=<id>` 返回 `claude-state-<id>` | ✅ APPROVED |
...
- **Issue:** Plan 01 SUMMARY 已识别此 carry-over（dispatcher 链路全无 `ClaudeAccountID:` 注入）；UAT Step 1 创建 host 后 `docker volume ls --filter label=com.claudedock.account_id=<id>` 返回空，证实 createHost 自动补 volume 完全未触发
...
2. **独立 GC 定时任务**（CONTEXT Deferred）— 当前删除依赖 admin 主动触发；建议 v3.1 cron 扫孤儿 volume，按 label `com.claudedock.managed=true` + DB 反查无对应 account 时清理。运维手册章节 7 已留占位。
...
- **Phase 34 (claudedock doctor v3 + 错误码统一)** 可立即开始：依赖的 `STATE_VOLUME_IN_USE_001` 错误码 + 6 类 audit 事件类型 + 运维手册基线已就位，Phase 34 doctor `mount` / `state` 检查项可直接复用 Phase 33 落地的命名规范与 label key。
...
- 必带 label：
  ...
  - `com.claudedock.managed=true` — 二级保险（运维 GC 时按此 label 过滤）
