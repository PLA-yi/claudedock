...
| D-02 | label `com.claudedock.account_id={id}` + `com.claudedock.managed=true` | ✓ | docker `--label k=v --label k=v` 多次 flag 已在 `worker.buildCreateArgs:186-188` 用同样模式（`--label claudedock.managed=true`）证明可行 |
...
| `internal/controlplane/http/admin_claude_accounts.go` | `github.com/jackc/pgx/v5`、`github.com/claudedock/claudedock/internal/agentapi`、`github.com/claudedock/claudedock/internal/store/repository`（其余标准库 `context` `errors` `log/slog` `net/http` `time`） |
...
func TestEnsureDockerVolume_Idempotent(t *testing.T) {
	...
	err := realEnsureDockerVolume(context.Background(), "claude-state-abc",
		map[string]string{"com.claudedock.account_id": "abc"})
	if err != nil {
