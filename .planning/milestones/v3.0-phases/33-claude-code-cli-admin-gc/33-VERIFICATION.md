...
| SC2 | `docker volume ls --filter label=com.claudedock.account_id=<id>` 返回唯一 volume `claude-state-<id>`（REQ-F7-A） | ✓ VERIFIED | `worker.go:949` `BuildClaudeStateVolumeName` 强制前缀 `claude-state-`；`worker.go:230-231` 写入双 label `com.claudedock.account_id=<id>` + `com.claudedock.managed=true`；单测 `TestBuildClaudeStateVolumeName_NonEmptyID_ReturnsPrefixedName` PASS；UAT Step 1 实测 `docker volume ls --filter label=...` 返回 `claude-state-<id>` 单值。                                                                                                                                                                                |
...
| 运维手册 verbatim 字段                                          | `grep -c "claude-state-\|com.claudedock.\|/var/lib/claude-persist\|STATE_VOLUME_IN_USE_001\|claude_account.\|orphan\|孤儿" runbook`               | 29 命中                                                | ✓ PASS   |
...
| ----------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-F7-A    | Plan 01 + Plan 02     | `~/.claude` 与 `~/.cache/claude` 通过独立 Docker named volume 持久化，命名 `claude-state-{id}` + label `com.claudedock.account_id` | ✓ SATISFIED   | `BuildClaudeStateVolumeName` + 双 label（worker.go:230-231）+ entrypoint symlink + admin host detail 显式 `persistent_volume_name` 字段（OOS-A19 边界守恒） |
...
2. **进入 Phase 34** — Phase 34 (claudedock doctor v3 + 错误码统一) 可立即开始，依赖的 `STATE_VOLUME_IN_USE_001` 错误码 + 6 类 audit 事件类型 + 运维手册基线已就位。
