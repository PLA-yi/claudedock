...
dependency-graph:
  requires:
    ...
    - claudedock.SessionConfig（Plan 02 已新增 — 本 plan 追加 IsSecondaryClient）
    - claudedock.runClaudeWithSession + writeLastSessionTmuxField + writeClientFile（Plan 02）
    - claudedock.SSHConnect / DetectTmux（Plan 02 export）
    - claudedock.LastSessionSnapshot.ClientRole（Plan 01 已加）
  provides:
    - claudedock.AcquireSyncLock(conn, accountID) (release func(), err error)
    - claudedock.ErrSyncLocked
    - claudedock.parseLastInt(s string) int（容错纯函数；exported 单测用）
  ...
  affects:
    - internal/claudedock/ssh.go::ConnectAndRunClaudeV3 — 替换 SyncSessionLock noop 默认 + SessionConfig 追加 IsSecondaryClient 透传（line 86-110, line 181）
    - internal/claudedock/mount_strategy.go::MountConfig — 末尾追加 IsSecondaryClient bool（+5 行；MountWorkspace 函数体 zero diff）
    - internal/claudedock/session.go::SessionConfig + writeClientFile + runClaudeWithSession（IsSecondaryClient 字段 + role 字符串切换 2 处）
    - internal/claudedock/integration_test.go — 末尾 +160 行 6 个新用例 + defaultFixtureSSHConfig helper
tech-stack:
  added: []
  patterns:
...
key-files:
  created:
    - internal/claudedock/sync_lock.go (122 LOC)
    - internal/claudedock/sync_lock_test.go (73 LOC，9 个单测 PASS)
  modified:
    - internal/claudedock/ssh.go (+27 行：AcquireSyncLock 闭包覆盖 + import errors + SessionConfig IsSecondaryClient 透传)
    - internal/claudedock/mount_strategy.go (+5 行：MountConfig 末尾追加 IsSecondaryClient bool)
    - internal/claudedock/session.go (+22 行：SessionConfig 字段 + writeClientFile role 形参 + runClaudeWithSession role 三元 + pTYAttachOnce role 三元)
    - internal/claudedock/integration_test.go (+160 行：6 个 TestIntegration_Phase32_* + defaultFixtureSSHConfig helper + import errors)
decisions:
  ...
  - "TestIntegration_Phase32_NetworkDisconnect30s 在 short mode 与 docker network 缺权限
    场景双重 t.Skip — 端到端 PTY 交互（claudedock→tmux→拔网→reconnect→buffer 完整）
...
锁路径选择论证：
- **/tmp/claudedock/locks/sync-<accountID>.lock**（CONTEXT D-17 修订；RESEARCH §6.2）
...
```
$ go build ./...                                                              PASS
$ go build -tags=integration ./internal/claudedock/...                       PASS
$ GOOS=linux  go vet ./internal/claudedock/...                               PASS
$ GOOS=darwin go vet ./internal/claudedock/...                               PASS
$ go vet -tags=integration ./internal/claudedock/...                         PASS
$ go test ./internal/claudedock/... -count=1 -short
ok      github.com/claudedock/claudedock/internal/claudedock         0.752s
ok      github.com/claudedock/claudedock/internal/claudedock/errcodes  0.453s
$ go test ./internal/claudedock/... -run "TestAcquireSyncLock|TestParseLastInt" -v
   9/9 PASS
$ rg "^func TestIntegration_Phase32_" internal/claudedock/integration_test.go | wc -l   → 6
$ rg -n "/var/lock" internal/claudedock/sync_lock.go                         无命中（D-17 验证）
$ rg -n "AcquireSyncLock" internal/claudedock/ssh.go                         命中 2 行
$ rg -n "ErrSyncLocked" internal/claudedock/ssh.go                           命中 2 行
$ rg -n "IsSecondaryClient" internal/claudedock/{mount_strategy,ssh,session}.go 共 8 行
...
- **Issue:** PLAN `<interfaces>` 仅示例 `writeLastSessionTmuxField` 的 role 切换；但文件注册表的 `client_role` 字段（session.go line 252 hard-code `"primary"`）若不同步切，doctor/explain 通过 `/workspace/.claudedock/clients/<pid>.json` 读 client_role 时会误判 secondary 为 primary（M13 静默降级回归 / T-32-21）。
...
- **Files modified:** internal/claudedock/session.go
- **Commit:** 42cf058

**2. Phase 31 mount_strategy.MountWorkspace 未实际调用 SyncSessionLock 闭包**
- **Found during:** Task 3.2 grep `rg -n "SyncSessionLock" internal/claudedock/mount_strategy.go`
...
- [x] internal/claudedock/sync_lock.go 存在（122 LOC）
- [x] internal/claudedock/sync_lock_test.go 存在（73 LOC，9 个单测 PASS）
- [x] internal/claudedock/ssh.go SyncSessionLock 注入闭包就位（line 86-110）+ SessionConfig IsSecondaryClient 透传（line 181）
- [x] internal/claudedock/mount_strategy.go MountConfig 末尾追加 IsSecondaryClient bool（+5 行）
- [x] internal/claudedock/session.go SessionConfig.IsSecondaryClient + writeClientFile role 形参 + runClaudeWithSession + pTYAttachOnce role 切换 4 处
- [x] internal/claudedock/integration_test.go 末尾追加 6 个 TestIntegration_Phase32_* + defaultFixtureSSHConfig helper
- [x] commit f6c4197 (Task 3.1) 存在
- [x] commit 42cf058 (Task 3.2) 存在
- [x] commit e577aa7 (Task 3.3) 存在
- [x] go build ./... PASS
- [x] go build -tags=integration ./internal/claudedock/... PASS
- [x] GOOS=linux + GOOS=darwin go vet PASS
- [x] go test ./internal/claudedock/... -count=1 -short PASS（含 9 新 + 22 既有）
