...
requirements:
  - REQ-F5-D
files_modified:
  - internal/claudedock/sync_lock.go
  - internal/claudedock/sync_lock_test.go
...
must_haves:
  truths:
    - "AcquireSyncLock(conn, accountID) 在容器内远程执行 'mkdir -p /tmp/claudedock/locks 2>/dev/null && flock -n -E 99 -F <lockPath> -c \"echo $$; exec sleep infinity\" & echo $!'，lockPath = /tmp/claudedock/locks/sync-<accountID>.lock（修订 D-17 — RESEARCH §6.2 选 /tmp/claudedock/locks 因 ubuntu:24.04 /var/lock 是 root-only）"
    ...
    - "(c) 与 (d) 用例 testing.Short() 跳过；CI 环境用 -tags=integration 才运行；scripts/test-fixture-up.sh 已就位（Phase 31 ship），本 plan 仅追加（如需）多端 fixture 配置（同 fixture 容器内启 2 个 claudedock 进程，无需新 service）"
  ...
  artifacts:
    - path: "internal/claudedock/sync_lock.go"
    ...
    - path: "internal/claudedock/sync_lock_test.go"
...
1. **账号级 flock 单例锁（REQ-F5-D / PITFALLS M15）**：sync_lock.go 通过容器内 `flock -n -E 99 -F /tmp/claudedock/locks/sync-<account_id>.lock -c "exec sleep infinity"` 实现单例；后端拿不到锁 → ErrSyncLocked → Phase 31 mount_strategy 既有逻辑降级 sshfs-only（本 plan **不**重写降级，仅注入实现）
...
internal/claudedock/sync_lock.go 导出：
...
// ErrSyncLocked 表示另一端 claudedock 已持有同一 claude_account 的 Mutagen 单例锁。
...
// 锁路径：/tmp/claudedock/locks/sync-<accountID>.lock（RESEARCH §6.2 修订 D-17）。
//
// 实现：远程 'mkdir -p /tmp/claudedock/locks 2>/dev/null && flock -n -E 99 -F <path> -c "echo $$; exec sleep infinity" & echo $!'
...
internal/claudedock/ssh.go ConnectAndRunClaudeV3 改造点（MountWorkspace 之前）：
...
internal/claudedock/mount_strategy.go MountConfig 追加 1 字段（与 Plan 02 追加的 SessionShortID/SessionTakeOver/LocalHostname 并列）：
...
- `<lockPath_q>` = `shellescape.Quote("/tmp/claudedock/locks/sync-<accountID>.lock")`
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/oauth_check.go（line 44-63 — sess.Run + 错误收敛模板）
    - internal/claudedock/mount_mutagen.go（line 116-124 — sess.CombinedOutput 读 stdout 模板）
    - internal/claudedock/ssh.go（line 230-238 — ssh.ExitError.ExitStatus() 解包模板）
    - internal/claudedock/mount_sshfs.go（shellescape.Quote 用法参考）
    - internal/claudedock/errcodes/codes.go + errcodes/session.go（SESSION_SYNC_LOCKED 已由 Plan 01 注册）
  ...
  <action>
    1. **internal/claudedock/sync_lock.go**：
    ...
    2. **internal/claudedock/sync_lock_test.go**：
  ...
  <acceptance_criteria>
    ...
    - `go test ./internal/claudedock/... -run "TestAcquireSyncLock_AnonReturnsNoop|TestParseLastInt" -count=1` 全部 PASS（≥ 7 用例）
    - `rg -n "func AcquireSyncLock" internal/claudedock/sync_lock.go` 命中 1 行
    - `rg -n "func parseLastInt" internal/claudedock/sync_lock.go` 命中 1 行
    - `rg -n "ErrSyncLocked" internal/claudedock/sync_lock.go` 命中（var 声明 + return 引用）
    - `rg -n "/tmp/claudedock/locks" internal/claudedock/sync_lock.go` 命中（路径字面值）
    - `rg -n "flock -n -E 99 -F" internal/claudedock/sync_lock.go` 命中（命令模板字面值）
    - `rg -n "exec sleep infinity" internal/claudedock/sync_lock.go` 命中（防 fork 关键参数）
    - `rg -n "shellescape.Quote" internal/claudedock/sync_lock.go` 命中（lockPath 必须 shellescape）
    - `rg -n "ExitStatus\\(\\) == 99" internal/claudedock/sync_lock.go` 命中
    - `rg -n "/var/lock" internal/claudedock/sync_lock.go` **必须无命中**（修订 D-17 — 路径已改 /tmp）
  </acceptance_criteria>
  <verify>
    <automated>go test ./internal/claudedock/... -run "TestAcquireSyncLock|TestParseLastInt" -count=1 &amp;&amp; go vet ./internal/claudedock/... &amp;&amp; ! rg -q "/var/lock" internal/claudedock/sync_lock.go</automated>
  </verify>
  <done>sync_lock.go 全部就位；anon 路径 noop + 锁拿不到 ErrSyncLocked + parseLastInt 边界覆盖；路径锁定 /tmp/claudedock/locks（修订 D-17）。Task 3.2 可在 ssh.go 内注入。</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/ssh.go（ConnectAndRunClaudeV3 全函数 — 找到 mountCfg 已构造、MountWorkspace 调用之前的位置）
    - internal/claudedock/mount_strategy.go（MountConfig 定义 + Plan 02 已加的 SessionShortID/SessionTakeOver/LocalHostname；本 task 在末尾追加 IsSecondaryClient bool）
    - internal/claudedock/mount_strategy.go（MountWorkspace 内 SyncSessionLock 调用点 — 验证 Phase 31 已实现 errSyncLocked 降级；本 task **不**改这段）
    - internal/claudedock/session.go（Plan 02 的 SessionConfig + writeLastSessionTmuxField；本 task 加 IsSecondaryClient 字段 + role 字符串切换）
    - internal/claudedock/sync_lock.go（Task 3.1）— AcquireSyncLock + ErrSyncLocked
    - internal/claudedock/errcodes/session.go（SESSION_SYNC_LOCKED 注册位置 — Plan 01）
    - .planning/phases/32-ssh-tmux/32-CONTEXT.md D-17 / D-18 / D-19
    - .planning/phases/32-ssh-tmux/32-RESEARCH.md §6.4（修订 D-18 注入位置）
    - .planning/phases/32-ssh-tmux/32-PATTERNS.md ssh.go 改造点 C
  </read_first>
  <action>
    1. **internal/claudedock/mount_strategy.go**：在 MountConfig struct 末尾追加 1 个字段（与 Plan 02 追加的 3 字段并列）：
    ...
    2. **internal/claudedock/ssh.go**：在 ConnectAndRunClaudeV3 中找到 mountCfg 构造完毕、MountWorkspace 调用之前的位置（grep `MountWorkspace` 定位）；在该位置之前插入 SyncSessionLock 覆盖：
    ...
    3. **internal/claudedock/ssh.go**：在 Plan 02 添加的 SessionConfig 构造段（OAuth 检查后），追加一行 IsSecondaryClient 字段透传：
    ...
    4. **internal/claudedock/session.go**：在 Plan 02 的 SessionConfig struct 末尾追加 1 个字段：
    ...
    5. 验证 mount_strategy.MountWorkspace 内 errSyncLocked 处理已在 Phase 31 ship — 用 grep 确认：
       ...
       ```bash
       rg -n "ErrSyncLocked|errSyncLocked|SyncSessionLock" internal/claudedock/mount_strategy.go
       ```
  ...
  <acceptance_criteria>
    - `go build ./...` 成功
    - `go vet ./...` 通过
    - `rg -n "AcquireSyncLock" internal/claudedock/ssh.go` 命中 1 行（在 ConnectAndRunClaudeV3 内）
    - `rg -n "ErrSyncLocked" internal/claudedock/ssh.go` 命中（errors.Is 判断）
    - `rg -n "SESSION_SYNC_LOCKED" internal/claudedock/ssh.go` 命中
    - `rg -n "IsSecondaryClient" internal/claudedock/mount_strategy.go` 命中 1 行（字段定义）
    - `rg -n "IsSecondaryClient" internal/claudedock/ssh.go` 命中 ≥ 1 行（设置 + 透传）
    - `rg -n "IsSecondaryClient" internal/claudedock/session.go` 命中 ≥ 2 行（字段 + 使用）
    - `rg -n "secondary" internal/claudedock/session.go` 命中（role 字符串）
    - `git diff internal/claudedock/mount_strategy.go` 仅追加 1 字段（用 `git diff -U0 internal/claudedock/mount_strategy.go | grep '^+' | grep -v '^+++' | wc -l` 输出 ≤ 5）
    - `git diff internal/claudedock/ssh.go` 改动行数 ≤ 25（覆盖 SyncSessionLock 闭包 + IsSecondaryClient 透传）
    - `go test ./internal/claudedock/... -count=1 -short` PASS（不破坏 Phase 31 / Plan 01 / Plan 02 单测）
  </acceptance_criteria>
  <verify>
    <automated>go build ./... &amp;&amp; go vet ./... &amp;&amp; go test ./internal/claudedock/... -count=1 -short</automated>
  </verify>
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/integration_test.go（Phase 31 已有 6 个 TestIntegration_* — 模式 + dockerExec helper + TestMain 优雅跳过）
    ...
    - internal/claudedock/sync_lock.go（Task 3.1）+ ssh.go SyncSessionLock 注入（Task 3.2）
    - internal/claudedock/sshfs_watcher.go（C3 防御 — 集成测试 (c) 30s 抖动场景隐含验收）
  ...
  <action>
    1. **internal/claudedock/integration_test.go**：在文件末尾追加 6 个新用例（与 Phase 31 6 个用例并列；TestMain 共享）：
       ...
       // 同一 accountID 在容器内只能有一个进程持有 /tmp/claudedock/locks/sync-<id>.lock。
       func TestIntegration_Phase32_SyncLockMutexes(t *testing.T) {
           ...
           // 验证 lockfile 存在
           out, _ := dockerExec(t, "ls", "/tmp/claudedock/locks/")
  ...
  <acceptance_criteria>
    - `go build -tags=integration ./internal/claudedock/...` 成功（编译通过 — 不需要 docker）
    - `go vet -tags=integration ./internal/claudedock/...` 通过
    - `rg -n "TestIntegration_Phase32_" internal/claudedock/integration_test.go` 命中 6 行（6 个新 Test 函数名）
    - `rg -n "TestIntegration_Phase32_PgrepNoSystemdLogind" internal/claudedock/integration_test.go` 命中
    - `rg -n "TestIntegration_Phase32_TmuxSurvivesSighupSshd" internal/claudedock/integration_test.go` 命中
    - `rg -n "TestIntegration_Phase32_SyncLockMutexes" internal/claudedock/integration_test.go` 命中
    - `rg -n "TestIntegration_Phase32_SyncLockAnonNoop" internal/claudedock/integration_test.go` 命中
    - `rg -n "TestIntegration_Phase32_DetectTmuxAvailable" internal/claudedock/integration_test.go` 命中
    - `rg -n "TestIntegration_Phase32_NetworkDisconnect30s" internal/claudedock/integration_test.go` 命中
    - `rg -n "AcquireSyncLock\\(" internal/claudedock/integration_test.go` 命中（用例直接调用）
    - `rg -n "DetectTmux\\(" internal/claudedock/integration_test.go` 命中
    - `rg -n "SSHConnect\\(" internal/claudedock/integration_test.go` 命中
    - `git diff scripts/test-fixture-up.sh` 应为空（本 plan 不改 fixture 脚本）
    - `git diff scripts/test-fixture-down.sh` 应为空
    - 在有 docker 环境（手测 / CI）：`bash scripts/test-fixture-up.sh && go test -tags=integration -count=1 -run "TestIntegration_Phase32_PgrepNoSystemdLogind|TestIntegration_Phase32_DetectTmuxAvailable|TestIntegration_Phase32_SyncLockAnonNoop" ./internal/claudedock/ && bash scripts/test-fixture-down.sh` 至少这 3 个最简用例 PASS（其它 3 个需要 docker 容器运行环境完整 — 接受在 CI 部分 skip）
    - 无 docker 时 TestMain 优雅 os.Exit(0) — `go test -tags=integration ./internal/claudedock/...` 不应报错（与 Phase 31 同行为）
  </acceptance_criteria>
  <verify>
    <automated>go build -tags=integration ./internal/claudedock/... &amp;&amp; go vet -tags=integration ./internal/claudedock/... &amp;&amp; rg -c "TestIntegration_Phase32_" internal/claudedock/integration_test.go | grep -q "^6$" || (echo "expect 6 TestIntegration_Phase32_* funcs" &amp;&amp; exit 1)</automated>
  </verify>
...
|----------|-------------|
| 客户端 → 容器 /tmp/claudedock/locks/ | mkdir + flock 远程命令；accountID 进入路径名 — 必须 shellescape |
...
|-----------|----------|-----------|-------------|-----------------|
| T-32-14 | Tampering | sync_lock.go remoteCmd 拼接 accountID 进入路径 | mitigate | accountID 来自 mountCfg.ClaudeAccountID（从 Phase 30 AuthResponse 读，gateway 控）；shellescape.Quote 包装 lockPath（grep 验证）；非 ASCII / 路径分隔符不会逃出 /tmp/claudedock/locks/ |
| T-32-15 | Information Disclosure | /tmp/claudedock/locks/sync-<account>.lock 文件名包含 accountID | accept | tmp 目录是容器内可见但不持久化；accountID 已是 hash（Phase 30 D-05 推导）；与 Plan 02 tmux session 命名等同等级 |
| T-32-16 | DoS | 多个 claudedock 反复 AcquireSyncLock 制造 sleep infinity 进程 | mitigate | flock -n 立即返回（不阻塞）；后端 ErrSyncLocked → 不创建第二个 sleep；release 显式 kill 不依赖超时；最坏情况是孤立 sleep（SSH 异常断开时 sshd SIGHUP 必然收割） |
...
| T-32-18 | Tampering | release func 远程 kill PID 错杀其它进程 | accept | PID 来自 echo $!（bash 子进程 PID 空间）；理论上 PID 复用可能误杀，但容器 PID 空间独立 + sleep infinity 通常存活时间 < claudedock 生命周期；最坏情况误杀其它 sleep — 不影响功能 |
| T-32-19 | Spoofing | 整个 plan 引入的 SSH 命令注入面（共 ≥ 8 个 dockerExec / sshOutput / sshRun） | mitigate | 全部参数走 shellescape；集成测试用例的字符串常量都是固定值（无外部输入）；新 grep 检查 `rg -n "fmt.Sprintf.*flock|fmt.Sprintf.*tmux" internal/claudedock/sync_lock.go` 命中处必须紧邻 shellescape.Quote |
| T-32-21 | InfoDisclosure | secondary 端误以为 full mount 模式（M13 静默降级回归） | mitigate | 三层可见性叠加：(a) ssh.go 在 ErrSyncLocked 时 stderr 输出 `[SESSION_SYNC_LOCKED]`（Plan 03 Task 3.2）；(b) Phase 31 mount_strategy 在 errSyncLocked 降级路径输出 `[MOUNT_AUTO_DOWNGRADED]`（已 ship）；(c) last-session.json `client_role='secondary'`（Plan 03 Task 3.2 透传链）— Phase 34 doctor / `claudedock explain` 复用任一通道排障 |
...
```bash
# 单元 + 编译 + lint
go build ./... && go build -tags=integration ./internal/claudedock/... \
  && go vet ./... && go vet -tags=integration ./internal/claudedock/... \
  && go test ./internal/claudedock/... -count=1 -short \
  && rg -c "TestIntegration_Phase32_" internal/claudedock/integration_test.go | grep -q "^6$"

# Plan 03 路径修订校验
! rg -q "/var/lock" internal/claudedock/sync_lock.go  # 必须无（修订 D-17 已改 /tmp）

# Plan 03 防御覆盖校验
rg -q "ErrSyncLocked" internal/claudedock/sync_lock.go internal/claudedock/ssh.go
rg -q "IsSecondaryClient" internal/claudedock/mount_strategy.go internal/claudedock/ssh.go internal/claudedock/session.go
rg -q "exec sleep infinity" internal/claudedock/sync_lock.go  # -F + exec 防 fork

# 集成测试（仅在有 docker 环境时）
if command -v docker >/dev/null && docker ps >/dev/null 2>&1; then
  bash scripts/test-fixture-up.sh \
    && go test -tags=integration -count=1 -run "TestIntegration_Phase32_PgrepNoSystemdLogind|TestIntegration_Phase32_DetectTmuxAvailable|TestIntegration_Phase32_SyncLockAnonNoop" -v ./internal/claudedock/
  bash scripts/test-fixture-down.sh
fi
```
</verification>
