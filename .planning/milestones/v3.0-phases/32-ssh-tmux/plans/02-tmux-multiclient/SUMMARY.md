...
dependency-graph:
  requires:
    ...
    - claudedock.RunKeepAlive / NewReconnector / NewBufferedStdin / FormatGiveUpMessage / ErrReconnectGaveUp（Plan 01）
    - claudedock.LastSessionSnapshot.{TmuxSession, ClientRole, ReconnectCount}（Plan 01）
    - claudedock.ansiGray / colorize / colorEnabled / fdHolder（Phase 31 + Plan 01）
    - mount_strategy.simpleHash8（Phase 31）
  provides:
    - claudedock.SessionConfig
    - claudedock.DetectTmux(conn) (bool, version, reason)
    - claudedock.GenerateShortSessionID() string
    - claudedock.runClaudeWithSession(ctx, conn, sshCfg, claudeArgs, sessionCfg, hasProxy) (int, error)
    - claudedock.RunSessionsLs(conn, w) error
    - claudedock.RunSessionsAttach(conn, name, hasProxy, cwd) (int, error)
    - claudedock.SSHConnect(cfg) — sshConnect 的 export 包装（cmd 层 sessions 子命令复用）
    - 客户端文件注册表 helpers: writeClientFile / removeClientFile / readClientHostnames（D-12 完整方案）
    - cobra 子命令 claudedock sessions {ls, attach}
  affects:
    - internal/claudedock/ssh.go::ConnectAndRunClaudeV3 — OAuth 检查后插入 DetectTmux + 路由
    - internal/claudedock/mount_strategy.go::MountConfig — 末尾追加 3 字段（SessionShortID/SessionTakeOver/LocalHostname）
    - cmd/claudedock/main.go — newSessionsCmd 注册 / DisableFlagParsing switch 追加 sessions / runRoot 剥离 --new-session/--take-over / KeepAlive 启动校验
tech-stack:
  added: []
  patterns:
    - 远程命令统一 shellescape.Quote / shellescape.QuoteCommand（SP-03，> 11 处）
    - 文件注册表 (`/workspace/.claudedock/clients/<pid>.json`) 替代环境变量传 hostname（D-12 修订）
...
key-files:
  created:
    - internal/claudedock/session.go (867 LOC)
    - internal/claudedock/session_test.go (329 LOC，22 用例 PASS)
    - cmd/claudedock/sessions.go (96 LOC)
  modified:
    - internal/claudedock/ssh.go (+44 行：ConnectAndRunClaudeV3 内插 DetectTmux/SessionConfig/路由 + 末尾追加 SSHConnect 包装；runClaude / sshConnect 函数体 zero diff)
    - internal/claudedock/mount_strategy.go (+6 行：MountConfig 末尾追加 SessionShortID/SessionTakeOver/LocalHostname 三字段，无 JSON tag)
    - cmd/claudedock/main.go (+22/-3 行：AddCommand newSessionsCmd / switch case sessions / 循环剥离 --new-session/--take-over / mountCfg 注入 + KeepAlive 启动校验)
decisions:
  ...
  - "SSHConnect (export) 在 ssh.go 末尾追加 1 行 forward — 不动 sshConnect 函数体，避免与 Plan 03 在 sshConnect 上的潜在叠加修改冲突；该包装仅供 cmd/claudedock/sessions.go 单点使用。"
...
落地 Phase 32 会话层 + 多端共享 attach：远程命令由 v2.0 裸 `claude <args>` 改为 `tmux new-session -A -d -s <claude-account_id_short8> ... \; attach-session`；多端默认共享（D-10 -A 行为）+ 第二端 banner 通过 `/workspace/.claudedock/clients/<pid>.json` 文件注册表识别其它 client 的 hostname + 活跃时间（D-12 完整方案）；`--new-session` / `--take-over` cobra flag + take-over 串行 detach；tmux 探测失败回退 v2.0 runClaude 不阻塞启动（D-15/D-16）；`claudedock sessions ls/attach` 子命令零控制面改造；KeepAliveInterval < 15s 启动期校验。
...
## cmd/claudedock/sessions.go 与 sync.go 结构对照
...
```go
// cmd/claudedock/main.go line 363-367
...
```
$ go test ./internal/claudedock/... -count=1 -short 2>&1 | tail -5
ok  	github.com/claudedock/claudedock/internal/claudedock         0.810s
ok  	github.com/claudedock/claudedock/internal/claudedock/errcodes  0.183s
```
...
| sessions ls/attach CLI 路由 | `claudedock --help` 含 sessions / `claudedock sessions --help` 含 ls + attach | 实测 PASS（输出见上） |
...
$ go run ./cmd/claudedock --help          → "sessions" 出现在 Available Commands  ✓
$ go run ./cmd/claudedock sessions --help → "ls" / "attach" 子命令可见            ✓
$ go test ./internal/claudedock/... -run "TestBuildTmuxSessionName|TestSanitize\
...
- **Files modified:** internal/claudedock/session.go (pTYAttachOnce line 698-714)
- **Commit:** cdad11e
...
- **Files modified:** internal/claudedock/session.go + internal/claudedock/ssh.go
- **Commit:** cdad11e + 07f200c

**3. SSHConnect export 包装在 ssh.go 末尾追加（Plan 03 接口预留）**
- **Found during:** Task 2.3 写 cmd/claudedock/sessions.go::connectForSessions 时
...
- **Files modified:** internal/claudedock/ssh.go
- **Commit:** 217fa12
...
- [x] internal/claudedock/session.go 存在（867 LOC）
- [x] internal/claudedock/session_test.go 存在（329 LOC，22 用例 PASS）
- [x] cmd/claudedock/sessions.go 存在（96 LOC）
- [x] internal/claudedock/ssh.go 修改 = ConnectAndRunClaudeV3 内插 + 末尾 SSHConnect（runClaude / sshConnect zero diff）
- [x] internal/claudedock/mount_strategy.go MountConfig 末尾追加 3 字段
- [x] cmd/claudedock/main.go 4 处改造完成
...
- [x] go build ./... PASS
- [x] GOOS=linux go vet ./internal/claudedock/... PASS
- [x] GOOS=darwin go vet ./internal/claudedock/... PASS
- [x] go run ./cmd/claudedock --help 含 sessions
- [x] go run ./cmd/claudedock sessions --help 含 ls + attach

## Plan 03 接入点提示

### A. mount_strategy SyncSessionLock 闭包注入位置

Plan 03 需要在 `internal/claudedock/ssh.go::ConnectAndRunClaudeV3` 内**MountWorkspace 之前**（即 line 85-89 当前已有的 SyncSessionLock 默认 noop 闭包之处）覆盖 mountCfg.SyncSessionLock 字段：
