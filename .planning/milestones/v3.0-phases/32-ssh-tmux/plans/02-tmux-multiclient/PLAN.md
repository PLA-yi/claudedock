...
must_haves:
  truths:
    ...
    - "第二端 banner（D-12 完整文件注册表方案 — RESEARCH §5.2 (a)）：claudedock 在 attach 前先 'tmux list-clients -t <session> -F #{client_pid}|#{client_activity}|#{client_tty}' 取既有 client PID 列表；attach 成功后立即远程查询本端 'tmux display-message -p #{client_pid}' 得 remote_tmux_client_pid，写入 /workspace/.claudedock/clients/<remote_tmux_client_pid>.json 文件注册表（schema_version=1 + hostname + tmux_client_pid + tmux_session + attach_at_unix + claude_account_id + client_role）；runClaudeWithSession defer 远程 'rm -f /workspace/.claudedock/clients/<remote_pid>.json' 清理；readClientHostnames(otherPids) 批量 cat JSON 文件解析 hostname，文件不存在 / JSON parse 失败 → 该 PID 在返回 map 中赋 'unknown-host'，**不**阻塞 banner 渲染（孤儿条目 — 即 PID 已不在 tmux list-clients 输出中的注册表 entry — 在本端 readClientHostnames 时跳过；本阶段被动清扫，主动清扫留 v3.1）"
    ...
    - "本地 hostname 通过 Go 端 os.Hostname() 取（在 cmd/claudedock/main.go 已注入 mountCfg.LocalHostname → SessionConfig.LocalHostname → writeClientFile 远程命令的 JSON 字段）；os.Hostname 失败 fallback 到 'unknown-host' 字面值；写入命令通过 shellescape.Quote 引用 JSON 字符串避免 shell 注入"
    - "claudedock sessions ls 远程 'tmux list-sessions -F #{session_name}|#{session_created}|#{session_attached}|#{session_windows}'，本地 text/tabwriter 渲染表格（SESSION / CREATED / CLIENTS / WINDOWS 4 列）；list-sessions 失败 → 输出 '当前容器内无活跃 tmux session'，退出码 0（D-13）"
    - "claudedock sessions attach <name>：远程 'tmux has-session -t <name>' 失败 → stderr [SESSION_NOT_FOUND] + exit ExitConfigError(=4)；成功 → 复用 runClaude 但 remoteCmd 替换为 'exec tmux attach-session -t <name>'（不包 claude）（D-14）"
    - "cmd/claudedock/main.go 注册 newSessionsCmd() + DisableFlagParsing switch 追加 'sessions' + runRoot 内手动剥离 --new-session / --take-over flag（与 --mount-mode 同一循环；--new-session / --take-over 是布尔无值 flag）+ 启动期校验 mountCfg.KeepAliveInterval >= 15s 失败时 stderr [SESSION_KEEPALIVE_TOO_AGGRESSIVE] + os.Exit(exitConfigError=4)（D-03 第 4 条）"
  ...
  artifacts:
    ...
    - path: "cmd/claudedock/sessions.go"
      provides: "newSessionsCmd cobra subcommand 树（ls / attach）+ runSessionsLs + runSessionsAttach（cobra 路由层；业务在 internal/claudedock/session.go）"
  ...
  key_links:
    ...
    - from: "cmd/claudedock/sessions.go"
      to: "internal/claudedock/session.go::RunSessionsLs/Attach"
...
- **Plan 01（Wave 1）必须先完成并 commit**：本 plan 直接 import：
  ...
  - `claudedock.RunKeepAlive / NewReconnector / NewBufferedStdin / RingBufCapacity / FormatGiveUpMessage / ErrReconnectGaveUp`（Plan 01 暴露）
  - `claudedock.LastSessionSnapshot.TmuxSession / ReconnectCount`（Plan 01 添加）
  - `claudedock.ansiGray`（Plan 01 添加）
...
2. **多端共享 attach（REQ-F5-A / B）**：默认 `new-session -A` 行为不踢人；第二端 banner 通过 `/workspace/.claudedock/clients/<pid>.json` 文件注册表识别其它 client 的 hostname + 活跃时间（修订 D-12）
...
internal/claudedock/session.go 导出：
...
// RunSessionsLs 由 cmd/claudedock/sessions.go 调用；远程 tmux list-sessions + 本地表格渲染。
// 失败（无 server）→ stdout 输出 "当前容器内无活跃 tmux session"，return nil（exit 0）。
func RunSessionsLs(conn *ssh.Client, w io.Writer) error

// RunSessionsAttach 由 cmd/claudedock/sessions.go 调用；远程 tmux has-session 校验 → attach。
...
//   - 写入 /workspace/.claudedock/clients/<remoteTmuxClientPid>.json （JSON schema 见 <remote_command_templates>）
...
//   - 远程 'rm -f /workspace/.claudedock/clients/<pid>.json'，错误忽略（孤儿条目下次 attach 被动跳过）
...
//   - 远程单 SSH session 多 cat：for pid in <pids>; do echo "===$pid==="; cat /workspace/.claudedock/clients/${pid}.json 2>/dev/null || true; done
...
internal/claudedock/reconnect.go 接口补强（Q4 RESOLVED — 共享 state 不暴露 *atomic.Int32 指针）：
...
internal/claudedock/input_buffer.go 接口补强（Q4 RESOLVED — 注入式连接 Reconnector）：
...
internal/claudedock/ssh.go MountConfig 追加（在 mount_strategy.go 的 MountConfig 上）：
...
cmd/claudedock/main.go 改造点见下方 <main_changes>。
</interfaces>
...
**registry 文件路径**：`/workspace/.claudedock/clients/<remote_tmux_client_pid>.json`（UID 1000 默认可写 /workspace；不污染 mergerfs/Mutagen — Plan 02 沿用 Phase 31 mutagen ignore 中已有的 `.claudedock/` 顶层匹配）。
...
printf '%s' <json_q> > /workspace/.claudedock/clients/$(tmux display-message -p '#{client_pid}').json
```
...
- `tmux display-message -p '#{client_pid}'` 返回当前 SSH session 内 attach 的 tmux client PID（远端 PID，与本地 claudedock 进程 PID 不同）
...
for pid in <pid1> <pid2> <pid3>; do
  echo "===$pid==="
  cat /workspace/.claudedock/clients/${pid}.json 2>/dev/null || true
done
```
...
```bash
rm -f /workspace/.claudedock/clients/<本端 remote_tmux_client_pid>.json
```
...
# 2. 命中 ≥ 1 时
tmux display-message -t <session_q> '[claudedock] 另一端已通过 --take-over 接管会话，本会话将在 3s 后断开'
sleep 3
...
<main_changes>
<!-- cmd/claudedock/main.go 4 处改动 — 与 PATTERNS §main.go 改造段对齐 -->
...
if newSession {
    mountCfg.SessionShortID = claudedock.GenerateShortSessionID() // 在 session.go 暴露 8 字符 base64url
}
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/oauth_check.go（DetectTmux mirror CheckOAuthCredentials line 44-63 — conn.NewSession + sess.Run + 错误收敛）
    - internal/claudedock/mount_strategy.go（buildSessionName line 435-459 + simpleHash8 line 461-470 — 直接复用 simpleHash8，不重新发明）
    - internal/claudedock/colors.go（colorize / colorEnabled — banner 渲染要用）
    - internal/claudedock/errcodes/codes.go（SESSION_TMUX_UNAVAILABLE / SESSION_NOT_FOUND）
    - internal/claudedock/mount.go（sshRun helper line 105-112 — 远程命令复用，不再新建）
  ...
  <action>
    ...
    1. **internal/claudedock/session.go** 包结构（package claudedock；imports 含 bytes / context / crypto/rand / encoding/base64 / encoding/json / fmt / io / os / strings / strconv / text/tabwriter / time + golang.org/x/crypto/ssh + al.essio.dev/pkg/shellescape + 内部 errcodes 包）。
    ...
    4. **命名 helpers**：
       ...
       // GenerateShortSessionID 暴露给 cmd/claudedock/main.go 在 --new-session flag 触发时调用。
    ...
    6. **文件注册表 helpers**（D-12 完整方案 — Q1 RESOLVED；本 task 必须落地，Task 2.1b 调用）：
       ...
       ```go
       const clientsRegistryDir = "/workspace/.claudedock/clients"
       ...
       //   2. 远程写入 /workspace/.claudedock/clients/<pid>.json
    ...
    9. **本 task 范围限制**：
       ...
       - **不**改 ssh.go / mount_strategy.go / cmd/claudedock/*（Task 2.2 / 2.3）
  </action>
  <acceptance_criteria>
    - `go build ./internal/claudedock/...` 成功
    - `go vet ./internal/claudedock/...` 通过
    - `go test ./internal/claudedock/... -run "TestBuildTmuxSessionName|TestSanitizeSessionName|TestGenerateShortSessionID|TestParseTmuxListClients|TestRenderActivityAge|TestBuildTmuxRemoteCmd|TestBuildClaudeCmd" -count=1` 全部 PASS（≥ 10 用例）
    - `rg -n "func DetectTmux" internal/claudedock/session.go` 命中 1 行
    - `rg -n "type SessionConfig struct" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func GenerateShortSessionID" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func buildTmuxSessionName" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func sanitizeSessionName" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func buildTmuxRemoteCmd" internal/claudedock/session.go` 命中 1 行
    - `rg -n "tmux new-session -A -d -s" internal/claudedock/session.go` 命中（D-10 模板字面）
    - `rg -n "func writeClientFile\\(" internal/claudedock/session.go` 命中 1 行（**REQ-F5-B 关键**）
    - `rg -n "func removeClientFile\\(" internal/claudedock/session.go` 命中 1 行（**REQ-F5-B 关键**）
    - `rg -n "func readClientHostnames\\(" internal/claudedock/session.go` 命中 1 行（**REQ-F5-B 关键**）
    - `rg -n "schema_version" internal/claudedock/session.go` 命中（clientFileSchema struct）
    - `rg -n "/workspace/.claudedock/clients" internal/claudedock/session.go` 命中（registry 路径常量）
    - `rg -n "tmux display-message -p" internal/claudedock/session.go` 命中（取 client_pid）
    - `rg -n "shellescape.Quote" internal/claudedock/session.go` 命中（≥ 5 次）
    - `rg -n "simpleHash8" internal/claudedock/session.go` 命中（复用 mount_strategy.simpleHash8，禁止重新发明）
    - **不应**命中：`rg -n "func runClaudeWithSession" internal/claudedock/session.go` 应**为空**（Task 2.1b 落地）
    - **不应**命中：`rg -n "func performTakeOver|func printAttachBanner|func RunSessionsLs|func RunSessionsAttach" internal/claudedock/session.go` 应**为空**（Task 2.1b 落地）
  </acceptance_criteria>
  <verify>
    <automated>go test ./internal/claudedock/... -run "TestBuildTmuxSessionName|TestSanitizeSessionName|TestGenerateShortSessionID|TestParseTmuxListClients|TestRenderActivityAge|TestBuildTmuxRemoteCmd|TestBuildClaudeCmd" -count=1 &amp;&amp; go vet ./internal/claudedock/... &amp;&amp; rg -q "func writeClientFile" internal/claudedock/session.go &amp;&amp; rg -q "func removeClientFile" internal/claudedock/session.go &amp;&amp; rg -q "func readClientHostnames" internal/claudedock/session.go</automated>
  </verify>
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/session.go（Task 2.1a 落地的 DetectTmux / SessionConfig / 命名 helpers / 文件注册表 helpers / parseTmuxListClients / renderActivityAge / sshOutput）
    - internal/claudedock/ssh.go（runClaude line 168-241 — runClaudePTYBare 复刻蓝本；保留 PTY/SIGWINCH/ExitError）
    - internal/claudedock/keepalive.go（Plan 01）— RunKeepAlive 签名
    - internal/claudedock/reconnect.go（Plan 01）— NewReconnector / ConnState / ErrReconnectGaveUp / RegisterStateListener / ReconnectCount / FormatGiveUpMessage
    - internal/claudedock/input_buffer.go（Plan 01）— NewBufferedStdin / SetReconnector / Flush / Close
    - internal/claudedock/last_session.go（Plan 01）— LastSessionSnapshot + 现有 Read/Write API（grep 找现网名字 LoadLastSession / SaveLastSession，禁止改 schema）
    - internal/claudedock/colors.go（colorize / colorEnabled / ansiGreen / ansiGray）
    - internal/claudedock/exitcodes.go（ExitConfigError=4 / ExitNetworkError=2）
    - internal/claudedock/errcodes/codes.go（SESSION_NOT_FOUND / SESSION_TAKEOVER_NOTIFIED / SESSION_TAKEOVER_FAILED）
  ...
  <action>
    ...
    1. **performTakeOver**（D-11，调 sshOutput / sshRun helper）：
       ...
       func performTakeOver(conn *ssh.Client, sessionName string) error {
           ...
           msg := "[claudedock] 另一端已通过 --take-over 接管会话，本会话将在 3s 后断开"
  ...
  <acceptance_criteria>
    - `go build ./internal/claudedock/...` 成功
    - `go vet ./internal/claudedock/...` 通过
    - `go test ./internal/claudedock/... -run "TestRunClaudePTYWithReconnect|TestPerformTakeOver|TestPrintAttachBanner|TestBuildTmuxSessionName|TestSanitizeSessionName|TestGenerateShortSessionID|TestParseTmuxListClients|TestRenderActivityAge|TestBuildTmuxRemoteCmd|TestBuildClaudeCmd" -count=1` 全部 PASS（≥ 14 用例 — Task 2.1a 的 10 + 本 task 的 4）
    - `rg -n "func runClaudeWithSession\\(" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func runClaudePTYWithReconnect" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func runClaudePTYBare" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func performTakeOver" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func printAttachBanner" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func RunSessionsLs" internal/claudedock/session.go` 命中 1 行
    - `rg -n "func RunSessionsAttach" internal/claudedock/session.go` 命中 1 行
    - `rg -n "RunKeepAlive" internal/claudedock/session.go` 命中（runClaudePTYWithReconnect 内 goroutine）
    - `rg -n "NewReconnector" internal/claudedock/session.go` 命中
    - `rg -n "NewBufferedStdin" internal/claudedock/session.go` 命中
    - `rg -n "ErrReconnectGaveUp" internal/claudedock/session.go` 命中（兜底分支）
    - `rg -n "writeClientFile\\(" internal/claudedock/session.go` 命中 ≥ 2 处（定义 + runClaudeWithSession 内调用）
    - `rg -n "removeClientFile\\(" internal/claudedock/session.go` 命中 ≥ 2 处（定义 + defer 清理）
    - `rg -n "readClientHostnames\\(" internal/claudedock/session.go` 命中 ≥ 2 处（定义 + printAttachBanner 内调用）
    - `rg -n "writeLastSessionTmuxField\\(" internal/claudedock/session.go` 命中 ≥ 2 处（定义 + runClaudeWithSession 内调用）
    - `rg -n "primary" internal/claudedock/session.go` 命中（writeLastSessionTmuxField 调用时第二参数 = "primary"）
  </acceptance_criteria>
  <verify>
    <automated>go test ./internal/claudedock/... -run "TestRunClaudePTYWithReconnect|TestPerformTakeOver|TestPrintAttachBanner|TestBuildTmuxSessionName|TestSanitizeSessionName|TestGenerateShortSessionID|TestParseTmuxListClients|TestRenderActivityAge|TestBuildTmuxRemoteCmd|TestBuildClaudeCmd" -count=1 &amp;&amp; go vet ./internal/claudedock/... &amp;&amp; rg -q "func runClaudeWithSession" internal/claudedock/session.go &amp;&amp; rg -q "func performTakeOver" internal/claudedock/session.go &amp;&amp; rg -q "func printAttachBanner" internal/claudedock/session.go</automated>
  </verify>
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/ssh.go（ConnectAndRunClaudeV3 全函数 — 找到 OAuth 检查后、return runClaude 前的位置 line 130-141）
    - internal/claudedock/mount_strategy.go（MountConfig struct 定义位置 — 在末尾追加 3 字段）
    - internal/claudedock/session.go（Task 2.1 暴露的 SessionConfig + DetectTmux + runClaudeWithSession + GenerateShortSessionID）
    - .planning/phases/32-ssh-tmux/32-CONTEXT.md D-15 / D-16 / D-28 / D-29
    - .planning/phases/32-ssh-tmux/32-PATTERNS.md ssh.go 改造点 B
  </read_first>
  <action>
    1. **internal/claudedock/mount_strategy.go**：在 MountConfig struct 末尾追加 3 个字段（保持现有字段顺序与 JSON tag 不动）：
    ...
    2. **internal/claudedock/ssh.go**：在 ConnectAndRunClaudeV3 函数末尾（line 139-141 区间，OAuth 检查之后、`return runClaude(...)` 之前）插入 SessionConfig 构造 + DetectTmux + 路由：
  ...
  <acceptance_criteria>
    - `go build ./...` 成功
    - `go vet ./internal/claudedock/... ./cmd/claudedock/...` 通过
    - `rg -n "DetectTmux" internal/claudedock/ssh.go` 命中 1 行（在 ConnectAndRunClaudeV3 内）
    - `rg -n "runClaudeWithSession" internal/claudedock/ssh.go` 命中 1 行（路由 call site）
    - `rg -n "SESSION_TMUX_UNAVAILABLE" internal/claudedock/ssh.go` 命中 1 行（fallback banner）
    - `rg -n "SessionShortID" internal/claudedock/mount_strategy.go` 命中 1 行
    - `rg -n "SessionTakeOver" internal/claudedock/mount_strategy.go` 命中 1 行
    - `rg -n "LocalHostname" internal/claudedock/mount_strategy.go` 命中 1 行
    - `git diff internal/claudedock/ssh.go` 中 runClaude 函数体（line 168-241）应 zero diff（用 `git diff -U0 internal/claudedock/ssh.go | grep '^+' | grep -v '^+++' | wc -l` 复核：新增行数仅在 ConnectAndRunClaudeV3 内、约 20 行）
    - `go test ./internal/claudedock/... -count=1 -short` 现有所有非集成测试 PASS（不破坏 Phase 31 单测）
  </acceptance_criteria>
  <verify>
    <automated>go build ./... &amp;&amp; go vet ./internal/claudedock/... ./cmd/claudedock/... &amp;&amp; go test ./internal/claudedock/... -count=1 -short</automated>
  </verify>
...
<task type="auto">
  <name>Task 2.3: cmd/claudedock/sessions.go cobra 子命令 + main.go 注册 + flag 剥离 + KeepAlive 启动校验</name>
  <files>
    cmd/claudedock/sessions.go
    cmd/claudedock/main.go
  </files>
  <read_first>
    - cmd/claudedock/sync.go（newSyncCmd line 18-37 — sessions.go 严格 mirror）
    - cmd/claudedock/main.go（line 92 AddCommand / line 96-101 DisableFlagParsing / line 244-269 runRoot 的 --mount-mode 剥离 / line 335 mountCfg 构造）
    - cmd/claudedock/main.go（runEnvCheck line 163-201 — sessions ls/attach 同 LoadConfig + AuthenticateAndWait + sshConnect 模板）
    - internal/claudedock/session.go（Task 2.1 暴露的 RunSessionsLs / RunSessionsAttach / GenerateShortSessionID / SessionConfig）
    - internal/claudedock/errcodes/codes.go（SESSION_KEEPALIVE_TOO_AGGRESSIVE）
    - .planning/phases/32-ssh-tmux/32-CONTEXT.md D-13 / D-14 / D-03 第 4 条 / D-29
    - .planning/phases/32-ssh-tmux/32-PATTERNS.md cmd/claudedock/sessions.go 段 + main.go 4 处改造
  </read_first>
  <action>
    1. **cmd/claudedock/sessions.go**（新文件，mirror sync.go 结构）：
       ...
       func newSessionsCmd() *cobra.Command {
           cmd := &cobra.Command{
               ...
               Long:          "查看 / attach 容器内由 claudedock 创建的 tmux 会话；零控制面改造，纯客户端逻辑。",
               SilenceUsage:  true,
               SilenceErrors: true,
           }
           lsCmd := &cobra.Command{
       ...
       func runSessionsLs(cmd *cobra.Command, args []string) error {
           ...
           if err != nil { return err }
           defer conn.Close()
           return claudedock.RunSessionsLs(conn, os.Stdout)
       }
       ...
       func runSessionsAttach(cmd *cobra.Command, args []string) error {
           ...
           code, err := claudedock.RunSessionsAttach(conn, args[0], false, "/workspace")
           if err != nil {
       ...
       func connectForSessions(ctx context.Context) (*ssh.Client, *claudedock.AuthResponse, error) {
           ...
           // sshConnect 是 internal/claudedock unexported；需暴露 SSHConnect 或用现有 ConnectAndRunClaudeV3 的 sshConnect 同等入口
           // 现有 cmd 层调用 sshConnect 的方式：通过 claudedock.RunEnvCheck / claudedock.RunSSHDoctor 内部调
           // 本 task 直接复用：在 internal/claudedock 暴露 SSHConnect(SSHConfig) (*ssh.Client, error) — 极简包装
       ...
       > **planner 注**：`claudedock.SSHConnect` 当前可能不导出（小写 sshConnect）；执行器在 ssh.go 末尾追加：
       > ```go
    ...
    2. **cmd/claudedock/main.go** 4 处改动（按 PATTERNS §main.go 改造段）：
       ...
       **D. mountCfg 构造后注入 + KeepAlive 启动校验**（line 335 附近 — 找到 `mountCfg := claudedock.MountConfig{...}` 块后）：

       ```go
       mountCfg.SessionTakeOver = takeOver
       if newSession {
           mountCfg.SessionShortID = claudedock.GenerateShortSessionID()
       }
       if hostname, _ := os.Hostname(); hostname != "" {
           mountCfg.LocalHostname = hostname
       }
       ...
       注：`errcodes` 在 main.go 应已 import（Phase 31 已用）；如未则补上 `"github.com/claudedock/claudedock/internal/claudedock/errcodes"` 与 `"time"`。
  ...
  <acceptance_criteria>
    - `go build ./...` 成功
    - `go vet ./...` 通过
    - `rg -n "newSessionsCmd" cmd/claudedock/main.go` 命中 1 行（AddCommand 调用）
    - `rg -n "func newSessionsCmd" cmd/claudedock/sessions.go` 命中 1 行
    - `rg -n "RunSessionsLs|RunSessionsAttach" cmd/claudedock/sessions.go` 各命中 1 行
    - `rg -n "\"sessions\"" cmd/claudedock/main.go` 命中（DisableFlagParsing switch case）
    - `rg -n -- "--new-session" cmd/claudedock/main.go` 命中（剥离逻辑）
    - `rg -n -- "--take-over" cmd/claudedock/main.go` 命中
    - `rg -n "SESSION_KEEPALIVE_TOO_AGGRESSIVE" cmd/claudedock/main.go` 命中（启动校验）
    - `rg -n "GenerateShortSessionID" cmd/claudedock/main.go` 命中（newSession 触发）
    - `rg -n "func SSHConnect" internal/claudedock/ssh.go` 命中 1 行（export 包装）
    - `./claudedock --help` 输出含 `sessions` 子命令（手测；CI 可用 `go run ./cmd/claudedock --help 2>&1 | grep sessions` 验证）
    - `./claudedock sessions --help` 输出含 `ls` / `attach` 两个子命令
    - `./claudedock sessions ls 2>&1` 在无 config 时优雅失败（"配置文件未找到" 等中文错误，**不**panic / **不**段错误）
  </acceptance_criteria>
  <verify>
    <automated>go build ./... &amp;&amp; go vet ./... &amp;&amp; go run ./cmd/claudedock --help 2>&amp;1 | grep -q sessions &amp;&amp; go run ./cmd/claudedock sessions --help 2>&amp;1 | grep -q "attach"</automated>
  </verify>
  <done>cobra `claudedock sessions ls` / `attach <name>` 子命令路由打通；--new-session / --take-over flag 正确剥离不透传给远端 claude；KeepAlive 启动校验已注入；--help 树正确显示 sessions 节点；现有 Phase 31 集成测试不破坏。</done>
</task>
...
| T-32-20 | DoS | conn-A 频率上限（多端 attach + take-over + reconnect 串行 SSH 拨号） | accept | Phase 29 D-14 sshd_config `MaxSessions 30` / `MaxStartups 60:30:120` 已就位（容器侧硬上限，单 host 任意 claudedock 端数总和 ≤ 30）+ Plan 01 reconnect 退避序列 1/2/4/8/30s 限制同 host 并发拨号节奏；M12 实质防御已在镜像层落地，本 plan 不引入新攻击面 |
...
1. **SC2（REQ-F4-A 30s 抖动 tmux 不丢进程）**：手动 / 集成测试（Plan 03 集成测试套件中验收，本 plan 提供必要 service）— `docker network disconnect <ctr> <net> ; sleep 30 ; docker network connect <ctr> <net>` 后 claudedock reconnect 成功 + tmux session 内 claude 进程仍在 + buffer 完整
2. **SC7（REQ-F4-C tmux 不可用降级）**：DetectTmux 返回 false 时 stderr 输出 [SESSION_TMUX_UNAVAILABLE]；claudedock **不退出非 0**，仍能进入 claude（fallback 走 v2.0 runClaude）
3. **SC8（REQ-F4-B sessions ls/attach）**：`go run ./cmd/claudedock sessions --help` 输出含 ls / attach 两个子命令；运行时 `tmux list-sessions` 失败（无 server）→ 输出"当前容器内无活跃 tmux session"，退出 0；`sessions attach nonexistent` → stderr [SESSION_NOT_FOUND] + 退出 ExitConfigError(=4)
...
go build ./... \
  && go vet ./... \
  && go test ./internal/claudedock/... -run "TestBuildTmuxSessionName|TestSanitizeSessionName|TestGenerateShortSessionID|TestParseTmuxListClients|TestRenderActivityAge|TestBuildTmuxRemoteCmd|TestBuildClaudeCmd" -count=1 \
  && go test ./internal/claudedock/... -count=1 -short \
  && go run ./cmd/claudedock --help 2>&1 | grep -q sessions \
  && go run ./cmd/claudedock sessions --help 2>&1 | grep -E "^\s+(ls|attach)"
```
</verification>
...
- cmd/claudedock/sessions.go 全新文件，mirror sync.go 结构
...
- `go run ./cmd/claudedock sessions --help` 输出有效；ls / attach 子命令可见
- 无新依赖加入 go.mod
...
- cmd/claudedock/sessions.go 与 sync.go 的结构对照
