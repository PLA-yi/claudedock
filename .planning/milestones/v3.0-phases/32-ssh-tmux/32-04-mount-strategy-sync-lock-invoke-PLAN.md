...
must_haves:
  truths:
    - "SC11（REQ-F5-D）同一 claude_account 第二个 claudedock 启动时，Mutagen sync 不重复创建（账号级单例锁）：mount_strategy.MountWorkspace 在 mountMutagen 之前调用 mountCfg.SyncSessionLock(mountCfg.ClaudeAccountID)，后连端只读 sshfs / mergerfs 视图"
    - "mount_strategy.MountWorkspace 在能力降级之后、tryOrder 循环之前调用 mountCfg.SyncSessionLock(mountCfg.ClaudeAccountID) —— `rg 'mountCfg\\.SyncSessionLock\\(' internal/claudedock/mount_strategy.go` 命中 ≥ 1 次（从 Phase 31 D-31 '字段预留' 升级为 '真实 invoke'）"
  ...
  artifacts:
    ...
    - path: "internal/claudedock/mount_strategy_test.go"
...
- **Plan 03（Wave 3 原始）必须已 ship**：本 plan 直接消费：
  - `claudedock.ErrSyncLocked`（Plan 03 Task 3.1）
...
**根因**：`internal/claudedock/mount_strategy.go::MountWorkspace` 函数体内**从未调用** `mountCfg.SyncSessionLock(accountID)`。Phase 31 D-31 仅在 `MountConfig` 上预留字段 + Plan 03 在 ssh.go 注入真实 `AcquireSyncLock` 闭包 + `TestIntegration_Phase32_SyncLockMutexes` 能证明锁语义本身正确，但下游 `MountWorkspace` 从未触发该闭包 —— 生产多端启动场景 flock 永远不会被拿到，两端 claudedock 仍会并行 mutagen create / sync，**M15 双写风险未被实际防御**。
...
internal/claudedock/sync_lock.go（Plan 03 Task 3.1）：
...
internal/claudedock/ssh.go ConnectAndRunClaudeV3（Plan 03 Task 3.2，line 87-111）：
...
internal/claudedock/mount_strategy.go MountConfig 现状（Plan 03 Task 3.2 已加 IsSecondaryClient）：
...
internal/claudedock/last_session.go DowngradeStep（Phase 31 ship）：
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/mount_strategy.go（line 115-227 MountWorkspace 全函数体；定位能力降级块 line 155-166 末尾 —— 必须在此之后、tryOrder 决策 line 168-181 之前插入 SyncSessionLock invoke）
    - internal/claudedock/mount_strategy.go（MountConfig struct line 74-101 —— 确认 SyncSessionLock + IsSecondaryClient 两字段 spelling；确认 applyDowngrade helper line 408-416 签名）
    - internal/claudedock/mount_strategy_test.go（现有 9 个测试；注意用例 line 133-160 MountWorkspace_Auto 表驱动用例 / line 187-218 Test_APFSCaseInsensitive_WritesLastSession / line 225-... hooks 注入方式 —— 本 task 新测试严格 mirror hooks 模板，**不污染**现有 table 用例）
    - internal/claudedock/sync_lock.go（AcquireSyncLock 签名 + ErrSyncLocked sentinel）
    - internal/claudedock/ssh.go line 87-111（Plan 03 已 ship 的 SyncSessionLock 注入闭包 —— 确认生产路径 mountCfg.SyncSessionLock 永不 nil；但本 plan 仍在 MountWorkspace 加 nil-guard 兼容 test 路径）
    - internal/claudedock/last_session.go line 32-40（DowngradeStep 字段名精确确认：From / To / ReasonCode / ReasonMessage，**不是** Reason）
  ...
  <acceptance_criteria>
    ...
    - `go test ./internal/claudedock/... -count=1 -short` PASS（包含现有 9 个 mount_strategy 测试 + 1 新增 TestMountWorkspace_SyncLocked）
    - `rg -n "mountCfg\\.SyncSessionLock\\(" internal/claudedock/mount_strategy.go` 命中 ≥ 1 次（**Gap #2 核心断言**）
    - `rg -n "errors\\.Is\\(.*ErrSyncLocked\\)" internal/claudedock/mount_strategy.go` 命中 1 次
    - `rg -n "sync_locked" internal/claudedock/mount_strategy.go` 命中 ≥ 1 次（ReasonCode 字符串字面值）
    - `rg -n "mountCfg\\.IsSecondaryClient\\s*=\\s*true" internal/claudedock/mount_strategy.go` 命中 1 次
    - `go test -run TestMountWorkspace_SyncLocked ./internal/claudedock/...` 退出码 0 且用例 PASS
    - `rg -n "func TestMountWorkspace_SyncLocked" internal/claudedock/mount_strategy_test.go` 命中 1 次
    - `git diff internal/claudedock/mount_strategy.go | grep '^+' | grep -v '^+++' | wc -l` 输出 ≤ 30（最小侵入：插入点 ~15-20 行 + 如需改动 cleanup 包装 ~5 行）
    - Phase 31 既有 9 个测试 `go test -run "TestMountWorkspace_Auto|Test_BannerColors|Test_APFSCaseInsensitive_WritesLastSession|TestMountWorkspace_Force" ./internal/claudedock/...` 全 PASS（nil-guard 保证现有用例 SyncSessionLock=nil 不触发新分支）
  </acceptance_criteria>
  <action>
    ...
    读 `internal/claudedock/mount_strategy.go`，找到能力降级块的末尾（现状 line 166）：
    ...
    **实际简化方案**（避免 defer 闭包复杂化函数控制流，执行器优先采纳这个）：
    ...
    追加到 `internal/claudedock/mount_strategy_test.go` 末尾（复用现有 `newHooks` / `hooks` / `strategyHooks` 模板）：
  ...
  <verify>
    <automated>go test ./internal/claudedock/... -count=1 -short &amp;&amp; rg -q "mountCfg\\.SyncSessionLock\\(|cfg\\.SyncSessionLock\\(" internal/claudedock/mount_strategy.go &amp;&amp; rg -q "errors\\.Is\\(.*ErrSyncLocked\\)" internal/claudedock/mount_strategy.go &amp;&amp; rg -q "sync_locked" internal/claudedock/mount_strategy.go &amp;&amp; rg -q "func TestMountWorkspace_SyncLocked\\b" internal/claudedock/mount_strategy_test.go</automated>
  </verify>
  <done>
...
| T-32G2-02 | DoS | 非 ErrSyncLocked 错误让 claudedock 启动失败 | accept | 原设计意图：flock 不可用（极旧镜像）意味着 M15 防御失效，应拒绝启动让用户感知；替代方案"静默降级"违反 M13；该用户体验权衡在 VERIFICATION.md `missing[2]` 明确"其它 lockErr → 错误传递"已认可 |
...
1. **根因修复 grep 断言**：`rg -n "mountCfg\\.SyncSessionLock\\(|cfg\\.SyncSessionLock\\(" internal/claudedock/mount_strategy.go` ≥ 1 hit —— 从 Phase 31 "字段预留" 升级为 "真实 invoke"
...
go build ./... \
  ...
  && rg -c "mountCfg\\.SyncSessionLock\\(|cfg\\.SyncSessionLock\\(" internal/claudedock/mount_strategy.go \
  && rg -c "errors\\.Is\\(.*ErrSyncLocked\\)" internal/claudedock/mount_strategy.go \
  && rg -c "sync_locked" internal/claudedock/mount_strategy.go \
  && go test -run "TestMountWorkspace_SyncLocked|TestMountWorkspace_SyncLockSuccess|TestMountWorkspace_SyncLockOtherError" ./internal/claudedock/... -v
```
...
- Gap #2 grep 验证：
  ...
  - `go test -run TestMountWorkspace_SyncLocked ./internal/claudedock/...` exit 0
- Phase 31 既有所有测试 PASS（证据：`go test ./internal/claudedock/... -count=1 -short` 尾部 OK）
</success_criteria>
...
- Gap #2 闭合后 SC11 端到端链路文字描述：`claudedock 启动 → MountWorkspace 调 mountCfg.SyncSessionLock → ssh.go 闭包调 AcquireSyncLock → 容器内 flock -n -E 99 → 第二端拿不到 → ErrSyncLocked → MountWorkspace 强制 ModeSSHFSOnly + DowngradeChain + IsSecondaryClient=true → SessionConfig 透传 → last-session.json client_role=secondary`
