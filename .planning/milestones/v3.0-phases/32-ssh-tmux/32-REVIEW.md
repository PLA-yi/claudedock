...
**File:** `internal/claudedock/session.go:703-712` （`pTYAttachOnce`）
...
**File:** `internal/claudedock/session.go:614-660, 671-766`
...
**File:** `internal/claudedock/session.go:724-730` （`pTYAttachOnce`），`internal/claudedock/input_buffer.go:58-88`
...
**File:** `internal/claudedock/input_buffer.go:90-141`
...
**File:** `internal/claudedock/session.go:200-209`
...
由于前面 `command -v tmux >/dev/null && exec tmux ...` 在 DetectTmux 已通过的前提下几乎必走，fallback 分支生产环境很难命中。但留着会让未来 tmux 远端突然消失 / 启动失败时 claudedock 直接报错而非降级，违背设计意图。
...
**File:** `internal/claudedock/sync_lock.go:63`

```go
lockPath := fmt.Sprintf("/tmp/claudedock/locks/sync-%s.lock", accountID)
```

后续 `shellescape.Quote(lockPath)` 防 shell 注入（OK），但若 `accountID` 含 `/` 或 `..`，flock 仍会去 `/tmp/claudedock/locks/sync-../../etc/passwd.lock` 这种位置创锁文件。当前 accountID 来自 gateway AuthResponse（受信），属于深度防御问题。
...
**File:** `internal/claudedock/last_session.go:67-74`
...
**File:** `internal/claudedock/keepalive.go:51-67`
...
**File:** `internal/claudedock/session.go:671-673`
...
**File:** `internal/claudedock/session.go:881-883`
...
### IN-06: `cmd/claudedock/sessions.go::runSessionsAttach` 在 os.Exit 前不会跑 deferred conn.Close

**File:** `cmd/claudedock/sessions.go:58-75`

```go
defer conn.Close()
code, err := claudedock.RunSessionsAttach(...)
if err != nil {
...
**File:** `internal/claudedock/errcodes/net.go:31-36` + `internal/claudedock/reconnect.go`
