...
files_modified:
  ...
  - internal/claudedock/mutagen_bin/darwin_amd64/mutagen
  - internal/claudedock/mutagen_bin/darwin_arm64/mutagen
  - internal/claudedock/mutagen_bin/linux_amd64/mutagen
  - internal/claudedock/mutagen_bin/linux_arm64/mutagen
...
must_haves:
  truths:
    ...
    - "internal/claudedock/mutagen_bin.go 通过 //go:embed 嵌入 4 平台 mutagen 二进制（按 GOOS_GOARCH 选择），ExtractTo 函数幂等：已存在且 mutagen version 含 0.18.1 直接复用，否则覆盖写入 0755"
  ...
  artifacts:
    ...
    - path: "internal/claudedock/errcodes/mount.go"
    ...
    - path: "internal/claudedock/errcodes/codes_test.go"
      provides: "注册表完整性单测：去重、命名正则、中文文案非空、NextAction 长度、≥15 条计数"
      contains: "TestErrcodesRegistry"
    - path: "internal/claudedock/mutagen_bin.go"
      provides: "go:embed Mutagen v0.18.1 多平台二进制 + ExtractTo 幂等抽取"
      contains: "//go:embed mutagen_bin"
    - path: "internal/claudedock/mutagen_bin/SHA256SUMS"
      provides: "4 平台 mutagen 二进制 sha256 列表（fetch 脚本写入，运行时校验）"
      contains: "mutagen"
    - path: "internal/claudedock/envcheck_fs.go"
      provides: "跨平台大小写敏感探测函数 IsCaseInsensitiveFS"
      contains: "IsCaseInsensitiveFS"
    - path: "scripts/fetch-mutagen-bins.sh"
      provides: "从 GitHub release 拉取 mutagen v0.18.1 4 平台二进制 + sha256 校验 + 落到 internal/claudedock/mutagen_bin/<plat>/mutagen"
      contains: "v0.18.1"
  key_links:
    - from: "internal/claudedock/mutagen_bin.go"
      to: "internal/claudedock/mutagen_bin/<GOOS_GOARCH>/mutagen"
      via: "go:embed mutagen_bin"
      pattern: "go:embed mutagen_bin"
    - from: "internal/claudedock/errcodes/codes_test.go"
      to: "internal/claudedock/errcodes/{mount,net}.go"
...
internal/claudedock/errcodes/codes.go 导出：
...
internal/claudedock/mutagen_bin.go 导出：

```go
package claudedock

// MutagenBinaryVersion 是 embed 的 Mutagen 客户端版本。Plan 02 的版本握手会比对此常量与远端 /etc/claudedock/mutagen.version。
const MutagenBinaryVersion = "v0.18.1"

// ExtractMutagenBinary 把当前 GOOS_GOARCH 的 embed mutagen 二进制写到 dst（建议 ~/.claudedock/bin/mutagen）。
// 行为：
...
|------|----------|----------------------|------------------------|
| MOUNT_MUTAGEN_VERSION_SKEW | SeverityError | Mutagen 客户端版本 (%s) 与容器内 agent 版本 (%s) 不一致，已降级到 sshfs-only | 升级容器镜像到 v3.0.0+ 或重装 claudedock |
...
| MOUNT_MUTAGEN_SAFETY_GUARD | SeverityFatal | 检测到本地目录 %s 为空但容器内 /workspace-hot 已有文件，拒绝同步以防反向清空 | 如确认从远端拉取，先 claudedock exec rsync /workspace-hot/ ./ |
| MOUNT_MUTAGEN_DAEMON_UNAVAILABLE | SeverityError | Mutagen daemon 启动失败: %s | 检查 ~/.claudedock/mutagen/ 目录权限，或重启 claudedock |
| MOUNT_MUTAGEN_SYNC_FAILED | SeverityError | Mutagen sync 创建失败: %s | 检查 SSH 连通性，或运行 claudedock doctor mount |
| MOUNT_MUTAGEN_TRANSPORT_FAILED | SeverityError | Mutagen ssh 子进程启动失败: %s | 检查本机 ssh 客户端是否可用，或安装 sshpass 作为后备 |
| MOUNT_SSHFS_FAILED | SeverityError | sshfs 挂载失败: %s | 检查 /dev/fuse 是否可用，或运行 claudedock doctor ssh |
| MOUNT_SSHFS_DISCONNECTED | SeverityWarn | sshfs 已断开 ≥15 秒，已从 mergerfs 摘除 /workspace-cold | 网络恢复后运行 claudedock doctor mount --fix 重新挂载 |
| MOUNT_MERGERFS_FAILED | SeverityError | mergerfs 挂载失败: %s | 检查容器是否启用 SYS_ADMIN + /dev/fuse，或运行 claudedock doctor mount |
| MOUNT_AUTO_DOWNGRADED | SeverityWarn | 文件映射已从 %s 降级到 %s，原因: [%s] %s | 运行 claudedock doctor mount 查看详细修复建议 |
| MOUNT_FORCE_MODE_FAILED | SeverityFatal | --mount-mode=%s 模式下 %s 层失败: %s | 移除 --mount-mode flag 让自动降级生效，或运行 claudedock doctor mount |
...
| NET_OAUTH_EXPIRED | SeverityFatal | Claude OAuth 凭证已过期（账号: %s） | 在容器内运行 claudedock exec claude login 重新登录 |
| NET_OAUTH_EXPIRING_SOON | SeverityWarn | Claude OAuth 凭证将在 %d 分钟后过期 | 建议尽快 claudedock exec claude login |
| NET_OAUTH_NOT_FOUND | SeverityFatal | 容器内未找到 Claude OAuth 凭证文件（账号: %s） | 在容器内运行 claudedock exec claude login 完成首次登录 |
</errcode_registry>
</context>
...
<task type="auto">
  ...
  <read_first>
    ...
    - internal/claudedock/ssh_doctor.go（v2.0 错误返回风格 — 中文 + fmt.Errorf 包装；errcodes 包必须保留 errors.As/Is 兼容）
  </read_first>
  <action>
    1. 创建包目录 internal/claudedock/errcodes/。

    2. internal/claudedock/errcodes/codes.go：
    ...
    3. internal/claudedock/errcodes/mount.go：在 init() 函数中按 <errcode_registry> 表前 12 条（MOUNT_*）顺次 MustRegister。code 与文案逐字符对齐表格内容（含中文标点 / 占位符 %s %d）。

    4. internal/claudedock/errcodes/net.go：在 init() 函数中按 <errcode_registry> 表后 3 条（NET_OAUTH_*）顺次 MustRegister。

    5. internal/claudedock/errcodes/codes_test.go：
  ...
  <acceptance_criteria>
    - 所有 4 个文件存在且 `gofmt -l` 输出为空：
      `gofmt -l internal/claudedock/errcodes/`
    - `go build ./internal/claudedock/errcodes/` 退出码 0
    - `go test ./internal/claudedock/errcodes/ -count=1 -v` 退出码 0；输出包含 `TestErrcodesRegistry`、`TestFormat_Render`、`TestFormat_UnknownCode`、`TestLookup_Hit`、`TestLookup_Miss` 五项 PASS
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/mount.go` 输出 12
    - `grep -c "MustRegister(Entry{" internal/claudedock/errcodes/net.go` 输出 3
    - `grep -E "MOUNT_MUTAGEN_VERSION_SKEW.*Code = " internal/claudedock/errcodes/codes.go` 命中 1 行
    - `grep -E "^[[:space:]]+建议: " internal/claudedock/errcodes/codes.go` 命中至少 1 行（Format 模板）
    - 测试 TestFormat_Render 字面断言通过 `[MOUNT_MUTAGEN_VERSION_SKEW] Mutagen 客户端版本 (v0.18.1)`
  </acceptance_criteria>
  <verify>
    <automated>go test ./internal/claudedock/errcodes/ -count=1 -v && [ "$(grep -c 'MustRegister(Entry{' internal/claudedock/errcodes/mount.go)" = 12 ] && [ "$(grep -c 'MustRegister(Entry{' internal/claudedock/errcodes/net.go)" = 3 ] && gofmt -l internal/claudedock/errcodes/ | wc -l | grep -q '^0$'</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/31-cli/31-RESEARCH.md §1.1（实际单文件 ~12MB / 共 ~49MB，claudedock 终态 ~80MB；GitHub release tarball 命名）
  ...
  <action>
    1. scripts/fetch-mutagen-bins.sh（可执行 chmod +x）：
       ...
       #   默认：拉取并写入 internal/claudedock/mutagen_bin/<plat>/mutagen
       #   --check-only：只校验已存在文件的 sha256，不联网
       set -euo pipefail

       VERSION="v0.18.1"
       BASE_URL="https://github.com/mutagen-io/mutagen/releases/download/${VERSION}"
       OUT_DIR="$(cd "$(dirname "$0")/.." && pwd)/internal/claudedock/mutagen_bin"
    ...
    2. internal/claudedock/mutagen_bin/.gitattributes（git 不做 EOL 转换；如果团队启用 git LFS，按需追加）：
    ...
    4. 在 .gitignore 中**不**排除 `internal/claudedock/mutagen_bin/`（这些二进制需进入 git，否则 go:embed 会失败）；同时确认 `scripts/fetch-mutagen-bins.sh` 不在已有 .gitignore 中。
  </action>
  <acceptance_criteria>
    ...
    - 4 个平台目录均存在：`for d in darwin_amd64 darwin_arm64 linux_amd64 linux_arm64; do test -d internal/claudedock/mutagen_bin/$d || exit 1; done`
    - `internal/claudedock/mutagen_bin/SHA256SUMS` 存在且至少 4 行（每行 `<sha>  <plat>/mutagen`）；如二进制由 CI 后续补齐，4 行至少含 PENDING-FETCH 占位
    - 联网执行 `scripts/fetch-mutagen-bins.sh --check-only` 退出码 0（前提：二进制已 fetch）；离线 commit 场景跳过此项但 SUMMARY.md 必须说明
    - `internal/claudedock/mutagen_bin/.gitattributes` 存在
  </acceptance_criteria>
  <verify>
    <automated>test -x scripts/fetch-mutagen-bins.sh && bash -n scripts/fetch-mutagen-bins.sh && for d in darwin_amd64 darwin_arm64 linux_amd64 linux_arm64; do test -d internal/claudedock/mutagen_bin/$d || exit 1; done && test -f internal/claudedock/mutagen_bin/SHA256SUMS && grep -F 'VERSION="v0.18.1"' scripts/fetch-mutagen-bins.sh</automated>
  </verify>
  <done>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    - .planning/phases/31-cli/31-CONTEXT.md（D-04 — extract 路径 ~/.claudedock/bin/mutagen + 幂等校验逻辑；D-09 修订为 Go probe）
    ...
    - internal/claudedock/config.go（ConfigDir 函数 — extract 父目录沿用 ~/.claudedock/）
    - internal/claudedock/errcodes/codes.go（本 plan Task 1.1 产出；Format 函数与 MOUNT_MUTAGEN_TRANSPORT_FAILED 常量）
  </read_first>
  <action>
    1. internal/claudedock/mutagen_bin.go：
       ...
       注意 import path：本仓库 module `github.com/claudedock/claudedock`（go.mod 已确认）。

    2. internal/claudedock/envcheck_fs.go：
    ...
    3. internal/claudedock/mutagen_bin_test.go：按 <behavior> 实现 4 个用例。
    ...
    4. internal/claudedock/envcheck_fs_test.go：按 <behavior> 实现 2 个用例，用 t.TempDir() 隔离。

    5. **关键导入注意**：本 plan 不在 claudedock 包内删/改 mount.go / ssh.go 任何代码（属于 Plan 02 的范围）。本 plan 新增的 mutagen_bin.go / envcheck_fs.go 不被任何现网代码 import，编译通过即可。
  </action>
  <acceptance_criteria>
    - `gofmt -l internal/claudedock/mutagen_bin.go internal/claudedock/envcheck_fs.go` 输出为空
    - `go vet ./internal/claudedock/...` 退出码 0
    - `go build ./internal/claudedock/...` 退出码 0（embed 路径必须存在 mutagen_bin/<plat>/mutagen 才能编译；占位文件即可）
    - `go test ./internal/claudedock/ -run Test_ExtractMutagenBinary -count=1 -v` 至少 1 个用例 PASS（Idempotent 在无真二进制环境可 SKIP）
    - `go test ./internal/claudedock/ -run Test_IsCaseInsensitiveFS -count=1 -v` 全 PASS
    - `grep -F '//go:embed mutagen_bin' internal/claudedock/mutagen_bin.go` 命中 1 行
    - `grep -F 'MutagenBinaryVersion = "v0.18.1"' internal/claudedock/mutagen_bin.go` 命中 1 行
    - `grep -E 'IsCaseInsensitiveFS\(dir string\) bool' internal/claudedock/envcheck_fs.go` 命中 1 行
    - 整仓 `go test ./... -count=1` 退出码 0（不破坏 v2.0 现有测试）
  </acceptance_criteria>
  <verify>
    <automated>go build ./internal/claudedock/... && go vet ./internal/claudedock/... && go test ./internal/claudedock/ -run 'Test_ExtractMutagenBinary|Test_IsCaseInsensitiveFS' -count=1 -v && go test ./... -count=1</automated>
  </verify>
  <done>
    claudedock 包新增 ExtractMutagenBinary（go:embed + 幂等抽取）+ IsCaseInsensitiveFS（跨平台 probe）两个对外函数；4 + 2 个单元测试 PASS；不破坏 v2.0 现有 mount_test / ssh_doctor_test 等任何回归测试。
  </done>
</task>
...
```bash
# 1. 包结构与 errcodes 注册表
test -d internal/claudedock/errcodes
test -f internal/claudedock/errcodes/codes.go
test -f internal/claudedock/errcodes/mount.go
test -f internal/claudedock/errcodes/net.go
test -f internal/claudedock/errcodes/codes_test.go
go test ./internal/claudedock/errcodes/ -count=1   # 应全 PASS
[ "$(grep -c 'MustRegister(Entry{' internal/claudedock/errcodes/mount.go)" = 12 ]
[ "$(grep -c 'MustRegister(Entry{' internal/claudedock/errcodes/net.go)" = 3 ]

# 2. mutagen_bin embed 与 fetch 脚本
test -x scripts/fetch-mutagen-bins.sh
bash -n scripts/fetch-mutagen-bins.sh
test -d internal/claudedock/mutagen_bin/darwin_amd64
test -d internal/claudedock/mutagen_bin/darwin_arm64
test -d internal/claudedock/mutagen_bin/linux_amd64
test -d internal/claudedock/mutagen_bin/linux_arm64
test -f internal/claudedock/mutagen_bin/SHA256SUMS
go build ./internal/claudedock/...                  # embed 路径完整即可编译

# 3. envcheck_fs 探测
go test ./internal/claudedock/ -run Test_IsCaseInsensitiveFS -count=1 -v

# 4. 整仓回归
go test ./... -count=1                                # 不允许破坏 v2.0 任何现有测试
gofmt -l internal/claudedock/                       # 输出必须为空
go vet ./...                                          # 退出码 0
```
...
| embed 二进制 → 用户 ~/.claudedock/bin/ | ExtractMutagenBinary 写入用户目录；若 embed 内容被篡改，写入的就是恶意二进制 |
...
| T-31-01-02 | Tampering | git 仓库内的 internal/claudedock/mutagen_bin/<plat>/mutagen | accept | 仓库受 git 控制（commit hash + LFS hash 不可篡改）；如启用 LFS 则 Object 哈希由 LFS 服务校验；4 个平台二进制不变更（锁定 v0.18.1）确保 commit diff 唯一 |
...
| T-31-01-04 | Denial of Service | embed 二进制 ~49MB 拖慢编译 / 二进制下载体积 | accept | RESEARCH §1.1 已记录权衡，claudedock 终态 ~80MB 可接受；v3.1 可分平台 build tag 优化 |
| T-31-01-05 | Spoofing | 用户已有的 brew mutagen daemon 与本进程冲突 | mitigate | RESEARCH §1.1 + Plan 02 ExtractMutagenBinary 抽取到独立目录 ~/.claudedock/bin/，且 MUTAGEN_DATA_DIRECTORY 强制 ~/.claudedock/mutagen/（Plan 02 落实），与 brew 默认路径完全隔离 |
...
- 已知限制：claudedock 二进制体积从 ~30MB 涨到 ~80MB（RESEARCH §1.1 已知）
- 与 Plan 02 的接口契约：Plan 02 的 mount_mutagen.go 直接 import errcodes 包 + 调 ExtractMutagenBinary(filepath.Join(home, ".claudedock/bin/mutagen"))
</output>
