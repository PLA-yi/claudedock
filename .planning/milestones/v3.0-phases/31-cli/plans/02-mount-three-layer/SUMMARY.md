...
requires:
  ...
  - "Plan 01 claudedock.ExtractMutagenBinary（占位 stub，CI 替换为真二进制）"
  - "Plan 01 claudedock.IsCaseInsensitiveFS（APFS 探测）"
...
affects:
  ...
  - "Wave 3 Plan 03 cmd/claudedock/sync.go：banner 后 ⚠ N 个文件同步冲突 ... 已就位，conflict 计数由 Plan 03 mount_mutagen.go sync list --template 解析"
  - "Phase 32 多端冲突：MountConfig.SyncSessionLock 已是 hook 字段（本阶段 noop），Phase 32 注入真实锁实现"
  - "Phase 34 doctor：复用 MutagenHealthCheck + 读 ~/.claudedock/last-session.json 展示降级链；exitcodes.go 表供 explain 子命令引用"
tech-stack:
  added: []
  patterns:
...
key-files:
  ...
  modified:
    - "internal/claudedock/ssh.go (新增 ConnectAndRunClaudeV3；ConnectAndRunClaude 转 V3 + sshfs-only)"
    - "cmd/claudedock/main.go (新增 --mount-mode flag 注册 + 手工解析 + 切到 ConnectAndRunClaudeV3)"
decisions:
  ...
  - "exitcodes.go 新增不动 cmd/claudedock/main.go 现有 exit* 常量（rename 留 Phase 34），仅 Plan 03 新代码引用 claudedock.Exit*"
metrics:
...
| 2.3 | ConnectAndRunClaudeV3 + cobra --mount-mode flag 接线 | 9a75372 | ssh.go / cmd/claudedock/main.go |
...
exitcodes_test.go (5 用例):
  ...
  Test_ExitCodes_V2Compat                  PASS  (0-5 与 cmd/claudedock/main.go 对齐)
...
- **Fix:** `gofmt -w internal/claudedock/mount_{merge,mutagen,strategy}.go`，全部消化。
- **Commit:** 5cee2a1

### Deferred Issues (Out of Scope)

- `internal/claudedock/envcheck.go` / `internal/claudedock/ssh_doctor_test.go` 仍然 gofmt -l 有警告（pre-existing，Plan 01 SUMMARY 已记录，本 plan 同样按 SCOPE BOUNDARY 不动）。
...
| 文件 | 行 | Stub 类型 | 由谁补齐 |
|------|----|----|----|
| `internal/claudedock/mutagen_bin/{darwin,linux}_{amd64,arm64}/mutagen` | 整文件 | 占位 shell stub（Plan 01 遗留） | CI build-images workflow `bash scripts/fetch-mutagen-bins.sh` |
| `internal/claudedock/mount_strategy.go::tryModeReal` | 311-356 | 生产路径已写但未端到端测过（仅 hooks 单测） | Plan 03 集成测试或 Phase 34 doctor 真实部署验证 |
...
1. **OAuth 检查 hook 点**：`internal/claudedock/ssh.go::ConnectAndRunClaudeV3` 在 mount ready 之后、`runClaude` 之前留有 `TODO(plan-03)` 注释（具体行号见 `git grep 'TODO(plan-03)' internal/claudedock/ssh.go`）。Plan 03 Task 3.2 在此插入 `CheckOAuthCredentials(connA, mountCfg.ClaudeAccountID)`，命中 NET_OAUTH_NOT_FOUND/EXPIRED 时返回 `claudedock.ExitOAuthNotFound/Expired`。
2. **conflict count 上报**：`mount_strategy.MountWorkspace` 已支持 `conflicts > 0` 时输出 `⚠ 有 N 个文件同步冲突，运行 claudedock sync conflicts 查看`。conflicts 来自 `mountMutagen` 返回的第二个返回值（当前总返回 0，因为本 plan 未集成 `mutagen sync list --template`）。Plan 03 在 mountMutagen 内部加上 sync list 调用 + parse template 即可。
3. **sync 子命令**：`cmd/claudedock/sync.go` 由 Plan 03 创建，复用 `claudedock.Mode` 与 `claudedock.Exit*` 常量。
4. **exitcodes 引用**：Plan 03 OAuth 检查 + mount_strategy ModeForce 失败路径必须使用 `claudedock.Exit*` 常量而非裸数字。
...
3. **exitcodes 与 cmd/claudedock/main.go 双套常量**：本 plan 新增 `claudedock.Exit*` 但未 rename `cmd/claudedock/main.go::exit*`，避免大范围 git diff 与潜在行为回归；rename 工作留 Phase 34。
...
- [x] `internal/claudedock/mount_mutagen.go` 存在
- [x] `internal/claudedock/mount_mutagen_test.go` 存在
- [x] `internal/claudedock/mount_merge.go` 存在
- [x] `internal/claudedock/mount_strategy.go` 存在
- [x] `internal/claudedock/mount_strategy_test.go` 存在
- [x] `internal/claudedock/exitcodes.go` 存在
- [x] `internal/claudedock/exitcodes_test.go` 存在
- [x] `internal/claudedock/ssh.go` 包含 `ConnectAndRunClaudeV3`
- [x] `cmd/claudedock/main.go` 包含 `--mount-mode` + `claudedock.ParseMode`
...
- [x] `claudedock --mount-mode=invalid` 输出 `错误: --mount-mode 必须是 ...` 并退出 4 (exitConfigError)
- [x] 关键命令字符级一致：mergerfs opts / setfattr / sshfs reconnect / mutagen --mode / --default-owner-beta / --ignore-vcs 全部 grep 命中
- [x] `! grep -F 'sync list --json' internal/claudedock/mount_mutagen.go` 命中（D-28 修订落实）
