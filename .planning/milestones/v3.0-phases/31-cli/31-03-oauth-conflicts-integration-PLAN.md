...
must_haves:
  truths:
    ...
    - "OAuth 失败优先级高于 mount 失败：mount 与 OAuth 在同一 errgroup 中并发，OAuth 返回 Fatal 错误时先 cleanup mount，stderr 输出 errcodes.Format(NET_OAUTH_*) + 退出对应命名常量（claudedock.ExitOAuthNotFound=6、claudedock.ExitOAuthExpired=7；引用 Plan 02 Task 2.4 创建的 exitcodes.go，避免裸数字与 v2.0 main.go ExitConfigError=4/ExitInternalError=5 撞码）；ExpiringSoon 仅警告不阻断"
    ...
    - "cmd/claudedock/sync.go 注册 sync conflicts 子命令；调 mutagen sync list --long 渲染冲突文件清单（path / alpha / beta / mtime）"
    - "internal/claudedock/integration_test.go（//go:build integration）覆盖 6 个 RESEARCH §6.2 关键场景；scripts/test-fixture-{up,down}.sh 用 docker compose 起停 Phase 29 镜像（不引入 testcontainers-go）"
    - "scripts/test-fixture-up.sh 创建临时 docker-compose.yml + 启动 1 个 Phase 29 镜像容器 + 等待 sshd ready；test-fixture-down.sh 销毁；脚本可重复执行（幂等）"
  artifacts:
    - path: "internal/claudedock/oauth_check.go"
      provides: "CheckOAuthCredentials + OAuthStatus + parseExpiresAt（纯函数）"
      contains: "func CheckOAuthCredentials"
    - path: "internal/claudedock/oauth_check_test.go"
      provides: "expiresAt 三态 + JSON 解析容错单测"
      contains: "Test_ParseExpiresAt"
    - path: "cmd/claudedock/sync.go"
      provides: "sync conflicts 子命令实现 + cobra 注册"
      contains: "sync conflicts"
    - path: "internal/claudedock/integration_test.go"
  ...
  key_links:
    - from: "internal/claudedock/ssh.go ConnectAndRunClaudeV3"
    ...
    - from: "cmd/claudedock/sync.go syncConflicts"
      to: "~/.claudedock/bin/mutagen sync list --long"
      via: "exec.Command 渲染冲突清单"
      pattern: "sync list --long"
    - from: "internal/claudedock/integration_test.go"
...
- **Plan 02（Wave 2）必须先完成**：依赖 ConnectAndRunClaudeV3 入口（OAuth 检查 hook 点已在 ssh.go 留 TODO）+ MountConfig 字段 + mount_mutagen.go 的 deps 注入接口（本 plan 在其上扩展 conflict count 解析逻辑）+ cmd/claudedock/main.go 的 cobra 子命令路由（本 plan 注册 sync 子命令树）+ **Plan 02 Task 2.4 创建的 internal/claudedock/exitcodes.go 命名常量**（本 plan ssh.go OAuth 检查必须引用 claudedock.ExitOAuthNotFound / ExitOAuthExpired，避免裸数字与 v2.0 4/5 撞码）
- 与 Plan 02 的并发改动文件：mount_mutagen.go（本 plan 追加 conflict 解析）、mount_strategy.go（本 plan 接入 conflict count → banner 警告）、ssh.go（本 plan 替换 TODO 注释为 CheckOAuthCredentials 调用）、cmd/claudedock/main.go（本 plan 注册 sync 子命令）— **本 plan 必须在 Plan 02 完成 commit 之后才能开始**，避免 git 冲突
</plan_dependencies>
...
2. **Mutagen conflict 冒泡（REQ-F1-E）**：CONTEXT D-28 + RESEARCH §1.1 修订（v0.18.1 不支持 --json，改 --template）— 启动后期解析 conflict count，banner 后输出中文警告；提供 `claudedock sync conflicts` 子命令查看清单。
...
Purpose: 兑现 ROADMAP §Phase 31 Success Criteria 第 6 条（C5 安全门 sync 必须未创建）+ 第 9 条（OAuth 过期不进 claude）+ 第 8 条（conflict 冲突中文冒泡 + claudedock sync conflicts 列出清单）；6 个集成测试是 Phase 35 真机验收前的 CI gate。
...
internal/claudedock/oauth_check.go 导出：
...
cmd/claudedock/sync.go 注册：

```go
// syncCmd 在 cmd/claudedock/main.go init 阶段被 rootCmd.AddCommand 注册。
// 路由：claudedock sync conflicts [--no-color]
// 行为：建立 SSH 连接（沿用 LoadConfig + EntryClient）→ 远程不调用，本地 ~/.claudedock/bin/mutagen sync list --long 解析渲染中文表格
...
```bash
$HOME/.claudedock/bin/mutagen sync list --template '{{range .}}{{.Name}}|{{len .Conflicts}}|{{.LastError}}{{"\n"}}{{end}}'
```

输出格式（每行一个 sync session）：

```
claudedock-abc123-deadbeef|0|
claudedock-xyz789-cafebabe|3|
```
...
- 写临时 docker-compose.yml 到 `/tmp/claudedock-fixture/docker-compose.yml`：
  ```yaml
  services:
    cc-fixture:
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - internal/claudedock/errcodes/codes.go（NET_OAUTH_EXPIRED / NET_OAUTH_EXPIRING_SOON / NET_OAUTH_NOT_FOUND 三个 Code）
    - internal/claudedock/mount.go（sshRun 函数 — 本 task 用 sess.Output 形式收集 stdout）
  </read_first>
  <action>
    1. internal/claudedock/oauth_check.go：
    ...
    2. internal/claudedock/oauth_check_test.go：按 <behavior> 9 个用例，全表驱动：
  ...
  <acceptance_criteria>
    - `gofmt -l internal/claudedock/oauth_check.go internal/claudedock/oauth_check_test.go` 输出为空
    - `go build ./...` 退出码 0
    - `go test ./internal/claudedock/ -run Test_ParseExpiresAt -count=1 -v` 至少 9 个子用例 PASS
    - 关键签名：
      `grep -F 'func CheckOAuthCredentials(connA *ssh.Client, claudeAccountID string) (*OAuthStatus, error)' internal/claudedock/oauth_check.go` 命中
      `grep -F 'func parseExpiresAt(rawJSON string, now time.Time) *OAuthStatus' internal/claudedock/oauth_check.go` 命中
    - 5min 阈值与 timeout 2 命令字面：
      `grep -F '5 * time.Minute' internal/claudedock/oauth_check.go` 命中
      `grep -F 'timeout 2 cat /home/claude/.claude/.credentials.json' internal/claudedock/oauth_check.go` 命中
    - 整仓回归：`go test ./... -count=1` 退出码 0
  </acceptance_criteria>
  <verify>
    <automated>go build ./... && go test ./internal/claudedock/ -run Test_ParseExpiresAt -count=1 -v && grep -F 'func CheckOAuthCredentials(connA *ssh.Client, claudeAccountID string) (*OAuthStatus, error)' internal/claudedock/oauth_check.go && grep -F 'timeout 2 cat /home/claude/.claude/.credentials.json' internal/claudedock/oauth_check.go && grep -F '5 * time.Minute' internal/claudedock/oauth_check.go</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <read_first>
    - internal/claudedock/ssh.go（Plan 02 留的 OAuth TODO 注释位置）
    - internal/claudedock/oauth_check.go（Task 3.1 产出 — CheckOAuthCredentials / OAuthStatus / OAuthState）
    - internal/claudedock/mount_mutagen.go（Plan 02 产出 — mountMutagen 现签名 cleanup, err；本 task 改为 cleanup, status, err）
    - internal/claudedock/mount_strategy.go（Plan 02 产出 — banner 输出 + last-session.json 写入 hook）
    - internal/claudedock/exitcodes.go（Plan 02 Task 2.4 产出 — ExitOAuthNotFound=6 / ExitOAuthExpired=7 命名常量；本 task ssh.go OAuth 检查必须引用，禁止裸数字）
    ...
    - cmd/claudedock/main.go 行 16-23（v2.0 现有 exit* 常量 0-5，本 task 不动）
  </read_first>
  <action>
    1. **internal/claudedock/ssh.go ConnectAndRunClaudeV3**：把 Plan 02 留的 TODO 注释替换为真实 OAuth 检查逻辑。
       ...
       注意 import：必须 import `github.com/claudedock/claudedock/internal/claudedock/errcodes`。

    2. **internal/claudedock/mount_mutagen.go**：扩展 mountMutagen 签名为 3 返回值（cleanup, status, err）。
    ...
    3. **internal/claudedock/mount_strategy.go**：调用 mountMutagen 的位置同步改 3 返回值；在 banner 输出后用 status.ConflictCount 决定是否插入警告：

       ```go
       // 假设 mutagenStatus 是 mountMutagen 返回的 status（仅 mode != ModeSSHFSOnly 时有意义）
       if mutagenStatus.ConflictCount > 0 {
           fmt.Fprintf(mountCfg.Logger, "⚠ 有 %d 个文件同步冲突，运行 claudedock sync conflicts 查看\n", mutagenStatus.ConflictCount)
       }
  ...
  <acceptance_criteria>
    ...
    - `go build ./...` 退出码 0
    - 关键调用链：
      `grep -F 'CheckOAuthCredentials(connA' internal/claudedock/ssh.go` 命中 1 行
      `grep -F 'errcodes.NET_OAUTH_EXPIRED' internal/claudedock/ssh.go` 命中 1 行
      `grep -F 'errcodes.NET_OAUTH_NOT_FOUND' internal/claudedock/ssh.go` 命中 1 行
      `grep -F 'errcodes.NET_OAUTH_EXPIRING_SOON' internal/claudedock/ssh.go` 命中 1 行
      `grep -F 'gateway 未返回 claude_account_id' internal/claudedock/ssh.go` 命中 1 行
    - mountMutagen 三返回值：
      `grep -E 'func mountMutagen\([^)]*\) \(cleanup func\(\), status MutagenSyncStatus, err error\)' internal/claudedock/mount_mutagen.go` 命中
      `grep -F 'sync list' internal/claudedock/mount_mutagen.go` 命中
      `grep -F '--template' internal/claudedock/mount_mutagen.go` 命中（不能是 --json）
    - banner conflict 警告：
      `grep -F '同步冲突' internal/claudedock/mount_strategy.go` 命中
      `grep -F 'claudedock sync conflicts' internal/claudedock/mount_strategy.go` 命中
    - exit codes 引用命名常量（修订 D-22 第 3 条 4/5 → 6/7，避开 v2.0 main.go ExitConfigError=4 / ExitInternalError=5 撞码）：
      `grep -F 'return ExitOAuthNotFound,' internal/claudedock/ssh.go` 命中（NotFound = 6）
      `grep -F 'return ExitOAuthExpired,' internal/claudedock/ssh.go` 命中（Expired = 7）
      `! grep -E 'return [0-9]+,' internal/claudedock/ssh.go | grep -E 'OAuth|NET_'`（OAuth 路径不允许裸数字）
    - 整仓回归：`go test ./... -count=1` 退出码 0（mount_strategy 12 降级矩阵随 mountMutagen 签名变更已在 Plan 02 mock 中处理；本 task 必须更新 mock 签名）
  </acceptance_criteria>
  <verify>
    <automated>go build ./... && go test ./... -count=1 && grep -F 'CheckOAuthCredentials(connA' internal/claudedock/ssh.go && grep -F 'errcodes.NET_OAUTH_EXPIRED' internal/claudedock/ssh.go && grep -F 'errcodes.NET_OAUTH_NOT_FOUND' internal/claudedock/ssh.go && grep -F 'gateway 未返回 claude_account_id' internal/claudedock/ssh.go && grep -F 'return ExitOAuthNotFound' internal/claudedock/ssh.go && grep -F 'return ExitOAuthExpired' internal/claudedock/ssh.go && grep -F '--template' internal/claudedock/mount_mutagen.go && ! grep -F 'sync list --json' internal/claudedock/mount_mutagen.go && grep -F '同步冲突' internal/claudedock/mount_strategy.go</automated>
  </verify>
  <done>
    OAuth 检查接入 ConnectAndRunClaudeV3 路径：claude_account_id 缺失跳过 + 三态分支落码 + 退出码引用 claudedock.ExitOAuthNotFound(6) / ExitOAuthExpired(7) 命名常量（修订 D-22 第 3 条 4/5 → 6/7，避开 v2.0 main.go 撞码）；mountMutagen 扩展 conflict count 解析（--template 模式）；banner 后输出中文冲突警告 + 写入 last-session.json；Plan 02 12 降级矩阵单测全部仍 PASS。
  </done>
</task>

<task type="auto">
  <name>Task 3.3: cmd/claudedock/sync.go 注册 sync conflicts 子命令</name>
  <files>
    cmd/claudedock/sync.go
    cmd/claudedock/main.go
  </files>
  <read_first>
    - cmd/claudedock/main.go（cobra 子命令注册模式 — initCmd / envCmd / sshCmd 范式）
    - internal/claudedock/config.go（LoadConfig）
    - internal/claudedock/mutagen_bin.go（ExtractMutagenBinary 用于确保本地有 mutagen 二进制）
    - .planning/phases/31-cli/31-CONTEXT.md（D-28 sync conflicts 子命令最小可行版本 — 不做 resolve）
  </read_first>
  <action>
    1. cmd/claudedock/sync.go：
       ...
       func runSyncConflicts(cmd *cobra.Command, args []string) error {
           ...
           env := append(os.Environ(),
               "MUTAGEN_DATA_DIRECTORY="+filepath.Join(home, ".claudedock", "mutagen"),
           )
    ...
    2. cmd/claudedock/main.go：在 main() 函数 rootCmd.AddCommand 调用处追加 newSyncCmd()：
  ...
  <acceptance_criteria>
    - `gofmt -l cmd/claudedock/` 输出为空
    - `go build ./...` 退出码 0
    - 路由注册：
      `grep -F 'newSyncCmd' cmd/claudedock/main.go` 命中
      `grep -E '"sync"' cmd/claudedock/main.go` 命中（DisableFlagParsing 路由）
      `grep -F 'sync list --long' cmd/claudedock/sync.go` 命中（使用 --long 而非 --json）
      `grep -F 'claudedock.ExtractMutagenBinary' cmd/claudedock/sync.go` 命中
    - 二进制烟测：
      ```bash
  ...
  <verify>
    <automated>go build -o /tmp/cc-test ./cmd/claudedock && /tmp/cc-test sync --help 2>&1 | grep -F 'conflicts' && grep -F 'newSyncCmd' cmd/claudedock/main.go && grep -F 'sync list --long' cmd/claudedock/sync.go && grep -F 'claudedock.ExtractMutagenBinary' cmd/claudedock/sync.go</automated>
  </verify>
  <done>
    claudedock sync conflicts 子命令注册完毕：cobra 路由通畅、ExtractMutagenBinary 自动确保本地二进制、调用 sync list --long 渲染冲突清单到 stdout；本任务**不**实现自动 resolve（OOS-A2）。
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    3. **internal/claudedock/integration_test.go**：
       ...
       // helper: 启 claudedock 二进制（已编译到 /tmp/claudedock-int），返回退出码 + stderr
       func runCloudClaude(t *testing.T, mode string, cwd string) (exitCode int, stderr string) {
           t.Helper()
           bin := "/tmp/claudedock-int"
           if _, err := os.Stat(bin); err != nil {
               // 编译一次
               if err := exec.Command("go", "build", "-o", bin, "./cmd/claudedock").Run(); err != nil {
                   t.Fatalf("编译 claudedock 失败: %v", err)
               }
           }
           ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
           defer cancel()

           // 注：claudedock 默认用 LoadConfig 读 ~/.claudedock/config.yaml。集成测试必须先注入 fixture 配置。
           ...
           cfgPath := filepath.Join(home, ".claudedock", "config.yaml")
       ...
       func TestIntegration_C4_VersionSkew_DowngradesToSSHFSOnly(t *testing.T) {
           // 篡改容器内版本文件
           _, _ = dockerExec(t, "sed", "-i", "s/v0.18.1/v0.99.99/", "/etc/claudedock/mutagen.version")
           defer dockerExec(t, "sed", "-i", "s/v0.99.99/v0.18.1/", "/etc/claudedock/mutagen.version")
       ...
       func TestIntegration_C5_SafetyGuard_BlocksSync(t *testing.T) {
           ...
           c.Env = append(os.Environ(), "MUTAGEN_DATA_DIRECTORY="+filepath.Join(home, ".claudedock", "mutagen"))
           out, _ := c.Output()
       ...
       func TestIntegration_F2B_KillMutagenAgent_DowngradesIn2s(t *testing.T) {
           // 此测试需要 claudedock 已成功 mount（mutagen 在跑）；先正常启动取得 baseline，然后 pkill
           // 测试模型：跑一个长时 claudedock（exec sleep），在另一个 goroutine pkill 后等待 stderr 出现降级
           // 简化：本用例只验证 pkill 之后再次 claudedock 启动会自动降级 mutagen-only 或 sshfs-only
       ...
       func TestIntegration_C3_NetemDrop_ColdBranchRemoved(t *testing.T) {
           ...
           // executor 自行决定本场景的实现深度；建议最低限度断言 claudedock --mount-mode=full 在拔网后不 hang > 60s 并打印 MOUNT_SSHFS_DISCONNECTED
           t.Skip("C3 集成场景由 Phase 35 真机验收完整覆盖；本测试占位以满足 RESEARCH §6.2 计数")
       }
       ```

       注意：
       - **凭证注入**：claudedock 启动需要 ~/.claudedock/config.yaml + 网关 / shortID / password。集成测试用 fixture 容器需要替代真实 Entry API 路径 — 这是个绕不开的问题。**executor 在 TestMain 中必须**：
         (a) 写临时 ~/.claudedock/config.yaml 指向 fixture 内置的 mock gateway 或
         (b) 直接调 `claudedock.ConnectAndRunClaudeV3`（绕过 main.go 的 LoadConfig + EntryClient 路径），把 SSHConfig 与 AuthResponse 直接构造。**推荐 (b)**：测试代码直接 import internal/claudedock 包，不走 cobra 路由 — 这样不需要 mock gateway。
         上面 runCloudClaude helper 写的是 (a) 路径示意；executor 在实现时改用 (b) 更稳健。
       - 集成测试不在 default `go test` 触发：必须 `go test -tags=integration ./internal/claudedock/`。
  ...
  <acceptance_criteria>
    - 文件存在性：
      `test -f internal/claudedock/integration_test.go`
      `test -x scripts/test-fixture-up.sh`
      `test -x scripts/test-fixture-down.sh`
    - build tag 正确：
      `head -3 internal/claudedock/integration_test.go | grep -F '//go:build integration'`
    - 默认 `go test ./internal/claudedock/` **不**跑集成测试（因 build tag）：
      `go test ./internal/claudedock/ -count=1` 退出码 0（且 verbose 输出不含 TestIntegration_*）
    - 6 个用例存在（用 grep test 函数名计数，不实际执行）：
      ```bash
      [ "$(grep -c '^func TestIntegration_' internal/claudedock/integration_test.go)" -ge 6 ]
      ```
    - 关键 RESEARCH §6.2 断言关键字：
      `grep -F 'pkill -9 mutagen-agent' internal/claudedock/integration_test.go` 命中
      `grep -F 'count=200' internal/claudedock/integration_test.go` 命中
      `grep -F 'expiresAt":0' internal/claudedock/integration_test.go` 命中
      `grep -F '/workspace-hot/seed' internal/claudedock/integration_test.go` 命中
    - fixture 脚本静态检查：
    ...
    - 整仓回归：`go test ./... -count=1` 退出码 0
    - 集成测试静态编译通过：`go test -tags=integration -count=1 -run x_no_run_x ./internal/claudedock/` 退出码 0（仅检查 build，不真实跑）
    - **未引入 testcontainers-go**：`grep -F 'testcontainers' go.mod || echo OK`
  </acceptance_criteria>
  <verify>
    <automated>head -3 internal/claudedock/integration_test.go | grep -F '//go:build integration' && [ "$(grep -c '^func TestIntegration_' internal/claudedock/integration_test.go)" -ge 6 ] && bash -n scripts/test-fixture-up.sh && bash -n scripts/test-fixture-down.sh && go test -tags=integration -count=1 -run x_no_run_x ./internal/claudedock/ && ! grep -F 'testcontainers' go.mod</automated>
  </verify>
  <done>
...
```bash
# 1. 文件齐备
test -f internal/claudedock/oauth_check.go
test -f internal/claudedock/oauth_check_test.go
test -f cmd/claudedock/sync.go
test -f internal/claudedock/integration_test.go
test -x scripts/test-fixture-up.sh
test -x scripts/test-fixture-down.sh

# 2. OAuth 关键命令字面与 5min 阈值
grep -F 'timeout 2 cat /home/claude/.claude/.credentials.json' internal/claudedock/oauth_check.go
grep -F '5 * time.Minute' internal/claudedock/oauth_check.go
grep -F 'CheckOAuthCredentials(connA' internal/claudedock/ssh.go
grep -F 'gateway 未返回 claude_account_id' internal/claudedock/ssh.go

# 3. Mutagen conflict 解析（--template 不是 --json）
grep -F '--template' internal/claudedock/mount_mutagen.go
! grep -F 'sync list --json' internal/claudedock/mount_mutagen.go

# 4. banner 中文冲突警告 + sync 子命令路由
grep -F '同步冲突' internal/claudedock/mount_strategy.go
grep -F 'claudedock sync conflicts' internal/claudedock/mount_strategy.go
grep -F 'newSyncCmd' cmd/claudedock/main.go

# 5. exit codes 引用命名常量（避开 v2.0 main.go 4/5 撞码）
grep -F 'return ExitOAuthNotFound,' internal/claudedock/ssh.go
grep -F 'return ExitOAuthExpired,' internal/claudedock/ssh.go
grep -F 'ExitOAuthNotFound    = 6' internal/claudedock/exitcodes.go
grep -F 'ExitOAuthExpired     = 7' internal/claudedock/exitcodes.go

# 6. 单元测试
go test ./internal/claudedock/ -run Test_ParseExpiresAt -count=1 -v   # 9 个子用例 PASS
go test ./internal/claudedock/errcodes/ -count=1                       # Plan 01 注册表回归
go test ./... -count=1                                                  # 整仓回归（不含 integration）

# 7. 集成测试静态编译
go test -tags=integration -count=1 -run x_no_run_x ./internal/claudedock/

# 8. 集成测试函数计数 + 关键场景关键字
[ "$(grep -c '^func TestIntegration_' internal/claudedock/integration_test.go)" -ge 6 ]
grep -F 'pkill -9 mutagen-agent' internal/claudedock/integration_test.go
grep -F 'expiresAt":0' internal/claudedock/integration_test.go
...
| Boundary | 描述 |
|----------|------|
| 远端容器 → claudedock（OAuth credentials JSON 内容） | conn-A 上 cat /home/claude/.claude/.credentials.json 的 stdout 直接被 JSON unmarshal；恶意容器可投毒 |
| claudedock → 本地 mutagen 进程（sync list stdout） | mountMutagen 解析 sync list 的 stdout 模板；mutagen 自身可信 |
...
|-----------|----------|-----------|-------------|-----------------|
| T-31-03-01 | Information Disclosure | OAuth credentials.json 内容（含 accessToken / refreshToken）通过 conn-A 经过本地内存 | accept | claudedock 进程已是「容器内运行 claude code」的合法持有者；token 在内存中存活时间仅 parseExpiresAt 一次调用，不写日志 / 不写文件 / 不出栈帧外。OAuth 检查只读 expiresAt 字段，accessToken / refreshToken 不被使用 |
...
| T-31-03-06 | Repudiation | OAuth Expired / NotFound 时退出码 4/5 模糊（用户不知道是 OAuth 失败） | mitigate | 退出前 stderr 必输出 errcodes.Format(NET_OAUTH_*)，含中文 NextAction 指引 claudedock exec claude login |
...
- claudedock sync conflicts 子命令注册 + ExtractMutagenBinary 自动准备 + 调 mutagen sync list --long
...
- exit codes 引用 claudedock.ExitOAuthNotFound(6) / ExitOAuthExpired(7) 命名常量（修订 D-22 第 3 条 4/5 → 6/7，避开 v2.0 main.go ExitConfigError=4 / ExitInternalError=5 撞码）；ssh.go OAuth 路径不允许裸数字 return
...
- 6 个集成测试场景的执行方式：`go test -tags=integration -count=1 -v ./internal/claudedock/`
- 已知占位：TestIntegration_C3_NetemDrop_ColdBranchRemoved 因 tc/netem 依赖留 Skip，最终验收在 Phase 35 真机
- 凭证注入方案的最终实现选择（直接 import internal/claudedock 包 vs 写临时 config.yaml + mock gateway）
