...
把 claudedock 端的文件映射从 v2.0 单层 sshfs 升级为 **Mutagen 热同步 + sshfs 冷兜底 + mergerfs 联合视图** 三层架构，实现 `--mount-mode={auto|full|mutagen-only|sshfs-only}` 显式降级状态机；并把 F7-C（连接前 OAuth 过期中文提示）织入 SSH 握手成功后、`claude` 进程拉起前的窗口期。

本阶段交付：

1. `internal/claudedock/mount.go` 拆分为 `mount_sshfs.go` / `mount_mutagen.go` / `mount_merge.go` / `mount_strategy.go` 四文件 + `errcodes/codes.go` 错误码注册表雏形
2. `go:embed` 集成 Mutagen v0.18.1 多平台二进制（darwin/linux × amd64/arm64），首次运行 extract 到 `~/.claudedock/bin/mutagen`；daemon 长期复用（不随 claudedock 退出停止）
3. 启动时 mutagen 客户端版本与容器内 `/etc/claudedock/mutagen.version` 比对（不一致直接降级 sshfs-only + 错误码 `MOUNT_MUTAGEN_VERSION_SKEW`，防御 PITFALLS C4）
...
10. Mutagen conflict 冒泡：每次启动从 `mutagen sync list --json` 读 conflict 计数，>0 时下次回车前 prompt 上方插入中文警告 `⚠ 有 N 个文件同步冲突，运行 claudedock sync conflicts 查看`（REQ-F1-E）
11. 并发时序：claudedock 启动两个 SSH connection（控制 conn-A / 数据 conn-B）；Mutagen sync 与 sshfs mount 在 conn-B 上由两个 goroutine 并发拉起；mergerfs 在两层均 ready 后由 SSH 远程命令在容器内挂载 `/workspace-hot=RW:/workspace-cold=NC,RO /workspace`（branch 拓扑由 Phase 29 D-12 锁定）
12. sshfs 抖动主动摘除（PITFALLS C3）：sshfs 挂参 `reconnect,ServerAliveInterval=15,ServerAliveCountMax=3,ConnectTimeout=10`；claudedock 后台 watcher 每 5s `mountpoint -q /workspace-cold`，连续 3 次失败（15s）从 mergerfs runtime branch 摘除并打 `MOUNT_SSHFS_DISCONNECTED` 警告
...
14. 错误码注册表雏形：本阶段引入 `MOUNT_*` / `NET_*` 前缀（≥ 11 条），注册到 `internal/claudedock/errcodes/codes.go`，每条带中文 `Message` + `NextAction`；Phase 34 doctor / explain 复用同一注册表（REQ-F8-A / REQ-F8-B 的 phase-31 落码部分）
...
- `claudedock doctor`（doctor 仅消费本阶段注册表，自身实现在 Phase 34）/ `claudedock explain` / `--fix` / `--json` → Phase 34
...
- **D-01**：`internal/claudedock/mount.go` 拆分为 4 个文件，原始单文件保留为 `mount_legacy_test.go` 仅用于回归（最终删除由 plan 决定）：
...
- **D-02**：错误码注册表新建包 `internal/claudedock/errcodes`：
...
- **D-03**：Mutagen 客户端二进制采用 `go:embed` 嵌入 4 个平台变体：`darwin_amd64` / `darwin_arm64` / `linux_amd64` / `linux_arm64`，每个 ~3MB，总计 ~12MB；版本严格锁定为 `v0.18.1`（与 Phase 29 D-05 容器内 mutagen-agent 一致）。embed 文件位于 `internal/claudedock/mutagen_bin/<platform>/mutagen`，构建脚本 `scripts/fetch-mutagen-bins.sh` 从 GitHub release 拉取 + sha256 校验 + 裁剪 `mutagen.exe` 等无关产物。仓库提交时 4 个二进制纳入 git LFS（如团队拒绝 LFS 则裸提交，4×3MB = 12MB 可接受）。  
  *[auto] Q1 推荐项：go:embed（替代：检测 brew install 引入用户摩擦、首次运行下载需出网且违背"零增量特权"目标）*
- **D-04**：首次运行时 extract 到 `~/.claudedock/bin/mutagen`，权限 `0755`；如 extract 目录已存在同名二进制，校验 `mutagen version` 输出包含 `0.18.1` 后直接复用，否则覆盖（防止用户旧版本污染）。
- **D-05**：Mutagen daemon 由 claudedock 启动时 `mutagen daemon start`（Mutagen 自身天然幂等，daemon 已在则 noop）；**claudedock 退出时不停 daemon**（长期复用，避免每次 ~500ms 启动开销 / 触发 ≤8s 基线）。daemon data 目录强制为 `~/.claudedock/mutagen/`，通过 `MUTAGEN_DATA_DIRECTORY` 环境变量隔离，避免与用户已装的 brew mutagen 冲突。  
  *[auto] Q2 推荐项：长期复用（替代：每次起停 daemon 增加首连耗时、违背 ≤8s 基线）*
- **D-06**：多 claudedock 并发同一 daemon：依赖 Mutagen daemon 自身的单实例约束（同一 `MUTAGEN_DATA_DIRECTORY` 只能跑一个 daemon），多 claudedock 共享同一 daemon；session 命名约定 `claudedock-{claude_account_id}-{cwd_hash8}`（cwd_hash8 = sha256(absCwd)[:8]），保证不同项目目录不碰撞。**账号级单例锁（REQ-F5-D）由 Phase 32 落地**，本阶段只确保 session 命名唯一、不主动抢占。
...
- **D-08**：Mutagen 默认 `--mode=two-way-resolved`，`alpha` = 本地 cwd（用户视角"权威"），`beta` = `/workspace-hot`（容器内 mergerfs 上层 RW branch）。理由：
  1. claudedock 典型工作流是"本地编辑 + 容器内 claude code 偶尔写入"，本地权威符合用户直觉
...
- **D-09**：macOS APFS 检测：claudedock 启动时执行 `diskutil info / | grep "Case-sensitive"`（macOS）/ 跳过（Linux），命中 `Case-sensitive: No` 时：
...
- **D-12**：默认 ignore 列表（写入运行时生成的 `~/.claudedock/mutagen-defaults.yml`）：`.git/` `node_modules/` `target/` `dist/` `*.pyc` `.venv/` `__pycache__/` `.next/` `build/` `.cache/` `.DS_Store`。用户工程级 `.mutagen.yml` 优先级高于默认（Mutagen 自身合并语义）。
- **D-13**：首次同步前安全门（防御 PITFALLS C5）：
  ...
  3. **触发动作**：错误码 `MOUNT_MUTAGEN_SAFETY_GUARD` + 中文提示`检测到本地目录为空但容器内 /workspace-hot 已有文件，拒绝执行同步以防止反向清空。如确认要从远端拉取，请先 claudedock exec rsync /workspace-hot/ ./` + **退出非 0**（不静默降级，C5 是数据安全风险，必须用户确认）
...
- **D-15**：状态机实现：`mount_strategy.go` 用 enum `Mode { ModeFull, ModeMutagenOnly, ModeSSHFSOnly, ModeFailed }`：
  ...
  2. 每次降级在 stderr 输出一行降级 banner：`[!] MOUNT_AUTO_DOWNGRADED: <from> → <to>，原因 <错误码> <中文>，运行 claudedock doctor 查看修复建议`（M13 防御：**任何静默降级视为缺陷**）
...
- **D-16**：「禁止静默降级」实现细节（M13 验收）：
  - stderr 写入用专用 logger（带 `[mount]` 前缀），与 claudedock 其它日志区分
  - 降级事件同时写入 `~/.claudedock/last-session.json`（schema：`{"timestamp", "intended_mode", "actual_mode", "downgrade_chain": [{"from", "to", "reason_code", "reason_message"}]}`）
  - Phase 34 doctor 第一屏读取 `last-session.json` 展示降级历史（M13 终验由 Phase 34 验证，本阶段产出数据）
- **D-17**：banner UI（连接成功后 claudedock 主流程拉起 claude 之前打印一次）：
  ```
  ✓ 文件映射就绪 [<mode>]
  ```
  颜色规则：
  ...
  使用现有 `internal/claudedock/colors.go`（如不存在则在本阶段新增 `colors.go` 极简实现，避免引入第三方 color 库）
...
- **D-19**：本阶段必须落地的错误码（注册到 `errcodes/mount.go` + `errcodes/net.go`，每条三要素：`Code` + 中文 `Message` 模板 + 中文 `NextAction`）：
  | Code | 触发 | 防御 |
  |------|------|------|
  | `MOUNT_MUTAGEN_VERSION_SKEW` | 客户端 mutagen 版本 ≠ 容器 `/etc/claudedock/mutagen.version` | C4 |
...
- **D-22**：检查时机：SSH 握手成功 + conn-A / conn-B 建立完毕之后、Mutagen sync create / sshfs mount 启动**之前**，作为独立 goroutine 在 conn-A 上并发执行（不阻塞 mount 路径）：
  ...
  3. 三态分支：
     - 文件不存在 / 输出为空 → `NET_OAUTH_NOT_FOUND`：中文提示 `Claude 账号未登录（账号: {claude_account_id}），请在容器内运行 claudedock exec claude login`，cleanup mount + 退出码 4
     - `expiresAt < now` → `NET_OAUTH_EXPIRED`：中文提示 `Claude OAuth 凭证已过期（账号: {claude_account_id}），请先重新登录`，cleanup mount + 退出码 5
     - `expiresAt - now < 5min` → `NET_OAUTH_EXPIRING_SOON`：警告打印（`[!] Claude OAuth 凭证将在 {N} 分钟后过期，建议尽快 claudedock exec claude login`）但**继续启动**
...
- **D-25**：claudedock 启动两个 SSH connection：
...
- **D-27**：sshfs 抖动主动摘除（PITFALLS C3）：
  ...
  - claudedock 启动后台 watcher goroutine：每 5s 在 conn-A 远程 `mountpoint -q /workspace-cold`，连续 3 次失败（≥15s）：
  ...
  - watcher 在 claudedock 退出时 stop（context cancel）

### Mutagen conflict 冒泡（REQ-F1-E）

- **D-28**：claudedock 启动后期（mount 全部 ready 之后）调用 `mutagen sync list --json`，若 `conflicts` 数组非空，记录 N 到 `~/.claudedock/last-session.json` 的 `conflict_count` 字段；同时在 banner 之后插入一行：
  ```
  ⚠ 有 N 个文件同步冲突，运行 claudedock sync conflicts 查看
  ```
  - `claudedock sync conflicts` 子命令本阶段实现最小可行版本：调用 `mutagen sync list --json` 后渲染冲突文件列表（路径 + alpha/beta side + last-modified），不做 resolve（resolve 由用户决定 mutagen 命令）
...
- **D-29**：从 Phase 30 `AuthResponse` 读取的字段（必须容忍缺失）：
  ...
  - `claude_account_id` → 缺失则跳过 OAuth 检查（D-24）+ Mutagen session 名退化为 `claudedock-{cwd_hash8}`（无账号维度隔离）
- **D-30**：本阶段不修改 `internal/claudedock/ssh.go:ConnectAndRunClaude` 函数签名；改动方式：
  ...
  3. `cmd/claudedock/main.go` 切到 V3 入口，`--mount-mode` flag 由 V3 接收
...
- **D-32**：本阶段为 Phase 34 预留接口：
  ...
  - `~/.claudedock/last-session.json` 写入约定 schema，doctor 第一屏读取
...
- `claudedock sync conflicts` 子命令的 cobra 注册位置（建议 `cmd/claudedock/sync.go` 新文件）
...
- `.planning/research/ARCHITECTURE.md` §claudedock 端 / §host-agent 边界 — 本阶段不扩展 host-agent endpoint，全部走 SSH

### 既有代码（直接改造对象）

- `internal/claudedock/mount.go` — 拆分目标，v2.0 单层 sshfs 实现的剥离起点
- `internal/claudedock/ssh.go` — `ConnectAndRunClaude` / `runClaude` / `sshConnect`；本阶段新增 V3 入口（D-30）
- `internal/claudedock/entry.go` — `AuthResponse` 字段（Phase 30 已扩展）
- `internal/claudedock/config.go` — `--mount-mode` flag 的 viper 绑定参考
- `cmd/claudedock/main.go` — cobra root command + flag 注册位置
- `internal/claudedock/mount_test.go` — v2.0 mount 测试，本阶段拆分时迁移到对应文件

### 二进制资源（本阶段引入）

- `internal/claudedock/mutagen_bin/{darwin_amd64,darwin_arm64,linux_amd64,linux_arm64}/mutagen` — go:embed 资源
- `scripts/fetch-mutagen-bins.sh` — 拉取脚本（新增）
- `~/.claudedock/bin/mutagen` — 运行时 extract 路径
- `~/.claudedock/mutagen/` — `MUTAGEN_DATA_DIRECTORY`
- `~/.claudedock/last-session.json` — 降级历史 + conflict count（Phase 34 doctor 第一屏读取）
...
- `internal/claudedock/mount.go` 现有 `mountWorkspace` / `waitForMount` / `fusermountCleanup` / `cleanupStaleFUSE` / `rmdirChain` / `sshRun` / `shellQuote` 函数全部可复用：剥离到 `mount_sshfs.go` 后是 sshfs 分支的完整实现
- `internal/claudedock/ssh.go:sshConnect` 工厂函数可被 `MountConfig` 复用（建立 conn-A / conn-B）
- `internal/claudedock/entry.go:AuthResponse` Phase 30 已扩展 4 个 v3 字段，本阶段直接读取
- `internal/claudedock/envcheck.go` 已有 macOS / Linux 检测模式（如 `runtime.GOOS == "darwin"`），APFS 检测可借鉴位置
- `internal/claudedock/ssh_doctor.go` 已实现一个 doctor 子命令骨架（v2.0 ssh 维度），Phase 34 在其上扩展五维度；本阶段 `errcodes` 包必须与之兼容
...
- 现有 `internal/claudedock/ssh_doctor.go` 已落 `claudedock ssh doctor` 子命令（quick-task 260417-0w4），Phase 34 doctor 在其上扩展五维度（不在本阶段动）
...
- ROADMAP §Phase 31 Success Criteria 第 4 条 `强 kill 容器内 mutagen-agent 后 ≤2s 内 claudedock stderr 中文输出降级到 sshfs-only` 是 plan-checker 验收锚点；测试用例必须真实 kill mutagen-agent（不能 mock），可用 `docker exec <ctr> pkill -9 mutagen-agent` 触发
...
- macOS 真机 APFS case-insensitive 是已知风险（PITFALLS M5 是 BLOCKER），D-09 的检测必须在 claudedock 启动早期完成（早于 Mutagen sync create），避免 case-sensitive 假设失败后 mutagen sync 已经创建产生脏数据
...
- **`--mutagen-force` flag 覆盖 50MB 白名单**：D-11 默认拒绝；高级用户场景留给 v3.1（需配套 `claudedock sync resume` 命令）
- **`claudedock sync resolve <pattern>` 自动解决冲突**：D-28 仅实现 `sync conflicts` 列表查看；自动解决需谨慎（OOS-A2 不暴露 5 种 mode），留 v3.1
- **「下次回车前 prompt 上方插入」严格语义**：REQ-F1-E 字面要求 PTY 拦截渲染，本阶段以「启动 banner 之后立即输出」近似实现；完整 PTY 拦截留 Phase 34 doctor 或 v3.1
- **Mutagen daemon 退出时 GC**：D-05 选定 daemon 长期复用不停；GC（最后一个 claudedock 退出后 30min 停 daemon）留 v3.1（涉及跨进程 ref count，复杂）
- **错误码 i18n（英文版本）**：本阶段中文硬编码；i18n 框架留 v3.1（OOS-A 未明确禁止但优先级低）
- **`claudedock doctor mount` 维度的真实实现**：本阶段只搭骨架（errcodes + last-session.json + MutagenHealthCheck 接口），完整实现在 Phase 34
