...
- **D-07** 构建期写入版本元数据：
  ...
  - `/etc/claudedock/tmux.version` ← 运行时 `tmux -V` 回填（构建期可留占位）
...
- **D-13** 新增 `/etc/tmux.conf`（容器级默认）：`set -ga terminal-overrides ",*:RGB"` / `set -g window-size latest` / `set -g aggressive-resize on` / `set -g history-limit 50000`；新增 `/etc/profile.d/claudedock.sh` 导出 `CLAUDE_CODE_TMUX_TRUECOLOR=1`。
...
| **B** 二进制预置 | mergerfs `.deb` / mutagen tarball / `/etc/claudedock/*.version` | D-04, D-05, D-07 | §1 / §2 |
| **C** entrypoint & 配置 | entrypoint.sh v3 阶段 / sshd_config / `/etc/tmux.conf` / `/etc/profile.d/claudedock.sh` | D-06, D-09, D-11, D-13, D-14, D-15 | §4 |
...
│    │                                                                        │
│    ├─ COPY entrypoint.sh / sshd_config / tmux.conf / claudedock.sh        │
│    │                                                                        │
...
│    │                                                                        │
│    ├─ RUN echo "v0.18.1"  > /etc/claudedock/mutagen.version               │
│    │     echo "2.41.1"  > /etc/claudedock/mergerfs.version                │
│    │                                                                        │
...
COPY deploy/docker/managed-user/claudedock.sh /etc/profile.d/claudedock.sh
...
prepare_mergerfs_check() {
  ...
  log "v3: mergerfs available ($ver) — mount deferred to claudedock (Phase 31)"
}
...
**本阶段防御：** Dockerfile 固化 `/etc/claudedock/mergerfs.version = 2.41.1`；镜像 README / host-preflight 文档写死参数字符串 `category.create=ff,func.readdir=cor:4,cache.attr=30,cache.entry=30,cache.readdir=true,cache.files=off,inodecalc=path-hash`（D-11）。实际挂载由 Phase 31 执行。
...
**本阶段防御：** sshfs 在镜像中已装（v2 遗留），本阶段**不改** sshfs；参数由 Phase 31 在 claudedock 端传入 `ServerAliveInterval=15,ServerAliveCountMax=3,ConnectTimeout=10,reconnect,-o auto_cache`。镜像层职责仅是保证 `sshfs -V` ≥ 3.7.x。
...
**断言：** `grep -R "2\.41\.1" /etc/claudedock/mergerfs.version`
...
**本阶段防御：** `/etc/tmux.conf` 硬编码 4 行（D-13）；`/etc/profile.d/claudedock.sh` 导出 `CLAUDE_CODE_TMUX_TRUECOLOR=1`。
...
1. **mutagen-agents.tar.gz 解压目标路径**
   - CONTEXT D-05 写：运行时由 claudedock（Phase 31）触发 agent extract
   - CONTEXT D-09-3 写：`prepare_mutagen_agent` 在 entrypoint 阶段 extract 到 `/usr/local/libexec/mutagen/agents/`
   - 两者有重复——**CONTEXT 内部一致性问题**。
   - 研究推断：D-05 的"runtime by claudedock"指的是 Mutagen daemon 会根据版本匹配**再次校验并使用**解压后的 agent；entrypoint 只是**预解压**一次（幂等 touch `.extracted` 标记）。
   - Planner action：按 D-09-3 解压 + touch 标记；D-05 的"runtime trigger"理解为 claudedock 负责启动 daemon，daemon 自然消费 `/usr/local/libexec/mutagen/agents/`。
...
4. **`CLOUD_CLAUDE_MERGERFS_BRANCHES` env 的读取位置**
   ...
   - 研究推断：env 变量**不在 entrypoint 读取**（entrypoint 不挂载 mergerfs）；由 Phase 31 claudedock 在执行 `mergerfs` CLI 前 `${CLOUD_CLAUDE_MERGERFS_BRANCHES:-"/workspace-hot=RW:/workspace-cold=NC,RO"}`。
