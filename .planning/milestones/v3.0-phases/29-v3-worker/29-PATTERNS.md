...
| AP7 | 在 entrypoint 里读取 `CLOUD_CLAUDE_MERGERFS_BRANCHES` env 并做分支组装 | D-12 + RESEARCH §10：本阶段只预留环境变量名，读取逻辑归 Phase 31 claudedock | 仅在 RESEARCH/运维文档中登记 env 名称 |
...
| N2 | `deploy/docker/managed-user/profile.d-claudedock.sh`（新文件） | 项目无既有 `/etc/profile.d/*.sh` 投放模式 | 简单 `export KEY=VALUE` 一行一条；RESEARCH §Code Examples 已给骨架 |
