...
tags: [entry-api, claudedock, agentapi, capability-handshake, omitempty]
...
provides:
  ...
  - "claudedock.AuthResponse 新字段 ImageVersion/SupportsMutagen/SupportsMergerfs/ClaudeAccountID，向后兼容旧 gateway"
...
tech-stack:
  ...
  patterns:
    ...
    - "跨模块（agentapi / controlplane / claudedock）JSON 键名同字面，测试以字符串断言锁定"
...
**在不新增 endpoint 的前提下，扩展 `/v1/entry/{shortId}/auth`、`HostActionRequest`、`claudedock.AuthResponse` 三处契约，把 Phase 30 Wave 1 的数据层产物接线到握手路径上，同时严格保持 v2 客户端兼容。**
...
- **Task 3 — claudedock 客户端升级：** `AuthResponse` 追加四个 omitempty 字段，新增 `entry_compat_test.go` 覆盖 v3 round-trip、v2 缺省场景、omitempty 序列化、以及通过 `httptest.Server` 的端到端 `Authenticate()` 回归。v2 gateway 响应依然能走 ready 分支并保持原 SSH 四元组校验。
...
- `internal/claudedock/entry.go` — `AuthResponse` 新增 4 个 omitempty 字段。
- `internal/claudedock/entry_compat_test.go` — 新增结构体层面的 marshal/unmarshal 兼容测试 + 基于 `httptest.Server` 的 v2/v3 gateway 端到端兼容回归。
...
- **T-30-07 (D — 兼容性回归)**：`TestHostActionRequest_V2Compat` / `TestHostActionRequest_ClaudeAccountID_ForwardCompat` / `TestAuthenticate_V2Gateway_NoExtensionsRequired` / `TestAuthResponse_MarshalOmitempty` 四条测试从 agentapi、claudedock 两侧共同保证 v2↔v3 payload 互操作。
...
- **Phase 31（Entry HTTP/CLI 流程）：** 可直接读取 `claudedock.AuthResponse.ImageVersion/SupportsMutagen/SupportsMergerfs/ClaudeAccountID`，无需任何控制面改动；缺省值全部安全（`false` / `""`）。
...
- `[ -f internal/claudedock/entry_compat_test.go ]` → FOUND
