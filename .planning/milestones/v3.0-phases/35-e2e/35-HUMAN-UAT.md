...
expected: 拔网 2 分钟后，claudedock 触发退避序列 ≥ 3 档（1/2/4/8/30s 中至少命中 3 个），网络恢复后 60s 内 tmux session 仍可 attach 并继续输入；最终失败时 stderr 出现 `(按 Enter 重试|claudedock doctor)` 中文提示

how-to-verify:
```bash
# 不需要 claudedock 已连，脚本会自己启
