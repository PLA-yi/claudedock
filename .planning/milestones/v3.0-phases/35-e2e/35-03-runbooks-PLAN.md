...
5. **CLI 客户端升级** — `curl -fsSL .../install.sh | bash` 流程 + 升级后 `claudedock --version` 应输出 `v3.0.x`
6. **升级后自检**：
   ```bash
   claudedock doctor --json | jq '.summary'       # 全部 pass
   claudedock explain MOUNT_MUTAGEN_VERSION_SKEW  # v3.0 新错误码可 explain
...
8. **快速诊断命令** — 3-5 条
9. **参考** — 引用到函数级：`internal/runtime/tasks/worker.go::createHost`、`internal/claudedock/doctor/doctor.go::RunDoctor`、`deploy/docker/managed-user/entrypoint.sh::prepare_persistent_state`
...
<task type="execute" id="35-03-T2">
  ...
  <read_first>
    ...
    - internal/claudedock/doctor/doctor.go（1-120 行 — 5 维度顺序、Status 字面量 pass/warn/fail/skip、Report schema_version=1）
    - internal/claudedock/doctor/ 目录 ls — 各维度 check 文件存在性
...
8. **快速诊断命令** — 3-5 条
9. **参考** — `internal/claudedock/doctor/doctor.go::RunDoctor` + 各维度 `.go` 文件（引到函数级）
...
3. **Mutagen 数据卷生命周期**（Phase 31-32 范畴）：
   ...
   - 磁盘膨胀诊断：`du -sh /var/lib/mutagen/` → 超过阈值 → `claudedock doctor` 输出 `DISK_MUTAGEN_DATA_BLOAT`
...
<task type="execute" id="35-03-T3">
  ...
  <read_first>
    - internal/claudedock/errcodes/codes.go（整份 — Registry / Format / 42 条 Code 常量）
    - internal/claudedock/errcodes/{auth,disk,mount,net,session,ssh,state,system}.go（ls 目录，各域 init MustRegister 列表）
    - internal/claudedock/errcodes/explanations.go（ExplainExempt + Extended 列表 — 区分"已有长说明"与"仅表格项"）
    - docs/runbooks/v3-claude-state-volumes.md（Pattern G 头部样式）
    - .planning/phases/35-e2e/35-PATTERNS.md Pattern H（错误码索引专用结构）
    - 若存在：`cmd/errcodes-dump/` 或 `claudedock explain --all`（确认生成命令的实际可用形式；若不存在，手册内注明"当前生成方式：遍历代码"）
  </read_first>
  <action>
...
3. **按域分组的 42 条表格**（Pattern H 信源，遍历 codes.go L120-178 + 各域 init）：
   ...
   **数量下限（硬约束）**：总条目数必须 ≥ 42（Phase 34 交付数量），可多不可少。实际数以 `grep -hE 'Code = "[A-Z]+_[A-Z]+' internal/claudedock/errcodes/*.go | wc -l` 为准。

4. **错误码使用 FAQ**：
   - 如何 grep 所有出现点 — `rg 'errcodes\.(MOUNT_|AUTH_|...)' internal/`
   - `claudedock explain <code>` 长说明在哪 — `explanations.go`
...
6. **快速诊断命令** — 3-5 条（含 `claudedock explain <code>`）
7. **参考** — `internal/claudedock/errcodes/codes.go::Registry` + `::Format` + `explanations.go::ExplainExempt`
...
```
  ...
  <acceptance_criteria>
    ...
    - 反向一致性：`comm -23 <(grep -hE 'Code\s*=\s*"[A-Z]+_[A-Z]+' internal/claudedock/errcodes/*.go | awk -F'"' '{print $2}' | sort -u) <(grep -oE '[A-Z]+_[A-Z]+_[A-Z0-9_]+' docs/runbooks/v3-error-code-index.md | grep -E '^(AUTH|DISK|MOUNT|NET|SESSION|SSH|STATE|SYSTEM)_' | sort -u)` 输出为空（注册表中每条 Code 都在手册中出现）
    - `grep -qF '快速诊断命令' docs/runbooks/v3-error-code-index.md` 退出码 0
    - `grep -qF '适用版本：v3.0' docs/runbooks/v3-error-code-index.md` 退出码 0
    - `grep -qF 'claudedock explain' docs/runbooks/v3-error-code-index.md` 退出码 0
...
# 错误码索引与注册表交叉一致
comm -23 \
  <(grep -hE 'Code\s*=\s*"[A-Z]+_[A-Z]+' internal/claudedock/errcodes/*.go | awk -F'"' '{print $2}' | sort -u) \
...
| T-35-03-04 | Tampering | error-code-index 与 errcodes/*.go 不同步 → 运维依据错误信息操作 | mitigate | 手册开篇声明"权威来源：claudedock explain"；CI acceptance criteria 做 `comm -23` 反向 diff 断言 |
</threat_model>
