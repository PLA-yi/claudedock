...
requires:
  - phase: 31-cli
    provides: errcodes/mount.go MOUNT_* 注册表（10 个 code）+ claudedock doctor --json schema_version=1
  - phase: 32-ssh-tmux
...
tech-stack:
  ...
  patterns:
    ...
    - "Pattern D 变体: 三层人工破坏 → docker exec claudedock doctor --json → jq -e select(.code==X) + 错误码命名正则 + warn/fail next_action 非空守恒"
...
patterns-established:
  ...
  - "Pattern D-M13 (Phase 35 静默降级回归): docker exec 破坏 → sleep 2s 让 CLI 渲染 → claudedock doctor --json → jq -e select(.code==X) + warn/fail next_action 守恒"
...
**BASE-03 弱网三场景 UAT 脚本 + M13 三层静默降级回归脚本就位：tc→iptables 两级 fallback、pgrep/tmux/token 三大无感知锚点量化、claudedock doctor MOUNT_* 错误码 + 中文 next_action 守恒断言，全程 trap EXIT 双兜底**
...
- **REQ-F3-C 失败提示验证**：脚本含字面量 grep 模板 `(按 Enter 重试|claudedock doctor)`
...
| 345 | `check_final_failure_prompt()` | grep `(按 Enter 重试\|claudedock doctor)`（REQ-F3-C） |
...
- **Issue:** 脚本最初仅在 `disrupt_start` 里 `info "网络破坏：..."` 才打印命令；但本机无 com.claudedock.managed=true 容器 → `detect_container` 直接 SKIP → 永远跑不到 disrupt_start → acceptance grep 永远 fail
...
- **Issue:** 多文件模式下 BSD/GNU `grep -oE` 都会输出 `internal/claudedock/errcodes/mount.go:MOUNT_MERGERFS_FAILED` 形式带文件名前缀；script side 输出 `MOUNT_MERGERFS_FAILED` 裸字面值；两者 `comm -23` 始终非空 → 假阳性 fail
...
- **Verification:** `comm -23 <(grep -hoE 'MOUNT_[A-Z_]+' scripts/degradation-regression.sh \| sort -u) <(grep -hoE 'MOUNT_[A-Z_]+' internal/claudedock/errcodes/mount.go internal/claudedock/errcodes/codes.go \| sort -u)` 输出空集
...
ac12 (按 Enter 重试|claudedock doctor) grep 模板                 ✓
...
- **30s 场景真机签字**：本机无 `com.claudedock.managed=true` 容器，UAT 脚本只能跑通 dry-run 预览。需在 staging 容器执行 `bash scripts/uat-network-resilience.sh --scenario=30s` 并把 JSON 报告（含 `pgrep_survived_full_duration=true ∧ buffer_diff_lines=0 ∧ token_replayed=true ∧ reconnect_success=true`）附到 v3-acceptance-report
