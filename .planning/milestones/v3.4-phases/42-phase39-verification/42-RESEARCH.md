...
这是一个纯验证/文档补齐阶段，不需要新增功能代码。所有需要验证的代码和测试已经存在。Phase 40 的手动测试（40-VERIFICATION-REPORT.md）已确认 `claudedock local up` 能真实启动容器并提供 SSH 连接，提供了额外的端到端验证证据。
...
|----|-------------|-----------------|
| LOCAL-01 | `claudedock local` 子命令支持一键启动本地容器 | `internal/local/local.go` LocalManager.Up 实现完整；`cmd/claudedock/local.go` cobra 注册；30 个 unit tests 通过；Phase 40 手动验证真实 Docker 启动 |
...
| UX-02 | `claudedock local` 支持 `down` / `status` 子命令 | `cmd/claudedock/local.go` runLocalDown + runLocalStatus；`internal/local/local.go` Down + Status 方法；TestDown + TestStatus 测试 |
</phase_requirements>
...
| `cmd/claudedock/local.go` | 132 | cobra local subcommand group (up/down/status) | go build OK |
...
|------|----|-----|--------|---------|
| cmd/claudedock/local.go::runLocalUp | internal/local/local.go::Up | LocalManager.Up() 调用 | WIRED | `cmd/claudedock/local.go:73` mgr.Up(cmd.Context()) |
```
...
# Build check
go build ./cmd/claudedock/

# CLI help verification
go run ./cmd/claudedock/ local --help
go run ./cmd/claudedock/ local up --help
go run ./cmd/claudedock/ local down --help
go run ./cmd/claudedock/ local status --help
...
# Anti-pattern scan
grep -rn "TODO\|FIXME\|HACK\|PLACEHOLDER" internal/local/ cmd/claudedock/local.go
```

## Open Questions

1. **是否需要在 VERIFICATION.md 中包含 Phase 40 的集成测试证据？**
   - What we know: Phase 40 手动验证报告确认 `claudedock local up` 真实启动了 Docker 容器并可通过 SSH 连接
