...
Phase 39 需要实现 `claudedock local` 子命令组，让用户在本地机器上一键启动 managed-user 容器。核心工作分为三块：(1) CLI 子命令扩展（cobra `local up/down/status`）；(2) entrypoint.sh 的 `MODE=local` 分支改造；(3) 可选的 sing-box egress 注入。
...
|----|-------------|-----------------|
| LOCAL-01 | `claudedock local` 子命令支持一键启动本地容器 | CLI 结构：cobra 子命令注册模式已验证（main.go line 93）；Docker 操作：复用 worker.go 的 runDocker + buildCreateArgs 模式；镜像管理：image.lock 已有 `local_dev_image_name` 字段 |
...
| UX-02 | `claudedock local` 支持 `down` / `status` 子命令 | 复用 `docker stop/rm` 和 `docker inspect` 模式（worker.go line 1096）；容器标签 `claudedock-local` 用于过滤 |
</phase_requirements>
...
```go
// Source: cmd/claudedock/main.go line 53-67
...
# 本地模式：可选 sing-box 启动
if [ "$MODE" = "local" ] && [ -f /etc/claudedock/sing-box-outbound.json ]; then
...
**Why it happens:** 固定容器名 `claudedock-local`
**How to avoid:** 容器名 = `claudedock-local-{project_path_hash}`，用项目路径 MD5 前 8 位区分
...
func localContainerName(projectDir string) string {
    ...
    return fmt.Sprintf("claudedock-local-%x", hash[:4])
}
```
...
|--------------|------------------|--------------|--------|
| 无本地模式 | `claudedock local` 子命令 | Phase 39 (this) | 新功能 |
...
- `cmd/claudedock/main.go` — CLI 骨架和 cobra 注册模式
