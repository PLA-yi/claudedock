...
**Phase Goal:** 为每个用户提供一键启动的本地 Docker "云主机"，支持 `claudedock local up/down/status` 管理容器生命周期、SSH 直连、devcontainer.json VS Code 集成、以及可选的 sing-box 全隧道出网。
...
|---|-------|--------|----------|
| 1 | `claudedock local up` 一键启动容器并输出 SSH 连接信息（host, port, user, password） | CODE-VERIFIED | `cmd/claudedock/local.go:47-91` runLocalUp 调用 mgr.Up() 并格式化输出 SSHInfo；`internal/local/local.go:81-146` Up 方法完整流程：密码生成 -> 容器名 -> buildCreateArgs -> docker create/start -> inspectSSHPort -> 返回 SSHInfo |
| 2 | `claudedock local down` 停止并清理本地容器，幂等操作 | CODE-VERIFIED | `cmd/claudedock/local.go:93-106` runLocalDown 调用 mgr.Down()；`internal/local/local.go:149-166` Down 方法：containerExists 检查 -> stop -> rm -f；不存在时返回 nil（幂等） |
| 3 | `claudedock local status` 显示容器运行状态和端口映射 | CODE-VERIFIED | `cmd/claudedock/local.go:108-132` runLocalStatus 调用 mgr.Status() 并格式化输出；`internal/local/local.go:169-198` Status 方法：inspectContainerStatus + inspectSSHPort + inspect image/created |
...
| 5 | entrypoint MODE=local 在有 egress 配置时启动 sing-box（tun 或 proxy 模式） | VERIFIED | `deploy/docker/managed-user/entrypoint.sh:316` `if [ "$MODE" = "local" ] && [ -f /etc/claudedock/sing-box-outbound.json ]`；行 323-328 tun 模式检测 + 降级为 proxy；行 332-370 proxy 模式配置 + 环境变量注入；行 373-383 sing-box 启动 |
...
| `cmd/claudedock/local.go` | cobra local 子命令组（up/down/status） | VERIFIED | 133 lines；导出：newLocalCmd；内部：runLocalUp, runLocalDown, runLocalStatus |
...
|------|----|-----|--------|---------|
| cmd/claudedock/local.go::runLocalUp | internal/local/local.go::Up | LocalManager.Up() 调用 | WIRED | `local.go:73` info, err := mgr.Up(cmd.Context()) |
| cmd/claudedock/local.go::runLocalDown | internal/local/local.go::Down | LocalManager.Down() 调用 | WIRED | `local.go:100` mgr.Down(cmd.Context()) |
| cmd/claudedock/local.go::runLocalStatus | internal/local/local.go::Status | LocalManager.Status() 调用 | WIRED | `local.go:115` mgr.Status(cmd.Context()) |
...
| 项目构建成功 | `go build ./cmd/claudedock/` | 无错误 | PASS |
| local 子命令组完整 | `claudedock local --help` | 显示 3 个子命令：down, status, up | PASS |
| up 子命令帮助正确 | `claudedock local up --help` | 显示 --egress-config 和 --port 标志 | PASS |
| down 子命令帮助正确 | `claudedock local down --help` | 正常显示 | PASS |
| status 子命令帮助正确 | `claudedock local status --help` | 正常显示 | PASS |
...
| 无反模式标记 | `grep -rn "TODO\|FIXME\|HACK\|PLACEHOLDER" internal/local/ cmd/claudedock/local.go entrypoint.sh devcontainer.json` | 无匹配 | PASS |
...
|-------------|------------|-------------|--------|----------|
| LOCAL-01 | 39-01 | `claudedock local up` 启动容器并返回 SSH 连接信息 | SATISFIED | `local.go:81-146` Up 方法完整实现；`cmd/claudedock/local.go:47-91` CLI 包装；测试 TestBuildCreateArgs (9 sub-tests) + TestInspectSSHPort + TestLocalContainerName |
| LOCAL-02 | 39-01 | `claudedock local down` 停止并清理容器 | SATISFIED | `local.go:149-166` Down 方法：containerExists 检查 + stop + rm -f；`cmd/claudedock/local.go:93-106` CLI 包装；幂等设计（不存在返回 nil） |
| LOCAL-03 | 39-01 | `claudedock local status` 显示容器运行状态 | SATISFIED | `local.go:169-198` Status 方法：inspectContainerStatus + inspectSSHPort + inspect image/created；`cmd/claudedock/local.go:108-132` CLI 包装；测试 TestInspectContainerStatus (running + not_found) |
...
|----------|--------------|--------|
| TODO/FIXME/HACK/PLACEHOLDER | internal/local/, cmd/claudedock/local.go, entrypoint.sh, devcontainer.json | None found |
...
|---|----------|-------|-----------------|
| 1 | `claudedock local up` 真实启动 Docker 容器 | 运行 `claudedock local up`，观察输出 | 容器启动成功，输出 SSH host/port/user/password |
| 2 | SSH 连接本地容器 | `ssh workspace@127.0.0.1 -p {port}`，输入密码 | 成功登录容器 shell |
| 3 | `claudedock local status` 显示运行状态 | 运行 `claudedock local status` | 显示容器 Running、端口映射、镜像名、创建时间 |
| 4 | `claudedock local down` 停止并清理容器 | 运行 `claudedock local down` | 容器被停止并删除 |
...
**需人工确认（Docker 运行时行为）：**
- `claudedock local up` 真实 Docker 容器创建和 SSH 连接
