...
|---|-----------|--------|----------|
| 1 | `claudedock doctor` 新增 remote-ssh 维度，能检测 VS Code Server 进程是否存在 | PASS | `checkVSCodeServerProcess()` in `doctor/remote_ssh.go`; RunDoctor step 8 in `doctor/doctor.go`; ValidArgs includes "remote-ssh" in `cmd/claudedock/doctor.go` |
