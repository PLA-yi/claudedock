...
control-plane / host-agent 不再创建 gw 容器；`container_proxy_provider` 简化为单容器路径；`claudedock-net-*` 自定义 bridge 退役；双绑互斥契约不变。

## Success Criteria (from ROADMAP)

1. 创建一台 host 后只生成一个 docker 容器（`claudedock-<id>`），`docker network ls` 不再出现 `claudedock-net-<HostID>`。
...
- **nft 已由 entrypoint 应用**：`apply_nft_or_die` (entrypoint L243-262) 在容器内自身 netns 应用 `/etc/claudedock/default-deny.nft`。Phase 54 host-agent **不再**进入 worker netns 跑 `worker_firewall_linux.go::ApplyWorkerFirewallRules`（见 D-54-7）。
...
| `gatewayContainerName` / `networkName` | L323-345 | 删除（无 gw 容器、无 claudedock-net 网络）|
...
| `GATEWAY_IMAGE := claudedock-sing-gateway:local` | 删 |
...
**Rationale**：
- 容器内 nft 由 entrypoint `apply_nft_or_die` 自己跑 (`nft -f /etc/claudedock/default-deny.nft`)，不再需要 host 侧通过 `nftables.WithNetNSFd` 进入 worker netns 写规则。
...
| Linux 真机 manual smoke | `make dev` 起控制面 + 在 Linux 宿主创建一个 host + `docker network ls` + `docker ps` 双断言 | SC1（只生成一个容器 + 无 `claudedock-net-*`）|
