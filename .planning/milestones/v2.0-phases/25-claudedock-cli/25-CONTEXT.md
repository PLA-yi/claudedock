# Phase 25: claudedock CLI 骨架与连接 - Context
...
交付用户侧 Go 二进制 `claudedock` 的最小可用闭环：`claudedock init` 将网关与凭据写入 `~/.claudedock/config.yaml`；无参数运行 `claudedock` 时完成控制面认证、在主机未就绪时等待或重试、建立到 SSH Proxy 的会话并进入容器内 Claude Code 交互环境。
...
### 配置模型与文件约定
- **D-03:** 配置文件路径固定为 `~/.claudedock/config.yaml`（与 CLI-02 / ROADMAP 一致）。配置目录权限 `0700`，文件权限 `0600`。
...
### 控制面集成（就绪与 SSH 参数）
- **D-06:** 使用现有 `POST {gateway}/v1/entry/{shortId}/auth`（JSON `password`）获取 SSH 参数与状态；行为与 `internal/controlplane/http/entry.go` 及 bootstrap 文档一致，**不新增**专用 claudedock API。
...
### 自有网关（CLI-05）
- **D-12:** `gateway` 完全由配置文件决定，**无**硬编码生产默认 URL；若缺少配置且非 `init` 流程，应明确报错并提示先执行 `claudedock init`。
...
- `.planning/research/ARCHITECTURE.md` — `cmd/claudedock`、数据流与 Pattern 1
...
### Reusable资产
- `internal/controlplane/http/entry.go`：Entry 认证与 SSH 参数返回的**唯一真实契约**，`claudedock` HTTP 客户端应与此对齐。
- `internal/sshproxy/proxy.go`：SSH 服务端行为与端口约定（文档中 `:2222`）的参照。

### 既定模式
- 仓库已有 `golang.org/x/crypto`；新增 `cmd/claudedock` 与现有 `cmd/control-plane` 并列，符合 `.planning/research/ARCHITECTURE.md` 中的目录建议。
