...
provides:
  - claudedock CLI 二进制（cobra 入口 + init 子命令 + 根命令主流程）
  - ~/.claudedock/config.yaml 配置读写模块
...
key-decisions:
  - "Entry API 为唯一认证契约，不新增专用 claudedock API"
...
patterns-established:
  - "internal/claudedock 包承载所有客户端逻辑，cmd/claudedock 仅为入口编排"
...
# Phase 25 Plan 01: claudedock CLI 骨架与连接 Summary
...
- `claudedock` 可构建二进制：无参数运行完成认证→轮询→SSH→PTY→远程 claude 全流程
- `claudedock init` 支持交互式/环境变量/flag 三种配置输入路径，写入 `~/.claudedock/config.yaml`（目录 0700、文件 0600）
...
- `cmd/claudedock/main.go` — CLI 入口，cobra 根命令和 init 子命令编排
- `internal/claudedock/config.go` — 配置结构体、路径解析、YAML 读写
- `internal/claudedock/entry.go` — Entry API HTTP 客户端、认证、就绪轮询、网关预检
- `internal/claudedock/ssh.go` — SSH 密码认证连接、PTY 申请、SIGWINCH 转发、远程 claude 启动
...
- `claudedock` 二进制可构建并运行，为 Phase 26（参数透传 + TTY/信号/退出码完整语义）提供客户端锚点
- `internal/claudedock` 包结构清晰，Phase 26 可直接在 SSH 模块上扩展信号转发和退出码透传
- Phase 27 目录映射可复用 SSH 连接基础设施

## Self-Check: PASSED

- [x] cmd/claudedock/main.go exists
- [x] internal/claudedock/config.go exists
- [x] internal/claudedock/entry.go exists
- [x] internal/claudedock/ssh.go exists
