...
# Phase 25：Code Review 报告（claudedock-cli）

**审查范围：** Phase 25 中与 `claudedock` CLI 相关的源码与依赖（`git` 范围 `6fd35c3^..8550bfe` 内上述 6 个文件）。  
...
主要风险在 **`internal/claudedock/ssh.go`**：远程 `claude` 以非零状态退出时，通过 `os.Exit` 直接退出进程，**不会执行恢复本地 TTY 的 `defer term.Restore`**，易导致终端停留在 raw 模式。另建议对 Entry URL 中的 `short_id` 做路径转义，并对 `go.mod` 中误标为 indirect 的直接依赖执行 `go mod tidy`。
...
**File:** `internal/claudedock/ssh.go:97-100`  
...
**File:** `internal/claudedock/entry.go:69`  
...
**File:** `cmd/claudedock/main.go:48-51`  
...
**File:** `internal/claudedock/entry.go:54-59`  
...
**File:** `cmd/claudedock/main.go:116-117`  
...
**File:** `go.mod:16-32`  
**Issue:** `github.com/spf13/cobra`、`gopkg.in/yaml.v3`、`golang.org/x/term` 等被标为 `// indirect`，但 `cmd/claudedock` 与 `internal/claudedock` 直接 import 它们。  
**Fix：** 在项目根执行 `go mod tidy`，将直接依赖移到 `require` 顶层并去掉错误的 indirect 标记。

### IN-02：错误分类依赖中文字符串子串

**File:** `cmd/claudedock/main.go:135-147`  
...
**File:** `internal/claudedock/ssh.go:82-90`  
...
| 要求 | 结论 |
|------|------|
| 可构建 `cmd/claudedock`，根命令无参走 auth→轮询→SSH+PTY | 满足（逻辑在 `main` + `internal/claudedock`）。 |
| `init` 写入 `~/.claudedock/config.yaml`，权限 0700/0600 | `SaveConfig` 满足。 |
