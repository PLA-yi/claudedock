...
overrides_applied: 0
human_verification:
  - test: "在已配置网关且容器可用的环境中，对比 `claude` 与 `claudedock` 传入相同参数（如 `-p`、`--model`）时远端行为与退出码是否一致"
...
**Phase Goal：** claudedock 的参数透传和终端交互与本地 claude 完全一致  
**Verified：** 2026-04-15T12:00:00Z  
**Status：** human_needed  
...
| 4 | 远端 claude 退出码原样为 claudedock 退出码 | ✓ VERIFIED | `ssh.go`：`*ssh.ExitError` 时 `return exitErr.ExitStatus(), nil`；`main.go`：`if exitCode != 0 { os.Exit(exitCode) }` |
...
|----------|----------|--------|---------|
| `internal/claudedock/ssh.go` | 参数、`QuoteCommand`、TTY/非 TTY、退出码返回 | ✓ VERIFIED | `gsd-tools verify artifacts` 通过；含 `shellescape.QuoteCommand`、`IsTerminal`、`RequestPty` 仅一处 |
| `cmd/claudedock/main.go` | cobra 透传、`--` 剥离、退出码 | ✓ VERIFIED | `DisableFlagParsing`、`ArbitraryArgs`、`args[0]=="--"`、`ConnectAndRunClaude(sshCfg, args)` |
| `go.mod` | shellescape 依赖 | ✓ VERIFIED | `al.essio.dev/pkg/shellescape v1.6.0` |

### Key Link Verification

`gsd-tools verify key-links` 对 PLAN 中 `pattern` 报告未匹配（例如 `ConnectAndRunClaude` 在源码中带 `claudedock.` 前缀；`ExitStatus()` 出现在 `ssh.go` 而非 `main`）。**手动核对如下：**

| From | To | Via | Status | Details |
|------|-----|-----|--------|---------|
| `cmd/claudedock/main.go` | `internal/claudedock/ssh.go` | `ConnectAndRunClaude` | ✓ WIRED | `exitCode, err := claudedock.ConnectAndRunClaude(sshCfg, args)` |
| `internal/claudedock/ssh.go` | `session.Start` | `QuoteCommand` 构建命令 | ✓ WIRED | `remoteCmd := ... QuoteCommand(...)` → `session.Start(remoteCmd)` |
| `internal/claudedock/ssh.go` | `cmd/claudedock/main.go` | 退出码上浮 | ✓ WIRED | 返回 `(int, error)`，`main` 中 `os.Exit(exitCode)`（非 `ExitStatus` 字面量于 main，语义等价 PLAN） |
...
|----------|---------|--------|--------|
| 可编译 | `go build -o /dev/null ./cmd/claudedock/` | 成功 | ✓ PASS |
| 远端命令构建 | `grep -n 'QuoteCommand' internal/claudedock/ssh.go` | 存在 | ✓ PASS |
| 无 ssh 内 os.Exit | `grep 'os.Exit' internal/claudedock/ssh.go` | 仅注释 | ✓ PASS |

未启动网关/SSH 服务；完整 `claudedock -p` 端到端留待人工或集成环境（符合 Step 7b 约束）。
