...
**潜在 backend 流转 Phase 51**：KILL-04 用例已内置兜底分支，若 gateway 未接 claudedock-net-* 自定义 bridge（如 backend 改用 macvlan / host network），用例 t.Skipf 并把现状落到 VERIFICATION 备注 + ROADMAP `Phase 51` 备注列。
...
| KILL-04 | `TestKillSwitch_04_NetworkDisconnect` | `docker network disconnect <claudedock-net-X> <gw>` → ≤ 3000ms 断网 + 0 包 + cleanup `network connect --ip <savedIP>` 恢复 | `ClassifyStressResult KILL-04_{Pass,FailOnPacketLeak}` × 2 + `PickGatewayBridgeNetwork` × 5 | — | PASS（darwin 单测） |
...
- **darwin 新增单测（`tests/e2e/helpers_test.go`）**：**29 个**全绿，覆盖：
  ...
  - `PickGatewayBridgeNetwork` × 5（claudedock-preferred / only-bridge / custom-fallback / empty / multi-claudedock）
...
|----|-----------------------|----------------|----------|
| D-50-1 | CONTEXT §Area 3 写「`docker network disconnect <bridge-net> <gateway>` 把 gateway 从 docker bridge 摘走」 | grep 源码确认 gateway 实际接的是专属自定义 bridge `claudedock-net-<HostID>`（`internal/network/container_proxy_provider.go:323 networkName`），**不是** docker 默认 `bridge`；worker 同时挂 default `bridge` + `claudedock-net-*` 两网。KILL-04 disconnect 的是前者，验证 worker 不允许 fallback 到 default bridge 直连。 | `PickGatewayBridgeNetwork` 纯函数按 `claudedock-net-` 前缀优先匹配；用例兜底分支识别 `has no claudedock-net` → t.Skipf 流转 Phase 51。详见 50-04-SUMMARY.md。 |
...
4. **KILL-04 `TestKillSwitch_04_NetworkDisconnect`**
   - 前置：同 KILL-01。
   - 验证：基线 worker curl exit 0 → `docker network disconnect claudedock-net-<HostID> <gw>` → ≤ 3000ms 内断网 + 0 包 → cleanup 内 `docker network connect --ip <savedIP>` 接回。
   - 关键观察点：worker 同时挂 default `bridge` + `claudedock-net-*`，本测就是要证「disconnect claudedock-net 后 worker 不会回落到 default bridge 默认路由」。
...
- `(*GoldenPath).DisconnectGatewayFromBridge(ctx) (savedNet, savedIP, err)` —— 自动挑选 `claudedock-net-*` 摘走 + 返回元数据供 reconnect；KILL-04 主接口。
...
- **潜在 backend GAP（条件性）**：KILL-04 用例内置兜底分支，若 ubuntu-24.04 真机 + Step 2..7 落地后 gateway 仍未接 `claudedock-net-*` 自定义 bridge（grep 当前 v3.6 源码已就绪，但分支预留），用例自动 t.Skipf + reason 提示，待 Phase 51 QUAL-* 收紧 → 转 PASS。当前 grep 显示源码已实现专属 bridge，本 phase 不主动列 GAP。
