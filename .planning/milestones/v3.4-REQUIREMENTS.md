# Requirements Archive: v3.4 多形态容器接入
...
- [x] **LOCAL-01**: `claudedock local` 子命令支持一键启动本地容器 → Phase 42 gap closure
...
- [x] **LOCAL-03**: 本地容器支持 sing-box 全隧道（可选配置） → Phase 42 gap closure
  - 通过 `claudedock local --egress-config <file>` 注入 sing-box outbound JSON
...
- [x] **UX-01**: `claudedock doctor` 新增 remote-ssh 诊断维度 — completed 2026-05-08 (41-VERIFICATION)
  - 检测 VS Code Server 进程是否存在
  - 检测 `~/.vscode-server/` 磁盘占用
  - 检测 forwarding channel 是否被拦截

- [x] **UX-02**: `claudedock local` 支持 `down` / `status` 子命令 → Phase 42 gap closure
