...
- **sing-box rule-set source format version 3 对应 sing-box 1.11.0**（不是 SUMMARY 说的 1.10），但 `type:"local" + format:"source"` + 文件 watch 在 1.10 就已经支持。Phase 45 的 placeholder `{"version":3,"rules":[]}` 需要 gateway 镜像至少跑 1.11+；本仓库 `claudedock-sing-gateway:local` 镜像版本需要在 plan 里勘察并明确。
...
| sing-box | 镜像 `claudedock-sing-gateway:local`（具体版本需 plan 阶段勘察） | gateway 容器运行时；必须 ≥ 1.11（rule-set source format v3） | `internal/network/container_proxy_provider.go:180` |
...
```bash
# 验证 go.mod Go 版本
grep "^go " /Users/.../claudedock/go.mod  # 1.25.7
# 验证 pgcrypto 已启用
...
`[VERIFIED: 仓库 grep]` Go 1.25.7、pgcrypto 已启用、镜像名 `claudedock-sing-gateway:local` 通过 env `CLAUDEDOCK_GATEWAY_IMAGE` 可覆盖。
...
| 密钥/环境变量 | `CLAUDEDOCK_GATEWAY_IMAGE` env（gateway 镜像覆盖）；`DATA_DIR` env（gateway 配置根目录） | 无新增，沿用 |
| 构建工件 / 已安装包 | gateway 容器镜像 `claudedock-sing-gateway:local` 内的 sing-box 二进制版本 | **必须 ≥ 1.11**（rule-set source format v3 才支持）。Plan 阶段第一步勘察镜像内版本，若 < 1.11 需在 Phase 45 内升级镜像或退到 v2 schema |
...
- gateway 镜像内 sing-box 版本 —— Plan 阶段第一个任务 `docker inspect claudedock-sing-gateway:local` + 容器内 `sing-box version`；若 < 1.11 需扩大 Phase 45 范围把镜像构建 Dockerfile 升 sing-box
...
|---|-------|---------|---------------|
| A1 | gateway 镜像 `claudedock-sing-gateway:local` 内 sing-box 版本 ≥ 1.11.0 | §Standard Stack / §Environment Availability | rule-set version 3 schema 不被识别 → sing-box 启动失败 → gateway 不健康 → 所有新 host 创建失败。Plan 阶段必须先验证；若 < 1.11 Phase 45 范围扩大 |
...
1. **gateway 镜像内 sing-box 实际版本？**
   - 我们已知：镜像名 `claudedock-sing-gateway:local`，env 可覆盖
...
4. **空 rule-set 文件是否需要包含 stub rule 兜底？**
   - 官方文档未明示 `rules: []` 运行时行为
   - 建议：Plan 阶段 dry-run sing-box 单独跑一次确认；若失败用 `[{"domain":"never.invalid.claudedock"}]` 兜底
