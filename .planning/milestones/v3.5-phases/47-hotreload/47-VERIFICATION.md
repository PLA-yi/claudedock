...
re_verification:
  ...
  gaps_closed:
    - "ApplyBypassRuleSet nft -f 事务（family+table+netns 全部对齐 ip claudedock + nsenter）"
    - "GET /v1/admin/hosts/{hostID}/bypass/consistency 真实生产路径"
    - "uat-bypass.sh I2/I9 nft family 修正为 ip claudedock"
...
gaps:
  - truth: "ActionReloadHostBypass 在 worker dispatch 命中后真正完成「nft -f 事务更新 @whitelist_v4 set」"
    ...
    reason: |
      ...
      (a) Plan 47-02 的 ConfigureBypassFirewall 把 whitelist_v4 set 添加到 worker netns 的
          claudedock 表（Family = nftables.TableFamilyIPv4 / nft family = "ip"，
    ...
    artifacts:
      - path: "internal/network/bypass_reload.go"
        issue: "常量 bypassNftFamily=\"inet\" / bypassNftTable=\"sbfw\" 与真实表 ip claudedock 不一致；nftRunner/nftJSONLister 缺 nsenter 进入 worker netns 的包装"
      - path: "internal/network/bypass_firewall.go"
        issue: "whitelist_v4 set 被加入调用方传入的 claudedock(IPv4) 表，未独立创建 inet sbfw 表；与 47-CONTEXT.md §nft 加固 + bypass_reload.go 假设的命名不对齐"
      - path: "internal/network/worker_firewall_linux.go"
        issue: "ConfigureBypassFirewall 在 claudedock(IPv4) 表挂载 set，未引入 inet sbfw 新表"
    missing:
      - "统一 nft 表族与表名：要么把 ConfigureBypassFirewall 改为创建 inet sbfw 表（与 47-CONTEXT.md/Plan 47-01 假设一致），要么把 bypass_reload.go 的常量改成 ip claudedock（与 Plan 47-02 现状一致）+ 同步更新 uat-bypass.sh"
    ...
    fix_summary: |
      ...
      bypassNftFamily="ip" / bypassNftTable="claudedock"，bypassNftSetName 引用
  ...
  - truth: "GET /v1/admin/hosts/{hostID}/bypass/consistency 真正返回 nft set 与 rule-set 文件 SHA-256 一致性"
    ...
    fix_summary: |
      ...
      前缀 + ip claudedock whitelist_v4 字面值）从 worker netns 内的真实 set 拉 JSON。
  ...
  - truth: "容器 netns output 链 policy=drop + 仅放行 sb-tun0 / uid+443 / @whitelist_v4 / mDNS/LLMNR/NetBIOS drop + 链末 log drop"
    ...
    reason: |
      ...
      assert_invariant_I2 / I9 都用 `nft list chain inet claudedock …`（uat-bypass.sh:236、341），
      实际表是 `ip claudedock`（Family TableFamilyIPv4）—— UAT 启用后会假报 I2/I9 缺失，并不能真正
      守护「policy=drop + log drop」不变量。
    artifacts:
      - path: "scripts/uat-bypass.sh"
        issue: "assert_invariant_I2 / I9 hardcode `nft list chain inet claudedock output`，与真实 ip claudedock 表族不符"
    missing:
      - "把 uat-bypass.sh 中所有 `inet claudedock` 改为 `ip claudedock`，或在统一 nft 表族后一并改为 inet sbfw"
      - "为 I2/I9 加 dry-run + 真实模式双断言（dry-run 只验证脚本文本，真实模式校验真表族）"
    fix_commit: "68f830c"
    fix_summary: |
      scripts/uat-bypass.sh assert_invariant_I2（line 236）/ I9（line 341）的
      `nft list chain inet claudedock output` 全部改为
      `nft list chain ip claudedock output`，与 gap #1 修复后 bypass_reload.go
      的 bypassNftFamily="ip" / bypassNftTable="claudedock" 保持单一事实源。
...
| 9   | uat-bypass.sh 的 I2 / I9 不变量真的能在生产 nft 表上查到「policy drop」/「mDNS drop 规则」 | FAILED | uat-bypass.sh:236 / 341 用 `nft list chain inet claudedock output`，但实际表是 `ip claudedock`（family IPv4）；脚本启用后会假报 I2/I9 缺失。详见 gap #3。 |
...
| nft 表族一致性（静态字面值） | `grep -n "inet claudedock\|inet sbfw" scripts/uat-bypass.sh internal/network/bypass_reload.go` | 多处 inet claudedock + inet sbfw 混用 | FAIL（family/table 错位） |
...
| scripts/uat-bypass.sh | 236, 341 | `nft list chain inet claudedock output` 写错 family（实际是 `ip claudedock`） | Blocker | I2 / I9 不变量启用后会持续假红 |
...
- **Plan 47-02 实现** 复用了 v3.4 留下的 `ip claudedock`（Family IPv4）表，set 跟随其他 worker netns 规则一起住在该表内，且只能通过 nftables.Conn + WithNetNSFd 操作。
...
- **uat-bypass.sh** 又自己拍脑袋写成 `inet claudedock`，与两边都不一致。
