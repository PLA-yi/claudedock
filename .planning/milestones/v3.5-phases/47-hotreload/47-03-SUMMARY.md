...
- 工程：`set -euo pipefail` + `trap cleanup EXIT INT TERM` 兜底 `docker restart claudedock-gw-<HID>`；退出码 0/1/2；token 在 dry-run 日志中遮蔽为 `***`（T-47-12 mitigation）
...
| 全包测试 | `go test ./... -count=1` | 全 PASS（19 个包，含 claudedock 47s 集成测试） |
