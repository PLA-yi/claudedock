...
**Requirements:** UI-01, UI-02, UI-03, UI-04 (Complete)
...
**Requirements:** PRB-01, PRB-02 (Complete)
...
**Requirements:** DEP-01, DEP-02, DEP-03, DEP-04, DEP-05 (Complete)
...
2. `DATABASE_URL=file:/data/claudedock.db` 正确连接 SQLite
3. `./data` 目录映射持久化数据库文件
...
**Requirements:** DOC-01, DOC-02, DOC-03, TEST-01, TEST-02 (Complete)
...
- **pgx/v5 残留**: `go.mod` 中 `github.com/jackc/pgx/v5` 因 `tests/e2e/helpers_linux.go` 的 `//go:build e2e && linux` 构建标签而保留，不影响生产二进制。待 E2E 测试体系适配 SQLite 后清理。
