...
- Go 中通过 `os.Args[0]` 不可改，实际方案：启动时写 PID file `~/.claudedock/cold-promoter.pid`，`pgrep` 检查命令改为 `pgrep -F ~/.claudedock/cold-promoter.pid` 或直接 `pgrep -f "cold-promoter"` 通过 goroutine 内嵌标签（不适用）。**实际方案：新增独立的轻量子进程 `cold-promoter` 或使用 goroutine + PID file。**
...
- 不做 `pkill -f cold-promoter`（可能误杀其他 claudedock 实例，单宿主机多用户场景不安全）
...
- 读取 PID file `~/.claudedock/cold-promoter.pid`
- `kill -0 <pid>` 判断进程存活
- 文件不存在 → `newSkip`
- 进程存活 → `newPass`
...
- doctor 通过读取共享状态（如 PID file 旁的状态文件 `~/.claudedock/promoter-state.json`）来获取
...
6. **快速诊断命令** -- `pgrep -f cold-promoter`、`cat ~/.claudedock/last-session.json | jq '.promotion_count'`、`docker exec <ctr> ls /tmp/.claudedock-mounts/<hash>/hot/`、`claudedock doctor mount --json | jq '.checks[] | select(.name | startswith("promotion"))'`
...
- `TestExplain_MOUNT_PROMOTER_FAILED`：`claudedock explain MOUNT_PROMOTER_FAILED` 输出含 ≥200 中文字符

## 12. 文件清单

新增文件：
- `internal/claudedock/cold_promoter.go` — ColdPromoter（inotify watcher）+ PromotionEngine
- `internal/claudedock/cold_promoter_test.go` — 单测
- `docs/runbooks/v31-cold-promotion.md` — 运维手册
- `tests/scripts/uat-v31-promotion.sh` — e2e UAT 脚本

修改文件：
- `internal/claudedock/mount_strategy.go` — tryModeReal Full 路径集成 ColdPromoter
- `internal/claudedock/last_session.go` — 新增 3 个 promotion 字段
- `internal/claudedock/errcodes/mount.go` — 注册 MOUNT_PROMOTER_FAILED
- `internal/claudedock/errcodes/codes.go` — 声明 MOUNT_PROMOTER_FAILED 常量
- `internal/claudedock/doctor/mount.go` — 新增 4 项 promotion check
