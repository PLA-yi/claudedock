...
1. **F1 · Git 仓库前置约束**（REQ-01 / SC#1）
   - 在 `cmd/claudedock/main.go::startSession`（cwd 解析后、Entry API auth 之前）执行本地 `git rev-parse --show-toplevel`；非 git 仓库立即拒绝挂载、不发起任何 SSH/SFTP/Mutagen 子进程
...
2. **F2 · 单文件大小熔断**（REQ-02 / REQ-03 / SC#2）
   - `~/.claudedock/config.yaml` 新增字段 `hot_sync_max_file_mb: int`（默认 50；零值/缺失走默认）
...
4. **F4 · doctor mount 扩展 5 项 check**（REQ-05 / SC#4）
   - `internal/claudedock/doctor/mount.go` 新增 5 项检查（沿用 `Checker interface{ Run; Fix }` + `runWithTimeout` + `RemoteRunner` 既有骨架，**不新建文件**）：
...
5. **F5 · 错误码注册表 + explain 长说明**（REQ-06 / SC#5）
   ...
   - `claudedock explain MOUNT_REQUIRE_GIT_REPO` 与 `claudedock explain MOUNT_OVERSIZED_FILE_SKIPPED` 子进程退出 0、长说明 ≥ 200 字（自动通过 v3.0 已实现的 `cmd/claudedock/explain.go` + Registry 闭环）
6. **F6 · CI 闸门**（SC#6）
...
- **D-01**：检测点选定 `cmd/claudedock/main.go::startSession`，**位置在 cwd 解析（`os.Getwd()`）之后、Entry API `AuthenticateAndWait` 之前**：
...
- **D-02**：检测函数封装在 `cmd/claudedock/git_check.go` 新文件（与 main.go 同包），导出 `requireGitRepo(cwd string) error`：
...
- **D-04**：`Config` struct 新增字段 `HotSyncMaxFileMB int` （yaml tag `hot_sync_max_file_mb,omitempty`）：
  ...
  3. yaml 注释由 `claudedock init` 生成（非本阶段交付，留 v3.4 init 文案优化；本阶段直接编辑 config.yaml 即可）
...
- **D-08**：stderr 一次性提示由 `mount_strategy.MountWorkspace` 在 `StartHotSync` 返回后输出（不在 hot_sync 内部边扫边打）：
  1. 格式：`[!] 跳过大文件 N 个（>%dMB），由 cold 兜底:\n  <rel1> (%dMB)\n  <rel2> (%dMB)\n  ... 仅显示前 5 条，完整列表见 ~/.claudedock/last-session.json`
...
- **D-11**：单元测试方案（实现在 `internal/claudedock/mount_sshfs_test.go`）：
...
- **D-13**：5 项 check 全部加到 `internal/claudedock/doctor/mount.go` 同文件（**不新建文件**），延续 v3.0 D-01 单文件多 Checker 模式：
  ...
  | `oversized_files_count` | 本地（读 `~/.claudedock/last-session.json`） | `MOUNT_OVERSIZED_FILE_SKIPPED` (warn) | 复用 `claudedock.LoadLastSession()`；nil snapshot 走 `STATE_LAST_SESSION_MISSING` skip；`OversizedFiles` 长度 0 → pass；>0 → warn + Details 列前 5 条 |
  ...
  | `git_proxy_enabled` | 本地（解析 LoadConfig） | `AUTH_CONFIG_MISSING` (warn) | 缺 `proxy_commands` 或不含 `git` → warn + 提示 `claudedock init` 重新生成；与 `DefaultProxyCommands` 比对 |
...
- **D-15**：`--fix` 不为本阶段 5 项新 check 提供自动修复：
  ...
  - `sshfs_cache_args` — 缺参数说明 claudedock 二进制本身有 bug 或被覆盖，自动修复=重启 claudedock
...
- **D-17**：`errcodes/mount.go` 注册 2 条 Entry（init() 内追加）：
  ...
  MustRegister(Entry{
      ...
      NextAction: "如需提高阈值，编辑 ~/.claudedock/config.yaml::hot_sync_max_file_mb；或在 .gitignore 加入该路径以避免警告",
  })
  ```
...
- **D-19**：`claudedock explain MOUNT_REQUIRE_GIT_REPO` 与 `claudedock explain MOUNT_OVERSIZED_FILE_SKIPPED` 自动通过 v3.0 已实现的 `cmd/claudedock/explain.go::runExplain` 路径生效；**不需要改动** explain.go 本身（依赖 Registry + ExtendedExplanations 自动注入），单测仅断言「子进程 exit 0 + stdout 含错误码 + stdout 字符数 ≥200」

### F6 · 测试与 CI

- **D-20**：单元测试矩阵：
  - `cmd/claudedock/git_check_test.go` — git_check 在 git 仓库 / 非 git 目录 / git 不存在三种场景的退出码与 stderr 输出
  - `internal/claudedock/hot_sync_oversized_test.go` — fixture 目录含「ignore 命中的 50MB 文件」+「未 ignore 的 60MB 文件」+「未 ignore 的 30MB 文件」，断言只有 60MB 文件进 OversizedFiles 列表
  - `internal/claudedock/last_session_test.go` 扩展 — 序列化/反序列化含 `OversizedFiles` 的 snapshot，断言 schema_version=1 + 字段顺序
  - `internal/claudedock/mount_sshfs_test.go` 新增 — D-11 的 fixture SFTP 计数测试（CI 装 sshfs 才跑，否则 skip）
  - `internal/claudedock/doctor/mount_test.go` 扩展 — 5 项新 check 的 pass / warn / fail / skip 矩阵
  - `internal/claudedock/errcodes/codes_test.go` 现有遍历测试自动覆盖 2 条新 code（命名 + Message + NextAction 非空）
  - `internal/claudedock/errcodes/explanations_test.go` 现有遍历测试自动覆盖 2 条长说明（≥200 字符 + 五段模板）
  - `cmd/claudedock/explain_test.go` 扩展 — 子进程 exit 0 + stdout 字符数 ≥200 断言新增 2 条
...
- `.planning/milestones/v3.0-phases/34-claudedock-doctor-v3/34-CONTEXT.md` §D-01 / D-02 / D-13 — Phase 34 doctor 包结构 / errcodes/explanations.go 长说明模板 / 第一屏布局；本阶段新增 5 项 check 必须延续同骨架
- `.planning/milestones/v3.0-phases/34-claudedock-doctor-v3/34-CONTEXT.md` §D-15 / D-19 — JSON schema_version=1 + 5 维度 check 分布；本阶段保持 schema_version=1
- `.planning/milestones/v3.0-phases/34-claudedock-doctor-v3/34-CONTEXT.md` §D-17 / D-18 — `claudedock explain` 子命令的 Registry + ExtendedExplanations 双层数据源；本阶段直接复用，不改动 explain.go

### 既有代码（直接改造对象）

- `internal/claudedock/config.go` — `Config` struct 增 `HotSyncMaxFileMB`；新增 `EffectiveHotSyncMaxFileMB()` 方法（D-04）
- `internal/claudedock/mount_sshfs.go` — `mountSSHFS` sshfs 命令追加 4 个缓存参数（D-10）
- `internal/claudedock/hot_sync.go` — `HotSyncConfig` 加 `MaxFileBytes`；`HotSyncEngine` walk 阶段加 size 过滤；`HotSyncStatus` 加 `OversizedFiles`（D-05 / D-06 / D-07）
- `internal/claudedock/last_session.go` — `LastSessionSnapshot` 新增 `OversizedFiles []OversizedFile` 字段；新建 `OversizedFile` struct（D-09）
- `internal/claudedock/mount_strategy.go` — `MountWorkspace` 注入 `MaxFileBytes` + 写 `last-session.json` 时塞 `OversizedFiles` + 输出 stderr 一次性提示（D-08 / D-09）
- `internal/claudedock/doctor/mount.go` — 5 项新 check 同文件追加（D-13）
- `internal/claudedock/errcodes/codes.go` — 2 个新 Code 常量（D-16）
- `internal/claudedock/errcodes/mount.go` — 2 条 MustRegister 注册（D-17）
- `internal/claudedock/errcodes/explanations.go` — 2 条 ExtendedExplanations 注册（D-18）
- `cmd/claudedock/main.go` — `startSession` 入口增 `requireGitRepo(cwd)` 调用，失败 `os.Exit(exitConfigError)`（D-01）
- `cmd/claudedock/git_check.go`（**新增**） — `requireGitRepo` 实现 + 单元测试 fixture（D-02）
- `cmd/claudedock/explain.go` — **不改动**（D-19，依赖 Registry 自动覆盖新 code）

### 既有代码（参考样板，不改动）

- `internal/claudedock/doctor/mount.go::checkMergerfsBranches` — D-13 `sshfs_cache_args` 远端 mount 参数 grep 实现的样板
- `internal/claudedock/doctor/mount.go::checkFUSEResidual` — D-13 `oversized_files_count` 用 Details map 列具体路径的样板
- `internal/claudedock/errcodes/explanations.go::init()` MOUNT_MUTAGEN_* 系列 — D-18 五段模板（触发场景 / 根本原因 / 复现方式 / 修复路径 / 关联文档）的格式参考
- `internal/claudedock/errcodes/codes_test.go` — D-21 注册表闭包测试（无需扩展，自动覆盖新 code）
- `internal/claudedock/last_session_test.go` — D-09 序列化测试扩展点
- `internal/claudedock/ignore.go::DefaultBinaryIgnorePatterns` + `IgnoreMatcher` — D-06 第一层过滤的语义边界
- `cmd/claudedock/explain_test.go::TestExplainUnknownCode_Exit4` — D-21 子进程 explain 测试扩展样板
...
- `internal/claudedock/errcodes/codes.go` 注册机制（`MustRegister` + `Format` + `Lookup`）— 直接增 2 条；包级 `Registry()` / `ExtendedExplanations` 自动闭包到 `claudedock explain` / `doctor` / `claudedock doctor --json`
- `internal/claudedock/last_session.go::LastSessionSnapshot` schema_version=1 + omitempty 字段追加策略（Phase 32 D-27 已示范）— 本阶段 OversizedFiles 字段沿用同模式，**不破坏 v3.0 已上线脚本**
- `internal/claudedock/doctor/mount.go::checkMergerfsBranches` 远端 mount 输出参数 grep 模式 — D-13 `sshfs_cache_args` check 几乎是它的镜像复制（替换 mergerfs → sshfs，wantList 替换为 4 个 cache 参数）
- `internal/claudedock/doctor/check.go::newPass / newWarn / newFail / newSkip` helper — 5 项新 check 直接调用，零模板代码
- `internal/claudedock/hot_sync.go::syncFileState{Size, ModTime}` — 已有 size 字段；D-06 size 过滤直接读 `info.Size()` 即可，无需新结构
- `internal/claudedock/ignore.go::IgnoreMatcher` — D-06 第一层过滤直接复用，确保「ignore 命中的 50MB 视频不被记两遍」
- `cmd/claudedock/explain.go` 已实现 Registry + ExtendedExplanations 双层数据源 — D-19 新增 2 条 code 自动覆盖，**零改动 explain.go**
- `cmd/claudedock/explain_test.go::TestExplainUnknownCode_Exit4` — D-21 新增 2 个子进程测试的样板（exit 0 + stdout 字符数断言）
...
- **D-19 explain 子命令** 自动覆盖：`claudedock explain MOUNT_REQUIRE_GIT_REPO` 等价于 `Registry()[MOUNT_REQUIRE_GIT_REPO]` + `ExtendedExplanations[MOUNT_REQUIRE_GIT_REPO]` 渲染，零代码改动
...
- ROADMAP §Phase 36 SC#5「`claudedock explain MOUNT_REQUIRE_GIT_REPO` 子进程退出 0、长说明 ≥200 字」对应 D-19；测试必须断言**子进程 stdout 字符数 ≥ 200**（用 `utf8.RuneCountInString` 计中文字符），不是 byte 数
...
- **`claudedock init` 文案优化**：D-04 新增的 `hot_sync_max_file_mb` 字段在 init 时不出现在 prompt（仅手动编辑 yaml 可配）；交互式引导留 v3.4
