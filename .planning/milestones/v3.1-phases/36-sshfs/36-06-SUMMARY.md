...
requires:
  ...
  - phase: 36-sshfs
    provides: "runRoot 中 git 检测前置；cmd/claudedock/git_check.go 模板（Plan 36-04）"
  - phase: 36-sshfs
...
provides:
  ...
  - "gitRevParseTopLevel 私有 helper（doctor 包不能 import cmd/claudedock，复制 4 行 exec 实现）"
...
tech-stack:
  added: []
  patterns:
    - "doctor 本地 check：直接调 ctx + os.Getwd() / os.Getenv() / claudedock.LoadConfig() / claudedock.LoadLastSession()，不走 ensureRemote()"
...
- `internal/claudedock/doctor/mount.go` 末尾追加 5 个 checker 函数 + `gitRevParseTopLevel` 私有 helper（4 行 `exec.Command("git","rev-parse","--show-toplevel")` 复制实现，doctor 包不能 import `cmd/claudedock` 主包）。
- `internal/claudedock/doctor/doctor.go` mount domain block 末尾追加 5 行 `append`：4 项本地 check 直接调用，`checkSSHFSCacheArgs` 唯一一项远端 check 走 `runWithTimeout(... checkSSHFSCacheArgs(c, remoteRunner))` 与既有 `checkMergerfsBranches` / `checkSSHFSMountpoint` 同模式。
- `internal/claudedock/doctor/mount_test.go` 末尾追加 13 条矩阵测试 + `scriptedRunner` 第二款 mock（按 script name map 返回不同 stdout）。
- 全部 13 条新测试 + 既有 doctor 包测试全 PASS（`go test ./internal/claudedock/doctor/... -v` 0 FAIL）。
- `make ci-gate`（含 `go test ./... -count=1 -short` + `ci-doctor-grep.sh`）全部 PASS：`OK: claudedock doctor M14 gate passed (schema=1 / next_action / 错误码)`。
- `go build ./...` 干净。
...
- `internal/claudedock/doctor/mount.go` — 追加 imports（`claudedock` 包）+ `gitRevParseTopLevel` 私有 helper + 5 个 checker 函数（共 ≈115 行新增）。
- `internal/claudedock/doctor/doctor.go` — mount domain block 末尾插入 6 行（1 行注释 + 5 行 append）。
- `internal/claudedock/doctor/mount_test.go` — imports 扩充（`os` / `path/filepath` / `strings` / `errcodes`）+ `scriptedRunner` mock + 13 条新测试函数（共 ≈220 行新增）。
...
- **D-02（错误码复用而非新增）**：`checkGitProxyEnabled` / `checkDefaultIgnoreLoaded` 走 `AUTH_CONFIG_MISSING` 而非新建专用警告码，与 plan 36-06 contract 一致（D-13 表格指定）。NextAction 复用现有「运行 claudedock init 重新配置网关与凭证」对当前两类警告语义略有偏差，但用户视角「打开配置文件检查」是正确的恢复路径。
- **D-03（test 隔离方式）**：`checkGitProxyEnabled` / `checkOversizedFilesCount` 走真实 `claudedock.LoadConfig()` / `claudedock.LoadLastSession()` 路径，通过 `t.Setenv("HOME", t.TempDir())` 隔离配置文件位置，避免在生产代码引入 var 注入点。与 Plan 36-04 `git_check_test.go` 用 `t.Setenv("PATH","")` 模拟 git 不可用同一思路（源码零侵入 + 真实代码路径覆盖）。
...
- **Issue:** Plan `<action>` 字面量 `newFail("mount", "require_git_repo", errcodes.MOUNT_REQUIRE_GIT_REPO, fmt.Sprintf("当前目录 %s 不在 git 仓库内", cwd))` 把已渲染好的字符串作为 `args[0]` 传给 `newFail`；`newFail` 内部 `fmt.Sprintf(entry.Message, args...)` 又会用 `entry.Message="当前目录 %s 不在 git 仓库内，claudedock 拒绝挂载以避免误同步整个家目录"` 二次渲染，结果会出现「当前目录 当前目录 /tmp/x 不在 git 仓库内 不在 git 仓库内，claudedock...」类双重内嵌。
...
- **Files modified:** `internal/claudedock/doctor/mount.go`
...
- **Issue:** Plan 字面量 `newWarn("mount", "git_proxy_enabled", errcodes.AUTH_CONFIG_MISSING)` 不传 args；`AUTH_CONFIG_MISSING.Message="~/.claudedock/config.yaml 不存在或解析失败: %s"` 含 `%s` 占位符。`newWarn` 在 `len(args)==0` 时取 raw `entry.Message`，最终用户看到字面 `%s` 字串。
...
**3. [Rule 1 - Bug] mount.go 顶部 imports 缺 claudedock 包路径**
- **Found during:** Task 1（首次 `go build` 失败）
- **Issue:** Plan `<action>` Step 1 列出「import 检查」一栏说明需追加 `claudedock` 包，但未直接给出 import 字面量；mount.go 原 imports 仅含 `os/exec / regexp / runtime / strings / fmt / os / context / errcodes`。
- **Fix:** 在 import 段追加 `"github.com/claudedock/claudedock/internal/claudedock"`，与 doctor.go 顶部 import 顺序一致；doctor 与 claudedock 是单向依赖（claudedock 不依赖 doctor），无循环 import 风险。
- **Files modified:** `internal/claudedock/doctor/mount.go`
...
- **Files modified:** `internal/claudedock/doctor/mount_test.go`
- **Verification:** `go test ./internal/claudedock/doctor/... -v` 全 PASS。
...
- `checkOversizedFilesCount` 真实调 `claudedock.LoadLastSession()`，pass/warn/skip 三路径全覆盖。
- `checkSSHFSCacheArgs` 真实走 `runner.RunScript("sshfs_mount", ...)` + 4 参数 Contains 检查，pass/fail/skip 三路径全覆盖。
- `checkGitProxyEnabled` 真实调 `claudedock.LoadConfig().EffectiveProxyCommands()`，pass/warn/skip 三路径全覆盖。
...
- **REQ-MOUNT-V31-05** 已闭合：mount 维度新 check 总数验收路径就位（CI ci-doctor-grep PASS，运行时 `claudedock doctor mount --json | jq '.checks | length'` 在远端 conn 可用时应得 9；当前本机无远端时 `mergerfs_branches` / `sshfs_mountpoint` / `sshfs_cache_args` 三项 Skip，仍为 9 个 entry，长度断言通过）。
...
- **文件存在校验：**
  - ✅ `internal/claudedock/doctor/mount.go` FOUND（已修改，新增 ≈115 行）
  - ✅ `internal/claudedock/doctor/doctor.go` FOUND（已修改，mount block +6 行）
  - ✅ `internal/claudedock/doctor/mount_test.go` FOUND（已修改，新增 ≈220 行 / 13 条测试）
...
- **acceptance criteria 重跑：**
  - ✅ `grep -c "checkRequireGitRepo\|checkOversizedFilesCount\|checkSSHFSCacheArgs\|checkGitProxyEnabled\|checkDefaultIgnoreLoaded" internal/claudedock/doctor/mount.go` = 10（5 函数定义 + 内部相互引用 = ≥5 ✓）
  - ✅ `grep "gitRevParseTopLevel" internal/claudedock/doctor/mount.go` 命中（私有 helper）
  - ✅ `grep -c "Phase 36" internal/claudedock/doctor/doctor.go` ≥ 1（注释行）
  - ✅ `grep "checkRequireGitRepo\|checkSSHFSCacheArgs\|checkDefaultIgnoreLoaded" internal/claudedock/doctor/doctor.go` 全部命中
  - ✅ `grep -c "TestCheckSSHFSCacheArgs\|TestCheckOversizedFilesCount\|TestCheckRequireGitRepo\|TestCheckGitProxyEnabled\|TestCheckDefaultIgnoreLoaded" internal/claudedock/doctor/mount_test.go` = 13（≥12 ✓）
  - ✅ `go build ./...` PASS
  - ✅ `go test ./internal/claudedock/doctor/... -v` 全 PASS（13 新 + 既有，0 FAIL）
  - ✅ `go test ./... -count=1` 全 PASS（无回归）
  - ✅ `make ci-gate` PASS（`OK: claudedock doctor M14 gate passed (schema=1 / next_action / 错误码)`）
