...
|---|---|---|---|---|
| `internal/claudedock/config.go` | config | accessor-default | `config.go::EffectiveProxyCommands` | exact |
| `internal/claudedock/mount_sshfs.go` | param-literal | string-literal | `mount_sshfs.go:48-51 sshfsCmd` | exact line |
| `internal/claudedock/hot_sync.go` | service | CRUD+batch | `hot_sync.go` `HotSyncEngine`/`initialSync`/`scanLocalSyncFiles` | exact |
| `internal/claudedock/last_session.go` | model | JSON-serialize | `last_session.go` Phase 32 D-27 omitempty 追加模式 | exact |
| `internal/claudedock/mount_strategy.go` | orchestrator | request-response | `mount_strategy.go:222-230 snapshot.ConflictCount` 赋值 + `writeLastSessionWarn` | exact |
| `internal/claudedock/doctor/mount.go` | checker | request-response | `checkMergerfsBranches`（remote grep） + `checkFUSEResidual`（Details map） | exact |
| `internal/claudedock/errcodes/codes.go` | registry | none | `MOUNT_APFS_CASE_INSENSITIVE` const 末尾插入位置 | exact |
| `internal/claudedock/errcodes/mount.go` | registry | none | `MustRegister(Entry{...})` init() 模式 | exact |
| `internal/claudedock/errcodes/explanations.go` | docs | none | `registerExplanation(MOUNT_MUTAGEN_*)` 五段模板 | exact |
| `cmd/claudedock/main.go` | CLI entry | request-response | `runRoot` L287-295 LoadConfig 后、L362-367 keepalive 校验后 exit 模式 | exact |
| `cmd/claudedock/git_check.go` (**new**) | utility | request-response | `main.go` exec.Command + `errcodes.Format` 用法 | role-match |
| `cmd/claudedock/git_check_test.go` (**new**) | test | unit | `explain_test.go` 真实二进制构建模式 / 临时目录真实测试 | role-match |
| `internal/claudedock/hot_sync_oversized_test.go` (**new**) | test | batch | `hot_sync.go` `scanLocalSyncFiles` + `IgnoreMatcher` 用法 | role-match |
| `internal/claudedock/mount_sshfs_test.go` (**new**, F3) | test | request-response | `explain_test.go::buildOnceExplainBin` t.Skip 模式 + pkg/sftp Handlers | role-match |
| `internal/claudedock/last_session_test.go` (**extended**) | test | JSON | 现有序列化断言模式 | exact |
| `internal/claudedock/doctor/mount_test.go` (**extended**) | test | request-response | 现有 mock RemoteRunner 矩阵 | exact |
| `cmd/claudedock/explain_test.go` (**extended**) | test | subprocess | `buildOnceExplainBin + runExplainBin` 模式 | exact |
| `internal/claudedock/doctor/doctor.go` (**extended**) | orchestrator | none | `report.Checks = append(...)` mount domain block L226-234 | exact |

---

## Pattern Assignments

### `internal/claudedock/config.go` (config, accessor-default)

**Analog:** `internal/claudedock/config.go::EffectiveProxyCommands`
...
### `internal/claudedock/mount_sshfs.go` (param-literal, string-literal)
...
### `internal/claudedock/hot_sync.go` (service, CRUD+batch)
...
### `internal/claudedock/last_session.go` (model, JSON-serialize)
...
### `internal/claudedock/mount_strategy.go` (orchestrator, request-response)
...
if hotStatus.ConflictCount > 0 {
    fmt.Fprintf(cfg.Logger, "⚠ 有 %d 个文件同步冲突，运行 claudedock sync conflicts 查看\n", hotStatus.ConflictCount)
}
...
### `internal/claudedock/doctor/mount.go` (checker, request-response)
...
func checkOversizedFilesCount(ctx context.Context) Check {
    snap, err := claudedock.LoadLastSession()
    if err != nil || snap == nil {
...
// doctor 包私有 helper（不能 import cmd/claudedock main package）：
...
### `internal/claudedock/errcodes/codes.go` (registry, none)
...
### `internal/claudedock/errcodes/mount.go` (registry, none)
...
### `internal/claudedock/errcodes/explanations.go` (docs, none)
...
### `cmd/claudedock/main.go` (CLI entry, request-response)
...
### `cmd/claudedock/git_check.go` (**new**, utility, request-response)
...
### `cmd/claudedock/git_check_test.go` (**new**, test, unit)
...
func TestRequireGitRepo_InGitRepo(t *testing.T) {
    // 使用当前 workspace（claudedock 本身是 git 仓库）
    cwd, _ := os.Getwd()
    if err := requireGitRepo(cwd); err != nil {
...
### `internal/claudedock/hot_sync_oversized_test.go` (**new**, test, batch)
...
### `internal/claudedock/mount_sshfs_test.go` (**new**, test, request-response)
...
### `internal/claudedock/last_session_test.go` (**extended**, test, JSON)
...
### `internal/claudedock/doctor/mount_test.go` (**extended**, test, request-response)
...
### `cmd/claudedock/explain_test.go` (**extended**, test, subprocess)
...
### 错误码格式化输出
**Source:** `internal/claudedock/errcodes/codes.go::Format`
...
**Source:** `cmd/claudedock/main.go` lines 282-285 / 362-367
...
**Source:** `internal/claudedock/last_session.go` Phase 32 D-27 注释块（lines 26-30）
...
**Analog search scope:** `internal/claudedock/`, `cmd/claudedock/`, `internal/claudedock/doctor/`, `internal/claudedock/errcodes/`
