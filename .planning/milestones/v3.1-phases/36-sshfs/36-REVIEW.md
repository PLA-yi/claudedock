...
files_reviewed_list:
  ...
  - internal/claudedock/hot_sync.go
  - internal/claudedock/hot_sync_oversized_test.go
  ...
  - cmd/claudedock/git_check.go
  - cmd/claudedock/git_check_test.go
...
**File：** `internal/claudedock/hot_sync.go:421-433`（`applyOversizedFilter`）+ `internal/claudedock/hot_sync.go:242-326`（`syncOnce`）+ `internal/claudedock/hot_sync.go:162-224`（`initialSync` 非 reset 分支）
...
**为何 Full 模式不受影响：** Full 走 `resetRemote=true` + 隐藏 hot staging（`/tmp/.claudedock-mounts/...`），`big.bin` 既未被上传到 hot 也不进入 `e.last`，状态机在 base / local / remote 三方都缺失，行为稳定。
...
**File：** `internal/claudedock/doctor/mount.go:236-257`
...
- `proxy_commands` 不含 git 时，doctor 输出："~/.claudedock/config.yaml 不存在或解析失败: proxy_commands 未包含 git"，并建议「运行 claudedock init」——而真实 config 完好、需要的是手工编辑 yaml 加 git。
- `CLOUD_CLAUDE_NO_DEFAULT_IGNORE=1` 时，输出："~/.claudedock/config.yaml 不存在或解析失败: CLOUD_CLAUDE_NO_DEFAULT_IGNORE=1，已禁用默认二进制黑名单"，同样语义错乱。
...
**Fix：** 在 `errcodes/mount.go` 注册两个新 Code（建议命名 `MOUNT_GIT_PROXY_DISABLED` / `MOUNT_DEFAULT_IGNORE_DISABLED`，Severity=Warn，Message 不带占位符或自定义占位符），让 doctor 的两条 check 各用专属 Code。或者退化方案：直接构造 `Check{Status: StatusWarn, Code: ..., Message: "...", NextAction: "..."}`，绕开 newWarn 的 sprintf 拼接，并把 NextAction 改成准确的「编辑 ~/.claudedock/config.yaml 的 proxy_commands 字段」。

### WR-02：`Config.HotSyncMaxFileMB` 用户配置目前完全失效（main.go 未注入 mountCfg）

**File：** `cmd/claudedock/main.go:352-357` + `internal/claudedock/ssh.go:54-110` + `internal/claudedock/config.go:38-45`
...
结果：用户在 `~/.claudedock/config.yaml` 写 `hot_sync_max_file_mb: 200` 不会生效；启动也不会有任何提示。这与 `MOUNT_OVERSIZED_FILE_SKIPPED` 长说明（`explanations.go:129`）告诉用户「编辑 ~/.claudedock/config.yaml 调高 hot_sync_max_file_mb」直接矛盾。
...
**Fix（最小改动）：**
```go
// cmd/claudedock/main.go runRoot 内构造 mountCfg 后：
...
**File：** `internal/claudedock/mount_strategy.go:257-264`
...
**File：** `internal/claudedock/mount_strategy.go:189-218`
...
**File：** `internal/claudedock/hot_sync.go:425-433` + `internal/claudedock/errcodes/mount.go:106-111`
...
### IN-02：`doctor.gitRevParseTopLevel` 与 `cmd/claudedock.requireGitRepo` 重复实现 4 行 exec 逻辑

**File：** `internal/claudedock/doctor/mount.go:152-158` + `cmd/claudedock/git_check.go:14-21`
...
**Fix：** 把 `requireGitRepo` 的核心 4 行（exec + 错误吞并）下沉到 `internal/claudedock/gitcheck`（新包，仅依赖 stdlib + errcodes），让 doctor 与 main 都 import；保留各自层的 stderr / Check 渲染。可放到 36-07 或 v3.1 backlog。

### IN-03：`convertSnapshotToBanner` 没有把 `OversizedFiles` 透传到 `DowngradeBanner`

**File：** `internal/claudedock/doctor/doctor.go:271-293`

**Issue：** doctor 第一屏 banner 显示 conflict_count / reconnect_count / tmux_session 等，但忽略了同样属于「上次会话状态」的 oversized_files 计数。当前用户拿到的信号只在 mount 维度的 `checkOversizedFilesCount`（带 `--domain mount` 才必现）；如果用户跑 `claudedock doctor --domain network` 看不到这条线索，会与「为什么我的大文件没同步」的工单脱钩。
...
**File：** `internal/claudedock/mount_strategy.go:529-535`
