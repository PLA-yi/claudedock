...
- `MountWorkspace` 成功分支：`snapshot.OversizedFiles = hotStatus.OversizedFiles`（D-09），并在 `printBanner` 之前打印 D-08 一次性 stderr 提示「`[!] 跳过大文件 N 个（>NMB），由 cold 兜底:`」+ 前 5 条文件 + 「`... 还有 N 个，见 ~/.claudedock/last-session.json`」。
...
- `internal/claudedock/hot_sync.go` — 三 struct 扩展 + StartHotSync 初始化 `engine.maxFileBytes` / 返回 `HotSyncStatus{OversizedFiles: engine.oversized}` + `initialSync` / `syncOnce` 调用 `applyOversizedFilter` + 新增 `applyOversizedFilter` 私有方法。
- `internal/claudedock/mount_strategy.go` — `MountConfig.HotSyncMaxFileMB` 字段 + `mountDefaultHotSyncMaxFileMB=50` 常量 + `effectiveHotSyncMaxFileMB()` accessor + `tryModeReal` 双路径注入 `MaxFileBytes` + `MountWorkspace` 成功分支写 `snapshot.OversizedFiles` + D-08 stderr 一次性提示块。
- `internal/claudedock/hot_sync_oversized_test.go` — 新建。`createFixtureFile` Truncate fixture helper + `applyTestOversizedFilter` 内联契约 helper + 三场景测试 `TestHotSyncOversized_60MB_NotIgnored` / `_IgnoreHit_NotCounted` / `_30MB_NotOversized`。
...
- **Files modified:** `internal/claudedock/hot_sync.go`
...
- **Files modified:** `internal/claudedock/mount_strategy.go`
- **Verification:** `grep "effectiveHotSyncMaxFileMB"` 命中 5 次（accessor 定义 + tryModeReal 注入 + D-08 stderr）；`go test ./internal/claudedock/...` 全包 PASS（mount_strategy_test.go 4 条 SyncLock 既有用例零回归，零值 cfg 走 accessor 默认 50）。
...
None — 无需外部配置。`HotSyncMaxFileMB` 默认 50MB，用户无需感知；如需调整，编辑 `~/.claudedock/config.yaml::hot_sync_max_file_mb`（Plan 36-02 已落地的字段）。

## Next Phase Readiness

- **Plan 36-04 / 36-06 doctor 集成**：`mount/oversized_files_count` check 可直接 `claudedock.LoadLastSession()` 读 `snap.OversizedFiles`（本 plan 已写入）。
...
- `OversizedFile.Path` 来自 `scanLocalSyncFiles` 的 `filepath.ToSlash(rel)`（已是 cwd 相对路径），写入 `~/.claudedock/last-session.json`（本地 0600）与 stderr，对应 plan threat register T-36-03-01 mitigate。
...
- 文件存在：
  - `internal/claudedock/hot_sync.go` FOUND
  - `internal/claudedock/mount_strategy.go` FOUND
  - `internal/claudedock/hot_sync_oversized_test.go` FOUND
