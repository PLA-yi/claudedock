...
- **D-01**：git 检测点在 `cmd/claudedock/main.go::runRoot`，位置在 `os.Getwd()` 之后、`AuthenticateAndWait` 之前；使用 `exec.Command("git", "rev-parse", "--show-toplevel").Run()`；退出码恒为 `exitConfigError`(=4)
- **D-02**：封装为 `cmd/claudedock/git_check.go`，导出 `requireGitRepo(cwd string) error`
...
- **D-13**：5 项新 check 全加到 `internal/claudedock/doctor/mount.go` 同文件
...
- **D-19**：`cmd/claudedock/explain.go` **不改动**
- **D-21**：`make ci-gate` 自动覆盖新增 code + 说明
- **D-22**：真机 e2e 签字留 Phase 37

### Claude's Discretion

- `OversizedFile` struct 的包位置（`claudedock` 包 or `last_session.go`，executor 选最自然位置）
...
|------------|-------------|----------------|-----------|
| F1 git 仓库前置校验 | CLI 本地（cmd/claudedock/） | — | 必须在网络操作前在本机执行 |
...
```
cmd/claudedock/main.go::runRoot

line 281: mode, err := claudedock.ParseMode(mountMode)
           ↓
line 287: cfg, err := claudedock.LoadConfig()        ← D-01 建议将 os.Getwd() 移到这里之后
           ↓
line 302: authResp, err := client.AuthenticateAndWait(...)  ← ⚠ 当前 Getwd 在此 AFTER
           ↓
line 332: cwd, err := os.Getwd()    ← 🔴 必须前移到 line 302 之前
           ↓
line 369: exitCode, err := claudedock.ConnectAndRunClaudeV3(sshCfg, args, cwd, ...)
```
...
if n := len(hStatus.OversizedFiles); n > 0 {
    ...
    if n > 5 {
        fmt.Fprintf(cfg.Logger, "  ... 还有 %d 个，见 ~/.claudedock/last-session.json\n", n-5)
    }
}
```
...
**`mountSSHFS` 函数位置**：`internal/claudedock/mount_sshfs.go:19`，被 `tryModeReal`（mount_strategy.go:~290）和 `mountWorkspace`（mount_sshfs.go:~90）调用。
...
**新文件**：`cmd/claudedock/git_check.go`（`package main`）
...
| 文件 | 改动 |
|------|------|
| `internal/claudedock/config.go` | `Config` 新增 `HotSyncMaxFileMB int` + `EffectiveHotSyncMaxFileMB()` accessor |
| `internal/claudedock/hot_sync.go` | `HotSyncConfig.MaxFileBytes int64`、`HotSyncEngine.maxFileBytes/oversized`、`HotSyncStatus.OversizedFiles`、`initialSync()` 后处理 |
| `internal/claudedock/last_session.go` | `LastSessionSnapshot.OversizedFiles []OversizedFile`、包级 `OversizedFile` struct |
| `internal/claudedock/mount_strategy.go` | `MountConfig` 注入 `MaxFileBytes`、`snapshot.OversizedFiles` 赋值、D-08 stderr 输出 |

**`OversizedFile` struct 放置**：建议放在 `last_session.go`（与 `LastSessionSnapshot` 同文件，减少跨包依赖）。`hot_sync.go` 直接用 `claudedock.OversizedFile`（同包）。
...
**D-11 fixture SFTP 测试**（`internal/claudedock/mount_sshfs_test.go` 新增）：
...
**`internal/claudedock/doctor/mount.go` 追加 5 个函数**：
...
| `checkOversizedFilesCount(ctx)` | 本地 | `claudedock.LoadLastSession()` → `len(snap.OversizedFiles)` | Pass(0) / Warn(>0) / Skip(nil) MOUNT_OVERSIZED_FILE_SKIPPED |
...
| `checkGitProxyEnabled(ctx)` | 本地 | `claudedock.LoadConfig()` → `cfg.EffectiveProxyCommands()` → contains("git") | Pass / Warn AUTH_CONFIG_MISSING |
| `checkDefaultIgnoreLoaded(ctx)` | 本地 | `os.Getenv("CLOUD_CLAUDE_NO_DEFAULT_IGNORE") == "1"` | Pass(未设) / Warn(已设) AUTH_CONFIG_MISSING |

**`checkRequireGitRepo` 不能 import cmd/ 包**（cmd/claudedock 是 main package）。在 `mount.go` 内写一个 12 行私有 helper：
```go
...
| 测试文件 | 覆盖场景 | CI 自动性 |
|---------|---------|---------|
| `cmd/claudedock/git_check_test.go`（新建） | git 仓库→pass；非 git→fail+错误码；无 git 命令→fail | `go test ./cmd/claudedock/...` |
| `internal/claudedock/hot_sync_oversized_test.go`（新建） | ignore 命中的大文件→不进 OversizedFiles；未 ignore 的 60MB→进；30MB→不进 | `go test ./internal/claudedock/...` |
| `internal/claudedock/last_session_test.go`（扩展） | 含 OversizedFiles 的 snapshot 序列化/反序列化；omitempty 空数组不出现 | `go test ./internal/claudedock/...` |
| `internal/claudedock/mount_sshfs_test.go`（新增函数） | D-11 fixture SFTP counting（sshfs 未安装 → Skip） | `go test -short` skip，CI 上 sshfs 可用时运行 |
| `internal/claudedock/doctor/mount_test.go`（扩展） | 5 项新 check 各 pass/warn/fail/skip 矩阵（mock runner + mock exec） | `go test ./internal/claudedock/doctor/...` |
| `internal/claudedock/errcodes/codes_test.go`（自动） | TestErrcodesRegistry 遍历覆盖 2 条新 code | 无需改动 |
| `internal/claudedock/errcodes/explanations_test.go`（自动） | TestAllCodesHaveExplanations 遍历覆盖 2 条新说明 | 无需改动 |
| `cmd/claudedock/explain_test.go`（扩展） | 子进程 exit 0 + stdout ≥200 字 — 2 条新 code | `go test ./cmd/claudedock/...` |
...
**规避**：使用 `os.Getwd()`——doctor 命令约定在工程目录下运行，与 claudedock main 语义一致。若 cwd 不是 git repo，doctor 输出 Fail 提示用户 cd 到 git 仓库再检查，符合预期。
...
**问题**：用户未 init 时 `claudedock.LoadConfig()` 返回错误，check 无法继续。
...
2. **`mount_sshfs_test.go` 中 fixture SFTP server 的 SSH 握手方式**：需要一个最小 SSH server 包装 sftp RequestServer。可参考 `golang.org/x/crypto/ssh` 的 `NewServerConn` API；testdata 证书用 `ssh.GenerateKey` 临时生成（与 `internal/claudedock/integration_test.go` 同模式）。
...
| 文件 | 必读行范围 | 目的 |
|------|----------|------|
| `cmd/claudedock/main.go` | 280–375（runRoot 全体） | F1 改动点：Getwd 前移位置、exit 常量定义 |
| `internal/claudedock/hot_sync.go` | 88–130（StartHotSync）、145–187（initialSync）、388–428（scanLocalSyncFiles） | F2 注入点 |
| `internal/claudedock/hot_sync.go` | 195–280（syncOnce）| L3 遗漏点——syncOnce 也需要过滤 |
| `internal/claudedock/last_session.go` | 全文（125 行） | F2 OversizedFile struct 落点 |
| `internal/claudedock/mount_strategy.go` | 280–320（tryModeReal，Full 路径） | F2 OversizedFiles 注入 + D-08 stderr 输出 |
| `internal/claudedock/mount_sshfs.go` | 19–50（mountSSHFS + sshfsCmd） | F3 字面量修改位置 |
| `internal/claudedock/config.go` | 全文（103 行） | F2 Config.HotSyncMaxFileMB + accessor 落点 |
| `internal/claudedock/ignore.go` | 100–115（LoadMountIgnorePatterns） | 确认 CLOUD_CLAUDE_NO_DEFAULT_IGNORE 行为（F4 doctor check） |
| `internal/claudedock/doctor/mount.go` | 全文（约 140 行） | F4 新增 5 check 的精确模板 |
| `internal/claudedock/doctor/doctor.go` | 226–234（mount domain block） | F4 新增 5 check 注册位置 |
| `internal/claudedock/doctor/check.go` | 全文（约 80 行） | F4 newPass/newWarn/newFail/newSkip/runWithTimeout API |
| `internal/claudedock/errcodes/codes.go` | 全文（最后 20 行 const 块） | F5 新增 2 const 插入位置 |
| `internal/claudedock/errcodes/mount.go` | 全文（约 70 行） | F5 MustRegister 模板 |
| `internal/claudedock/errcodes/explanations.go` | init() 函数（约 40–600 行） | F5 registerExplanation 调用模板（五段格式） |
| `cmd/claudedock/explain_test.go` | 全文（约 100 行） | F6 buildOnceExplainBin + runExplainBin 复用 |
| `internal/claudedock/errcodes/codes_test.go` | 全文（约 80 行） | F6 确认无需改动 + 命名限制（NextAction ≤80 rune） |
...
- `internal/claudedock/hot_sync.go`（583 行全文阅读）
- `internal/claudedock/mount_sshfs.go`（95 行全文阅读）
- `internal/claudedock/mount_strategy.go`（400 行全文阅读）
- `internal/claudedock/last_session.go`（125 行全文阅读）
- `internal/claudedock/config.go`（103 行全文阅读）
- `internal/claudedock/doctor/mount.go`、`check.go`、`doctor.go` 相关段落
- `internal/claudedock/errcodes/codes.go`、`mount.go`、`explanations.go`、`codes_test.go`、`explanations_test.go`
- `cmd/claudedock/main.go`（runRoot 全文）
- `cmd/claudedock/explain.go`、`explain_test.go`
- `internal/claudedock/ignore.go`（LoadMountIgnorePatterns）
