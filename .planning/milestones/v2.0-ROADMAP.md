...
- 🚧 **v2.0 claudedock 透明远程 CLI** — Phases 24-28 (in progress)
...
### 🚧 v2.0 claudedock 透明远程 CLI

**Milestone Goal:** 交付一个可替代原生 `claude` 命令的 Go 二进制文件 `claudedock`，用户 `alias claude=claudedock` 后输入 `claude` 的体验与本地完全一致——实际运行在远端配好代理出口的 Docker 容器里，本地目录通过 sshfs slave 实时映射到容器内。

- [x] **Phase 24: 受管镜像 FUSE 硬化与容器参数** — 镜像预装 sshfs/fuse3，Worker 附加 FUSE 设备权限，SSH Proxy 零改造验证 (completed 2026-04-14)
- [x] **Phase 25: claudedock CLI 骨架与连接** — Go 二进制 claudedock 的配置、认证和远端容器连接闭环（1 plan） (completed 2026-04-15)
...
**Success Criteria** (what must be TRUE):
  ...
  3. SSH Proxy 现有多 session channel 和 exec 转发能力无需代码改动即可支持 claudedock 的连接模式
**Plans**: 1 plan
Plans:
- [x] 24-01-PLAN.md — 镜像 FUSE 预装、容器参数附加与 SSH Proxy 零改造验证

### Phase 25: claudedock CLI 骨架与连接
**Goal**: 用户可以运行 claudedock 命令完成配置、认证和远端容器连接
**Depends on**: Phase 24
**Requirements**: CLI-01, CLI-02, CLI-04, CLI-05
**Success Criteria** (what must be TRUE):
  1. 用户运行 `claudedock`（无参数）后，CLI 自动连接网关、认证、等待容器就绪，并进入远端 Claude Code 会话
  2. 用户运行 `claudedock init` 后，网关地址和凭证持久化到 `~/.claudedock/config.yaml`，后续运行自动读取
  3. 网关不可达、认证失败、容器未就绪时，CLI 输出清晰的中文错误提示并返回合适的退出码
  4. 用户可以在 config.yaml 中配置自有网关地址，CLI 连接到该地址而非默认地址
**Plans**: 1 plan
Plans:
- [x] 25-01-PLAN.md — claudedock 配置/init、Entry 认证与 SSH+PTY 闭环

### Phase 26: 参数透传与终端体验
**Goal**: claudedock 的参数透传和终端交互与本地 claude 完全一致
**Depends on**: Phase 25
...
**Success Criteria** (what must be TRUE):
  ...
  4. 容器内 Claude Code 退出码透传给本地 claudedock 进程，脚本可基于退出码判断结果
**Plans**: 1 plan
Plans:
...
**Success Criteria** (what must be TRUE):
  1. 用户运行 claudedock 时，CLI 自动在第二个 SSH session 上通过 sshfs slave 将当前目录映射到容器 /workspace
...
**Success Criteria** (what must be TRUE):
  ...
  3. 完整流程（claudedock → SSH Proxy → 目录映射 → Claude Code 运行）在生产环境端到端通过
**Plans**: 2 plans
Plans:
...
| 25. claudedock CLI 骨架与连接 | 1/1 | Complete    | 2026-04-15 |
