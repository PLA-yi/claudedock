...
**Domain:** 透明替代 `claude` 的远程 CLI（claudedock / v2.0 里程碑）  
...
| **可重复的配置与凭据** | 网关地址、租户/用户身份、认证方式需可保存、可迁移、可自动化 | MEDIUM | `init` + 配置文件路径约定（如 `~/.claudedock/`）属于行业惯例 |
...
| **与既有接入方式共存（SSH / 管理后台）** | 用户可能混用「纯 SSH」与「claudedock」；不应要求推翻现有心智 | MEDIUM | 文档与行为上明确：claudedock 是叠加路径，而非唯一入口 |
...
|---------|-------------------|------------|-------|
| **透明命令替换（`alias claude=claudedock`）** | 零迁移成本，脚本与肌肉记忆不改 | MEDIUM | 比「先 ssh 再手动执行」少一步认知负担 |
...
|---------|---------------|-----------------|-------------|
| **默认双向实时同步整个 `$HOME` 或超大 monorepo** | 「像网盘一样什么都同步」 | 延迟、冲突、CPU/IO、排除规则爆炸；难以解释「为什么慢」 | 默认仅映射当前 repo；文档化 `.claudedockignore`；大仓用显式子目录或阶段性同步 |
...
- [ ] **单一二进制 `claudedock` + `init` 配置** — 可安装、可指向私有网关；凭据落盘路径清晰。
...
| Feature | VS Code Remote SSH | GitHub Codespaces CLI | Gitpod `gp` | Our Approach（claudedock） |
|---------|-------------------|------------------------|-------------|------------------------------|
...
---
*Feature research for: claudedock 透明远程 CLI（v2.0）*  
*Researched: 2026-04-15*
