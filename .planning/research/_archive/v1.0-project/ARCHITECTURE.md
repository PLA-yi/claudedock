...
**Domain:** claudedock 透明远程 CLI（客户端二进制 + 本地目录 ↔ 远端容器实时映射）  
...
在 **v1.x 已交付** 的分层上，v2.0 仅增加 **用户侧 `claudedock` 二进制** 与 **目录同步/挂载策略**；控制面、host-agent、Docker 创建、网络 Provider、React 后台 **保持不变**，除非单独列出「可选修改」。
...
|-----------|------------------|-------------------------|
| **claudedock** | 读 `~/.claudedock/config`；建立到网关 `SSH Proxy` 的连接；编排「挂载 → exec claude」；TTY/信号/退出码 | Go，`golang.org/x/crypto/ssh`，可参考 `tools/cloud-dev/main.go` |
...
```
cmd/claudedock/          # 正式交付的单一二进制入口（待建或从 tools 提升）
...
- **`claudedock` 放在 `cmd/`：** 与「可安装 CLI」定位一致，便于版本发布与 `go install`。  
...
**What:** 用户只打开到 `SSH Proxy` 的 SSH 连接；`claudedock` 在其上创建多个 `session`——其一跑容器内 `sshfs -o slave`，本地对接 SFTP；其二请求 `pty` + `exec`/`shell` 运行 `claude` 及子进程。  
...
**Trade-offs:** 组件多、需明确同步根与权限；不一定与「当前 shell 目录」天然一致，需在 `claudedock` 内做路径约定或一次性配置。
...
### 端到端：`claude`（实为 claudedock）→ 容器内 Claude Code

以下覆盖 **非交互一次执行** 与 **交互 TTY** 两种场景；差别仅在第二个 session 请求 `exec` 还是 `pty-req`+`shell`。

```
用户终端输入: claudedock [与原生 claude 相同参数]
    ↓
claudedock 解析参数、读取 ~/.claudedock/config（网关地址、short_id、凭证）
    ↓
...
│ claudedock: NewSession → Start("sshfs -o slave ... <mount>")            │
...
│ claudedock: NewSession → RequestPty（若交互）或 exec                    │
...
| 路径 | 是否经过 claudedock | 目录来源 |
|------|----------------------|----------|
| `curl` bootstrap 后 `ssh short_id@gateway` | 否 | 主要为宿主机持久化的 `/workspace` |
| `claudedock` | 是 | 用户机当前目录经 sshfs/Mutagen 映射到约定 mountpoint |
...
3. **出网流量：** 仍由现有 `network.Provider` + 容器内 tun/代理链保证，**不**因 claudedock 改变。
...
**结论：** 目录映射 **不属于** `internal/sshproxy` 的一层服务；应落在 **claudedock（客户端）** 与 **镜像/容器内工具（sshfs/fuse）** 的组合；Mutagen 若采用，属于 **同步侧车进程**，架构上平行于 SSH 会话转发。
...
| **RepoResolver / 认证** | **默认无需改** | short_id + 密码/公钥 已满足 `claudedock` 非交互登录。 |
...
- 为 `claudedock` 提供 **轻量健康检查**（例如公开 `GET /healthz` 或已有入口）仅用于预检网关可达性。  
...
| 集成点 | 方向 | 契约 |
|--------|------|------|
| **SSH Proxy :22** | claudedock → Proxy → 容器 | SSH 协议；用户名为 host short_id |
...
|----------|---------------|-------|
| claudedock ↔ SSH Proxy | TCP + SSH | 映射与 claude 执行均为普通 session |
| SSH Proxy ↔ 容器 | 每 session 一次 `ssh.Dial` | 见 Pattern 3 |
| claudedock ↔ 控制面 | 可选 HTTPS | 仅主机启动/状态，非映射数据面 |

## 新增 vs 修改组件

| 类型 | 组件 | 说明 |
|------|------|------|
| **新增** | `claudedock` 二进制（`cmd/claudedock` 或等价） | init、SSH 客户端、sshfs 编排、TTY/信号、参数透传 |
...
2. **claudedock MVP：** 仅 SSH + **单 session** `exec` 跑 `claude`（映射到容器已有 `/workspace` 用于冒烟）— 验证透传。  
...
**Do this instead:** 挂载全部由 `claudedock` 发普通 exec/session。
...
- 产品：`.planning/PROJECT.md`（v2.0 claudedock 目标与「SSH Proxy 零改造」）

---
*Architecture research for: claudedock 透明远程 CLI 与现有 SSH Proxy 集成*  
*Researched: 2026-04-15*
