...
| 弱网容忍 | **OpenSSH `ServerAliveInterval` + claudedock 自实现重连** | 复用现有 OpenSSH 10.2p1 | CLI 端实现，不新引入 mosh / autossh / ET |
...
| 传输方式（v3.0 采用） | OpenSSH 透传（`ssh://` URL，复用 claudedock 现有 SSH 通道与认证流） | https://mutagen.io/documentation/transports/ssh/ |
...
2. **OpenSSH 透传**：传输层直接调用本机 `ssh`/`scp`，不嵌入第三方 SSH 库，自动复用 claudedock 已经握手好的 SSH 配置和密钥。这条对项目意义重大：v2.0 的 SSH proxy + Entry API 认证流不需要任何改造。
...
1. **客户端**：claudedock 二进制内嵌一份 `mutagen` 客户端（要么 `go install` 然后 `go:embed`，要么用 `os/exec` 调用旁路安装的 mutagen——**推荐 embed 静态调用**，避免用户机器有无 mutagen 的差异）。
...
3. **mutagen daemon**：客户端侧 Mutagen 需要本地 daemon。claudedock 启动时检测并 `mutagen daemon start`，退出时不停（让长会话复用）。
4. **session 命名**：以 `claude-account-{id}` 命名，方便 doctor 检查。

### 配置（建议默认）

```yaml
# 客户端写入 ~/.mutagen.yml（claudedock 自动生成）
sync:
  defaults:
...
**白名单上限策略**（F1 的"≤50MB"要求）：claudedock 在创建 session 前先 `du -sb` 扫描候选目录，超过 50MB 的目录拒绝放进 Mutagen branch，自动改用 sshfs 兜底。Mutagen 自己没有"总量超过则拒绝"的开关，必须在 CLI 层守门。
...
claudedock 检测三层任一失败时，按顺序降级并告知用户：
...
| mergerfs 挂载失败 | mergerfs 进程异常 / mountpoint 检测失败 | 直接 bind mount mutagen branch（如可用）或 sshfs branch | "联合视图未就绪，已切到单层挂载，请稍后用 `claudedock doctor` 排查" |
...
- `slave`：保留 v2.0 的 slave 模式，继续用 claudedock 端的嵌入 SFTP server。
...
2. **socket-per-session** 简洁可控：每个 claude_account 容器内只跑一个 tmux server，socket 在 `/tmp/tmux-${UID}/default`，claudedock 用 `tmux ls / has-session / new-session -d -s claude / attach -t claude` 即可完成所有控制。
...
- **TTY/信号/exit code 透传**：tmux 自己处理 SIGWINCH 和 PTY，对 claudedock 的 v2.0 透传逻辑透明。
...
3. **claudedock `--new-session`**：远端命令改为 `tmux new-session -d -s claude-$(date +%s) && tmux attach -t claude-$(...)` 强制建独立 session。
4. **冲突提示**：claudedock 在 attach 前先 `tmux list-clients -t claude`，若已有 client 且未传 `--new-session`，给出中文提示并继续 attach（共享）；如果用户已经传了 `--new-session`，则建新名字 session 并提示当前总共有几个独立 session。
...
| 决定项 | 取值 |
|--------|------|
| 客户端 SSH 库 | 复用 v2.0 的 `golang.org/x/crypto/ssh`（claudedock 内置） |
...
| 重连 | claudedock 自实现 reconnect with exponential backoff（1s → 2s → 4s → 8s → 30s 上限），重连复用本地缓存的 Entry API token，**不重新弹密码** |
...
3. **mosh 不能传输任意二进制 / scrollback**：与 claudedock "TTY/信号/退出码完全透传"承诺有冲突（mosh 自己有屏幕预测和重绘协议，不是 raw bytes）。
...
- autossh 只解决"ssh 进程死了自动重启"的问题，但 claudedock 自己就是 wrapper（处理认证、挂载、tmux、错误码）。再套 autossh 等于嵌套两层 wrapper，错误码、信号、终端恢复全部要重新打洞。
- autossh 最近一次 GitHub release 是 1.4f（2021-04-02），主线开发停滞，仅靠发行版打 1.4g 补丁。

**正确做法**：把 autossh 干的事（"ssh 退出 → 等待 → 重连"）直接写在 claudedock 里，配合现有 Entry API token 缓存做无密码重连。
...
不引入新组件 = 零兼容性风险。仅扩展 claudedock 自身代码。
...
v2.0 已经定义了 7 个稳定的退出码（CLOUDCLAUDE_E_*），bootstrap 脚本和 claudedock 共用。v3.0 的新错误路径**不要新发明编码体系**，只在分类前缀上扩展：
...
2. 中文描述（短）
3. "下一步该做什么"（短，可以引用 `claudedock doctor [子检查]`）
...
不引入新组件。`claudedock doctor` 在 v2.0 的 cobra 树下扩展子命令：
...
|------------|---------------------|------------------|
| Go | 1.26.1 | claudedock 与 control-plane 同栈 |
...
|-------|-----|-------------|
| mosh | UDP 60000-61000 与 sing-box tun + nftables 默认拒绝模型不兼容；mosh 1.4.0 自 2022 起无新 release | OpenSSH ServerAliveInterval + claudedock 自实现 reconnect |
| autossh | 仅做"ssh 死了重启"，claudedock 已是 wrapper，套娃增加错误码复杂度；上次 release 2021 | claudedock 内置重连逻辑 |
...
# Mutagen agent bundle（不放客户端，客户端在 claudedock 二进制里 embed）
...
COPY rootfs/usr/local/bin/claudedock-mount-stack.sh /usr/local/bin/
COPY rootfs/etc/profile.d/claudedock-tmux.sh /etc/profile.d/
```
...
*Stack research for: claudedock v3.0 远端开发体验升级（F1–F8）*
*Researched: 2026-04-18*
