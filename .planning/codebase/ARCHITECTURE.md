...
ClaudeDock 是一个面向单宿主机的容器化 SSH 云主机平台。用户从一个很短的 `curl` 入口开始，在终端里输入用户名和密码，等待专属 Docker 容器启动完成后，直接进入该容器内的 SSH 会话。所有网络流量都必须通过指定出口 IP 的全局隧道路由发送。

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│                        claudedock CLI (终端用户)                           │
│                    `cmd/claudedock/` — Go + Cobra                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                              Entry Short-Link                                │
...
│              Unix socket 通信 (`/run/claudedock/host-agent.sock`)      │
├─────────────────────────────────────────────────────────────────────────────┤
...
| claudedock CLI | 终端用户入口，认证、挂载、SSH 会话 | `cmd/claudedock/main.go` |
...
- **Used by:** Admin Dashboard, claudedock CLI, curl bootstrap
...
7. claudedock CLI 建立 SSH 连接并启动 claude code
   - `cmd/claudedock/main.go` → `ConnectAndRunClaudeV3()`
...
**claudedock CLI:**
- Location: `cmd/claudedock/main.go`
- Triggers: 终端用户直接执行
...
- 业务层: 使用 `internal/claudedock/errcodes` 注册表定义结构化错误码
