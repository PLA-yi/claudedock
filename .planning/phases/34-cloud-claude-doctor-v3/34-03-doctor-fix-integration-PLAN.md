...
// isFusermountIdempotent — RESEARCH §4.2：exit 1 + stderr "not mounted" 视为 OK。
func isFusermountIdempotent(err error) bool {
	msg := strings.ToLower(err.Error())
	return strings.Contains(msg, "not mounted") ||
		strings.Contains(msg, "no such file") ||
		strings.Contains(msg, "device or resource busy") == false && strings.Contains(msg, "not found")
}
...
<task type="execute">
  <name>Task 3.3: 修改 network/mount.go — 填充 Details["host_port"] / ["mountpoints"] 等 Fixer 依赖字段</name>
...
- 过滤第 1 段 `DIFFPATH` 降级 banner 输出（awk 已守护 section boundary）
  ...
  <acceptance_criteria>
    ...
    - `grep -qE '错误码:.*\[A-Z\]' scripts/ci-doctor-grep.sh` 命中（断言 3）
