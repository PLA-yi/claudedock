...
**Shipped:** v3.0 远端开发体验升级 (2026-04-23)

v3.0 把 v2.0 cloud-claude 从"能跑"升级到"能成为日常开发主战场"——三层文件系统（Mutagen 热同步 + sshfs 冷兜底 + mergerfs 联合视图）替换 sshfs 性能天花板，SSH 弱网容忍（KeepAlive 15s/4 + 退避重连 + BufferedStdin 灰色未确认本地 echo）+ tmux 默认包装实现 30s 抖动无感知 + 多端共享 attach + 账号级 Mutagen 单例锁，Claude Code OAuth 跨容器持久化（`claude-state-{account_id}` named volume + admin DELETE 联动 volume rm），`cloud-claude doctor` 5 维度 18 项 check + 6 类自动 fix + JSON schema_v1，错误码统一为 `<DOMAIN>_<KIND>_<NUM>` 4 段（42 条 8 域 Registry + 38 条长说明 + `cloud-claude explain <code>` 子命令）。

**v3.0 ship 总览:**
- 8 phases, 30 plans (含 1 P0 hotfix Phase 29.1 + 2 gap-closure plans in P32 + 3 post-execution patches in P33)
- 208 commits, 255 files (+72,879 / -335 lines, 含 Mutagen 4 平台 ~49MB go:embed)
- Codebase: Go 29,535 + TS+TSX 11,772 + Shell 4,459 = 45,766 LOC
- 2026-04-18 → 2026-04-23 (5 active execution days)
- Critical Pitfalls C1-C8 + M13/M14 全部覆盖；4 条核心 E2E flow WIRED（cross-phase integration check）
- 33/34 REQ satisfied，仅 BASE-02 真机签字 deferred-to-ship；3 项真机 UAT 跟踪在 35-HUMAN-UAT.md

## Current Milestone: v3.1 映射语义补齐与懒加载

**Goal:** 把"远端开发体验"从"代码热同步 + 二进制全量 sshfs 透传"升级为"代码强约束（git 仓库 / 可配大小熔断）+ 二进制懒加载（首次读触发晋升 hot）"，让 v3.0 暴露的 5 大映射 gap 全部可观测、可回归。

**Target features:**

主线 A · 映射前置约束与 FUSE 缓存（Phase 36）
- F1：非 git 仓库拒绝挂载（明确错误码 + 中文 next_action + 退出码）
- F2：单文件大小熔断（默认 50MB，用户可配 `hot_sync_max_file_mb`），超阈值不进热同步、走 cold 兜底，stderr 一次性提示
- F3：sshfs 命令默认开 FUSE page cache（`cache=yes,kernel_cache,auto_cache,cache_timeout=300`），同会话重复读 0 额外 RTT
- F4：`cloud-claude doctor mount` 扩展 5 项检查 + 2 条新错误码接入 `explain`（rustc 风格 ≥200 字）

主线 B · 冷文件读触发晋升（Phase 37）
- F5：容器内 cold 分支常驻 `cold-promoter` watcher（inotify `IN_OPEN` / `IN_ACCESS`），LIFO cleanup 回收
- F6：命中文件异步经 `connB` SFTP 拉到 hot 分支，去重 + 5s 防抖 + 指数退避（1/2/4s）+ 3 次熔断
- F7：mergerfs 自然读 hot 优先，第二次读不走 SFTP；`CLOUD_CLAUDE_NO_PROMOTION=1` 全量关闭
- F8：`doctor` 新增 4 项晋升可观测指标（alive / queue / total / failed）+ runbook + e2e UAT 脚本

**性能/体验验收基线:**
- 非 git 仓库挂载 100% 拒绝、退出码恒定、单测锁定
- 60MB fixture 不进热同步、cold 视图可读
- 同会话同文件二次读 SFTP read count = 1（FUSE page cache 命中）
- 首次读 PNG → 晋升 → 第二次读 SFTP read count = 0（mergerfs 走 hot）
- e2e UAT 脚本 PASS + macOS / Linux 双平台真机签字

**Out of scope（推迟到 v3.2 或更后）:**
- bash 命令整体 proxy 到本机（割裂容器内 Claude 环境一致性，缓存 + 晋升后语义已充分）
- Windows 客户端支持（路径同名 + 大小写敏感性 + FUSE 客户端栈缺失，单独立项）
- 热同步改 inotify/fsevents 替代秒级轮询（不动主路径降低回归面，单独评估）
- rename / move 检测优化（非本里程碑核心 gap）
- 跨会话持久缓存（hot 分支退出后保留）

## Backlog（从 v3.0 ship 后沉淀，与 v3.1 主线不耦合）

> 与 v3.1 主线（映射语义补齐）不耦合，待后续里程碑收敛或并行处理。

- **ENH-V3.1-01** 容器预热与空闲回收策略（控制面资源调度）
...
- **ENH-V3.1-04** Mutagen → 自研 hot-sync 切换的 spec doc 修订（v3.0 隐式设计变更，REQUIREMENTS.md 文字漂移）
- 3 项真机签字 ship 闸门（M5 APFS / BASE-03 2min / C6 Ubuntu 25.04）— v3.0 deferred-to-ship
- v3.0 tech debt 批次清理（≈14 项 Phase 32/33 WR/HR/MR 系列 + spec/code 数字漂移）
...
**Latest shipped:** v3.0 远端开发体验升级 (2026-04-23)
**Codebase:** ~45,766 LOC (29,535 Go + 11,772 TypeScript + 4,459 Shell)
**Tech stack:** Go 1.25.7 + PostgreSQL + Docker + sing-box + React 19 + Vite + cobra + sshfs/SFTP + Mutagen v0.18.1 + mergerfs 2.41.1 + tmux 3.6a
...
### Active (v3.1 映射语义补齐与懒加载)

主线 A · 映射前置约束与 FUSE 缓存（Phase 36）
- [ ] **MOUNT-V31-01** 非 git 仓库时拒绝挂载，stderr 输出 `MOUNT_REQUIRE_GIT_REPO` 错误码 + 中文 next_action，退出码恒定
- [ ] **MOUNT-V31-02** 单文件 ≥ `hot_sync_max_file_mb`（默认 50MB）不进热同步，由 cold sshfs 兜底，stderr 一次性提示 + `last-session.json` 记录
- [ ] **MOUNT-V31-03** sshfs 命令默认开 FUSE page cache（`cache=yes,kernel_cache,auto_cache,cache_timeout=300`）
- [ ] **MOUNT-V31-04** `cloud-claude doctor mount` 新增 5 项检查（require_git_repo / oversized_files_count / sshfs_cache_args / git_proxy_enabled / default_ignore_loaded）
- [ ] **MOUNT-V31-05** 错误码注册表新增 `MOUNT_REQUIRE_GIT_REPO` / `MOUNT_OVERSIZED_FILE_SKIPPED`，附 ≥200 字 `cloud-claude explain` 长说明

主线 B · 冷文件读触发晋升（Phase 37）
- [ ] **MOUNT-V31-06** 容器内 cold 分支根目录常驻 `cold-promoter` watcher（inotify `IN_OPEN` / `IN_ACCESS`），LIFO cleanup 回收
- [ ] **MOUNT-V31-07** 命中事件后异步经 `connB` SFTP 拉到 hot 分支：单文件 5s 防抖 + 指数退避（1/2/4s）+ 3 次熔断
- [ ] **MOUNT-V31-08** mergerfs 自然读 hot 优先；第二次读 SFTP read count = 0；`last-session.json` 新增 `promotion_count` / `promotion_bytes`
- [ ] **MOUNT-V31-09** `CLOUD_CLAUDE_NO_PROMOTION=1` 关闭晋升；用户主动读不二次过滤 ignore（已读 = 用户意图）
- [ ] **MOUNT-V31-10** `doctor mount` 新增 4 项晋升可观测指标（promoter_alive / promotion_queue_depth / promotion_total / promotion_failed_total）
- [ ] **MOUNT-V31-11** runbook `docs/runbooks/v31-cold-promotion.md`（原理图 + env var + 故障排查 + mergerfs 边界）
- [ ] **MOUNT-V31-12** e2e UAT 脚本 `tests/scripts/uat-v31-promotion.sh`（fixture + 全场景断言 + JSON 报告 + CI 接入）
...
| 自研 hot-sync 替代 Mutagen 客户端架构（隐式设计变更） | Mutagen 客户端协议复杂度高于预期；Phase 31 实际用 ssh + tar 自研 hot-sync 实现等价语义；Entry API 仅需 `SupportsMergerfs` 即可触发降级 | ⚠️ Doc Drift — REQUIREMENTS.md 仍写 Mutagen，行为正确但 spec 未跟进，v3.1 修订 |
...
---
*Last updated: 2026-04-23 — v3.1 milestone started (映射语义补齐与懒加载, 2 phases, 12 REQ-MOUNT-V31-XX)*
