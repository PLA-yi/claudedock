...
**Shipped:** v3.6 端到端测试体系与网络隔离验证 (2026-05-14, tag v3.6)
**Next:** v3.7（待定，`/gsd-new-milestone` 进入；P1 起手收尾建议为 TD-1 fixture 校准 + TD-3 Scenario.Start Step 2..7 真实接入，触发 9 项 deferred-to-CI 签字）

v3.6 把项目自己的端到端测试体系一次性长出来：`tests/e2e/` testcontainers-go + testify/suite + Scenario builder API + waitFor 4 变体 + ArtifactDumper + `collect-artifacts.sh` 一键采集 + hosted ubuntu-24.04 双 job workflow；MVS / LEAK / KILL 三套真实跑路径的 e2e 用例骨架就位；Phase 51 把 9 项代码层质量加固（verify 多源轮询 / 多目标 / 全 nameserver / namespace functional option / nft counter + linklocal-drop / cap-drop NET_RAW / `-race -shuffle=on` 默认 / goleak / 双绑 pre-check）一次性落地。

**v3.6 ship 总览:**

- 8 phases (45-52), 39 plans（含 51-09 双绑互斥 API 收口）
- 213 commits（一日冲刺 2026-05-14）
- 生产代码新增 ≈ 445 行（集中在 `internal/network/` + `internal/controlplane/http/` + `internal/store/repository/` + `internal/runtime/tasks/`）
- 38/38 v1 REQ satisfied；8/8 phase VERIFICATION.md passed；跨 phase 接口全 WIRED
- darwin 维度六道闸全绿；121 PASS 纯函数单测；harness 自身 23 PASS
- 唯一新依赖：`go.uber.org/goleak v1.3.0`（QUAL-08）
- 8 项 tech debt 全部非阻塞 ship；9 项 human verification 全部 deferred-to-CI

### v3.6 关键交付

E2E 测试骨架（Phase 45 + 52）

- `tests/e2e/` testcontainers-go + testify/suite + `harness.Scenario` builder API（`.WithControlPlane().WithSingBoxGateway(...).WithHost(...).WithUser(...)`）
- `harness.WaitFor` + 4 个语义化变体（`WaitForLog` / `WaitForPort` / `WaitForHTTP` / `WaitForExec`），`scripts/lint-no-bare-sleep.sh` 双层守护防回退
- `ArtifactDumper.Collect(ctx, name)` 失败归档 hook + 5 子目录（logs/network/docker/postgres/system）
- `.github/workflows/e2e.yml` 双 job（lint + e2e，hosted ubuntu-24.04）+ paths 强制守护 + `if: failure()` PR 评论 + `actions/upload-artifact@v4` retention 30 天
- `tests/e2e/harness/collect-artifacts.sh` 172 行一键采集（`SCRIPT_VERSION="v1"` 锁定）+ 7 单测（含 `NoAbsoluteUserPathsInScript` 静态扫描）

MVS / LEAK / KILL 三套 e2e 用例（Phase 46-50）

- `StartGoldenPath(t) *GoldenPath` + `RunBootstrapScript` + 24 个纯函数单测
- `EgressIPSources()` 三源轮询（ip.me / ifconfig.io / ipinfo.io）+ `Vote` 多数派裁决（公网 IP 等值 deferred-to-CI）
- `ClassifyDNSResult` Tunneled/Denied OR 语义 + `DefaultDenyMatrix` 4 target × 3s timeout × 并发
- `BootstrapExitCodeContract` 与 `BootstrapErrorEntries` 编译期 cross-check（被测 binary 改为 `cloud-bootstrap.sh`，以源码为准）
- `ExpiryEventType="host.stop.expired"` + `EXPIRY_SCAN_INTERVAL` env + `HostHealthRecoveryContract{30s/60s}` + `EgressIPDoubleBindContract{409}`
- `KillswitchTimingContract{3s/5s}` + netshoot privileged sidecar tcpdump + `ClassifyResolvConfDNSOutcome` 6 分支 OR 表
- 8 条防泄漏（LEAK-01..08）每条都通过 host eth0 抓包 / `nft list ruleset` / `getpcaps` 等独立 oracle 做断言
- 4 条 kill-switch 压力（SIGKILL / tun0 down / Pumba `gaiaadm/pumba:0.10.0` netem 1000ms / docker network disconnect）

代码层质量加固（Phase 51）

- verify.go `verifyEgressIPMulti` + `voteEgressIP` 三源轮询（复用 Phase 46 Vote 语义）
- verify.go `verifyLeakBlockedMulti` 多 target 默认对齐 `DefaultDenyMatrix`
- verify.go `parseAllNameservers` 遍历全 nameserver 行
- namespace.go `WithProbeWindow / WithMaxRetries` functional option
- worker_firewall_linux 全规则 `expr.Counter` + 注入 `169.254.0.0/16 counter drop comment "linklocal-drop"`
- worker `--cap-drop NET_RAW` + 删 SYS_ADMIN，NET_ADMIN 按 sing-box tun 在 worker netns 创建 tun0 的依赖保留（折中）
- Makefile + ci.yml `-race -shuffle=on -count=1` 默认；goleak.VerifyTestMain 三包接入
- 51-09 admin_bindings.go 双绑 pre-check + `ErrCodeEgressIPAlreadyBound` 常量 + 409 + 中英双语 message（闭 Phase 47 D-47-3 gap）
...
**v3.6 起手收尾候选（建议作为 v3.7 第一周 P0 收尾）：**
...
- ✓ goleak.VerifyTestMain 接入 `internal/network` / `cmd/cloud-claude` / `internal/controlplane/app` 三包 — v3.6 (Phase 51 QUAL-08)

### Active (v3.7 候选 — 待 `/gsd-new-milestone` 收敛)

无 active 项 —— v3.6 已 shipped，下一里程碑目标待定。建议作为 v3.7 起手第一周收尾的高优先级项：
...
| MVS-05 被测 binary 改为 `deploy/bootstrap/cloud-bootstrap.sh`（ROADMAP 草稿描述为 cloud-claude） | grep 源码后实际错误码 10/11/12/13 由 `bootstrap_errors.go` 定义 + `cloud-bootstrap.sh case "$error_code"` 映射；按 CONTEXT §Area 3「以源码为准」决策落地 | ✓ Good — v3.6 Phase 46 |
...
---
*Last updated: 2026-05-14 — v3.6 端到端测试体系与网络隔离验证 shipped (8 phase / 39 plan / 38 REQ satisfied / 8 tech debt deferred). Tag v3.6. Next milestone awaits `/gsd-new-milestone`.*
