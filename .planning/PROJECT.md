...
## Current Milestone: v3.5 网络白名单与 DNS 拆分解析

**Goal:** 让管理员能在后台配置代理白名单（IP/CIDR/域名/端口），让特定流量绕过 sing-box tun 代理隧道走宿主机直连，同时保留 zero DNS leak 的安全承诺。

**Target features:**

- 主线 A · sing-box 静态配置改造 + 拆分 DNS 模型 + 容器 `/etc/resolv.conf` 接管 + 白名单数据模型 + `loopback`/`lan` 内置预设
- 主线 B · 管理员 API（CRUD + preview + apply + rollback）+ 护栏校验 + React 后台 Bypass Tab
- 主线 C · Agent 热更新链路（rule-set 文件 watch + nftables atomic set 更新 + 健康检查 + 失败回滚）+ fail-closed 加固 + E2E 流量验证（10 条安全不变量 CI 化）

**关键技术决策（已研究确定）：**

- 两段式配置：静态 config.json（变更需重启）+ 动态 local rule-set 文件（sing-box 1.10+ 文件 watch 自动 reload）
- 拆分 DNS：内网域名走 `dns-local`，公网白名单域名走代理 DoH（保护查询隐私）
- fail-closed：容器 netns nftables `output policy drop` + 仅放行 `oifname sb-tun0` + sing-box uid 锁定直连代理 IP
- DNS 入口锁：容器 `/etc/resolv.conf` 改为只读挂载指向 sing-box tun IP（172.19.0.1），从当前 8.8.8.8 占位改造
...
**Latest shipped:** v3.4 多形态容器接入 (2026-05-08, tag v3.4.0)
**Codebase:** ~43,300+ LOC (40,702 Go + 2,594 Shell)
**Tech stack:** Go 1.26.1 + PostgreSQL + Docker + sing-box + React 19 + Vite + cobra + sshfs/SFTP + hot-sync (ssh+tar) + mergerfs 2.41.1 + tmux 3.6a + inotify
...
### Active (v3.5 网络白名单与 DNS 拆分解析)

- [ ] sing-box 静态配置改造为两段式：静态 config.json + 动态 local rule-set 文件 watch
- [ ] 拆分 DNS 模型：内网域名走 `dns-local`，公网白名单域名走代理 DoH（保护查询隐私）
- [ ] 容器 `/etc/resolv.conf` 接管：只读挂载指向 sing-box tun IP（172.19.0.1）替代 8.8.8.8 占位
- [ ] 白名单数据模型：`host_bypass_presets` / `host_bypass_rules` / `host_bypass_bindings` / `host_bypass_snapshots` / `host_bypass_audit_log` 五张表
- [ ] 系统内置预设：`loopback`（强制开启）+ `lan` 两个 seed
- [ ] 管理员可在后台对每个 host CRUD 白名单规则（IP/CIDR/域名/端口）
- [ ] 管理员可在后台勾选预设组合并实时预览生效的 sing-box 配置 diff
- [ ] 护栏校验：全量绕过 / 顶级域名 / 自我矛盾 / 数量上限四类硬拦截 + 关键字过短二次确认
- [ ] Agent 热更新链路：写 rule-set 文件 → sing-box 文件 watch 自动 reload + nftables atomic set 原子更新
- [ ] 配置下发失败自动回滚到上一个 snapshot 并告警
- [ ] fail-closed 加固：容器 netns nftables `output policy drop` + 仅放行 `oifname sb-tun0` + sing-box uid 锁定直连代理 IP + IPv6 容器内全禁
- [ ] E2E 流量验证：白名单流量从 eth0 出 + 非白名单走 proxy + DNS 不泄漏，10 条安全不变量 CI 化
...
- [ ] 热同步改 inotify/fsevents 替代秒级轮询
- [ ] rename / move 检测优化
- [ ] cn-dev / oss-dev / ai-api 白名单预设 + 远程 rule-set 拉取（v3.5 P1）
- [ ] 白名单灰度按钮：先在一台测试 host 验证，OK 后再批量 apply（v3.5 P1）
- [ ] 用户自助配置白名单（区分管理员 / 用户角色权限）
...
---
*Last updated: 2026-05-12 — v3.5 milestone initialized (网络白名单与 DNS 拆分解析). Phases TBD by roadmapper.*
