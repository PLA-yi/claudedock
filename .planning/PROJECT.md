...
ClaudeDock 是一个面向单宿主机的容器化 SSH 云主机平台，既供自己使用，也面向出海团队和开发团队销售。用户从一个很短的 `curl` 入口开始，在终端里输入用户名和密码，等待专属 Docker "云主机"启动完成后，直接进入该容器内的 SSH 会话。
...
**Goal:** 将部署从 5 服务精简至 2 服务，数据库从 PostgreSQL 切换至 SQLite，admin 前端与 sing-box 探针均内嵌至 control-plane 单一二进制。

**Target features:**

- PostgreSQL → SQLite（纯 Go modernc.org/sqlite，WAL 模式，data 目录映射持久化）
- Admin 前端 dist embed 进 Go 二进制，control-plane 直出静态服务（去掉独立 nginx 容器）
- Sing-box 探针二进制内嵌至 control-plane（去掉独立预拉取 sing-box 镜像）
- 统一端口/环境变量（8090→8080，去掉 POSTGRES_*、ADMIN_PORT 等遗留）
- Docker Compose 5 服务 → 2 服务（control-plane + managed-user）

**Not in scope:**

- ❌ managed-user 容器和业务 API 契约不变
- ❌ E2E 测试体系暂不深入适配（本次仅保证单元测试通过）

**Status:** ✅ Shipped — 5 phases (58-62), 21/21 REQ satisfied, audit passed

## Shipped Milestone: v4.0 sing-box 同容器化 (Shipped 2026-05-27, tag v4.0)

**Goal:** 把 sing-box 从独立 `claudedock-gw-*` 容器搬进用户容器内部，消灭 "user 容器 default 路由不指向 gw" 这一整类回归泄漏；单容器架构带来 breaking change，故升 major v4.0。

**Target features:**

- **F1** managed-user 镜像内置 sing-box，entrypoint 作 PID 1 + setuid 降权
- **F2** 控制面去掉 gw 容器路径与 `claudedock-net-*` 自定义 bridge，`container_proxy_provider` 简化为单容器
- **F3** sing-box 配置由控制面注入（root:root 0600 → 启动后从 fs 删除，运行时仅内存常驻），用户读不到、改不了
- **F4** DNS 强制走 sing-box stub resolver，nft 默认拒绝兜底：外部 53/853 + DoT/DoH 一律 drop
- **F5** sing-box 死 → 容器死（PID 1 fail-closed）→ `restart=on-failure` 拉起
- **F6** e2e 单容器化重构，v3.6 LEAK/KILL 80% 复用 + 新增 SEC-01..03 同容器安全断言
- **F7** CI paths filter 扩面 + `make e2e` 本地入口（顺手堵 v3.6 留的 "偶尔改坏没拦住" 痛点）

**Not in scope:**

- ❌ 生产用户迁移：v4.0 是 breaking change，存量容器直接重建（决策 D-V4-5）
- ❌ Restart GoldenPath e2e（bug 类消失，不需要测）
- ❌ sing-box 协议层变更（VMess/VLESS/Trojan/SS 等仍走 sing-box 原生 outbound）

**Locked decisions:**

| ID | Decision | Choice |
|---|---|---|
| D-V4-1 | sing-box 进程身份 | B —— 建完 tun 后 `process.user` setuid 降权（NET_ADMIN 不在用户进程） |
| D-V4-2 | config 凭据保护 | 强化 A→B —— root:root 0600 + 启动后 rm + 只能面板改 |
| D-V4-3 | DNS 策略 | A 强化 —— sing-box stub resolver + nft drop 外部 53/853 + DoT/DoH |
| D-V4-4 | sing-box 死亡 | A —— PID 1 fail-closed，sing-box 死 → 容器死 |
| D-V4-5 | 迁移策略 | 不迁移，v4.0 breaking，容器重建 |
| D-V4-6 | e2e 改造 | 按拆分草案（80% 复用 + 新增 SEC-01..03 + 删 Restart） |

**Status:** ✅ Shipped — 4 phases (53-56), 13 plans, 28/28 REQ satisfied, audit passed

## Current State

**Shipped:** v4.2.0 容器合并 · SQLite 迁移 · 配置统一 (2026-06-01)
**Active:** v4.2.0 shipped 2026-06-01, awaiting next milestone

v4.2.0 将部署从 5 服务精简至 2 服务，数据库从 PostgreSQL 切换至 SQLite（纯 Go modernc.org/sqlite），admin 前端与 sing-box 探针均内嵌至 control-plane 单一二进制。

**v4.2.0 ship 总览:**

- 5 phases (58-62), 3 plans (Phase 58)
- ~15 commits (2026-06-01 一日完成)
- 核心变更：移除 pgx/v5，引入 modernc.org/sqlite + database/sql；21 个迁移文件 PG→SQLite；queries.go + queries_bypass.go 重写
- Docker Compose: 5 services → 2 services（control-plane + managed-user）
- Admin UI + sing-box 探针均内嵌至 control-plane 单一二进制，统一端口 :8080
- 所有单元测试从 testcontainers PG 切换到内存 SQLite
- 21/21 REQ satisfied；build + test PASS
- 唯一遗留：pgx/v5 因 e2e 测试构建标签保留在 go.mod 中（非阻塞）

### v4.2.0 关键交付

SQLite 数据库层迁移（Phase 58）

- 移除 pgx/v5，引入 modernc.org/sqlite + google/uuid（纯 Go，零 CGO）
- migrator 重写为 database/sql + embed.FS 内嵌迁移文件
- 21 个迁移文件 PG→SQLite 语法改写（UUID/TIMESTAMPTZ/JSONB → TEXT，BOOLEAN → INTEGER）
...
- router.go 注册静态文件 handler，API 路由优先级高于静态文件
- vite.config.ts 代理 target 统一为 :8080

Sing-box 探针内嵌（Phase 60）

- control-plane Dockerfile 下载 sing-box v1.13.3 到 /usr/local/bin/
- startLocalSingBox 优先使用宿主机二进制，不存在才回退 Docker

部署精简与配置统一（Phase 61）

- docker-compose.yml 5 服务精简至 2 服务，移除 postgres/admin/sing-box
- 统一端口 :8080，移除 POSTGRES_* 环境变量
- Makefile 移除 db/db-stop/db-reset 目标和 PG 检测
- deploy/scripts 移除 PostgreSQL 交互和检查

文档同步与测试适配（Phase 62）

- README/文档站架构图、环境变量表、FAQ 全面更新
- repository 测试从 testcontainers PG 切换到内存 SQLite
- app_test.go 配置常量更新

以下为历史已发布里程碑摘要（已折叠）。

<details>
<summary>📦 v3.5 网络白名单与 DNS 拆分解析 (已归档)</summary>
...
v3.4 扩展容器接入方式：Cloud 版 SSH Proxy 支持 full port forwarding（VS Code Remote-SSH），本地版 `claudedock local` 支持 Dev Containers 工作流，doctor 新增 remote-ssh 维度和 sshd_config 主动验证。
...
- `claudedock local up/down/status` cobra 子命令组 + `internal/local` 包
...
**v4.0 后续收尾候选（建议作为 v4.1 第一周 P0 收尾）：**

- **F-V4-DID** — 本地 Docker-in-Docker runner（`make e2e-linux` 在 Ubuntu 容器里跑 `make e2e`），让 macOS 一条命令复现 CI 全套 Linux 维度断言（v4.0 deferred）
- **F-V4-TMPFS** — sing-box config 走 tmpfs 注入而非 fs 写后删（更强凭据隔离）（v4.0 deferred）

- **V36-TD-3** P1 — Phase 45 Plan 02 Scenario.Start Step 2..7 真实接入（控制面子进程 / admin login / fixture 三件套 / PrepareGateway / PrepareHost）—— Linux runner 真机跑通 9 项 deferred-to-CI 签字的共同前置
- **V36-TD-1** P1 — Phase 49 LEAK-08 fixture `proc_status_clean.txt` NET_ADMIN 期望校准（与 Phase 51 QUAL-06 折中保留 NET_ADMIN 一致）
- **V36-Linux-Signoff** P0 — hosted ubuntu-24.04 上跑通 9 项 human verification（MVS-06/07/08 + MVS-09/10 + LEAK-01..08 + KILL-01..04 + VERIFY-1/2）
- **V36-TD-7** P2 — `DATABASE_URL` 透传给 CI failure step，让 `collect-artifacts.sh` 真实 `pg_dump --schema-only`
- **V36-TD-2** P2 — `dockerExecHandle` 临时 ContainerHandle 实现替换为正式 testcontainers Container（KILL-04）
- **V36-TD-5** P2 — fixture SQL bcrypt hash 动态生成 helper（避免硬编码过期 hash）
- **V36-TD-6** P2 — hosted ubuntu-24.04 上 `/dev/net/tun` 偶发不可用的 fixture preflight 兜底
- **V36-TD-4** P2 — host-agent per-host health API（多宿主机阶段视情况引入）
- **V36-TD-8** P3 — fork PR 上 PR 评论 403 的 fallback（`pull_request_target` + 严格输入校验，待 fork PR 成为常态时引入）
- **V36-V2** v2 范围 —— E2E-FULL-01..05（并发 N=50/100 容器 / conntrack flush / Tetragon TracingPolicy / host 重启清理 / Hurl 控制面 API 断言）+ E2E-PERF-01..02（冷启动 ≤15s + 完整矩阵 ≤60min nightly）
...
- **ENH-NEXT-07** `claudedock local --sync-config` 从云端拉取 egress IP 配置
...
**Goal:** 把 claudedock 从"能跑起来"升级到"长期可依赖的远程开发日常工具"——彻底解决 sshfs 性能天花板、网络抖动会话丢失、多端使用冲突等 v2.0 实际使用中暴露的核心体验问题。
...
主线 C · 运维与体验配套
- F6：`claudedock doctor` 全面升级（network/auth/ssh/mount 三层/disk 五维度自检 + 一键修复） — ✓ Phase 34
...
**Latest shipped:** v4.2.0 容器合并 · SQLite 迁移 · 配置统一 (2026-06-01)
**Codebase:** ~120,000+ LOC (Go + TS/TSX + Shell)
**Tech stack:** Go 1.26.1 + SQLite (modernc.org/sqlite, WAL 模式) + Docker 28 + sing-box 1.13.3 + nftables（`google/nftables` + `nft -f` 事务）+ React 19.2 + Vite 8 + cobra + sshfs/SFTP + hot-sync (ssh+tar) + mergerfs 2.41.1 + tmux 3.6a + inotify + vitest 4 + Radix UI
...
v2.0 已交付：
- Go 单一二进制 `claudedock`，cobra 入口 + init 配置管理 + Entry API 认证轮询
...
- tmux 默认包装 (`exec tmux new-session -A -s claude-<account_id>`) + 多端共享 attach + `claudedock sessions ls/attach` + `--new-session`/`--take-over` + 账号级 Mutagen 单例锁
...
- `claudedock doctor` 5 维度（network/auth/ssh/mount/disk）18 项 check + 6 类自动 fix + JSON schema_v1 + 退出码 0/1/2 + 第一屏降级历史 banner + ci-doctor-grep.sh M14 闸门
- 错误码统一 `<DOMAIN>_<KIND>_<NUM>` 4 段（42 条 8 域 Registry + 38 条 ≥200 字符长说明 + `claudedock explain <code>` rustc 风格）
...
- ✓ Go 单一二进制 claudedock，用户可 alias claude=claudedock 透明替代原生 claude 命令 — v2.0
- ✓ claudedock init 配置网关地址、用户凭证，持久化到 ~/.claudedock/config.yaml — v2.0
...
- ✓ claudedock 用 Mutagen 热同步源码白名单（≤50MB、按扩展名/路径过滤），替换原纯 sshfs 方案 — v3.0 (Phase 31)
- ✓ claudedock 同时挂载 sshfs 冷兜底，覆盖未同步文件（懒读、按需走网络）— v3.0 (Phase 31)
...
- ✓ 网络恢复后 claudedock 自动提示并支持重连，无需重新认证（退避 1/2/4/8/30s + token 复用）— v3.0 (Phase 32)
...
- ✓ `claudedock doctor` 覆盖 network/auth/ssh/mount（mutagen+sshfs+mergerfs 三层）/disk 五个维度 — v3.0 (Phase 34)
...
- ✓ `claudedock explain <code>` 子命令对每个错误码给详细中文说明（rustc 风格）— v3.0 (Phase 34)
...
- ✓ `claudedock local up/down/status` 子命令，不依赖 control-plane 直接启动本地容器 — v3.4 (Phase 39)
...
- ✓ `claudedock doctor` 新增 remote-ssh 维度（5 项检查 + 6 个错误码 + 20 单测）— v3.4 (Phase 41)
...
- ✓ `tests/e2e/` testcontainers-go + testify/suite 接入 + `harness.Scenario` builder API（声明式控制面/host-agent/Postgres/用户容器/sing-box gateway 拓扑）— v3.6 (Phase 45)
- ✓ `harness.WaitFor` + 4 个语义化变体（Log/Port/HTTP/Exec）+ `scripts/lint-no-bare-sleep.sh` 双层守护（禁止裸 sleep）— v3.6 (Phase 45)
- ✓ `ArtifactDumper` 失败归档 hook + 5 子目录占位 + `collect-artifacts.sh` 一键采集脚本 + `actions/upload-artifact@v4` retention 30 天 — v3.6 (Phase 45 + Phase 52)
- ✓ `.github/workflows/e2e.yml` 双 job（lint + e2e，hosted ubuntu-24.04）+ paths 强制守护 + `if: failure()` PR 评论 — v3.6 (Phase 45 + Phase 52)
- ✓ MVS-01 bootstrap 黄金路径 e2e 骨架（`StartGoldenPath` + 24 单测）— v3.6 (Phase 46)
- ✓ MVS-02 容器内出口 IP 匹配验证（`EgressIPSources` 三源轮询 + `Vote` 多数派裁决）— v3.6 (Phase 46)
- ✓ MVS-03 DNS 强制走 tun OR 防火墙拒绝（`ClassifyDNSResult` Tunneled/Denied OR 语义）— v3.6 (Phase 46)
- ✓ MVS-04 默认拒绝矩阵（`DefaultDenyMatrix` 4 target × 3s timeout × 并发）— v3.6 (Phase 46)
- ✓ MVS-05 CLI 错误码契约（`BootstrapExitCodeContract` 与 `bootstrap_errors.go` 编译期 cross-check；binary 为 `cloud-bootstrap.sh`）— v3.6 (Phase 46)
- ✓ MVS-06 到期容器自动停止 + `host.stop.expired` 审计事件 + `EXPIRY_SCAN_INTERVAL` 环境变量 — v3.6 (Phase 47)
- ✓ MVS-07 出口 IP 双绑互斥（API pre-check 409 + `ErrCodeEgressIPAlreadyBound` 中英双语 message）— v3.6 (Phase 47 + 51-09)
- ✓ MVS-08 host-agent 心跳与恢复（`HostHealthRecoveryContract{30s/60s}`）— v3.6 (Phase 47)
- ✓ MVS-09 sing-box 崩溃后容器立即断网（`KillswitchTimingContract{3s/5s}` + netshoot privileged sidecar）— v3.6 (Phase 48)
- ✓ MVS-10 resolv.conf 篡改免疫（`ClassifyResolvConfDNSOutcome` 6 分支 OR 表）— v3.6 (Phase 48)
- ✓ 8 条防泄漏不变量（DNS 明文/DoT/ICMP/IPv6/IMDS/raw socket/link-local/capability）每条独立 oracle 断言 — v3.6 (Phase 49 + Phase 51)
- ✓ 4 条 kill-switch 压力（SIGKILL/tun0 down/Pumba `gaiaadm/pumba:0.10.0` netem/network disconnect）— v3.6 (Phase 50)
- ✓ verify.go 多源轮询（`verifyEgressIPMulti` + `voteEgressIP`）+ 多目标泄漏检测（`verifyLeakBlockedMulti`）+ 全 nameserver 校验（`parseAllNameservers`）— v3.6 (Phase 51 QUAL-01..03)
- ✓ namespace.go `WithProbeWindow` / `WithMaxRetries` functional option — v3.6 (Phase 51 QUAL-04)
- ✓ worker_firewall_linux 全规则 `expr.Counter` + 显式 `169.254.0.0/16 counter drop comment "linklocal-drop"` — v3.6 (Phase 51 QUAL-05)
- ✓ worker `--cap-drop NET_RAW` + 删 SYS_ADMIN，NET_ADMIN 按 sing-box tun 依赖保留（折中）— v3.6 (Phase 51 QUAL-06)
- ✓ Makefile + ci.yml `-race -shuffle=on -count=1` 默认 — v3.6 (Phase 51 QUAL-07)
- ✓ goleak.VerifyTestMain 接入 `internal/network` / `cmd/claudedock` / `internal/controlplane/app` 三包 — v3.6 (Phase 51 QUAL-08)

- ✓ managed-user 镜像内置 sing-box 1.13.3（file cap cap_net_admin+eip）+ singbox 系统账号 uid=9000 + 删除 workspace sudo — v4.0 (Phase 53)
- ✓ entrypoint v4.0 fail-closed 启动序列（sing-box 降权运行 → tun0 → nft default-deny → DNS stub → config shred → PID 1 fail-closed → sshd）— v4.0 (Phase 53)
- ✓ nft default-deny ruleset + DNS 强制 sing-box stub resolver + 外部 53/853/DoT/DoH 一律 drop — v4.0 (Phase 53)
- ✓ container_proxy_provider 单容器化（-364 行）+ gw 容器/bridge/teardownGateway 整体退役 — v4.0 (Phase 54)
- ✓ sing-box config 注入 root:singbox 0640 + entrypoint 启动后 shred — v4.0 (Phase 54)
- ✓ 双绑互斥契约保持（ErrCodeEgressIPAlreadyBound + 409 + 中英双语 message）— v4.0 (Phase 54)
- ✓ Scenario builder 单容器化 API + LEAK/KILL/MVS/GoldenPath 迁移 + SEC-01..03 安全断言 — v4.0 (Phase 55)
- ✓ CI paths filter 扩面（+6 路径）+ `make e2e` 本地入口 + lint/vet 对齐 — v4.0 (Phase 56)

### Active (v4.1 候选 — 待 `/gsd-new-milestone` 收敛)

无 active 项 —— v4.0 已 shipped，下一里程碑目标待定。建议作为 v4.1 起手收尾的高优先级项：

- [ ] **V36-TD-3** Scenario.Start Step 2..7 真实接入（Linux runner 真机跑通的共同前置）
- [ ] **V36-TD-1** Phase 49 LEAK-08 fixture NET_ADMIN 期望校准
- [ ] **V36-Linux-Signoff** 9 项 deferred-to-CI 在 hosted ubuntu-24.04 上一次性签字（MVS-06/07/08 + MVS-09/10 + LEAK-01..08 + KILL-01..04 + VERIFY-1/2）

### Future (延后到 v3.7 之后)

待 `/gsd-new-milestone` 进入定义。

### Archived (v1.3 claude-shell) — 能力并入 v3.4

v1.3 的"本地容器"和"指纹伪装"能力与 v3.4 的 `claudedock local` 目标重叠。为避免维护两套本地 Docker 编排逻辑，v1.3 能力拆分并入 v3.4 Phase 39：

- Phase 17 镜像基线 → 已并入 managed-user 镜像（v3.0+）
- Phase 18 网络隔离 → 并入 v3.4 Phase 39（本地版 sing-box sidecar + veth 注入）
- Phase 19 CLI 骨架 → 并入 v3.4 Phase 39（`claudedock local` 子命令）
- Phase 20 TTY 透传 → 不再需要（本地 publish 端口，SSH 用系统 OpenSSH）
- Phase 21 指纹伪造 → entrypoint `MODE=local` 深度伪装分支（快速任务级别）
- Phase 22 验证自检 → `claudedock local verify` / `doctor --local`
...
| Entry API 为 claudedock 唯一认证契约 | 复用控制面现有实现，不新增专用 API | ✓ Good — 零服务端改造 |
...
| doctor 纯本地 + SSH 实现，不给 host-agent 加 endpoint | 保持特权边界清晰，避免 control/data plane 混淆 | ✓ Good — Phase 34 SC#9 守恒（`rg "agentapi\." internal/claudedock/doctor/` 命中 0） |
...
| `claudedock explain <code>` 对标 `rustc --explain` | 为非研发用户（运维、客服）提供错误码反查能力，不需要读源码 | ✓ Good — Phase 34 落 38 条 ≥200 字符长说明，exit 0/4 两态 |
...
| Mutagen 二进制 go:embed 集成（Q1 (a)） | 用户体验"一条命令" — 不依赖本机 brew install / 首次下载；4 平台 v0.18.1 二进制 ~49MB 嵌入 claudedock | ✓ Good — Phase 31，CI build-images workflow 拉取真实文件 |
...
| persistent volume 单 volume `claude-state-{claude_account_id}` (Q4) | 运维更简单（vs. 双 volume creds + cache 分开）；label `com.claudedock.account_id` 支撑 GC | ✓ Good — Phase 30 数据模型 + Phase 33 实现 |
...
| v1.3 归档，能力并入 v3.4 | v1.3 的"本地容器"和"指纹伪装"能力与 v3.4 `claudedock local` 目标重叠；维护两套本地 Docker 编排逻辑无意义 | 避免重复开发；Phase 18→39（网络）、Phase 19→39（CLI）、Phase 21→entrypoint `MODE=local` |
...
| nft 表族 = `ip claudedock`，`nftRunner` / `nftJSONLister` 自带 `nsenter -t <pid> -n --` 包装 | 与 47-02 现状对齐，单点事实源 `bypassNftFamily/Table`；Phase 47 初次验证教训：单测 PASS ≠ 生产路径 OK | ✓ Good — v3.5 Phase 47（commit 60c9896） |
...
| e2e 测试体系基于 testcontainers-go + testify/suite + Scenario builder API | 复用成熟开源生态（vs 自研框架）；testify/suite 提供 `SetupSuite/TearDownSuite` 生命周期；Scenario builder 一行声明拓扑避免复制粘贴启动代码 | ✓ Good — v3.6 Phase 45 |
| 测试 harness 禁止裸 `time.Sleep`，统一走 `waitFor(ctx, predicate, timeout)` | 裸 sleep 是 e2e flake 的头号来源；waitFor 4 变体（Log/Port/HTTP/Exec）覆盖常见等待场景；`lint-no-bare-sleep.sh` 双层 grep 守护防回退 | ✓ Good — v3.6 Phase 45 |
| GoldenPath 抽象 + 纯函数 + 锁定常量作为跨 phase e2e 共享契约 | Phase 48/49/50 高频复用 Phase 46/47 的 `StartGoldenPath` 方法、`EgressIPSources`/`ClassifyDNSResult`/`DefaultDenyMatrix` 纯函数、`BootstrapExitCodeContract`/`HostHealthRecoveryContract` 锁定表，避免每 phase 重新发明 e2e 入口 | ✓ Good — v3.6 Phase 46-50 |
| 出口 IP 三源 + Vote 多数派裁决，某源全部超时按"投票"语义裁决 | 单源容易抖动导致 false-positive；Vote 在 ≥2 源一致时即 PASS，抗外网波动；Phase 51 QUAL-01 在生产 verify.go 也复用同一语义 | ✓ Good — v3.6 Phase 46 + Phase 51 |
| DNS / resolv.conf 篡改用 "Tunneled OR Denied" OR 语义二选一即 PASS | 不同 host 环境下 sing-box dns hijack 与 nft drop 表达方式可能不同；OR 语义吃掉环境差异，只关心"流量未旁路"这一核心契约 | ✓ Good — v3.6 Phase 46 MVS-03 + Phase 48 MVS-10 |
| MVS-05 被测 binary 改为 `deploy/bootstrap/cloud-bootstrap.sh`（ROADMAP 草稿描述为 claudedock） | grep 源码后实际错误码 10/11/12/13 由 `bootstrap_errors.go` 定义 + `cloud-bootstrap.sh case "$error_code"` 映射；按 CONTEXT §Area 3「以源码为准」决策落地 | ✓ Good — v3.6 Phase 46 |
| host eth0 tcpdump 改走 netshoot privileged sidecar | 不能假定 e2e runner 上有 tcpdump 二进制；`docker run --network host --cap-add NET_RAW/NET_ADMIN nicolaka/netshoot:v0.13` 是稳定可移植路径；新增 `E2E_TCPDUMP_IMAGE` / `E2E_ALLOW_HOST_TCPDUMP` env 覆盖 | ✓ Good — v3.6 Phase 48 + Phase 49 + Phase 50 |
| Pumba 固定 tag `gaiaadm/pumba:0.10.0` | 避免 latest 漂移导致 netem 行为不一致；`BuildPumbaNetemArgs` + `ParsePumbaOutput` 把 Pumba 行为契约化 | ✓ Good — v3.6 Phase 50 KILL-03 |
| worker NET_ADMIN 折中保留（QUAL-06 删 NET_RAW + SYS_ADMIN，NET_ADMIN 不删） | sing-box 需在 worker netns 创建 tun0 设备，删 NET_ADMIN 会导致 tun 起不来；CONTEXT §Area 4 允许的折中；fixture 期望校准列 TD-1 | ⚠️ Revisit — v3.6 Phase 51 QUAL-06；待 sing-box tun 替代方案出现后重评 |
| nft 全规则 `expr.Counter` + 显式 `169.254.0.0/16 counter drop comment "linklocal-drop"` | counter 是观测属性不改 drop 行为；显式 link-local drop 比依赖链末兜底 sbfw-drop 更明确，e2e artifact 也能直接读 counter 命中数排障 | ✓ Good — v3.6 Phase 51 QUAL-05 |
| `go test ./... -race -shuffle=on -count=1` 默认化 | race detector 持续守护竞争条件；shuffle 暴露隐式顺序依赖；count=1 避免缓存 + 抵抗 flake | ✓ Good — v3.6 Phase 51 QUAL-07 |
| goleak.VerifyTestMain 仅 ignore 已知设计内常驻 goroutine（`broadcast.(*Hub).cleanupLoop`） | 严格 ignore list 比放宽更早暴露真泄漏；新增依赖 `go.uber.org/goleak v1.3.0` 是本里程碑唯一允许的新生产依赖 | ✓ Good — v3.6 Phase 51 QUAL-08 |
| 双绑互斥 API pre-check 用 `ErrCodeEgressIPAlreadyBound` + 409 + 中英双语 message + host_id/egress_ip_id 字段回显 | 比依赖 DB UNIQUE 约束的 500 错误更友好；前端可直接展示中文 message 和占用 host_id；51-09 一次性闭 Phase 47 D-47-3 gap | ✓ Good — v3.6 Phase 51-09 |
| CI e2e 走 hosted ubuntu-24.04（弃用 self-hosted Linux runner） | 与 v3.5 uat-bypass.yml 同款 runner 池，降低维护成本；hosted runner 在 paths/`/dev/net/tun` preflight 兜底后能稳定跑大部分 e2e；自管 runner 留给 v2 大并发矩阵再评估 | ✓ Good — v3.6 Phase 45 E2E-03 |
| CONTEXT §Area 4「darwin 编译 + 纯函数单测 PASS = passed；Linux 真机断言 deferred-to-CI 非阻塞 ship」 | 本里程碑 9 项签字点全 deferred-to-CI；TD-3 完成后 9 项可一次性签；不阻塞 ship 路径 | ✓ Good — v3.6 全 phase |

| sing-box 进程建好 tun + auto_route 后 setuid 降权（process.user 字段）| NET_ADMIN 不在用户进程；file cap 比容器整体 SYS_ADMIN 更精细 | ✓ Good — D-V4-1, v4.0 Phase 53 |
| sing-box config root:root 0600 + 启动后 fs 删除 | 防用户读上游凭据；运行时仅内存常驻 | ✓ Good — D-V4-2, v4.0 Phase 53+54 |
| PID 1 fail-closed：sing-box 死 → 容器死 → restart=on-failure 拉起 | 不允许 sing-box 崩溃后直连外网 | ✓ Good — D-V4-4, v4.0 Phase 53 |
| v4.0 是 breaking change，存量容器直接重建，不做 sidecar/inline 双模式共存 | 维护双路径成本远大于收益 | ✓ Good — D-V4-5, v4.0 |
| container_singbox_config.go 复用 buildGatewayProxyOutbound/buildGatewayDirectOutbound | 减少 diff 面积，从已退役的 gateway_singbox_config.go 迁移 | ✓ Good — v4.0 tech debt cleanup |
...
---
*Last updated: 2026-06-01 — v4.2.0 容器合并 · SQLite 迁移 · 配置统一 milestone shipped & archived (5 phases: 58-62, 21/21 REQ, audit passed). Awaiting next milestone.*
