...
func buildOnceExplainBin(t *testing.T) string {
	...
	explainBinOnce.Do(func() {
		tmpDir, err := os.MkdirTemp("", "claudedock-explain-test-*")
		if err != nil {
			explainBinErr = err
			return
		}
		explainBinPath = tmpDir + "/claudedock"
	...
	if explainBinErr != nil {
		t.Fatalf("编译 claudedock 失败: %v", explainBinErr)
	}
...
// claudedock explain MOUNT_HOT_SYNC_FAILED 必须 exit 0 + stdout 含错误码字面量 + "建议:" 子串。
...
// claudedock explain mount_hot_sync_failed（小写）必须 exit 4，禁止自动修正。
