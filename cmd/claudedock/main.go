...
func main() {
	rootCmd := &cobra.Command{
		...
		Long:               "连接远端云主机并启动 Claude Code 交互会话。\n首次使用请先运行 claudedock init 配置网关与凭证。",
		Version:            Version,
		SilenceUsage:       true,
		SilenceErrors:      true,
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	if err := claudedock.SaveConfig(cfg); err != nil {
		return err
	}

	path, _ := claudedock.ConfigPath()
	fmt.Printf("配置已保存到 %s\n", path)
	return nil
}
...
func runRoot(cmd *cobra.Command, args []string) error {
	...
	mountCfg := claudedock.MountConfig{
		...
		// WR-02 修复：把 ~/.claudedock/config.yaml 中 hot_sync_max_file_mb 透传到
		...
		// 「编辑 ~/.claudedock/config.yaml 调高 hot_sync_max_file_mb」承诺直接矛盾。
	...
	exitCode, err := claudedock.ConnectAndRunClaudeV3(
