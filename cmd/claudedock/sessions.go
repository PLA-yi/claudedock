// cmd/claudedock/sessions.go — Phase 32 Plan 02 Task 2.3
//
// claudedock sessions 子命令树（mirror cmd/claudedock/sync.go 结构）：
//   - sessions ls       列出当前 tmux 会话（远程 tmux list-sessions）
//   - sessions attach   attach 到指定 tmux 会话（远程 tmux has-session 校验 → exec attach）
//
// 业务逻辑全部在 internal/claudedock.RunSessionsLs / RunSessionsAttach（session.go）；
...
func connectForSessions(ctx context.Context) (*ssh.Client, error) {
	...
	return claudedock.SSHConnect(sshCfg)
}
