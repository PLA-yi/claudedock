...
// newSyncCmd 构造 claudedock sync 父命令树。
//
// 自研 hot sync 当前只暴露 conflicts 查询：
// 读取 ~/.claudedock/last-session.json 的 conflict_count，提示用户最近一次会话
// 是否出现自动 resolved 的双向冲突。逐文件清单暂未持久化。
func newSyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:           "sync",
		Short:         "热同步相关工具",
		Long:          "查看 claudedock 自研热同步的最近一次冲突计数等状态。\n当前仅实现 sync conflicts 子命令；逐文件冲突清单留后续版本补齐。",
		SilenceUsage:  true,
		SilenceErrors: true,
	}
	...
	conflictsCmd := &cobra.Command{
		Use:           "conflicts",
		Short:         "查看最近一次热同步冲突计数",
		Long:          "读取 ~/.claudedock/last-session.json 中的 conflict_count。\n当前自研热同步会在运行时实时提示冲突，但尚未持久化逐文件清单。",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func runSyncConflicts(cmd *cobra.Command, args []string) error {
	...
	fmt.Println("当前版本仅持久化冲突计数；逐文件冲突会在运行 claudedock 时实时提示。")
	return nil
}
