...
用户安装 `claudedock` 并执行 `init` 填入上述信息后，在项目目录下运行 `claudedock` 即可。当前目录会通过 sshfs 挂载到容器内同名路径。
...
也可从 [Releases](https://github.com/claudedock/claudedock/releases) 下载或 `go build ./cmd/claudedock`。
...
按提示输入网关地址、Short ID 和密码，写入 `~/.claudedock/config.yaml`。
...
claudedock
claudedock -p "帮我重构这个函数"
```

**会话管理：**

```bash
claudedock                  # 默认 attach 已有会话
claudedock --new-session    # 新建独立会话
claudedock --take-over      # 接管主会话，踢掉其他客户端
claudedock sessions         # 列出当前会话
```

**映射模式：**

```bash
claudedock --mount-mode=auto         # 默认：HotSync 优先，失败降级 SSHFS
claudedock --mount-mode=full         # HotSync + SSHFS 双轨
claudedock --mount-mode=sshfs-only   # 纯 SSHFS
```

**自检与排障：**

```bash
claudedock doctor                     # 五维度全面自检
claudedock doctor mount --fix         # 检查挂载并自动修复
claudedock explain MOUNT_SSHFS_DISCONNECTED  # 错误码解释
claudedock env check                  # 检查远端时区、出口 IP、FUSE 等
```

**常用配置（`~/.claudedock/config.yaml`）：**
