...
| SQLite (modernc.org/sqlite) | pure-Go 1.26 | 持久化保存用户、容器、出口 IP 绑定、会话、到期时间和审计事件 | 纯 Go WAL 模式单文件数据库，零运维，适合单机产品。 |
...
| sing-box + Linux netns | 协议稳定 / 跟随宿主机发行版 | 为每个容器提供全隧道出网能力 | 通过 sing-box tun 设备在容器 namespace 内实现全流量代理，支持 SOCKS5、VMess、VLESS、Shadowsocks、Trojan、HTTP 等协议。 |
...
| `database/sql` + `modernc.org/sqlite` | Go 1.26 | SQLite 驱动和查询执行 | 当控制面开始落地用户、任务、绑定和事件表时直接使用。 |
...
# 后端运行基线
# - Go 1.26.1
# - SQLite (modernc.org/sqlite, WAL 模式)
# - Docker Engine 28.x
...
| SQLite | 仅适合需要独立 PG 进程的场景（本项目 v4.2.0 已迁移至 SQLite） |
| OpenSSH 10.2p1 | Web Terminal / 自定义 PTY 代理 | 只有在 SSH 主路径稳定后，才考虑补浏览器终端。 |
| sing-box tun + netns 全隧道 | 单纯 SOCKS / HTTP 代理 | 只适合非强约束实验，无法满足“所有流量不可泄漏”的要求。 |
## 明确不推荐
| 不要用 | 原因 | 替代方案 |
|--------|------|----------|
...
| 组件 A | 兼容组件 | 备注 |
|--------|----------|------|
| Go 1.26.1 | SQLite (modernc.org/sqlite) | 纯 Go 驱动，无 CGO，正常驱动支持路径 |
...
- https://modernc.org/sqlite — 确认 pure-Go SQLite 驱动为当前推荐方案
...
- 持久化层：使用 SQLite (WAL 模式) 保存系统真实状态
## 关键边界
