...
[English](README.en.md) | [文档](https://claudedock.github.io/claudedock/)
...
ClaudeDock 做的事情很简单：**把你的 Claude Code 包装成一个坐在洛杉矶家里用 Windows 电脑的普通美国人。** 从 IP 到系统指纹到 TLS 握手，每一层都做了伪装。Anthropic 看到的就是一个正常的美国住宅用户，不是什么奇奇怪怪的云服务。
...
创建 `claudedock` 系统用户，构建二进制和镜像，安装 systemd 单元并启动。

### 环境变量

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `DATABASE_URL` | SQLite 数据库路径 | `file:/data/claudedock.db` |
...
Bug 报告和功能建议请提交 [Issue](https://github.com/claudedock/claudedock/issues)。
...
完整文档见 [GitHub Pages](https://claudedock.github.io/claudedock/)：快速开始、部署指南、配置参考、架构说明、API 参考、故障排查。
